...
class HintsReader implements AutoCloseable, Iterable<HintsReader.Page> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(HintsReader.class);

    private static final transient Logger logger = LoggerFactory.getLogger(HintsReader.class);

    // don't read more than 512 KB of hints at a time.
    private static final transient int PAGE_SIZE = 512 << 10;

    private final transient HintsDescriptor descriptor;

    private final transient File file;

    private final transient ChecksummedDataInput input;
    ...
    final class Page {
        ...
        public final transient InputPosition position;
    ...
    final class HintsIterator extends AbstractIterator<Hint> {
        ...
        private final transient InputPosition offset;

        private final transient long now = System.currentTimeMillis();
        ...
        private Hint readHint(int size) throws IOException {
            ...
            try {
                ...
                logger.warn("Failed to read a hint for {}: {} - table with id {} is unknown in file {}", StorageService.instance.getEndpointForHostId(descriptor.hostId), descriptor.hostId, e.id, descriptor.fileName());
                input.skipBytes(Ints.checkedCast(size - input.bytesPastLimit()));
                // set the return value to null and let following code to update/check the CRC
                hint = null;
            }
            if (input.checkCrc())
                return hint;
            // log a warning and skip the corrupted entry
            logger.warn("Failed to read a hint for {}: {} - digest mismatch for hint at position {} in file {}", StorageService.instance.getEndpointForHostId(descriptor.hostId), descriptor.hostId, input.getPosition() - size - 4, descriptor.fileName());
            return null;
        }
    }
