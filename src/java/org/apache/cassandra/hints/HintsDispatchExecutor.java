...
final class HintsDispatchExecutor {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(HintsDispatchExecutor.class);

    private static final transient Logger logger = LoggerFactory.getLogger(HintsDispatchExecutor.class);
    ...
    private final transient Predicate<InetAddressAndPort> isAlive;

    private final transient Map<UUID, Future> scheduledDispatches;
    ...
    private final class TransferHintsTask implements Runnable {
         ...
         */
        private final transient Supplier<UUID> hostIdSupplier;
        ...
        private boolean transfer(UUID hostId) {
            catalog.stores().map(store -> new DispatchHintsTask(store, hostId)).forEach(Runnable::run);
    ...
    private final class DispatchHintsTask implements Runnable {
        ...
        private final transient HintsStore store;
