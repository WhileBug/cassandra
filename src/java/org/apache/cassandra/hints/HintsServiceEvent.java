...
final class HintsServiceEvent extends DiagnosticEvent {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(HintsServiceEvent.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(HintsServiceEvent.class);

    enum HintsServiceEventType {

        DISPATCHING_STARTED, DISPATCHING_PAUSED, DISPATCHING_RESUMED, DISPATCHING_SHUTDOWN
    }

    private final transient HintsServiceEventType type;

    private final transient HintsService service;

    private final transient boolean isDispatchPaused;

    private final transient boolean isShutdown;

    private final transient boolean dispatchExecutorIsPaused;

    private final transient boolean dispatchExecutorHasScheduledDispatches;
