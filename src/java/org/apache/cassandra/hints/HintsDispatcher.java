...
final class HintsDispatcher implements AutoCloseable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(HintsDispatcher.class);

    private static final transient Logger logger = LoggerFactory.getLogger(HintsDispatcher.class);
    ...
    private final transient HintsReader reader;

    final transient UUID hostId;

    final transient InetAddressAndPort address;

    private final transient int messagingVersion;

    private final transient BooleanSupplier abortRequested;

    private transient InputPosition currentPagePosition;
    ...
    private Action sendHintsAndAwait(HintsReader.Page page) {
        ...
        for (Callback cb : callbacks) {
            ...
            if (outcome == Callback.Outcome.SUCCESS)
                success++;
            else if (outcome == Callback.Outcome.FAILURE)
                failures++;
            else if (outcome == Callback.Outcome.TIMEOUT)
                timeouts++;
        }
    ...
    private static final class Callback implements RequestCallback {
        ...
        enum Outcome {
            ...
            SUCCESS, TIMEOUT, FAILURE, INTERRUPTED
        }

        private final transient long start = approxTime.now();

        private final transient SimpleCondition condition = new SimpleCondition();

        private volatile transient Outcome outcome;

        private final transient long hintCreationNanoTime;
