...
public class DatabaseDescriptor {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(DatabaseDescriptor.class);
    ...
    private static final transient Logger logger = LoggerFactory.getLogger(DatabaseDescriptor.class);

    /**
     * Tokens are serialized in a Gossip VersionedValue String.  VV are restricted to 64KB
     * when we send them over the wire, which works out to about 1700 tokens.
     */
    private static final transient int MAX_NUM_TOKENS = 1536;
     ...
     */
    static final transient long LOWEST_ACCEPTED_TIMEOUT = 10L;

    private static transient IEndpointSnitch snitch;

    // leave null so we can fall through to getLocalHost
    private static transient InetAddress listenAddress;

    private static transient InetAddress broadcastAddress;

    private static transient InetAddress rpcAddress;

    private static transient InetAddress broadcastRpcAddress;

    private static transient SeedProvider seedProvider;

    private static transient IInternodeAuthenticator internodeAuthenticator = new AllowAllInternodeAuthenticator();

    /* Hashing strategy Random or OPHF */
    private static transient IPartitioner partitioner;

    private static transient String paritionerName;

    private static transient Config.DiskAccessMode indexAccessMode;

    private static transient IAuthenticator authenticator;

    private static transient IAuthorizer authorizer;

    private static transient INetworkAuthorizer networkAuthorizer;

    // Don't initialize the role manager until applying config. The options supported by CassandraRoleManager
    // depend on the configured IAuthenticator, so defer creating it until that's been set.
    private static transient IRoleManager roleManager;

    private static transient long preparedStatementsCacheSizeInMB;

    private static transient long keyCacheSizeInMB;

    private static transient long counterCacheSizeInMB;

    private static transient long indexSummaryCapacityInMB;

    private static transient String localDC;

    private static transient Comparator<Replica> localComparator;

    private static transient EncryptionContext encryptionContext;

    private static transient boolean hasLoggedConfig;

    private static transient DiskOptimizationStrategy diskOptimizationStrategy;

    private static transient boolean clientInitialized;

    private static transient boolean toolInitialized;

    private static transient boolean daemonInitialized;

    private static final transient int searchConcurrencyFactor = Integer.parseInt(System.getProperty(Config.PROPERTY_PREFIX + "search_concurrency_factor", "1"));

    private static volatile transient boolean disableSTCSInL0 = Boolean.getBoolean(Config.PROPERTY_PREFIX + "disable_stcs_in_l0");

    private static final transient boolean unsafeSystem = Boolean.getBoolean(Config.PROPERTY_PREFIX + "unsafesystem");

    // turns some warnings into exceptions for testing
    private static final transient boolean strictRuntimeChecks = Boolean.getBoolean("cassandra.strict.runtime.checks");

    public static volatile transient boolean allowUnlimitedConcurrentValidations = Boolean.getBoolean("cassandra.allow_unlimited_concurrent_validations");

    private static transient Function<CommitLog, AbstractCommitLogSegmentManager> commitLogSegmentMgrProvider = c -> DatabaseDescriptor.isCDCEnabled() ? new CommitLogSegmentManagerCDC(c, DatabaseDescriptor.getCommitLogLocation()) : new CommitLogSegmentManagerStandard(c, DatabaseDescriptor.getCommitLogLocation());
    ...
    public static Config loadConfig() throws ConfigurationException {
        ...
        ConfigurationLoader loader = loaderClass == null ? new YamlConfigurationLoader() : FBUtilities.construct(loaderClass, "configuration loading");
    ...
    private static void applySimpleConfig() {
        ...
        if (conf.local_system_data_file_directory != null) {
            ...
            if (freeBytes < ONE_GB)
                logger.warn("Only {} free in the system data volume. Consider adding more capacity or removing obsolete snapshots", FBUtilities.prettyPrintMemory(freeBytes));
        }
    ...
    public static Collection<String> tokensFromString(String tokenString) {
        ...
        if (tokenString != null)
            for (String token : StringUtils.split(tokenString, ',')) tokens.add(token.trim());
        return tokens;
    }
    ...
    public static String[] getLocalSystemKeyspacesDataFileLocations() {
        ...
        return conf.data_file_directories.length == 0 ? conf.data_file_directories : new String[] { conf.data_file_directories[0] };
    }
    ...
    private enum ByteUnit {
        ...
        KIBI_BYTES(2048 * 1024, 1024), MEBI_BYTES(2048, 1024 * 1024);
