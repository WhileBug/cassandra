...
public class FullQueryLoggerOptionsCompositeData {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(FullQueryLoggerOptionsCompositeData.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(FullQueryLoggerOptionsCompositeData.class);

    private static final transient String[] ITEM_NAMES = new String[] { "log_dir", "archive_command", "roll_cycle", "block", "max_queue_weight", "max_log_size", "max_archive_retries" };

    private static final transient String[] ITEM_DESC = new String[] { "directory where FQL data are stored", "archive command for FQL data", "how often to roll BinLog segments so they can potentially be reclaimed", "indicates if the BinLog should block if the it falls behind or should drop bin log records", "maximum weight of in memory queue for records waiting to be written to the binlog file before blocking or dropping the log records", "maximum size of the rolled files to retain on disk before deleting the oldest file", "number of times to retry an archive command" };
