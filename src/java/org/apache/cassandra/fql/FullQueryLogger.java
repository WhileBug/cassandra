...
public class FullQueryLogger implements QueryEvents.Listener {
    ...
    public static class Batch extends AbstractLogEntry {
        ...
        public Batch(BatchStatement.Type batchType, List<String> queries, List<List<ByteBuffer>> values, QueryOptions queryOptions, QueryState queryState, long batchTimeMillis) {
            ...
            // cached weight
            weight += // queries + values lists
            Integer.BYTES + // batchType and two lists references
            2 * EMPTY_LIST_SIZE + 3 * OBJECT_REFERENCE_SIZE;
    ...
    private static abstract class AbstractLogEntry extends BinLog.ReleaseableWriteMarshallable implements WeightedQueue.Weighable {
        ...
        public int weight() {
            ...
            OBJECT_HEADER_SIZE + // protocolVersion
            Long.BYTES + Integer.BYTES + OBJECT_REFERENCE_SIZE + // queryOptionsBuffer
            EMPTY_BYTEBUF_SIZE + // generatedTimestamp
            queryOptionsBuffer.capacity() + // generatedNowInSeconds
            Long.BYTES + Integer.BYTES + // keyspace
            OBJECT_REFERENCE_SIZE + Ints.checkedCast(ObjectSizes.sizeOf(keyspace));
        }
    }
}
