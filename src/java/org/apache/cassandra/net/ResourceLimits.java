...
public abstract class ResourceLimits {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ResourceLimits.class);
    ...
    public static class Concurrent implements Limit {
        ...
        private volatile transient long limit;

        private static final transient AtomicLongFieldUpdater<Concurrent> limitUpdater = AtomicLongFieldUpdater.newUpdater(Concurrent.class, "limit");

        private volatile transient long using;

        private static final transient AtomicLongFieldUpdater<Concurrent> usingUpdater = AtomicLongFieldUpdater.newUpdater(Concurrent.class, "using");
        ...
        public Outcome release(long amount) {
            ...
            if (using < 0L) {
                ...
                throw new UnrecoverableIllegalStateException("Internode messaging byte limits that are shared between connections is invalid (using=" + using + ")");
            }
    ...
    public static class EndpointAndGlobal {
        ...
        final transient Limit endpoint;

        final transient Limit global;
    ...
    public enum Outcome {
        ...
        SUCCESS, INSUFFICIENT_ENDPOINT, INSUFFICIENT_GLOBAL, BELOW_LIMIT, ABOVE_LIMIT
    }
}
