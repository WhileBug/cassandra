...
public final class InboundMessageHandlers {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(InboundMessageHandlers.class);

    private final transient InetAddressAndPort self;

    private final transient InetAddressAndPort peer;

    private final transient int queueCapacity;

    private final transient ResourceLimits.Limit endpointReserveCapacity;

    private final transient ResourceLimits.Limit globalReserveCapacity;

    private final transient InboundMessageHandler.WaitQueue endpointWaitQueue;

    private final transient InboundMessageHandler.WaitQueue globalWaitQueue;

    private final transient InboundCounters urgentCounters = new InboundCounters();

    private final transient InboundCounters smallCounters = new InboundCounters();

    private final transient InboundCounters largeCounters = new InboundCounters();

    private final transient InboundCounters legacyCounters = new InboundCounters();

    private final transient InboundMessageCallbacks urgentCallbacks;

    private final transient InboundMessageCallbacks smallCallbacks;

    private final transient InboundMessageCallbacks largeCallbacks;

    private final transient InboundMessageCallbacks legacyCallbacks;

    private final transient InternodeInboundMetrics metrics;

    private final transient MessageConsumer messageConsumer;

    private final transient HandlerProvider handlerProvider;

    private final transient Collection<InboundMessageHandler> handlers = new CopyOnWriteArrayList<>();

    static class GlobalResourceLimits {

        final transient ResourceLimits.Limit reserveCapacity;

        final transient InboundMessageHandler.WaitQueue waitQueue;
     ...
     */
    private volatile transient long closedReceivedCount, closedReceivedBytes;

    private static final transient AtomicLongFieldUpdater<InboundMessageHandlers> closedReceivedCountUpdater = AtomicLongFieldUpdater.newUpdater(InboundMessageHandlers.class, "closedReceivedCount");

    private static final transient AtomicLongFieldUpdater<InboundMessageHandlers> closedReceivedBytesUpdater = AtomicLongFieldUpdater.newUpdater(InboundMessageHandlers.class, "closedReceivedBytes");

    private volatile transient long closedThrottledCount, closedThrottledNanos;

    private static final transient AtomicLongFieldUpdater<InboundMessageHandlers> closedThrottledCountUpdater = AtomicLongFieldUpdater.newUpdater(InboundMessageHandlers.class, "closedThrottledCount");

    private static final transient AtomicLongFieldUpdater<InboundMessageHandlers> closedThrottledNanosUpdater = AtomicLongFieldUpdater.newUpdater(InboundMessageHandlers.class, "closedThrottledNanos");

    private volatile transient long closedCorruptFramesRecovered, closedCorruptFramesUnrecovered;

    private static final transient AtomicLongFieldUpdater<InboundMessageHandlers> closedCorruptFramesRecoveredUpdater = AtomicLongFieldUpdater.newUpdater(InboundMessageHandlers.class, "closedCorruptFramesRecovered");

    private static final transient AtomicLongFieldUpdater<InboundMessageHandlers> closedCorruptFramesUnrecoveredUpdater = AtomicLongFieldUpdater.newUpdater(InboundMessageHandlers.class, "closedCorruptFramesUnrecovered");
    ...
    interface HandlerProvider {
        ...
        InboundMessageHandler provide(FrameDecoder decoder, ConnectionType type, Channel channel, InetAddressAndPort self, InetAddressAndPort peer, int version, int largeMessageThreshold, int queueCapacity, ResourceLimits.Limit endpointReserveCapacity, ResourceLimits.Limit globalReserveCapacity, InboundMessageHandler.WaitQueue endpointWaitQueue, InboundMessageHandler.WaitQueue globalWaitQueue, InboundMessageHandler.OnHandlerClosed onClosed, InboundMessageCallbacks callbacks, Consumer<Message<?>> consumer);
    }
}
