...
public class OutboundConnections {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(OutboundConnections.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(OutboundConnections.class);

    private static final transient Logger logger = LoggerFactory.getLogger(OutboundConnections.class);

    @VisibleForTesting
    public static final transient int LARGE_MESSAGE_THRESHOLD = Integer.getInteger(Config.PROPERTY_PREFIX + "otcp_large_message_threshold", 1024 * 64) - Math.max(Math.max(LegacyLZ4Constants.HEADER_LENGTH, FrameEncoderCrc.HEADER_AND_TRAILER_LENGTH), FrameEncoderLZ4.HEADER_AND_TRAILER_LENGTH);

    private final transient SimpleCondition metricsReady = new SimpleCondition();

    private volatile transient InternodeOutboundMetrics metrics;

    private final transient ResourceLimits.Limit reserveCapacity;

    private transient OutboundConnectionSettings template;

    public final transient OutboundConnection small;

    public final transient OutboundConnection large;

    public final transient OutboundConnection urgent;
    ...
    synchronized Future<Void> scheduleClose(long time, TimeUnit unit, boolean flushQueues) {
        ...
        releaseMetrics();
        return new FutureCombiner(apply(c -> c.scheduleClose(time, unit, flushQueues)));
    }
    ...
    synchronized Future<Void> close(boolean flushQueues) {
        // immediately release our metrics, so that if we need to re-open immediately we can safely register a new one
        releaseMetrics();
        return new FutureCombiner(apply(c -> c.close(flushQueues)));
    }
    ...
    private static ConnectionType connectionTypeFor(Message<?> msg, ConnectionType specifyConnection) {
        ...
        if (msg.serializedSize(current_version) > LARGE_MESSAGE_THRESHOLD) {
            if (msg.verb().priority == Verb.Priority.P0) {
                NoSpamLogger.log(logger, NoSpamLogger.Level.WARN, 1, TimeUnit.MINUTES, "Enqueued URGENT message which exceeds large message threshold");
                if (logger.isTraceEnabled())
                    logger.trace("{} message with size {} exceeded large message threshold {}", msg.verb(), msg.serializedSize(current_version), LARGE_MESSAGE_THRESHOLD);
            }
            return LARGE_MESSAGES;
        }
        return msg.verb().priority == Verb.Priority.P0 ? URGENT_MESSAGES : SMALL_MESSAGES;
    }
    ...
    private static class UnusedConnectionMonitor {
        ...
        static class Counts {
            ...
            final transient long small, large, urgent;
        ...
        final transient MessagingService messagingService;

        transient ObjectObjectHashMap<InetAddressAndPort, Counts> prevEndpointToCounts = new ObjectObjectHashMap<>();
        ...
        private void closeUnusedSinceLastRun() {
            ...
            for (OutboundConnections connections : messagingService.channelManagers.values()) {
                ...
                if (cur.small != prev.small && cur.large != prev.large && cur.urgent != prev.urgent)
                    continue;
                if (cur.small == prev.small && cur.large == prev.large && cur.urgent == prev.urgent && !Gossiper.instance.isKnownEndpoint(connections.template.to)) {
                    logger.info("Closing outbound connections to {}, as inactive and not known by Gossiper", connections.template.to);
