...
public class InboundMessageHandler extends AbstractMessageHandler {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(InboundMessageHandler.class);

    private static final transient Logger logger = LoggerFactory.getLogger(InboundMessageHandler.class);

    private static final transient NoSpamLogger noSpamLogger = NoSpamLogger.getLogger(logger, 1L, TimeUnit.SECONDS);

    private static final transient Message.Serializer serializer = Message.serializer;
    ...
    private final transient InboundMessageCallbacks callbacks;

    private final transient Consumer<Message<?>> consumer;
    ...
    protected void processCorruptFrame(CorruptFrame frame) throws Crc.InvalidCrc {
        ...
        } else if (// first frame of a large message
        null == largeMessage) {
            receivedBytes += frame.frameSize;
            corruptFramesUnrecovered++;
    ...
    String id(boolean includeReal) {
        if (!includeReal)
            return id();
        return SocketFactory.channelId(peer, (InetSocketAddress) channel.remoteAddress(), self, (InetSocketAddress) channel.localAddress(), type, channel.id().asShortText());
    }
    ...
    private class LargeMessage extends AbstractMessageHandler.LargeMessage<Header> {
        ...
        private Message deserialize() {
            try (ChunkedInputPlus input = ChunkedInputPlus.of(buffers)) {
                ...
                // closing the input will have ensured that the buffers were released no matter what
                buffers.clear();
            }
            return null;
        }
    ...
    private class ProcessLargeMessage extends ProcessMessage {
        ...
        private final transient LargeMessage message;
        ...
        @Override
        void releaseResources() {
            // releases buffers if they haven't been yet (by deserialize() call)
            message.releaseBuffers();
        }
    }
}
