...
public class OutboundConnectionSettings {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(OutboundConnectionSettings.class);

    private static final transient String INTRADC_TCP_NODELAY_PROPERTY = Config.PROPERTY_PREFIX + "otc_intradc_tcp_nodelay";

    /**
     * Enabled/disable TCP_NODELAY for intradc connections. Defaults to enabled.
     */
    private static final transient boolean INTRADC_TCP_NODELAY = Boolean.parseBoolean(System.getProperty(INTRADC_TCP_NODELAY_PROPERTY, "true"));
    ...
    public final transient IInternodeAuthenticator authenticator;

    public final transient InetAddressAndPort to;

    // may be represented by a different IP address on this node's local network
    public final transient InetAddressAndPort connectTo;

    public final transient EncryptionOptions encryption;

    public final transient Framing framing;

    public final transient Integer socketSendBufferSizeInBytes;

    public final transient Integer applicationSendQueueCapacityInBytes;

    public final transient Integer applicationSendQueueReserveEndpointCapacityInBytes;

    public final transient ResourceLimits.Limit applicationSendQueueReserveGlobalCapacityInBytes;

    public final transient Boolean tcpNoDelay;

    public final transient int flushLowWaterMark, flushHighWaterMark;

    public final transient Integer tcpConnectTimeoutInMS;

    public final transient Integer tcpUserTimeoutInMS;

    public final transient AcceptVersions acceptVersions;

    public final transient InetAddressAndPort from;

    public final transient SocketFactory socketFactory;

    public final transient OutboundMessageCallbacks callbacks;

    public final transient OutboundDebugCallbacks debug;

    public final transient EndpointMessagingVersions endpointToVersion;
    ...
    public String toString() {
        return String.format("peer: (%s, %s), framing: %s, encryption: %s", to, connectTo, framing, SocketFactory.encryptionOptionsSummary(encryption));
    }

    public OutboundConnectionSettings withAuthenticator(IInternodeAuthenticator authenticator) {
        return new OutboundConnectionSettings(authenticator, to, connectTo, encryption, framing, socketSendBufferSizeInBytes, applicationSendQueueCapacityInBytes, applicationSendQueueReserveEndpointCapacityInBytes, applicationSendQueueReserveGlobalCapacityInBytes, tcpNoDelay, flushLowWaterMark, flushHighWaterMark, tcpConnectTimeoutInMS, tcpUserTimeoutInMS, acceptVersions, from, socketFactory, callbacks, debug, endpointToVersion);
    }

    @SuppressWarnings("unused")
    public OutboundConnectionSettings toEndpoint(InetAddressAndPort endpoint) {
        return new OutboundConnectionSettings(authenticator, endpoint, connectTo, encryption, framing, socketSendBufferSizeInBytes, applicationSendQueueCapacityInBytes, applicationSendQueueReserveEndpointCapacityInBytes, applicationSendQueueReserveGlobalCapacityInBytes, tcpNoDelay, flushLowWaterMark, flushHighWaterMark, tcpConnectTimeoutInMS, tcpUserTimeoutInMS, acceptVersions, from, socketFactory, callbacks, debug, endpointToVersion);
    }
    ...
    public OutboundConnectionSettings withApplicationReserveSendQueueCapacityInBytes(Integer applicationReserveSendQueueEndpointCapacityInBytes, ResourceLimits.Limit applicationReserveSendQueueGlobalCapacityInBytes) {
        return new OutboundConnectionSettings(authenticator, to, connectTo, encryption, framing, socketSendBufferSizeInBytes, applicationSendQueueCapacityInBytes, applicationReserveSendQueueEndpointCapacityInBytes, applicationReserveSendQueueGlobalCapacityInBytes, tcpNoDelay, flushLowWaterMark, flushHighWaterMark, tcpConnectTimeoutInMS, tcpUserTimeoutInMS, acceptVersions, from, socketFactory, callbacks, debug, endpointToVersion);
    }
    ...
    public int socketSendBufferSizeInBytes() {
        return socketSendBufferSizeInBytes != null ? socketSendBufferSizeInBytes : DatabaseDescriptor.getInternodeSocketSendBufferSizeInBytes();
    }

    public int applicationSendQueueCapacityInBytes() {
        return applicationSendQueueCapacityInBytes != null ? applicationSendQueueCapacityInBytes : DatabaseDescriptor.getInternodeApplicationSendQueueCapacityInBytes();
    }

    public ResourceLimits.Limit applicationSendQueueReserveGlobalCapacityInBytes() {
        return applicationSendQueueReserveGlobalCapacityInBytes != null ? applicationSendQueueReserveGlobalCapacityInBytes : instance().outboundGlobalReserveLimit;
    }

    public int applicationSendQueueReserveEndpointCapacityInBytes() {
        return applicationSendQueueReserveEndpointCapacityInBytes != null ? applicationSendQueueReserveEndpointCapacityInBytes : DatabaseDescriptor.getInternodeApplicationReceiveQueueReserveEndpointCapacityInBytes();
    }

    public int tcpConnectTimeoutInMS() {
        return tcpConnectTimeoutInMS != null ? tcpConnectTimeoutInMS : DatabaseDescriptor.getInternodeTcpConnectTimeoutInMS();
    }
    ...
    public AcceptVersions acceptVersions(ConnectionCategory category) {
        return acceptVersions != null ? acceptVersions : category.isStreaming() ? MessagingService.accept_streaming : MessagingService.accept_messaging;
    }
    ...
    public String connectToId() {
        return !to.equals(connectTo()) ? to.toString() : to.toString() + '(' + connectTo().toString() + ')';
    }
    ...
    public Framing framing(ConnectionCategory category) {
        ...
        return shouldCompressConnection(getEndpointSnitch(), getBroadcastAddressAndPort(), to) ? Framing.LZ4 : Framing.CRC;
    }
    ...
    public OutboundConnectionSettings withDefaults(ConnectionCategory category) {
        ...
        return new OutboundConnectionSettings(authenticator(), to, connectTo(), encryption(), framing(category), socketSendBufferSizeInBytes(), applicationSendQueueCapacityInBytes(), applicationSendQueueReserveEndpointCapacityInBytes(), applicationSendQueueReserveGlobalCapacityInBytes(), tcpNoDelay(), flushLowWaterMark, flushHighWaterMark, tcpConnectTimeoutInMS(), tcpUserTimeoutInMS(category), acceptVersions(category), from(), socketFactory(), callbacks(), debug(), endpointToVersion());
    }
    ...
    static boolean shouldCompressConnection(IEndpointSnitch snitch, InetAddressAndPort localHost, InetAddressAndPort remoteHost) {
        return (DatabaseDescriptor.internodeCompression() == Config.InternodeCompression.all) || ((DatabaseDescriptor.internodeCompression() == Config.InternodeCompression.dc) && !isInLocalDC(snitch, localHost, remoteHost));
    }
