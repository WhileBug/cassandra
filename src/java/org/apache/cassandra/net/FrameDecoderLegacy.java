...
class FrameDecoderLegacy extends FrameDecoder {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(FrameDecoderLegacy.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(FrameDecoderLegacy.class);

    private final transient int messagingVersion;

    private transient int remainingBytesInLargeMessage = 0;
        ...
        try {
            if (stash != null) {
                ...
                if (length <= LARGE_MESSAGE_THRESHOLD) {
                    ...
                    stash.limit(length);
                    // we may be over capacity from earlier doubling
