...
class FrameDecoderLegacyLZ4 extends FrameDecoderLegacy {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(FrameDecoderLegacyLZ4.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(FrameDecoderLegacyLZ4.class);

    private static final transient BufferPool bufferPool = BufferPools.forNetworking();
    ...
    private static class LZ4Decoder extends ChannelInboundHandlerAdapter {
        ...
        private static final transient XXHash32 xxhash = XXHashFactory.fastestInstance().hash32();

        private static final transient LZ4SafeDecompressor decompressor = LZ4Factory.fastestInstance().safeDecompressor();

        private final transient BufferPoolAllocator allocator;

        LZ4Decoder(BufferPoolAllocator allocator) {
            this.allocator = allocator;
        }

        private final transient Deque<ShareableBytes> frames = new ArrayDeque<>(4);

        // total # of frames decoded between two subsequent invocations of channelReadComplete()
        private transient int decodedFrameCount = 0;
        ...
        public void channelRead(ChannelHandlerContext ctx, Object msg) throws CorruptLZ4Frame {
            ...
            try {
                ...
                while (!frames.isEmpty()) ctx.fireChannelRead(frames.poll());
            }
            if (null != error)
                throw error;
        }
        ...
        private static final class Header {
            ...
            transient long magicNumber;

            transient byte token;

            transient int compressedLength;

            transient int uncompressedLength;

            transient int checksum;
            ...
            void validate() throws CorruptLZ4Frame {
                ...
                if (uncompressedLength == 0 && compressedLength != 0 || uncompressedLength != 0 && compressedLength == 0 || !isCompressed() && uncompressedLength != compressedLength) {
                    except("Stream corrupted: compressedLength(%d) and decompressedLength(%d) mismatch", compressedLength, uncompressedLength);
                }
            }
        }

        private final transient Header header = new Header();
        ...
        private transient ByteBuffer stash;
