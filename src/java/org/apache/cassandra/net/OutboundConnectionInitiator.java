...
public class OutboundConnectionInitiator<SuccessType extends OutboundConnectionInitiator.Result.Success> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(OutboundConnectionInitiator.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(OutboundConnectionInitiator.class);

    private static final transient Logger logger = LoggerFactory.getLogger(OutboundConnectionInitiator.class);

    private final transient ConnectionType type;

    private final transient OutboundConnectionSettings settings;

    // for pre40 nodes
    private final transient int requestMessagingVersion;

    private final transient Promise<Result<SuccessType>> resultPromise;
    ...
    private class Handler extends ByteToMessageDecoder {
        ...
        public void channelActive(final ChannelHandlerContext ctx) {
            ...
            AsyncChannelPromise.writeAndFlush(ctx, msg.encode(), future -> {
                if (!future.isSuccess())
        ...
        protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
            try {
                ...
                if (useMessagingVersion > 0) {
                    ...
                    if (// this clause is for impersonating 3.0 node in testing only
                    peerMessagingVersion == requestMessagingVersion || peerMessagingVersion > settings.acceptVersions.max) {
                        switch(settings.framing) {
                            case CRC:
                            case UNPROTECTED:
    ...
    public static class Result<SuccessType extends Result.Success> {
        ...
        public static class Success<SuccessType extends Success> extends Result<SuccessType> {
            ...
            public final transient int messagingVersion;
        ...
        public static class MessagingSuccess extends Success<MessagingSuccess> {
            ...
            public final transient FrameEncoder.PayloadAllocator allocator;
        ...
        static class Retry<SuccessType extends Success> extends Result<SuccessType> {
            ...
            final transient int withMessagingVersion;
        ...
        static class Incompatible<SuccessType extends Success> extends Result<SuccessType> {
            ...
            final transient int closestSupportedVersion;
        ...
        final transient Outcome outcome;
