...
public class Message<T> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Message.class);

    private static final transient Logger logger = LoggerFactory.getLogger(Message.class);

    private static final transient NoSpamLogger noSpam1m = NoSpamLogger.getLogger(logger, 1, TimeUnit.MINUTES);

    public final transient Header header;

    public final transient T payload;
    ...
    private static final transient EnumMap<ParamType, Object> NO_PARAMS = new EnumMap<>(ParamType.class);
    ...
    private static final transient long NO_ID = 0L;

    private static final transient AtomicInteger nextId = new AtomicInteger(0);
     ...
     */
    static final transient int PROTOCOL_MAGIC = 0xCA552DFA;
    ...
    public static class Header {
        ...
        public final transient Verb verb;

        public final transient InetAddressAndPort from;

        public final transient long createdAtNanos;

        public final transient long expiresAtNanos;

        private final transient int flags;

        private final transient Map<ParamType, Object> params;
    ...
    public static class Builder<T> {
        ...
        private transient Verb verb;

        private transient InetAddressAndPort from;

        private transient T payload;

        private transient int flags = 0;

        private final transient Map<ParamType, Object> params = new EnumMap<>(ParamType.class);

        private transient long createdAtNanos;

        private transient long expiresAtNanos;

        private transient long id;

        private transient boolean hasId;
        ...
        public Builder<T> ofVerb(Verb verb) {
            ...
            if (expiresAtNanos == 0 && verb != null && createdAtNanos != 0)
                expiresAtNanos = verb.expiresAtNanos(createdAtNanos);
            if (// default to sending from self if we're a request verb
            !this.verb.isResponse() && from == null)
    ...
    public static final class Serializer {
        ...
        private static final transient int CREATION_TIME_SIZE = 4;
        ...
        private <T> Message<T> deserializePost40(DataInputPlus in, InetAddressAndPort peer, int version) throws IOException {
            ...
            // payload size, not needed by payload deserializer
            skipUnsignedVInt(in);
        ...
        // protocol magic + id + createdAt
        private static final transient int PRE_40_MESSAGE_PREFIX_SIZE = 12;

        private void skipHeaderPre40(DataInputPlus in) throws IOException {
            // magic, id, createdAt
            in.skipBytesFully(PRE_40_MESSAGE_PREFIX_SIZE);
            // from
         ...
         */
        private static final transient long TIMESTAMP_WRAPAROUND_GRACE_PERIOD_START = 0xFFFFFFFFL - MINUTES.toMillis(15L);

        private static final transient long TIMESTAMP_WRAPAROUND_GRACE_PERIOD_END = MINUTES.toMillis(15L);
        ...
        static long calculateCreationTimeNanos(int messageTimestampMillis, MonotonicClockTranslation timeSnapshot, long currentTimeNanos) {
            ...
            } else // if the message timestamp wrapped, but we still haven't, add one highBit
        ...
        private Map<ParamType, Object> deserializeParams(DataInputPlus in, int version) throws IOException {
            ...
            for (int i = 0; i < count; i++) {
                ParamType type = version >= VERSION_40 ? ParamType.lookUpById(Ints.checkedCast(in.readUnsignedVInt())) : ParamType.lookUpByAlias(in.readUTF());
                int length = version >= VERSION_40 ? Ints.checkedCast(in.readUnsignedVInt()) : in.readInt();
                if (null != type) {
                    ...
                    // forward compatibiliy with minor version changes
        ...
        private long serializedParamsSize(Map<ParamType, Object> params, int version) {
            long size = version >= VERSION_40 ? computeUnsignedVIntSize(params.size()) : sizeof(params.size());
            for (Map.Entry<ParamType, Object> kv : params.entrySet()) {
                ParamType type = kv.getKey();
                Object value = kv.getValue();
        ...
        private <T> int payloadSize(Message<T> message, int version) {
            long payloadSize = message.payload != null && message.payload != NoPayload.noPayload ? message.getPayloadSerializer().serializedSize(message.payload, version) : 0;
    ...
    private static <In, Out> IVersionedAsymmetricSerializer<In, Out> getPayloadSerializer(Verb verb, long id, InetAddressAndPort from) {
        return null != verb.serializer() ? verb.serializer() : instance().callbacks.responseSerializer(id, from);
    }
    ...
    private transient int payloadSize30 = -1;

    private transient int payloadSize3014 = -1;

    private transient int payloadSize40 = -1;
