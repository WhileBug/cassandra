...
public class InboundConnectionSettings {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(InboundConnectionSettings.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(InboundConnectionSettings.class);

    public final transient IInternodeAuthenticator authenticator;

    public final transient InetAddressAndPort bindAddress;

    public final transient ServerEncryptionOptions encryption;

    public final transient Integer socketReceiveBufferSizeInBytes;

    public final transient Integer applicationReceiveQueueCapacityInBytes;

    public final transient AcceptVersions acceptMessaging;

    public final transient AcceptVersions acceptStreaming;

    public final transient SocketFactory socketFactory;

    public final transient Function<InetAddressAndPort, InboundMessageHandlers> handlers;

    private InboundConnectionSettings(IInternodeAuthenticator authenticator, InetAddressAndPort bindAddress, ServerEncryptionOptions encryption, Integer socketReceiveBufferSizeInBytes, Integer applicationReceiveQueueCapacityInBytes, AcceptVersions acceptMessaging, AcceptVersions acceptStreaming, SocketFactory socketFactory, Function<InetAddressAndPort, InboundMessageHandlers> handlers) {
    ...
    public String toString() {
        return format("address: (%s), nic: %s, encryption: %s", bindAddress, FBUtilities.getNetworkInterface(bindAddress.address), SocketFactory.encryptionOptionsSummary(encryption));
    }

    public InboundConnectionSettings withAuthenticator(IInternodeAuthenticator authenticator) {
        return new InboundConnectionSettings(authenticator, bindAddress, encryption, socketReceiveBufferSizeInBytes, applicationReceiveQueueCapacityInBytes, acceptMessaging, acceptStreaming, socketFactory, handlers);
    }
    ...
    public InboundConnectionSettings withDefaults() {
        ...
        if (bindAddress.port != DatabaseDescriptor.getStoragePort() && bindAddress.port != DatabaseDescriptor.getSSLStoragePort())
            throw new ConfigurationException(format("Local endpoint port %d doesn't match YAML configured port %d or legacy SSL port %d", bindAddress.port, DatabaseDescriptor.getStoragePort(), DatabaseDescriptor.getSSLStoragePort()));
