...
public interface CQL3Type {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CQL3Type.class);

    static final transient Logger logger = LoggerFactory.getLogger(CQL3Type.class);
    ...
    public abstract class Raw {
        ...
        protected final transient boolean frozen;
        ...
        private static class RawCollection extends Raw {
            ...
            private final transient CollectionType.Kind kind;

            private final transient CQL3Type.Raw keys;

            private final transient CQL3Type.Raw values;
            ...
            public RawCollection freeze() {
                CQL3Type.Raw frozenKeys = null != keys && keys.supportsFreezing() ? keys.freeze() : keys;
                CQL3Type.Raw frozenValues = null != values && values.supportsFreezing() ? values.freeze() : values;
            ...
            public String toString() {
                ...
                switch(kind) {
                    case LIST:
                        return start + "list<" + values + '>' + end;
                    case SET:
                        return start + "set<" + values + '>' + end;
                    case MAP:
                        return start + "map<" + keys + ", " + values + '>' + end;
                }
                throw new AssertionError();
            }
        }
            ...
            public CQL3Type prepare(String keyspace, Types udts) throws InvalidRequestException {
                if (name.hasKeyspace()) {
                    ...
                    if (!keyspace.equals(name.getKeyspace()))
                        throw new InvalidRequestException(String.format("Statement on keyspace %s cannot refer to a user type in keyspace %s; " + "user types can only be used in the keyspace they are defined in", keyspace, name.getKeyspace()));
                } else {
        ...
        private static class RawTuple extends Raw {
            ...
            private final transient List<CQL3Type.Raw> types;

            private RawTuple(List<CQL3Type.Raw> types) {
                super(true);
                this.types = types.stream().map(t -> t.supportsFreezing() ? t.freeze() : t).collect(toList());
            }
