...
public final class Duration {
    ...
    public static final transient long NANOS_PER_MICRO = 1000L;

    public static final transient long NANOS_PER_MILLI = 1000 * NANOS_PER_MICRO;

    public static final transient long NANOS_PER_SECOND = 1000 * NANOS_PER_MILLI;

    public static final transient long NANOS_PER_MINUTE = 60 * NANOS_PER_SECOND;

    public static final transient long NANOS_PER_HOUR = 60 * NANOS_PER_MINUTE;

    public static final transient int DAYS_PER_WEEK = 7;

    public static final transient int MONTHS_PER_YEAR = 12;
    ...
    private Duration(int months, int days, long nanoseconds) {
        ...
        assert (months >= 0 && days >= 0 && nanoseconds >= 0) || ((months <= 0 && days <= 0 && nanoseconds <= 0));
        this.months = months;
        this.days = days;
    ...
    private static class Builder {
        ...
        private void validate(long units, long limit, String unitName) {
            checkTrue(units <= limit, "Invalid duration. The total number of %s must be less or equal to %s", unitName, Integer.MAX_VALUE);
        }
        ...
        private void validateOrder(int unitIndex) {
            ...
            if (unitIndex <= currentUnitIndex)
                throw invalidRequest("Invalid duration. The %s should be after %s", getUnitName(currentUnitIndex), getUnitName(unitIndex));
