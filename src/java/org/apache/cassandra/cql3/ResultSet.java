...
public class ResultSet {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ResultSet.class);

    public static final transient Codec codec = new Codec();

    public final transient ResultMetadata metadata;

    public final transient List<List<ByteBuffer>> rows;
    ...
    public static class Codec implements CBCodec<ResultSet> {
        ...
        public int encodedSize(ResultSet rs, ProtocolVersion version) {
            ...
            for (List<ByteBuffer> row : rs.rows) {
                for (int i = 0; i < rs.metadata.columnCount; i++) size += CBUtil.sizeOfValue(row.get(i));
            }
            return size;
        }
    }
    ...
    public static class ResultMetadata {
        ...
        public static final transient CBCodec<ResultMetadata> codec = new Codec();

        public static final transient ResultMetadata EMPTY = new ResultMetadata(MD5Digest.compute(new byte[0]), EnumSet.of(Flag.NO_METADATA), null, 0, null);

        private final transient EnumSet<Flag> flags;
        ...
        public final transient List<ColumnSpecification> names;

        private final transient int columnCount;

        private transient PagingState pagingState;

        private final transient MD5Digest resultMetadataId;
        ...
        public boolean equals(Object other) {
            ...
            return Objects.equals(flags, that.flags) && Objects.equals(names, that.names) && columnCount == that.columnCount && Objects.equals(pagingState, that.pagingState);
        }
    ...
    public static class PreparedMetadata {
        ...
        public static final transient CBCodec<PreparedMetadata> codec = new Codec();

        private final transient EnumSet<Flag> flags;

        public final transient List<ColumnSpecification> names;

        private final transient short[] partitionKeyBindIndexes;
