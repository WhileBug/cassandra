...
public final class CreateIndexStatement extends AlterSchemaStatement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CreateIndexStatement.class);
    ...
    private final transient List<IndexTarget.Raw> rawIndexTargets;

    private final transient IndexAttributes attrs;
    ...
    private void validateIndexTarget(TableMetadata table, IndexTarget target) {
        ...
        if (column.type.isFrozenCollection() && target.type != Type.FULL)
            throw ire("Cannot create %s() index on frozen column %s. Frozen collections are immutable and must be fully " + "indexed by using the 'full(%s)' modifier", target.type, column, column);
        if (!column.type.isFrozenCollection() && target.type == Type.FULL)
    ...
    public static final class Raw extends CQLStatement.Raw {
        ...
        private final transient QualifiedName indexName;
        ...
        public CreateIndexStatement prepare(ClientState state) {
            String keyspaceName = tableName.hasKeyspace() ? tableName.getKeyspace() : indexName.hasKeyspace() ? indexName.getKeyspace() : state.getKeyspace();
