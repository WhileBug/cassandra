...
public final class AlterKeyspaceStatement extends AlterSchemaStatement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AlterKeyspaceStatement.class);

    private static final transient boolean allow_alter_rf_during_range_movement = Boolean.getBoolean(Config.PROPERTY_PREFIX + "allow_alter_rf_during_range_movement");

    private static final transient boolean allow_unsafe_transient_changes = Boolean.getBoolean(Config.PROPERTY_PREFIX + "allow_unsafe_transient_changes");
    ...
    private void validateNoRangeMovements() {
        if (allow_alter_rf_during_range_movement)
            return;
        Stream<InetAddressAndPort> unreachableNotAdministrativelyInactive = Gossiper.instance.getUnreachableMembers().stream().filter(endpoint -> !FBUtilities.getBroadcastAddressAndPort().equals(endpoint) && !Gossiper.instance.isAdministrativelyInactiveState(endpoint));
