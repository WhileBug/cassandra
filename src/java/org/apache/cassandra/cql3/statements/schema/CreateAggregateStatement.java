...
public final class CreateAggregateStatement extends AlterSchemaStatement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CreateAggregateStatement.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CreateAggregateStatement.class);

    private final transient String aggregateName;

    private final transient List<CQL3Type.Raw> rawArgumentTypes;

    private final transient CQL3Type.Raw rawStateType;

    private final transient FunctionName stateFunctionName;

    private final transient FunctionName finalFunctionName;

    private final transient Term.Raw rawInitialValue;
    ...
    SchemaChange schemaChangeEvent(KeyspacesDiff diff) {
        ...
        return new SchemaChange(created ? Change.CREATED : Change.UPDATED, Target.AGGREGATE, keyspaceName, aggregateName, rawArgumentTypes.stream().map(CQL3Type.Raw::toString).collect(toList()));
    }
    ...
    Set<IResource> createdResources(KeyspacesDiff diff) {
        ...
        FunctionsDiff<UDAggregate> udasDiff = diff.altered.get(0).udas;
        assert udasDiff.created.size() + udasDiff.altered.size() == 1;
        return udasDiff.created.isEmpty() ? ImmutableSet.of() : ImmutableSet.of(FunctionResource.functionFromCql(keyspaceName, aggregateName, rawArgumentTypes));
    }
    ...
    public static final class Raw extends CQLStatement.Raw {
        ...
        private final transient FunctionName aggregateName;

        private final transient List<CQL3Type.Raw> rawArgumentTypes;

        private final transient CQL3Type.Raw rawStateType;

        private final transient String stateFunctionName;

        private final transient String finalFunctionName;
        ...
        public CreateAggregateStatement prepare(ClientState state) {
            ...
            return new CreateAggregateStatement(keyspaceName, aggregateName.name, rawArgumentTypes, rawStateType, new FunctionName(keyspaceName, stateFunctionName), null != finalFunctionName ? new FunctionName(keyspaceName, finalFunctionName) : null, rawInitialValue, orReplace, ifNotExists);
        }
    }
}
