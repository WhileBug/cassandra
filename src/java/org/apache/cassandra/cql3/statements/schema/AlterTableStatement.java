...
public abstract class AlterTableStatement extends AlterSchemaStatement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AlterTableStatement.class);
    ...
    private static class AddColumns extends AlterTableStatement {
        ...
        private static class Column {
            ...
            private final transient ColumnIdentifier name;

            private final transient CQL3Type.Raw type;
        ...
        private final transient Collection<Column> newColumns;
        ...
        private void addColumn(KeyspaceMetadata keyspace, TableMetadata table, Column column, TableMetadata.Builder tableBuilder, Views.Builder viewsBuilder) {
            ...
            if (null != droppedColumn) {
                ...
                if (!type.isValueCompatibleWith(droppedColumn.type)) {
                    throw ire("Cannot re-add previously dropped column '%s' of type %s, incompatible with previous type %s", name, type.asCQL3Type(), droppedColumn.type.asCQL3Type());
                }
                if (droppedColumn.isStatic() != isStatic) {
                    throw ire("Cannot re-add previously dropped column '%s' of kind %s, incompatible with previous kind %s", name, isStatic ? ColumnMetadata.Kind.STATIC : ColumnMetadata.Kind.REGULAR, droppedColumn.kind);
                }
    ...
    private static class DropColumns extends AlterTableStatement {
        ...
        private final transient Set<ColumnIdentifier> removedColumns;
        ...
        private void dropColumn(KeyspaceMetadata keyspace, TableMetadata table, ColumnIdentifier column, TableMetadata.Builder builder) {
            ...
            if (!dependentIndexes.isEmpty()) {
                throw ire("Cannot drop column %s because it has dependent secondary indexes (%s)", currentColumn, join(", ", transform(dependentIndexes, i -> i.name)));
            }
    ...
    private static class RenameColumns extends AlterTableStatement {
        ...
        private final transient Map<ColumnIdentifier, ColumnIdentifier> renamedColumns;
        ...
        private void renameColumn(KeyspaceMetadata keyspace, TableMetadata table, ColumnIdentifier oldName, ColumnIdentifier newName, TableMetadata.Builder tableBuilder, Views.Builder viewsBuilder) {
            ...
            if (null != table.getColumn(newName)) {
                throw ire("Cannot rename column %s to %s in table '%s'; another column with that name already exists", oldName, newName, table);
            }
            // TODO: some day try and find a way to not rely on Keyspace/IndexManager/Index to find dependent indexes
            Set<IndexMetadata> dependentIndexes = Keyspace.openAndGetStore(table).indexManager.getDependentIndexes(column);
            if (!dependentIndexes.isEmpty()) {
                throw ire("Can't rename column %s because it has dependent secondary indexes (%s)", oldName, join(", ", transform(dependentIndexes, i -> i.name)));
            }
            for (ViewMetadata view : keyspace.views.forTable(table.id)) {
    ...
    private static class AlterOptions extends AlterTableStatement {
        ...
        public KeyspaceMetadata apply(KeyspaceMetadata keyspace, TableMetadata table) {
            ...
            if (!isEmpty(keyspace.views.forTable(table.id)) && params.gcGraceSeconds == 0) {
                throw ire("Cannot alter gc_grace_seconds of the base table of a " + "materialized view to 0, since this value is used to TTL " + "undelivered updates. Setting gc_grace_seconds too low might " + "cause undelivered updates to expire " + "before being replayed.");
            }
    ...
    private static class DropCompactStorage extends AlterTableStatement {
        ...
        private static final transient Logger logger = LoggerFactory.getLogger(AlterTableStatement.class);

        private static final transient NoSpamLogger noSpamLogger = NoSpamLogger.getLogger(logger, 5L, TimeUnit.MINUTES);
        ...
        public KeyspaceMetadata apply(KeyspaceMetadata keyspace, TableMetadata table) {
            ...
            Set<Flag> flags = table.isCounter() ? ImmutableSet.of(Flag.COMPOUND, Flag.COUNTER) : ImmutableSet.of(Flag.COMPOUND);
        ...
        private void validateCanDropCompactStorage() {
            ...
            for (InetAddressAndPort node : StorageService.instance.getTokenMetadata().getAllEndpoints()) {
                if (MessagingService.instance().versions.knows(node) && MessagingService.instance().versions.getRaw(node) < MessagingService.VERSION_40) {
                    before4.add(node);
                    continue;
                }
                ...
                try {
                    boolean has2xSStables = onComma.splitToList(sstableVersionsString).stream().anyMatch(v -> v.compareTo("big-ma") <= 0);
                    if (has2xSStables)
            ...
            if (!before4.isEmpty())
                throw new InvalidRequestException(format("Cannot DROP COMPACT STORAGE as some nodes in the cluster (%s) " + "are not on 4.0+ yet. Please upgrade those nodes and run " + "`upgradesstables` before retrying.", before4));
            if (!preC15897nodes.isEmpty())
                throw new InvalidRequestException(format("Cannot guarantee that DROP COMPACT STORAGE is safe as some nodes " + "in the cluster (%s) do not have https://issues.apache.org/jira/browse/CASSANDRA-15897. " + "Please upgrade those nodes and retry.", preC15897nodes));
            if (!with2xSStables.isEmpty())
                throw new InvalidRequestException(format("Cannot DROP COMPACT STORAGE as some nodes in the cluster (%s) " + "has some non-upgraded 2.x sstables. Please run `upgradesstables` " + "on those nodes before retrying", with2xSStables));
        }
    }
    ...
    public static final class Raw extends CQLStatement.Raw {
        ...
        // ADD
        private final transient List<AddColumns.Column> addedColumns = new ArrayList<>();

        // DROP
        private final transient Set<ColumnIdentifier> droppedColumns = new HashSet<>();

        // will use execution timestamp if not provided by query
        private transient Long timestamp = null;

        // RENAME
        private final transient Map<ColumnIdentifier, ColumnIdentifier> renamedColumns = new HashMap<>();

        // OPTIONS
        public final transient TableAttributes attrs = new TableAttributes();
        ...
        public AlterTableStatement prepare(ClientState state) {
            ...
            switch(kind) {
                ...
                case DROP_COLUMNS:
                    return new DropColumns(keyspaceName, tableName, droppedColumns, timestamp);
                case RENAME_COLUMNS:
                    return new RenameColumns(keyspaceName, tableName, renamedColumns);
                case ALTER_OPTIONS:
                    return new AlterOptions(keyspaceName, tableName, attrs);
