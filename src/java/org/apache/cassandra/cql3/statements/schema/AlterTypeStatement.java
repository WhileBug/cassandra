...
public abstract class AlterTypeStatement extends AlterSchemaStatement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AlterTypeStatement.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AlterTypeStatement.class);

    protected final transient String typeName;
    ...
    private static final class AddField extends AlterTypeStatement {
        ...
        private final transient FieldIdentifier fieldName;
        ...
        UserType apply(KeyspaceMetadata keyspace, UserType userType) {
            ...
            if (!tablesWithTypeInPartitionKey.isEmpty()) {
                throw ire("Cannot add new field %s of type %s to user type %s as the type is being used in partition key by the following tables: %s", fieldName, type, userType.getCqlTypeName(), String.join(", ", transform(tablesWithTypeInPartitionKey, TableMetadata::toString)));
            }
        ...
        private static Collection<TableMetadata> findTablesReferencingTypeInPartitionKey(KeyspaceMetadata keyspace, UserType userType) {
            ...
            filter(keyspace.tablesAndViews(), table -> any(table.partitionKeyColumns(), column -> column.type.referencesUserType(userType.name))).forEach(tables::add);
            return tables;
        }
    }
    ...
    private static final class RenameFields extends AlterTypeStatement {
        ...
        private final transient Map<FieldIdentifier, FieldIdentifier> renamedFields;
        ...
        UserType apply(KeyspaceMetadata keyspace, UserType userType) {
            List<String> dependentAggregates = keyspace.functions.udas().filter(uda -> null != uda.initialCondition() && uda.stateType().referencesUserType(userType.name)).map(uda -> uda.name().toString()).collect(toList());
            if (!dependentAggregates.isEmpty()) {
                throw ire("Cannot alter user type %s as it is still used in INITCOND by aggregates %s", userType.getCqlTypeName(), join(", ", dependentAggregates));
            }
            List<FieldIdentifier> fieldNames = new ArrayList<>(userType.fieldNames());
    ...
    public static final class Raw extends CQLStatement.Raw {
        ...
        // ADD
        private transient FieldIdentifier newFieldName;

        private transient CQL3Type.Raw newFieldType;

        // RENAME
        private final transient Map<FieldIdentifier, FieldIdentifier> renamedFields = new HashMap<>();
        ...
        public AlterTypeStatement prepare(ClientState state) {
            ...
            switch(kind) {
                ...
                case RENAME_FIELDS:
                    return new RenameFields(keyspaceName, typeName, renamedFields);
                case ALTER_FIELD:
                    return new AlterField(keyspaceName, typeName);
            }
