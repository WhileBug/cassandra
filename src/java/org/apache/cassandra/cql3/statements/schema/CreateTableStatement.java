...
public final class CreateTableStatement extends AlterSchemaStatement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CreateTableStatement.class);

    private static final transient Logger logger = LoggerFactory.getLogger(CreateTableStatement.class);

    private final transient String tableName;

    private final transient Map<ColumnIdentifier, CQL3Type.Raw> rawColumns;

    private final transient Set<ColumnIdentifier> staticColumns;

    private final transient List<ColumnIdentifier> partitionKeyColumns;

    private final transient List<ColumnIdentifier> clusteringColumns;

    private final transient LinkedHashMap<ColumnIdentifier, Boolean> clusteringOrder;

    private final transient TableAttributes attrs;

    private final transient boolean ifNotExists;

    private final transient boolean useCompactStorage;
    ...
    private static class DefaultNames {
        ...
        private static final transient String DEFAULT_CLUSTERING_NAME = "column";

        private static final transient String DEFAULT_COMPACT_VALUE_NAME = "value";

        private final transient Set<String> usedNames;

        private transient int clusteringIndex = 1;

        private transient int compactIndex = 0;
    ...
    public static TableMetadata.Builder parse(String cql, String keyspace) {
        return CQLFragmentParser.parseAny(CqlParser::createTableStatement, cql, "CREATE TABLE").keyspace(keyspace).prepare(// works around a messy ClientState/QueryProcessor class init deadlock
        null).builder(Types.none());
    }
    ...
    public final static class Raw extends CQLStatement.Raw {
        ...
        private transient boolean useCompactStorage = false;

        private final transient Map<ColumnIdentifier, CQL3Type.Raw> rawColumns = new HashMap<>();

        private final transient Set<ColumnIdentifier> staticColumns = new HashSet<>();

        private final transient List<ColumnIdentifier> clusteringColumns = new ArrayList<>();

        private transient List<ColumnIdentifier> partitionKeyColumns;

        private final transient LinkedHashMap<ColumnIdentifier, Boolean> clusteringOrder = new LinkedHashMap<>();
        ...
        public CreateTableStatement prepare(ClientState state) {
            ...
            return new CreateTableStatement(keyspaceName, name.getName(), rawColumns, staticColumns, partitionKeyColumns, clusteringColumns, clusteringOrder, attrs, ifNotExists, useCompactStorage);
        }
