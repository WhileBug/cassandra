...
public final class DropFunctionStatement extends AlterSchemaStatement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(DropFunctionStatement.class);

    private final transient String functionName;

    private final transient Collection<CQL3Type.Raw> arguments;

    private final transient boolean argumentsSpeficied;
    ...
    public Keyspaces apply(Keyspaces schema) {
        String name = argumentsSpeficied ? format("%s.%s(%s)", keyspaceName, functionName, join(", ", transform(arguments, CQL3Type.Raw::toString))) : format("%s.%s", keyspaceName, functionName);
        ...
        if (functions.size() > 1 && !argumentsSpeficied) {
            throw ire("'DROP FUNCTION %s' matches multiple function definitions; " + "specify the argument types by issuing a statement like " + "'DROP FUNCTION %s (type, type, ...)'. You can use cqlsh " + "'DESCRIBE FUNCTION %s' command to find all overloads", functionName, functionName, functionName);
        }
        ...
        String dependentAggregates = keyspace.functions.aggregatesUsingFunction(function).map(a -> a.name().toString()).collect(joining(", "));
    ...
    public static final class Raw extends CQLStatement.Raw {
        ...
        private final transient List<CQL3Type.Raw> arguments;

        private final transient boolean argumentsSpecified;
