...
public class UpdateStatement extends ModificationStatement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(UpdateStatement.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(UpdateStatement.class);

    private static final transient Constants.Value EMPTY = new Constants.Value(ByteBufferUtil.EMPTY_BYTE_BUFFER);
    ...
    public void addUpdateForKey(PartitionUpdate.Builder updateBuilder, Clustering<?> clustering, UpdateParameters params) {
        if (updatesRegularRows()) {
            ...
            for (int i = 0, isize = updates.size(); i < isize; i++) updates.get(i).execute(updateBuilder.partitionKey(), params);
            updateBuilder.add(params.buildRow());
        }
        if (updatesStaticRow()) {
            params.newRow(Clustering.STATIC_CLUSTERING);
            List<Operation> staticOps = getStaticOperations();
            for (int i = 0, isize = staticOps.size(); i < isize; i++) staticOps.get(i).execute(updateBuilder.partitionKey(), params);
            updateBuilder.add(params.buildRow());
        }
    }
    ...
    public static class ParsedInsert extends ModificationStatement.Parsed {
        ...
        private final transient List<ColumnIdentifier> columnNames;

        private final transient List<Term.Raw> columnValues;
        ...
        protected ModificationStatement prepareInternal(TableMetadata metadata, VariableSpecifications bindVariables, Conditions conditions, Attributes attrs) {
            ...
            StatementRestrictions restrictions = new StatementRestrictions(type, metadata, whereClause.build(), bindVariables, applyOnlyToStaticColumns, false, false);
    ...
    public static class ParsedInsertJson extends ModificationStatement.Parsed {
        ...
        private final transient Json.Raw jsonValue;

        private final transient boolean defaultUnset;
    ...
    public static class ParsedUpdate extends ModificationStatement.Parsed {
        ...
        private final transient List<Pair<ColumnIdentifier, Operation.RawUpdate>> updates;
