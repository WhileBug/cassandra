...
public abstract class ModificationStatement implements CQLStatement.SingleKeyspaceCqlStatement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ModificationStatement.class);

    protected static final transient Logger logger = LoggerFactory.getLogger(ModificationStatement.class);

    private final static transient MD5Digest EMPTY_HASH = MD5Digest.wrap(new byte[] {});

    public static final transient String CUSTOM_EXPRESSIONS_NOT_ALLOWED = "Custom index expressions cannot be used in WHERE clauses for UPDATE or DELETE statements";

    private static final transient ColumnIdentifier CAS_RESULT_COLUMN = new ColumnIdentifier("[applied]", false);

    protected final transient StatementType type;

    protected final transient VariableSpecifications bindVariables;
    ...
    private final transient Operations operations;

    private final transient RegularAndStaticColumns updatedColumns;

    private final transient Conditions conditions;

    private final transient RegularAndStaticColumns conditionColumns;

    private final transient RegularAndStaticColumns requiresRead;
    ...
    final void addUpdates(UpdatesCollector collector, List<ByteBuffer> keys, QueryOptions options, boolean local, long timestamp, int nowInSeconds, long queryStartNanoTime) {
        if (hasSlices()) {
    ...
    private <V> void validateClustering(Clustering<V> clustering) {
        ...
        for (V v : clustering.getRawValues()) {
            if (v != null && accessor.size(v) > FBUtilities.MAX_UNSIGNED_SHORT)
                throw new InvalidRequestException(String.format("Key length of %d is longer than maximum of %d", clustering.dataSize(), FBUtilities.MAX_UNSIGNED_SHORT));
        }
    }
    ...
    private UpdateParameters makeUpdateParameters(Collection<ByteBuffer> keys, NavigableSet<Clustering<?>> clusterings, QueryOptions options, boolean local, long timestamp, int nowInSeconds, long queryStartNanoTime) {
        if (clusterings.contains(Clustering.STATIC_CLUSTERING))
            return makeUpdateParameters(keys, new ClusteringIndexSliceFilter(Slices.ALL, false), options, DataLimits.cqlLimits(1), local, timestamp, nowInSeconds, queryStartNanoTime);
    ...
    public static abstract class Parsed extends QualifiedStatement {
        ...
        private final transient List<Pair<ColumnIdentifier, ColumnCondition.Raw>> conditions;
