...
public class SelectStatement implements CQLStatement.SingleKeyspaceCqlStatement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SelectStatement.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SelectStatement.class);

    private static final transient Logger logger = LoggerFactory.getLogger(SelectStatement.class);

    public static final transient int DEFAULT_PAGE_SIZE = 10000;

    public final transient VariableSpecifications bindVariables;

    public final transient TableMetadata table;

    public final transient Parameters parameters;

    private final transient Selection selection;

    private final transient Term limit;

    private final transient Term perPartitionLimit;

    private final transient StatementRestrictions restrictions;

    private final transient boolean isReversed;

    /**
     * The <code>AggregationSpecification</code> used to make the aggregates.
     */
    private final transient AggregationSpecification aggregationSpec;

    /**
     * The comparator used to orders results when multiple keys are selected (using IN).
     */
    private final transient Comparator<List<ByteBuffer>> orderingComparator;

    // Used by forSelection below
    private static final transient Parameters defaultParameters = new Parameters(Collections.emptyMap(), Collections.emptyList(), false, false, false);
    ...
    public ResultMessage.Rows execute(QueryState state, QueryOptions options, long queryStartNanoTime) {
        ...
        return execute(Pager.forDistributedQuery(pager, cl, state.getClientState()), options, selectors, pageSize, nowInSec, userLimit, queryStartNanoTime);
    }
    ...
    private static abstract class Pager {
        ...
        protected transient QueryPager pager;
        ...
        public static class NormalPager extends Pager {
            ...
            private final transient ClientState clientState;
            ...
            public PartitionIterator fetchPage(int pageSize, long queryStartNanoTime) {
                logger_IC.info("[InconsistencyDetector][org.apache.cassandra.cql3.statements.SelectStatement.Pager.NormalPager.clientState]=" + org.json.simple.JSONValue.toJSONString(clientState).replace("\n", "").replace("\r", ""));
                logger_IC.info("[InconsistencyDetector][org.apache.cassandra.cql3.statements.SelectStatement.Pager.NormalPager.clientState]=" + org.json.simple.JSONValue.toJSONString(clientState).replace("\n", "").replace("\r", ""));
    ...
    public static class RawStatement extends QualifiedStatement {
        ...
        public final transient List<RawSelector> selectClause;

        public final transient WhereClause whereClause;

        public final transient Term.Raw limit;

        public final transient Term.Raw perPartitionLimit;
        ...
        public SelectStatement prepare(boolean forView) throws InvalidRequestException {
            ...
            Selection selection = prepareSelection(table, selectables, bindVariables, resultSetOrderingColumns, restrictions);
            if (parameters.isDistinct) {
            ...
            AggregationSpecification aggregationSpec = getAggregationSpecification(table, selection, restrictions, parameters.isDistinct);
            checkFalse(aggregationSpec == AggregationSpecification.AGGREGATE_EVERYTHING && perPartitionLimit != null, "PER PARTITION LIMIT is not allowed with aggregate queries.");
            ...
            return new SelectStatement(table, bindVariables, parameters, selection, restrictions, isReversed, aggregationSpec, orderingComparator, prepareLimit(bindVariables, limit, keyspace(), limitReceiver()), prepareLimit(bindVariables, perPartitionLimit, keyspace(), perPartitionLimitReceiver()));
        }
        ...
        private Selection prepareSelection(TableMetadata table, List<Selectable> selectables, VariableSpecifications boundNames, Set<ColumnMetadata> resultSetOrderingColumns, StatementRestrictions restrictions) {
            ...
            if (// wildcard query
            selectables.isEmpty()) {
                return hasGroupBy ? Selection.wildcardWithGroupBy(table, boundNames, parameters.isJson, restrictions.returnStaticContentOnPartitionWithNoRows()) : Selection.wildcard(table, parameters.isJson, restrictions.returnStaticContentOnPartitionWithNoRows());
            }
        ...
        private StatementRestrictions prepareRestrictions(TableMetadata metadata, VariableSpecifications boundNames, boolean selectsOnlyStaticColumns, boolean forView) throws InvalidRequestException {
            return new StatementRestrictions(StatementType.SELECT, metadata, whereClause, boundNames, selectsOnlyStaticColumns, parameters.allowFiltering, forView);
        }
        ...
        private static void validateDistinctSelection(TableMetadata metadata, Selection selection, StatementRestrictions restrictions) throws InvalidRequestException {
            checkFalse(restrictions.hasClusteringColumnsRestrictions() || (restrictions.hasNonPrimaryKeyRestrictions() && !restrictions.nonPKRestrictedColumns(true).stream().allMatch(ColumnMetadata::isStatic)), "SELECT DISTINCT with WHERE clause only supports restriction by partition key and/or static columns.");
            Collection<ColumnMetadata> requestedColumns = selection.getColumns();
            for (ColumnMetadata def : requestedColumns) checkFalse(!def.isPartitionKey() && !def.isStatic(), "SELECT DISTINCT queries must only request partition key columns and/or static columns (not %s)", def.name);
            // If it's a key range, we require that all partition key columns are selected so we don't have to bother
            // with post-query grouping.
            if (!restrictions.isKeyRange())
                return;
            for (ColumnMetadata def : metadata.partitionKeyColumns()) checkTrue(requestedColumns.contains(def), "SELECT DISTINCT queries must request all the partition key columns (missing %s)", def.name);
        }
         ...
         */
        private AggregationSpecification getAggregationSpecification(TableMetadata metadata, Selection selection, StatementRestrictions restrictions, boolean isDistinct) {
        ...
        private boolean isReversed(TableMetadata table, Map<ColumnMetadata, Boolean> orderingColumns, StatementRestrictions restrictions) throws InvalidRequestException {
            ...
            for (Map.Entry<ColumnMetadata, Boolean> entry : orderingColumns.entrySet()) {
                ...
                checkTrue(def.isClusteringColumn(), "Order by is currently only supported on the clustered columns of the PRIMARY KEY, got %s", def.name);
                while (i != def.position()) {
                    checkTrue(restrictions.isColumnRestrictedByEq(table.clusteringColumns().get(i++)), "Order by currently only supports the ordering of columns following their declared order in the PRIMARY KEY");
                }
                i++;
        ...
        public String toString() {
            return MoreObjects.toStringHelper(this).add("name", qualifiedName).add("selectClause", selectClause).add("whereClause", whereClause).add("isDistinct", parameters.isDistinct).toString();
        }
    }
    ...
    public static class Parameters {
        ...
        public final transient Map<ColumnIdentifier, Boolean> orderings;

        public final transient List<ColumnIdentifier> groups;

        public final transient boolean isDistinct;

        public final transient boolean allowFiltering;

        public final transient boolean isJson;

        public Parameters(Map<ColumnIdentifier, Boolean> orderings, List<ColumnIdentifier> groups, boolean isDistinct, boolean allowFiltering, boolean isJson) {
    ...
    private static class SingleColumnComparator extends ColumnComparator<List<ByteBuffer>> {
        ...
        private final transient Comparator<ByteBuffer> comparator;
    ...
    private static class CompositeComparator extends ColumnComparator<List<ByteBuffer>> {
        ...
        private final transient List<Comparator<ByteBuffer>> orderTypes;

        private final transient List<Integer> positions;
