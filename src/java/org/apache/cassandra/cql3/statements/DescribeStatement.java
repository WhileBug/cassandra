...
public abstract class DescribeStatement<T> extends CQLStatement.Raw implements CQLStatement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(DescribeStatement.class);

    private static final transient String KS = "system";

    private static final transient String CF = "describe";

    /**
     * The columns returned by the describe queries that only list elements names (e.g. DESCRIBE KEYSPACES, DESCRIBE TABLES...)
     */
    private static final transient List<ColumnSpecification> LIST_METADATA = ImmutableList.of(new ColumnSpecification(KS, CF, new ColumnIdentifier("keyspace_name", true), UTF8Type.instance), new ColumnSpecification(KS, CF, new ColumnIdentifier("type", true), UTF8Type.instance), new ColumnSpecification(KS, CF, new ColumnIdentifier("name", true), UTF8Type.instance));
     ...
     */
    private static final transient List<ColumnSpecification> ELEMENT_METADATA = ImmutableList.<ColumnSpecification>builder().addAll(LIST_METADATA).add(new ColumnSpecification(KS, CF, new ColumnIdentifier("create_statement", true), UTF8Type.instance)).build();

    /**
     * "Magic version" for the paging state.
     */
    private static final transient int PAGING_STATE_VERSION = 0x0001;

    static final transient String SCHEMA_CHANGED_WHILE_PAGING_MESSAGE = "The schema has changed since the previous page of the DESCRIBE statement result. " + "Please retry the DESCRIBE statement.";

    private transient boolean includeInternalDetails;
    ...
    public static final class Listing extends DescribeStatement<SchemaElement> {
        ...
        private final transient java.util.function.Function<KeyspaceMetadata, Stream<? extends SchemaElement>> elementsProvider;
        ...
        protected Stream<? extends SchemaElement> describe(ClientState state, Keyspaces keyspaces) {
            ...
            Stream<KeyspaceMetadata> stream = keyspace == null ? keyspaces.stream().sorted(SchemaElement.NAME_COMPARATOR) : Stream.of(validateKeyspace(keyspace, keyspaces));
    ...
    public static class Element extends DescribeStatement<SchemaElement> {
        ...
        private final transient BiFunction<KeyspaceMetadata, String, Stream<? extends SchemaElement>> elementsProvider;
        ...
        @Override
        protected Stream<? extends SchemaElement> describe(ClientState state, Keyspaces keyspaces) {
            String ks = keyspace == null ? checkNotNull(state.getRawKeyspace(), "No keyspace specified and no current keyspace") : keyspace;
        ...
        return new DescribeStatement<List<Object>>() {
            ...
            protected List<ColumnSpecification> metadata(ClientState state) {
                ...
                if (mustReturnsRangeOwnerships(state.getRawKeyspace()))
                    builder.add(new ColumnSpecification(KS, CF, new ColumnIdentifier("range_ownership", true), MapType.getInstance(UTF8Type.instance, ListType.getInstance(UTF8Type.instance, false), false)));
