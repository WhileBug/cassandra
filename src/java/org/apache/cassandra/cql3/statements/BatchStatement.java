...
public class BatchStatement implements CQLStatement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(BatchStatement.class);
    ...
    private final transient VariableSpecifications bindVariables;

    private final transient List<ModificationStatement> statements;

    // Columns modified for each table (keyed by the table ID)
    private final transient Map<TableId, RegularAndStaticColumns> updatedColumns;
    ...
    private final transient RegularAndStaticColumns conditionColumns;

    private final transient boolean updatesRegularRows;

    private final transient boolean updatesStaticRow;

    private final transient Attributes attrs;

    private final transient boolean hasConditions;

    private final transient boolean updatesVirtualTables;

    private static final transient Logger logger = LoggerFactory.getLogger(BatchStatement.class);

    private static final transient String UNLOGGED_BATCH_WARNING = "Unlogged batch covering {} partitions detected " + "against table{} {}. You should use a logged batch for " + "atomicity, or asynchronous writes for performance.";

    private static final transient String LOGGED_BATCH_LOW_GCGS_WARNING = "Executing a LOGGED BATCH on table{} {}, configured with a " + "gc_grace_seconds of 0. The gc_grace_seconds is used to TTL " + "batchlog entries, so setting gc_grace_seconds too low on " + "tables involved in an atomic batch might cause batchlog " + "entries to expire before being replayed.";

    public static final transient BatchMetrics metrics = new BatchMetrics();
    ...
    public short[] getPartitionKeyBindVariableIndexes() {
        boolean affectsMultipleTables = !statements.isEmpty() && !statements.stream().map(s -> s.metadata().id).allMatch(isEqual(statements.get(0).metadata().id));
        // Use the TableMetadata of the first statement for partition key bind indexes.  If the statements affect
        // multiple tables, we won't send partition key bind indexes.
        return (affectsMultipleTables || statements.isEmpty()) ? null : bindVariables.getPartitionKeyBindVariableIndexes(statements.get(0).metadata());
    }
    ...
    public Iterable<org.apache.cassandra.cql3.functions.Function> getFunctions() {
        ...
        for (ModificationStatement statement : statements) statement.addFunctionsTo(functions);
        return functions;
    }

    public void authorize(ClientState state) throws InvalidRequestException, UnauthorizedException {
        for (ModificationStatement statement : statements) statement.authorize(state);
    }
    ...
    public void validate(ClientState state) throws InvalidRequestException {
        for (ModificationStatement statement : statements) statement.validate(state);
    }
    ...
    public List<? extends IMutation> getMutations(BatchQueryOptions options, boolean local, long batchTimestamp, int nowInSeconds, long queryStartNanoTime) {
        ...
        for (int i = 0, isize = statements.size(); i < isize; i++) {
            ...
            for (int stmtIdx = 0, stmtSize = stmtPartitionKeys.size(); stmtIdx < stmtSize; stmtIdx++) perKeyCountsForTable.add(stmtPartitionKeys.get(stmtIdx));
        }
    ...
    public static class Parsed extends QualifiedStatement {
        ...
        private final transient Attributes.Raw attrs;

        private final transient List<ModificationStatement.Parsed> parsedStatements;
        ...
        public boolean isFullyQualified() {
            for (ModificationStatement.Parsed statement : parsedStatements) if (!statement.isFullyQualified())
                return false;
            return true;
        }
        ...
        public void setKeyspace(ClientState state) throws InvalidRequestException {
            for (ModificationStatement.Parsed statement : parsedStatements) statement.setKeyspace(state);
        }
    ...
    private static class MultiTableColumnsBuilder {
        ...
        private final transient Map<TableId, RegularAndStaticColumns.Builder> perTableBuilders = new HashMap<>();
        ...
        public Map<TableId, RegularAndStaticColumns> build() {
            ...
            for (Map.Entry<TableId, RegularAndStaticColumns.Builder> p : perTableBuilders.entrySet()) m.put(p.getKey(), p.getValue().build());
            return m;
        }
    }
