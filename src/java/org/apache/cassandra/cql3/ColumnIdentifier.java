...
public class ColumnIdentifier implements IMeasurableMemory, Comparable<ColumnIdentifier> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ColumnIdentifier.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ColumnIdentifier.class);

    private static final transient Pattern PATTERN_DOUBLE_QUOTE = Pattern.compile("\"", Pattern.LITERAL);

    private static final transient String ESCAPED_DOUBLE_QUOTE = Matcher.quoteReplacement("\"\"");

    public final transient ByteBuffer bytes;
     ...
     */
    public final transient long prefixComparison;

    private final transient boolean interned;

    private static final transient Pattern UNQUOTED_IDENTIFIER = Pattern.compile("[a-z][a-z0-9_]*");

    private static final transient long EMPTY_SIZE = ObjectSizes.measure(new ColumnIdentifier(ByteBufferUtil.EMPTY_BYTE_BUFFER, "", false));

    private static final transient ConcurrentMap<InternedKey, ColumnIdentifier> internedInstances = new MapMaker().weakValues().makeMap();
    ...
    public long unsharedHeapSize() {
        return EMPTY_SIZE + ObjectSizes.sizeOnHeapOf(bytes) + ObjectSizes.sizeOf(text);
    }

    public long unsharedHeapSizeExcludingData() {
        return EMPTY_SIZE + ObjectSizes.sizeOnHeapExcludingData(bytes) + ObjectSizes.sizeOf(text);
    }
