...
public abstract class MultiColumnRestriction implements SingleRestriction {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MultiColumnRestriction.class);
    ...
    public final boolean hasSupportingIndex(IndexRegistry indexRegistry) {
        for (Index index : indexRegistry.listIndexes()) if (isSupportedBy(index))
            return true;
        return false;
    }
    ...
    public static class EQRestriction extends MultiColumnRestriction {
        ...
        protected final transient Term value;
        ...
        public SingleRestriction doMergeWith(SingleRestriction otherRestriction) {
            throw invalidRequest("%s cannot be restricted by more than one relation if it includes an Equal", getColumnsInCommons(otherRestriction));
        }

        @Override
        protected boolean isSupportedBy(Index index) {
            for (ColumnMetadata column : columnDefs) if (index.supportsExpression(column, Operator.EQ))
                return true;
            return false;
        }
    ...
    public abstract static class INRestriction extends MultiColumnRestriction {
        ...
        @Override
        public SingleRestriction doMergeWith(SingleRestriction otherRestriction) {
            throw invalidRequest("%s cannot be restricted by more than one relation if it includes a IN", getColumnsInCommons(otherRestriction));
        }

        @Override
        protected boolean isSupportedBy(Index index) {
            for (ColumnMetadata column : columnDefs) if (index.supportsExpression(column, Operator.IN))
                return true;
            return false;
        }
    ...
    public static class InRestrictionWithValues extends INRestriction {
        ...
        protected final transient List<Term> values;
    ...
    public static class InRestrictionWithMarker extends INRestriction {
        ...
        protected final transient AbstractMarker marker;
    ...
    public static class SliceRestriction extends MultiColumnRestriction {
        ...
        @Override
        protected boolean isSupportedBy(Index index) {
            for (ColumnMetadata def : columnDefs) if (slice.isSupportedBy(def, index))
                return true;
            return false;
        }
        ...
        public SingleRestriction doMergeWith(SingleRestriction otherRestriction) {
            ...
            if (!getFirstColumn().equals(otherRestriction.getFirstColumn())) {
                ColumnMetadata column = getFirstColumn().position() > otherRestriction.getFirstColumn().position() ? getFirstColumn() : otherRestriction.getFirstColumn();
    ...
    public static class NotNullRestriction extends MultiColumnRestriction {
        ...
        @Override
        public SingleRestriction doMergeWith(SingleRestriction otherRestriction) {
            throw invalidRequest("%s cannot be restricted by a relation if it includes an IS NOT NULL clause", getColumnsInCommons(otherRestriction));
        }

        @Override
        protected boolean isSupportedBy(Index index) {
            for (ColumnMetadata column : columnDefs) if (index.supportsExpression(column, Operator.IS_NOT))
                return true;
            return false;
        }
