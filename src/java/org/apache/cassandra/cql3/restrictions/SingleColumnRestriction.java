...
public abstract class SingleColumnRestriction implements SingleRestriction {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SingleColumnRestriction.class);
     ...
     */
    protected final transient ColumnMetadata columnDef;
    ...
    public static class SliceRestriction extends SingleColumnRestriction {
        ...
        @Override
        public void addRowFilterTo(RowFilter filter, IndexRegistry indexRegistry, QueryOptions options) {
            for (Bound b : Bound.values()) if (hasBound(b))
    ...
    public static final class ContainsRestriction extends SingleColumnRestriction {
        ...
        // for CONTAINS
        private transient List<Term> values = new ArrayList<>();

        // for CONTAINS_KEY
        private transient List<Term> keys = new ArrayList<>();

        // for map[key] = value
        private transient List<Term> entryKeys = new ArrayList<>();

        // for map[key] = value
        private transient List<Term> entryValues = new ArrayList<>();
        ...
        @Override
        public void addRowFilterTo(RowFilter filter, IndexRegistry indexRegistry, QueryOptions options) {
            for (ByteBuffer value : bindAndGet(values, options)) filter.add(columnDef, Operator.CONTAINS, value);
            for (ByteBuffer key : bindAndGet(keys, options)) filter.add(columnDef, Operator.CONTAINS_KEY, key);
            ...
            for (int i = 0; i < eks.size(); i++) filter.addMapEquality(columnDef, eks.get(i), Operator.EQ, evs.get(i));
        }
    ...
    public static final class LikeRestriction extends SingleColumnRestriction {
        ...
        private static final transient ByteBuffer LIKE_WILDCARD = ByteBufferUtil.bytes("%");
