...
public class FunctionCall extends Term.NonTerminal {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(FunctionCall.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(FunctionCall.class);

    private final transient ScalarFunction fun;

    private final transient List<Term> terms;
    ...
    public static class Raw extends Term.Raw {
        ...
        private transient FunctionName name;

        private final transient List<Term.Raw> terms;
        ...
        public Term prepare(String keyspace, ColumnSpecification receiver) throws InvalidRequestException {
            ...
            if (!scalarFun.testAssignment(keyspace, receiver).isAssignable()) {
                if (OperationFcts.isOperation(name))
                    throw invalidRequest("Type error: cannot assign result of operation %s (type %s) to %s (type %s)", OperationFcts.getOperator(scalarFun.name()), scalarFun.returnType().asCQL3Type(), receiver.name, receiver.type.asCQL3Type());
                throw invalidRequest("Type error: cannot assign result of function %s (type %s) to %s (type %s)", scalarFun.name(), scalarFun.returnType().asCQL3Type(), receiver.name, receiver.type.asCQL3Type());
            }
            if (fun.argTypes().size() != terms.size())
                throw invalidRequest("Incorrect number of arguments specified for function %s (expected %d, found %d)", fun, fun.argTypes().size(), terms.size());
