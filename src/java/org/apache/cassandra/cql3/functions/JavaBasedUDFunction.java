...
public final class JavaBasedUDFunction extends UDFunction {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(JavaBasedUDFunction.class);

    private static final transient String BASE_PACKAGE = "org.apache.cassandra.cql3.udf.gen";

    private static final transient Pattern JAVA_LANG_PREFIX = Pattern.compile("\\bjava\\.lang\\.");

    private static final transient Logger logger = LoggerFactory.getLogger(JavaBasedUDFunction.class);

    private static final transient AtomicInteger classSequence = new AtomicInteger();
    ...
    private static final transient UDFExecutorService executor = new UDFExecutorService(new NamedThreadFactory("UserDefinedFunctions", Thread.MIN_PRIORITY, udfClassLoader, new SecurityThreadGroup("UserDefinedFunctions", null, UDFunction::initializeThread)), "userfunction");

    private static final transient EcjTargetClassLoader targetClassLoader = new EcjTargetClassLoader();

    private static final transient UDFByteCodeVerifier udfByteCodeVerifier = new UDFByteCodeVerifier();

    private static final transient ProtectionDomain protectionDomain;

    private static final transient IErrorHandlingPolicy errorHandlingPolicy = DefaultErrorHandlingPolicies.proceedWithAllProblems();

    private static final transient IProblemFactory problemFactory = new DefaultProblemFactory(Locale.ENGLISH);

    private static final transient CompilerOptions compilerOptions;
     ...
     */
    private static final transient String[] javaSourceTemplate;
    ...
    static {
        ...
        try (InputStream input = JavaBasedUDFunction.class.getResource("JavaSourceUDF.txt").openConnection().getInputStream()) {
            ...
            for (int i = 0; st.hasMoreElements(); i++) javaSourceTemplate[i] = st.nextToken();
        } catch (IOException e) {
    ...
    private final transient JavaUDF javaUDF;

    private static final transient Pattern patternJavaDriver = Pattern.compile("com\\.datastax\\.driver\\.core\\.");
    ...
    static final class EcjCompilationUnit implements ICompilationUnit, ICompilerRequestor, INameEnvironment {
        ...
        transient List<IProblem> problemList;

        private final transient String className;

        private final transient char[] sourceCode;
        ...
        public char[][] getPackageName() {
            ...
            for (int i = 0; i < result.length; i++) result[i] = izer.nextToken().toCharArray();
            return result;
        }
    ...
    static final class EcjTargetClassLoader extends SecureClassLoader {
        ...
        // 
        private final transient Map<String, byte[]> classes = new ConcurrentHashMap<>();
