...
public class UDAggregate extends AbstractFunction implements AggregateFunction, SchemaElement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(UDAggregate.class);

    protected static final transient Logger logger = LoggerFactory.getLogger(UDAggregate.class);

    private final transient AbstractType<?> stateType;

    private final transient TypeCodec stateTypeCodec;

    private final transient TypeCodec returnTypeCodec;

    protected final transient ByteBuffer initcond;

    private final transient ScalarFunction stateFunction;

    private final transient ScalarFunction finalFunction;
    ...
    public boolean referencesUserType(ByteBuffer name) {
        return any(argTypes(), t -> t.referencesUserType(name)) || returnType.referencesUserType(name) || (null != stateType && stateType.referencesUserType(name)) || stateFunction.referencesUserType(name) || (null != finalFunction && finalFunction.referencesUserType(name));
    }
    ...
    public boolean equals(Object o) {
        ...
        return equalsWithoutTypesAndFunctions(that) && argTypes.equals(that.argTypes) && returnType.equals(that.returnType) && Objects.equal(stateFunction, that.stateFunction) && Objects.equal(finalFunction, that.finalFunction) && ((stateType == that.stateType) || ((stateType != null) && stateType.equals(that.stateType)));
    }
    ...
    public Optional<Difference> compare(Function function) {
        ...
        if (!equalsWithoutTypesAndFunctions(other) || ((null == finalFunction) != (null == other.finalFunction)) || ((null == stateType) != (null == other.stateType)))
