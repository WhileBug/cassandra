...
public interface Selectable extends AssignmentTestable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Selectable.class);
    ...
    public static class WithTerm implements Selectable {
         ...
         */
        private static final transient ColumnIdentifier bindMarkerNameInSelection = new ColumnIdentifier("[selection]", true);

        private final transient Term.Raw rawTerm;
        ...
        public static class Raw implements Selectable.Raw {
            ...
            private final transient Term.Raw term;
    ...
    public static class WritetimeOrTTL implements Selectable {
        ...
        public final transient boolean isWritetime;
        ...
        public Selector.Factory newSelectorFactory(TableMetadata table, AbstractType<?> expectedType, List<ColumnMetadata> defs, VariableSpecifications boundNames) {
            if (column.isPrimaryKeyColumn())
                throw new InvalidRequestException(String.format("Cannot use selection function %s on PRIMARY KEY part %s", isWritetime ? "writeTime" : "ttl", column.name));
            if (column.type.isMultiCell())
                throw new InvalidRequestException(String.format("Cannot use selection function %s on non-frozen %s %s", isWritetime ? "writeTime" : "ttl", column.type.isCollection() ? "collection" : "UDT", column.name));
        ...
        public static class Raw implements Selectable.Raw {
            ...
            private final transient Selectable.RawIdentifier id;
    ...
    public static class WithFunction implements Selectable {
        ...
        public final transient Function function;

        public final transient List<Selectable> args;
        ...
        public static class Raw implements Selectable.Raw {
            ...
            private final transient FunctionName functionName;

            private final transient List<Selectable.Raw> args;
            ...
            public static Raw newNegation(Selectable.Raw arg) {
                return new Raw(FunctionName.nativeFunction(OperationFcts.NEGATION_FUNCTION_NAME), Collections.singletonList(arg));
            }

            @Override
            public Selectable prepare(TableMetadata table) {
                List<Selectable> preparedArgs = new ArrayList<>(args.size());
                for (Selectable.Raw arg : args) preparedArgs.add(arg.prepare(table));
                ...
                } else // Also, COUNT(x) is equivalent to COUNT(*) for any non-null term x (since count(x) don't care about it's argument outside of check for nullness) and
                ...
                if (functionName.equalsNativeFunction(FunctionName.nativeFunction("count")) && preparedArgs.size() == 1 && (preparedArgs.get(0) instanceof WithTerm) && (((WithTerm) preparedArgs.get(0)).rawTerm instanceof Constants.Literal)) {
    ...
    public static class WithToJSonFunction implements Selectable {
        ...
        @Override
        public String toString() {
            return new StrBuilder().append(ToJsonFct.NAME).append("(").appendWithSeparators(args, ", ").append(")").toString();
        }
    ...
    public static class WithCast implements Selectable {
        ...
        private final transient CQL3Type type;

        private final transient Selectable arg;
        ...
        public static class Raw implements Selectable.Raw {
            ...
            private final transient Selectable.Raw arg;
    ...
    public static class WithFieldSelection implements Selectable {
        ...
        public final transient Selectable selected;

        public final transient FieldIdentifier field;
        ...
        public Selector.Factory newSelectorFactory(TableMetadata table, AbstractType<?> expectedType, List<ColumnMetadata> defs, VariableSpecifications boundNames) {
            ...
            if (!type.isUDT()) {
                throw new InvalidRequestException(String.format("Invalid field selection: %s of type %s is not a user type", selected, type.asCQL3Type()));
            }
        ...
        public static class Raw implements Selectable.Raw {
            ...
            private final transient Selectable.Raw selected;
    ...
    public static class BetweenParenthesesOrWithTuple implements Selectable {
         ...
         */
        private final transient List<Selectable> selectables;
        ...
        @Override
        public Factory newSelectorFactory(TableMetadata cfm, AbstractType<?> expectedType, List<ColumnMetadata> defs, VariableSpecifications boundNames) {
            AbstractType<?> type = getExactTypeIfKnown(cfm.keyspace);
            if (type == null) {
                type = expectedType;
                if (type == null)
                    throw invalidRequest("Cannot infer type for term %s in selection clause (try using a cast to force a type)", this);
            }
        ...
        private Factory newBetweenParenthesesSelectorFactory(TableMetadata cfm, AbstractType<?> expectedType, List<ColumnMetadata> defs, VariableSpecifications boundNames) {
        ...
        private Factory newTupleSelectorFactory(TableMetadata cfm, TupleType tupleType, List<ColumnMetadata> defs, VariableSpecifications boundNames) {
            SelectorFactories factories = createFactoriesAndCollectColumnDefinitions(selectables, tupleType.allTypes(), cfm, defs, boundNames);
        ...
        public static class Raw implements Selectable.Raw {
            ...
            private final transient List<Selectable.Raw> raws;
    ...
    public static class WithList implements Selectable {
        ...
        public Factory newSelectorFactory(TableMetadata cfm, AbstractType<?> expectedType, List<ColumnMetadata> defs, VariableSpecifications boundNames) {
            ...
            for (int i = 0, m = selectables.size(); i < m; i++) expectedTypes.add(listType.getElementsType());
            SelectorFactories factories = createFactoriesAndCollectColumnDefinitions(selectables, expectedTypes, cfm, defs, boundNames);
    ...
    public static class WithSet implements Selectable {
        ...
        public Factory newSelectorFactory(TableMetadata cfm, AbstractType<?> expectedType, List<ColumnMetadata> defs, VariableSpecifications boundNames) {
            ...
            List<AbstractType<?>> expectedTypes = new ArrayList<>(selectables.size());
            for (int i = 0, m = selectables.size(); i < m; i++) expectedTypes.add(setType.getElementsType());
            SelectorFactories factories = createFactoriesAndCollectColumnDefinitions(selectables, expectedTypes, cfm, defs, boundNames);
    ...
    public static class WithMapOrUdt implements Selectable {
         ...
         * identified.
         */
        private final transient TableMetadata cfm;

        /**
         * The Map or UDT raw elements.
         */
        private final transient List<Pair<Selectable.Raw, Selectable.Raw>> raws;
        ...
        @Override
        public TestResult testAssignment(String keyspace, ColumnSpecification receiver) {
            return receiver.type.isUDT() ? UserTypes.testUserTypeAssignment(receiver, getUdtFields((UserType) receiver.type)) : Maps.testMapAssignment(receiver, getMapEntries(cfm));
        }
        ...
        private Factory newMapSelectorFactory(TableMetadata cfm, List<ColumnMetadata> defs, VariableSpecifications boundNames, AbstractType<?> type) {
            ...
            return MapSelector.newFactory(type, getMapEntries(cfm).stream().map(p -> Pair.create(p.left.newSelectorFactory(cfm, mapType.getKeysType(), defs, boundNames), p.right.newSelectorFactory(cfm, mapType.getValuesType(), defs, boundNames))).collect(Collectors.toList()));
        }

        private Factory newUdtSelectorFactory(TableMetadata cfm, AbstractType<?> expectedType, List<ColumnMetadata> defs, VariableSpecifications boundNames) {
            ...
            for (Pair<Selectable.Raw, Selectable.Raw> raw : raws) {
                if (!(raw.left instanceof RawIdentifier))
                    throw invalidRequest("%s is not a valid field identifier of type %s ", raw.left, ut.getNameAsString());
                FieldIdentifier fieldName = ((RawIdentifier) raw.left).toFieldIdentifier();
                int fieldPosition = ut.fieldPosition(fieldName);
                if (fieldPosition == -1)
                    throw invalidRequest("Unknown field '%s' in value of user defined type %s", fieldName, ut.getNameAsString());
                AbstractType<?> fieldType = ut.fieldType(fieldPosition);
                factories.put(fieldName, raw.right.prepare(cfm).newSelectorFactory(cfm, fieldType, defs, boundNames));
            }
        ...
        @Override
        public String toString() {
            return raws.stream().map(p -> String.format("%s: %s", p.left instanceof RawIdentifier ? p.left : p.left.prepare(cfm), p.right.prepare(cfm))).collect(Collectors.joining(", ", "{", "}"));
        }

        private List<Pair<Selectable, Selectable>> getMapEntries(TableMetadata cfm) {
            return raws.stream().map(p -> Pair.create(p.left.prepare(cfm), p.right.prepare(cfm))).collect(Collectors.toList());
        }
    ...
    public static class WithTypeHint implements Selectable {
        ...
        public static class Raw implements Selectable.Raw {
            ...
            private final transient CQL3Type.Raw typeRaw;

            private final transient Selectable.Raw raw;
    ...
    public static final class RawIdentifier implements Selectable.Raw {
        ...
        private final transient boolean quoted;
        ...
        public FieldIdentifier toFieldIdentifier() {
            return quoted ? FieldIdentifier.forQuoted(text) : FieldIdentifier.forUnquoted(text);
        }
    ...
    public static class WithElementSelection implements Selectable {
        ...
        public final transient Term.Raw element;
    ...
    public static class WithSliceSelection implements Selectable {
        ...
        public final transient Term.Raw from;

        public final transient Term.Raw to;
        ...
        public static class Raw implements Selectable.Raw {
            ...
            // Both from and to can be null if they haven't been provided
            private final transient Term.Raw from;

            private final transient Term.Raw to;
