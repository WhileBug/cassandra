...
public final class TableParams {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TableParams.class);
    ...
    public final transient double bloomFilterFpChance;

    public final transient double crcCheckChance;

    public final transient int gcGraceSeconds;

    public final transient int defaultTimeToLive;

    public final transient int memtableFlushPeriodInMs;

    public final transient int minIndexInterval;

    public final transient int maxIndexInterval;

    public final transient SpeculativeRetryPolicy speculativeRetry;

    public final transient SpeculativeRetryPolicy additionalWritePolicy;

    public final transient CachingParams caching;

    public final transient CompactionParams compaction;

    public final transient CompressionParams compression;

    public final transient ImmutableMap<String, ByteBuffer> extensions;

    public final transient boolean cdc;

    public final transient ReadRepairStrategy readRepair;

    private TableParams(Builder builder) {
        comment = builder.comment;
        bloomFilterFpChance = builder.bloomFilterFpChance == null ? builder.compaction.defaultBloomFilterFbChance() : builder.bloomFilterFpChance;
    ...
    public boolean equals(Object o) {
        ...
        return comment.equals(p.comment) && bloomFilterFpChance == p.bloomFilterFpChance && crcCheckChance == p.crcCheckChance && gcGraceSeconds == p.gcGraceSeconds && defaultTimeToLive == p.defaultTimeToLive && memtableFlushPeriodInMs == p.memtableFlushPeriodInMs && minIndexInterval == p.minIndexInterval && maxIndexInterval == p.maxIndexInterval && speculativeRetry.equals(p.speculativeRetry) && caching.equals(p.caching) && compaction.equals(p.compaction) && compression.equals(p.compression) && extensions.equals(p.extensions) && cdc == p.cdc && readRepair == p.readRepair;
    }

    @Override
    public int hashCode() {
        return Objects.hashCode(comment, bloomFilterFpChance, crcCheckChance, gcGraceSeconds, defaultTimeToLive, memtableFlushPeriodInMs, minIndexInterval, maxIndexInterval, speculativeRetry, caching, compaction, compression, extensions, cdc, readRepair);
    }

    @Override
    public String toString() {
        return MoreObjects.toStringHelper(this).add(Option.COMMENT.toString(), comment).add(Option.BLOOM_FILTER_FP_CHANCE.toString(), bloomFilterFpChance).add(Option.CRC_CHECK_CHANCE.toString(), crcCheckChance).add(Option.GC_GRACE_SECONDS.toString(), gcGraceSeconds).add(Option.DEFAULT_TIME_TO_LIVE.toString(), defaultTimeToLive).add(Option.MEMTABLE_FLUSH_PERIOD_IN_MS.toString(), memtableFlushPeriodInMs).add(Option.MIN_INDEX_INTERVAL.toString(), minIndexInterval).add(Option.MAX_INDEX_INTERVAL.toString(), maxIndexInterval).add(Option.SPECULATIVE_RETRY.toString(), speculativeRetry).add(Option.CACHING.toString(), caching).add(Option.COMPACTION.toString(), compaction).add(Option.COMPRESSION.toString(), compression).add(Option.EXTENSIONS.toString(), extensions).add(Option.CDC.toString(), cdc).add(Option.READ_REPAIR.toString(), readRepair).toString();
    }

    public void appendCqlTo(CqlBuilder builder, boolean isView) {
        // option names should be in alphabetical order
        builder.append("additional_write_policy = ").appendWithSingleQuotes(additionalWritePolicy.toString()).newLine().append("AND bloom_filter_fp_chance = ").append(bloomFilterFpChance).newLine().append("AND caching = ").append(caching.asMap()).newLine().append("AND cdc = ").append(cdc).newLine().append("AND comment = ").appendWithSingleQuotes(comment).newLine().append("AND compaction = ").append(compaction.asMap()).newLine().append("AND compression = ").append(compression.asMap()).newLine().append("AND crc_check_chance = ").append(crcCheckChance).newLine();
        if (!isView) {
            builder.append("AND default_time_to_live = ").append(defaultTimeToLive).newLine();
        }
        builder.append("AND extensions = ").append(extensions.entrySet().stream().collect(toMap(Entry::getKey, e -> "0x" + ByteBufferUtil.bytesToHex(e.getValue()))), false).newLine().append("AND gc_grace_seconds = ").append(gcGraceSeconds).newLine().append("AND max_index_interval = ").append(maxIndexInterval).newLine().append("AND memtable_flush_period_in_ms = ").append(memtableFlushPeriodInMs).newLine().append("AND min_index_interval = ").append(minIndexInterval).newLine().append("AND read_repair = ").appendWithSingleQuotes(readRepair.toString()).newLine().append("AND speculative_retry = ").appendWithSingleQuotes(speculativeRetry.toString());
    }
    ...
    public static final class Builder {
        ...
        private transient Double bloomFilterFpChance;

        private transient double crcCheckChance = 1.0;

        // 10 days
        private transient int gcGraceSeconds = 864000;

        private transient int defaultTimeToLive = 0;

        private transient int memtableFlushPeriodInMs = 0;

        private transient int minIndexInterval = 128;

        private transient int maxIndexInterval = 2048;

        private transient SpeculativeRetryPolicy speculativeRetry = PercentileSpeculativeRetryPolicy.NINETY_NINE_P;

        private transient SpeculativeRetryPolicy additionalWritePolicy = PercentileSpeculativeRetryPolicy.NINETY_NINE_P;

        private transient CachingParams caching = CachingParams.DEFAULT;

        private transient CompactionParams compaction = CompactionParams.DEFAULT;

        private transient CompressionParams compression = CompressionParams.DEFAULT;

        private transient ImmutableMap<String, ByteBuffer> extensions = ImmutableMap.of();

        private transient boolean cdc;

        private transient ReadRepairStrategy readRepair = ReadRepairStrategy.BLOCKING;
