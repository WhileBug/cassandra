...
public final class Functions implements Iterable<Function> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Functions.class);
    ...
    private final transient ImmutableMultimap<FunctionName, Function> functions;
    ...
    public Collection<UDFunction> getUdfs(FunctionName name) {
        return functions.get(name).stream().filter(Filter.UDF).map(f -> (UDFunction) f).collect(Collectors.toList());
    }
    ...
    public Collection<UDAggregate> getUdas(FunctionName name) {
        return functions.get(name).stream().filter(Filter.UDA).map(f -> (UDAggregate) f).collect(Collectors.toList());
    }
    ...
    public Optional<Function> find(FunctionName name, List<AbstractType<?>> argTypes, Filter filter) {
        return get(name).stream().filter(filter.and(fun -> typesMatch(fun.argTypes(), argTypes))).findAny();
    }
    ...
    public static boolean typesMatch(List<AbstractType<?>> t1, List<AbstractType<?>> t2) {
        if (t1.size() != t2.size())
            return false;
        for (int i = 0; i < t1.size(); i++) if (!typesMatch(t1.get(i), t2.get(i)))
            return false;
        return true;
    }
    ...
    public static int typeHashCode(List<AbstractType<?>> types) {
        int h = 0;
        for (AbstractType<?> type : types) h = h * 31 + typeHashCode(type);
        return h;
    }
    ...
    public Functions without(FunctionName name, List<AbstractType<?>> argTypes) {
        Function fun = find(name, argTypes).orElseThrow(() -> new IllegalStateException(String.format("Function %s doesn't exists", name)));
        return without(fun);
    }
    ...
    public Functions withAddedOrUpdated(Function function) {
        return builder().add(Iterables.filter(this, f -> !(f.name().equals(function.name()) && Functions.typesMatch(f.argTypes(), function.argTypes())))).add(function).build();
    }
    ...
    public static final class Builder {
        ...
        final transient ImmutableMultimap.Builder<FunctionName, Function> functions = new ImmutableMultimap.Builder<>();
    ...
    public static final class FunctionsDiff<T extends Function> extends Diff<Functions, T> {
        ...
        static final transient FunctionsDiff NONE = new FunctionsDiff<>(Functions.none(), Functions.none(), ImmutableList.of());
