...
public final class ColumnMetadata extends ColumnSpecification implements Selectable, Comparable<ColumnMetadata> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ColumnMetadata.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ColumnMetadata.class);

    public static final transient Comparator<Object> asymmetricColumnDataComparator = (a, b) -> ((ColumnData) a).column().compareTo((ColumnMetadata) b);

    public static final transient int NO_POSITION = -1;
     ...
     */
    private final transient long comparisonOrder;
    ...
    private static long comparisonOrder(Kind kind, boolean isComplex, long position, ColumnIdentifier name) {
        ...
        return (((long) kind.ordinal()) << 61) | (isComplex ? 1L << 60 : 0) | (position << 48) | (name.prefixComparison >>> 16);
    }
    ...
    public ColumnMetadata(String ksName, String cfName, ColumnIdentifier name, AbstractType<?> type, int position, Kind kind) {
        ...
        // The position really only make sense for partition and clustering columns (and those must have one),
    ...
    private static Comparator<CellPath> makeCellPathComparator(Kind kind, AbstractType<?> type) {
        if (kind.isPrimaryKeyKind() || !type.isMultiCell())
            return null;
        AbstractType<?> nameComparator = type.isCollection() ? ((CollectionType) type).nameComparator() : ((UserType) type).nameComparator();
        return (path1, path2) -> {
    ...
    private boolean equalsWithoutType(ColumnMetadata other) {
        return name.equals(other.name) && kind == other.kind && position == other.position && ksName.equals(other.ksName) && cfName.equals(other.cfName);
    }
    ...
    Optional<Difference> compare(ColumnMetadata other) {
        ...
        return type.asCQL3Type().toString().equals(other.type.asCQL3Type().toString()) ? Optional.of(Difference.DEEP) : Optional.of(Difference.SHALLOW);
    }
    ...
    public String debugString() {
        return MoreObjects.toStringHelper(this).add("name", name).add("type", type).add("kind", kind).add("position", position).toString();
    }
    ...
    public static String toCQLString(Iterator<ColumnMetadata> defs) {
        ...
        while (defs.hasNext()) sb.append(", ").append(defs.next().name);
        return sb.toString();
    }

    public void appendNameAndOrderTo(CqlBuilder builder) {
        builder.append(name.toCQLString()).append(' ').append(clusteringOrder().toString());
    }
    ...
    public AbstractType<?> cellValueType() {
        ...
        return type instanceof CollectionType && type.isMultiCell() ? ((CollectionType) type).valueComparator() : type;
    }
    ...
    public boolean isCounterColumn() {
        if (// Possible with, for example, supercolumns
