...
public final class ReplicationParams {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ReplicationParams.class);

    public static final transient String CLASS = "class";

    public final transient Class<? extends AbstractReplicationStrategy> klass;

    public final transient ImmutableMap<String, String> options;
    ...
    public String toString() {
        ...
        for (Map.Entry<String, String> entry : options.entrySet()) helper.add(entry.getKey(), entry.getValue());
        return helper.toString();
    }

    public void appendCqlTo(CqlBuilder builder) {
        String classname = "org.apache.cassandra.locator".equals(klass.getPackage().getName()) ? klass.getSimpleName() : klass.getName();
        builder.append("{'class': ").appendWithSingleQuotes(classname);
        options.forEach((k, v) -> {
            builder.append(", ").appendWithSingleQuotes(k).append(": ").appendWithSingleQuotes(v);
        });
        builder.append('}');
    }
}
