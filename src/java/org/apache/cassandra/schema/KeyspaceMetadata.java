...
public final class KeyspaceMetadata implements SchemaElement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(KeyspaceMetadata.class);
    ...
    public final transient KeyspaceParams params;

    public final transient Tables tables;

    public final transient Views views;

    public final transient Types types;

    public final transient Functions functions;
    ...
    public KeyspaceMetadata withUpdatedUserType(UserType udt) {
        return new KeyspaceMetadata(name, kind, params, tables.withUpdatedUserType(udt), views.withUpdatedUserTypes(udt), types.withUpdatedUserType(udt), functions.withUpdatedUserType(udt));
    }
    ...
    public TableMetadata getTableOrViewNullable(String tableOrViewName) {
        ...
        return view == null ? tables.getNullable(tableOrViewName) : view.metadata;
    }
    ...
    public Optional<TableMetadata> findIndexedTable(String indexName) {
        for (TableMetadata table : tablesAndViews()) if (table.indexes.has(indexName))
    ...
    public boolean equals(Object o) {
        ...
        return name.equals(other.name) && kind == other.kind && params.equals(other.params) && tables.equals(other.tables) && views.equals(other.views) && functions.equals(other.functions) && types.equals(other.types);
    }

    @Override
    public String toString() {
        return MoreObjects.toStringHelper(this).add("name", name).add("kind", kind).add("params", params).add("tables", tables).add("views", views).add("functions", functions).add("types", types).toString();
    }
    ...
    public void validate() {
        if (!SchemaConstants.isValidName(name)) {
            throw new ConfigurationException(format("Keyspace name must not be empty, more than %s characters long, " + "or contain non-alphanumeric-underscore characters (got \"%s\")", SchemaConstants.NAME_LENGTH, name));
        }
    ...
    public AbstractReplicationStrategy createReplicationStrategy() {
        return AbstractReplicationStrategy.createReplicationStrategy(name, params.replication.klass, StorageService.instance.getTokenMetadata(), DatabaseDescriptor.getEndpointSnitch(), params.replication.options);
    }
    ...
    public static final class KeyspaceDiff {
        ...
        public final transient KeyspaceMetadata before;

        public final transient KeyspaceMetadata after;

        public final transient TablesDiff tables;

        public final transient ViewsDiff views;

        public final transient TypesDiff types;

        public final transient FunctionsDiff<UDFunction> udfs;

        public final transient FunctionsDiff<UDAggregate> udas;

        private KeyspaceDiff(KeyspaceMetadata before, KeyspaceMetadata after, TablesDiff tables, ViewsDiff views, TypesDiff types, FunctionsDiff<UDFunction> udfs, FunctionsDiff<UDAggregate> udas) {
            this.before = before;
            this.after = after;
            this.tables = tables;
            this.views = views;
        ...
        private static Optional<KeyspaceDiff> diff(KeyspaceMetadata before, KeyspaceMetadata after) {
            ...
            FunctionsDiff<UDFunction> udfs = FunctionsDiff.NONE;
            @SuppressWarnings("unchecked")
            FunctionsDiff<UDAggregate> udas = FunctionsDiff.NONE;
