...
final class SchemaMigrationDiagnostics {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SchemaMigrationDiagnostics.class);
    ...
    static void unknownLocalSchemaVersion(InetAddressAndPort endpoint, UUID theirVersion) {
        if (isEnabled(MigrationManagerEventType.UNKNOWN_LOCAL_SCHEMA_VERSION))
            service.publish(new SchemaMigrationEvent(MigrationManagerEventType.UNKNOWN_LOCAL_SCHEMA_VERSION, endpoint, theirVersion));
    }
    ...
    static void taskRequestSend(InetAddressAndPort endpoint) {
        if (isEnabled(MigrationManagerEventType.TASK_REQUEST_SEND))
            service.publish(new SchemaMigrationEvent(MigrationManagerEventType.TASK_REQUEST_SEND, endpoint, null));
    }
