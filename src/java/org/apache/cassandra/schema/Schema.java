...
public final class Schema implements SchemaProvider {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Schema.class);

    public static final transient Schema instance = new Schema();

    private volatile transient Keyspaces keyspaces = Keyspaces.none();

    // UUID -> mutable metadata ref map. We have to update these in place every time a table changes.
    private final transient Map<TableId, TableMetadataRef> metadataRefs = new NonBlockingHashMap<>();

    // (keyspace name, index name) -> mutable metadata ref map. We have to update these in place every time an index changes.
    private final transient Map<Pair<String, String>, TableMetadataRef> indexMetadataRefs = new NonBlockingHashMap<>();

    // Keyspace objects, one per keyspace. Only one instance should ever exist for any given keyspace.
    private final transient Map<String, Keyspace> keyspaceInstances = new NonBlockingHashMap<>();

    private volatile transient UUID version;

    private final transient List<SchemaChangeListener> changeListeners = new CopyOnWriteArrayList<>();
    ...
    private void loadNew(KeyspaceMetadata ksm) {
        ksm.tablesAndViews().forEach(metadata -> metadataRefs.put(metadata.id, new TableMetadataRef(metadata)));
        ksm.tables.indexTables().forEach((name, metadata) -> indexMetadataRefs.put(Pair.create(ksm.name, name), new TableMetadataRef(metadata)));
    ...
    synchronized void unload(KeyspaceMetadata ksm) {
        ...
        ksm.tablesAndViews().forEach(t -> metadataRefs.remove(t.id));
        ksm.tables.indexTables().keySet().forEach(name -> indexMetadataRefs.remove(Pair.create(ksm.name, name)));
    ...
    public List<String> getNonLocalStrategyKeyspaces() {
        return keyspaces.stream().filter(keyspace -> keyspace.params.replication.klass != LocalStrategy.class).map(keyspace -> keyspace.name).collect(Collectors.toList());
    }
    ...
    public TableMetadataRef getTableMetadataRef(String keyspace, String table) {
        ...
        return tm == null ? null : metadataRefs.get(tm.id);
    }
    ...
    public static final class TransformationResult {
        ...
        public final transient RuntimeException exception;

        public final transient KeyspacesDiff diff;

        public final transient Collection<Mutation> mutations;
