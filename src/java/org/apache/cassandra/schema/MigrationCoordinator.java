...
public class MigrationCoordinator {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MigrationCoordinator.class);

    private static final transient Logger logger = LoggerFactory.getLogger(MigrationCoordinator.class);

    private static final transient Future<Void> FINISHED_FUTURE = Futures.immediateFuture(null);

    private static transient LongSupplier getUptimeFn = () -> ManagementFactory.getRuntimeMXBean().getUptime();
    ...
    private static final transient int MIGRATION_DELAY_IN_MS = CassandraRelevantProperties.MIGRATION_DELAY.getInt();

    private static final transient int MAX_OUTSTANDING_VERSION_REQUESTS = 3;

    public static final transient MigrationCoordinator instance = new MigrationCoordinator();

    /**
     * @see CassandraRelevantProperties#SCHEMA_PULL_BACKOFF_DELAY_MS
     */
    private static final transient long BACKOFF_DELAY_MS = CassandraRelevantProperties.SCHEMA_PULL_BACKOFF_DELAY_MS.getInt();

    /**
     * Holds the timestamps in ms for last pull request attempts.
     */
    private final transient WeakHashMap<InetAddressAndPort, Long> lastPullAttemptTimestamps = new WeakHashMap<>();
    ...
    private static final transient Set<UUID> IGNORED_VERSIONS = getIgnoredVersions();
    ...
    static class VersionInfo {
        ...
        final transient UUID version;

        /**
         * The set of endpoints containing this schema version
         */
        final transient Set<InetAddressAndPort> endpoints = Sets.newConcurrentHashSet();

        /**
         * The set of endpoints from which we are already fetching the schema
         */
        final transient Set<InetAddressAndPort> outstandingRequests = Sets.newConcurrentHashSet();

        /**
         * The queue of endpoints from which we are going to fetch the schema
         */
        final transient Deque<InetAddressAndPort> requestQueue = new ArrayDeque<>();

        /**
         * Threads waiting for schema synchronization are waiting until this object is signalled
         */
        private final transient WaitQueue waitQueue = new WaitQueue();

        /**
         * Whether this schema version have been received
         */
        volatile transient boolean receivedSchema;
        ...
        public String toString() {
            return "VersionInfo{" + "version=" + version + ", outstandingRequests=" + outstandingRequests + ", requestQueue=" + requestQueue + ", waitQueue.waiting=" + waitQueue.getWaiting() + ", receivedSchema=" + receivedSchema + '}';
        }
    }

    private final transient Map<UUID, VersionInfo> versionInfo = new HashMap<>();

    private final transient Map<InetAddressAndPort, UUID> endpointVersions = new HashMap<>();

    private final transient AtomicInteger inflightTasks = new AtomicInteger();

    private final transient Set<InetAddressAndPort> ignoredEndpoints = getIgnoredEndpoints();
    ...
    public synchronized Map<UUID, Set<InetAddressAndPort>> outstandingVersions() {
        ...
        for (VersionInfo info : versionInfo.values()) if (!info.wasReceived())
    ...
    protected boolean shouldPullImmediately(InetAddressAndPort endpoint, UUID version) {
        if (Schema.instance.isEmpty() || getUptimeFn.getAsLong() < MIGRATION_DELAY_IN_MS) {
            ...
            logger.debug("Immediately submitting migration task for {}, " + "schema versions: local={}, remote={}", endpoint, Schema.schemaVersionToString(Schema.instance.getVersion()), Schema.schemaVersionToString(version));
            return true;
        }
        return false;
    }
    ...
    class Callback implements RequestCallback<Collection<Mutation>> {
        ...
        final transient InetAddressAndPort endpoint;

        final transient VersionInfo info;
