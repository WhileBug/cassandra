...
public final class CompactionParams {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CompactionParams.class);

    private static final transient Logger logger = LoggerFactory.getLogger(CompactionParams.class);

    public enum Option {

        CLASS, ENABLED, MIN_THRESHOLD, MAX_THRESHOLD, PROVIDE_OVERLAPPING_TOMBSTONES;
    ...
    public static final transient int DEFAULT_MIN_THRESHOLD = 4;

    public static final transient int DEFAULT_MAX_THRESHOLD = 32;

    public static final transient boolean DEFAULT_ENABLED = true;

    public static final transient TombstoneOption DEFAULT_PROVIDE_OVERLAPPING_TOMBSTONES = TombstoneOption.valueOf(System.getProperty("default.provide.overlapping.tombstones", TombstoneOption.NONE.toString()).toUpperCase());

    public static final transient Map<String, String> DEFAULT_THRESHOLDS = ImmutableMap.of(Option.MIN_THRESHOLD.toString(), Integer.toString(DEFAULT_MIN_THRESHOLD), Option.MAX_THRESHOLD.toString(), Integer.toString(DEFAULT_MAX_THRESHOLD));

    public static final transient CompactionParams DEFAULT = new CompactionParams(SizeTieredCompactionStrategy.class, DEFAULT_THRESHOLDS, DEFAULT_ENABLED, DEFAULT_PROVIDE_OVERLAPPING_TOMBSTONES);

    private final transient Class<? extends AbstractCompactionStrategy> klass;

    private final transient ImmutableMap<String, String> options;

    private final transient boolean isEnabled;

    private final transient TombstoneOption tombstoneOption;
    ...
    public static CompactionParams create(Class<? extends AbstractCompactionStrategy> klass, Map<String, String> options) {
        ...
        String overlappingTombstoneParm = options.getOrDefault(Option.PROVIDE_OVERLAPPING_TOMBSTONES.toString(), DEFAULT_PROVIDE_OVERLAPPING_TOMBSTONES.toString()).toUpperCase();
        ...
        if (!tombstoneOptional.isPresent()) {
            throw new ConfigurationException(format("Invalid value %s for 'provide_overlapping_tombstones' compaction sub-option - must be one of the following [%s].", overlappingTombstoneParm, StringUtils.join(TombstoneOption.values(), ", ")));
        }
    ...
    public int minCompactionThreshold() {
        ...
        return threshold == null ? DEFAULT_MIN_THRESHOLD : Integer.parseInt(threshold);
    }

    public int maxCompactionThreshold() {
        String threshold = options.get(Option.MAX_THRESHOLD.toString());
        return threshold == null ? DEFAULT_MAX_THRESHOLD : Integer.parseInt(threshold);
    }
    ...
    public static Class<? extends AbstractCompactionStrategy> classFromName(String name) {
        ...
        if (!AbstractCompactionStrategy.class.isAssignableFrom(strategyClass)) {
            throw new ConfigurationException(format("Compaction strategy class %s is not derived from AbstractReplicationStrategy", className));
        }
    ...
    @Override
    public String toString() {
        return MoreObjects.toStringHelper(this).add("class", klass.getName()).add("options", options).toString();
    }
