...
public final class ViewMetadata implements SchemaElement {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ViewMetadata.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ViewMetadata.class);

    public final transient TableId baseTableId;

    public final transient String baseTableName;

    public final transient boolean includeAllColumns;
    ...
    public boolean equals(Object o) {
        ...
        return baseTableId.equals(other.baseTableId) && includeAllColumns == other.includeAllColumns && whereClause.equals(other.whereClause) && metadata.equals(other.metadata);
    }
    ...
    @Override
    public int hashCode() {
        return new HashCodeBuilder(29, 1597).append(baseTableId).append(includeAllColumns).append(whereClause).append(metadata).toHashCode();
    }

    @Override
    public String toString() {
        return new ToStringBuilder(this).append("baseTableId", baseTableId).append("baseTableName", baseTableName).append("includeAllColumns", includeAllColumns).append("whereClause", whereClause).append("metadata", metadata).toString();
    }
    ...
    public ViewMetadata withUpdatedUserType(UserType udt) {
        return referencesUserType(udt.name) ? copy(metadata.withUpdatedUserType(udt)) : this;
    }

    public ViewMetadata withRenamedPrimaryKeyColumn(ColumnIdentifier from, ColumnIdentifier to) {
        return new ViewMetadata(baseTableId, baseTableName, includeAllColumns, whereClause.renameIdentifier(from, to), metadata.unbuild().renamePrimaryKeyColumn(from, to).build());
    }

    public ViewMetadata withAddedRegularColumn(ColumnMetadata column) {
        return new ViewMetadata(baseTableId, baseTableName, includeAllColumns, whereClause, metadata.unbuild().addColumn(column).build());
    }
    ...
    public void appendCqlTo(CqlBuilder builder, boolean internals, boolean ifNotExists) {
        ...
        builder.append(metadata.toString()).append(" AS").newLine().increaseIndent().append("SELECT ");
        if (includeAllColumns) {
            builder.append('*');
        } else {
            builder.appendWithSeparators(metadata.allColumnsInSelectOrder(), (b, c) -> b.append(c.name), ", ");
        }
        builder.newLine().append("FROM ").appendQuotingIfNeeded(metadata.keyspace).append('.').appendQuotingIfNeeded(baseTableName).newLine().append("WHERE ").append(whereClause.toString()).newLine();
