...
public final class CachingParams {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CachingParams.class);
    ...
    private static final transient String ALL = "ALL";

    private static final transient String NONE = "NONE";

    static final transient boolean DEFAULT_CACHE_KEYS = true;

    static final transient int DEFAULT_ROWS_PER_PARTITION_TO_CACHE = 0;

    public static final transient CachingParams CACHE_NOTHING = new CachingParams(false, 0);

    public static final transient CachingParams CACHE_KEYS = new CachingParams(true, 0);

    public static final transient CachingParams CACHE_EVERYTHING = new CachingParams(true, Integer.MAX_VALUE);

    @VisibleForTesting
    public static transient CachingParams DEFAULT = new CachingParams(DEFAULT_CACHE_KEYS, DEFAULT_ROWS_PER_PARTITION_TO_CACHE);

    final transient boolean cacheKeys;

    final transient int rowsPerPartitionToCache;
    ...
    private static int rowsPerPartitionFromString(String value) {
        ...
        throw new ConfigurationException(format("Invalid value '%s' for caching sub-option '%s':" + " only '%s', '%s', and integer values are allowed", value, Option.ROWS_PER_PARTITION, ALL, NONE));
    }
    ...
    @Override
    public String toString() {
        return format("{'%s' : '%s', '%s' : '%s'}", Option.KEYS, keysAsString(), Option.ROWS_PER_PARTITION, rowsPerPartitionAsString());
    }
