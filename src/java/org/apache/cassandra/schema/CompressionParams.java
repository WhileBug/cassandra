...
public final class CompressionParams {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CompressionParams.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CompressionParams.class);

    private static final transient Logger logger = LoggerFactory.getLogger(CompressionParams.class);

    private static volatile transient boolean hasLoggedSsTableCompressionWarning;

    private static volatile transient boolean hasLoggedChunkLengthWarning;

    private static volatile transient boolean hasLoggedCrcCheckChanceWarning;

    public static final transient int DEFAULT_CHUNK_LENGTH = 1024 * 16;

    // Since pre-4.0 versions do not understand the
    public static final transient double DEFAULT_MIN_COMPRESS_RATIO = 0.0;

    // new compression parameter we can't use a
    // different default value.
    public static final transient IVersionedSerializer<CompressionParams> serializer = new Serializer();

    public static final transient String CLASS = "class";

    public static final transient String CHUNK_LENGTH_IN_KB = "chunk_length_in_kb";

    public static final transient String ENABLED = "enabled";

    public static final transient String MIN_COMPRESS_RATIO = "min_compress_ratio";

    public static final transient CompressionParams DEFAULT = new CompressionParams(LZ4Compressor.create(Collections.<String, String>emptyMap()), DEFAULT_CHUNK_LENGTH, calcMaxCompressedLength(DEFAULT_CHUNK_LENGTH, DEFAULT_MIN_COMPRESS_RATIO), DEFAULT_MIN_COMPRESS_RATIO, Collections.emptyMap());

    public static final transient CompressionParams NOOP = new // 4 KiB is often the underlying disk block size
    CompressionParams(// 4 KiB is often the underlying disk block size
    NoopCompressor.create(Collections.emptyMap()), 1024 * 4, Integer.MAX_VALUE, DEFAULT_MIN_COMPRESS_RATIO, Collections.emptyMap());

    private static final transient String CRC_CHECK_CHANCE_WARNING = "The option crc_check_chance was deprecated as a compression option. " + "You should specify it as a top-level table option instead";

    @Deprecated
    public static final transient String SSTABLE_COMPRESSION = "sstable_compression";

    @Deprecated
    public static final transient String CHUNK_LENGTH_KB = "chunk_length_kb";

    @Deprecated
    public static final transient String CRC_CHECK_CHANCE = "crc_check_chance";

    private final transient ICompressor sstableCompressor;

    private final transient int chunkLength;

    // In content we store max length to avoid rounding errors causing compress/decompress mismatch.
    private final transient int maxCompressedLength;

    // In configuration we store min ratio, the input parameter.
    private final transient double minCompressRatio;

    // Unrecognized options, can be used by the compressor
    private final transient ImmutableMap<String, String> otherOptions;

    // TODO: deprecated, should now be carefully removed. Doesn't affect schema code as it isn't included in equals() and hashCode()
    private volatile transient double crcCheckChance = 1.0;
    ...
    public static CompressionParams noCompression() {
        logger_IC.info("[InconsistencyDetector][org.apache.cassandra.schema.CompressionParams.DEFAULT]=" + org.json.simple.JSONValue.toJSONString(DEFAULT).replace("\n", "").replace("\r", ""));
        logger_IC.info("[InconsistencyDetector][org.apache.cassandra.schema.CompressionParams.DEFAULT]=" + org.json.simple.JSONValue.toJSONString(DEFAULT).replace("\n", "").replace("\r", ""));
    ...
    private static Map<String, String> copyOptions(Map<? extends CharSequence, ? extends CharSequence> co) {
        ...
        for (Map.Entry<? extends CharSequence, ? extends CharSequence> entry : co.entrySet()) compressionOptions.put(entry.getKey().toString(), entry.getValue().toString());
    ...
    public boolean equals(Object obj) {
        ...
        return Objects.equal(sstableCompressor, cp.sstableCompressor) && chunkLength == cp.chunkLength && otherOptions.equals(cp.otherOptions) && minCompressRatio == cp.minCompressRatio;
    }

    @Override
    public int hashCode() {
        return new HashCodeBuilder(29, 1597).append(sstableCompressor).append(chunkLength).append(otherOptions).append(minCompressRatio).toHashCode();
    }
    ...
    static class Serializer implements IVersionedSerializer<CompressionParams> {
        ...
        public CompressionParams deserialize(DataInputPlus in, int version) throws IOException {
            ...
            // Earlier Cassandra cannot use uncompressed chunks.
