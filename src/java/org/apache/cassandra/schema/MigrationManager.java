...
public class MigrationManager {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MigrationManager.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MigrationManager.class);

    private static final transient Logger logger = LoggerFactory.getLogger(MigrationManager.class);

    public static final transient MigrationManager instance = new MigrationManager();
    ...
    private static boolean shouldPushSchemaTo(InetAddressAndPort endpoint) {
        ...
        return !endpoint.equals(FBUtilities.getBroadcastAddressAndPort()) && MessagingService.instance().versions.knows(endpoint) && MessagingService.instance().versions.getRaw(endpoint) == MessagingService.current_version;
    }
    ...
    public static KeyspacesDiff announce(SchemaTransformation transformation, boolean locally) {
        ...
        Future<Schema.TransformationResult> future = MIGRATION.submit(() -> Schema.instance.transform(transformation, locally, now));
    ...
    public static class MigrationsSerializer implements IVersionedSerializer<Collection<Mutation>> {
        ...
        public static transient MigrationsSerializer instance = new MigrationsSerializer();

        public void serialize(Collection<Mutation> schema, DataOutputPlus out, int version) throws IOException {
            out.writeInt(schema.size());
            for (Mutation mutation : schema) Mutation.serializer.serialize(mutation, out, version);
        }

        public Collection<Mutation> deserialize(DataInputPlus in, int version) throws IOException {
            int count = in.readInt();
            Collection<Mutation> schema = new ArrayList<>(count);
            for (int i = 0; i < count; i++) schema.add(Mutation.serializer.deserialize(in, version));
            return schema;
        }

        public long serializedSize(Collection<Mutation> schema, int version) {
            int size = TypeSizes.sizeof(schema.size());
            for (Mutation mutation : schema) size += mutation.serializedSize(version);
            return size;
        }
    }
}
