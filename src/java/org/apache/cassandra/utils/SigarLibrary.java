...
public class SigarLibrary {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SigarLibrary.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SigarLibrary.class);

    private transient Logger logger = LoggerFactory.getLogger(SigarLibrary.class);

    public static final transient SigarLibrary instance = new SigarLibrary();

    private transient Sigar sigar;

    private transient FileSystemMap mounts = null;

    private transient boolean initialized = false;

    private transient long INFINITY = -1;

    // number of files that can be opened
    private transient long EXPECTED_MIN_NOFILE = 10000l;

    // number of processes
    private transient long EXPECTED_NPROC = 32768l;

    // address space
    private transient long EXPECTED_AS = INFINITY;
    ...
    public void warnIfRunningInDegradedMode() {
        if (initialized) {
            ...
            if (swapEnabled || !goodAddressSpace || !goodFileLimits || !goodProcNumber) {
                logger.warn("Cassandra server running in degraded mode. Is swap disabled? : {},  Address space adequate? : {}, " + " nofile limit adequate? : {}, nproc limit adequate? : {} ", !swapEnabled, goodAddressSpace, goodFileLimits, goodProcNumber);
            } else {
