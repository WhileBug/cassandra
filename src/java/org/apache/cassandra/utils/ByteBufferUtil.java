...
public class ByteBufferUtil {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ByteBufferUtil.class);

    public static final transient ByteBuffer EMPTY_BYTE_BUFFER = ByteBuffer.wrap(new byte[0]);

    /**
     * Represents an unset value in bound variables
     */
    public static final transient ByteBuffer UNSET_BYTE_BUFFER = ByteBuffer.wrap(new byte[] {});

    public static final transient ByteBuffer[] EMPTY_ARRAY = new ByteBuffer[0];
    ...
    public static ByteBuffer[] minimizeBuffers(ByteBuffer[] src) {
        ...
        for (int i = 0; i < src.length; i++) dst[i] = src[i] != null ? minimalBufferFor(src[i]) : null;
        return dst;
    }
