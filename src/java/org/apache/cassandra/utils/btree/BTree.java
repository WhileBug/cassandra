...
public class BTree {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(BTree.class);
     ...
     */
    public static final transient int BRANCH_SHIFT = Integer.getInteger("cassandra.btree.branchshift", 5);

    private static final transient int BRANCH_FACTOR = 1 << BRANCH_SHIFT;

    public static final transient int MIN_KEYS = BRANCH_FACTOR / 2 - 1;

    public static final transient int MAX_KEYS = BRANCH_FACTOR - 1;

    // An empty BTree Leaf - which is the same as an empty BTree
    private static final transient Object[] EMPTY_LEAF = new Object[1];

    private static final transient int[][] DENSE_SIZE_MAPS = buildBalancedSizeMaps(BRANCH_SHIFT);

    private static final transient long[] PERFECT_DENSE_SIZE_ON_HEAP = sizeOnHeapOfPerfectTrees(BRANCH_SHIFT);
    ...
    private static <C, I extends C, O extends C> Object[] buildLeaf(BulkIterator<I> insert, int size, UpdateFunction<I, O> updateF) {
        // ensure that we have an odd-length array
        ...
        if (!isSimple(updateF)) {
            ...
            for (int i = 0; i < size; i++) values[i] = updateF.insert((I) values[i]);
        }
        return values;
    }
    ...
    public static <Compare, Existing extends Compare, Insert extends Compare> Object[] update(Object[] toUpdate, Object[] insert, Comparator<? super Compare> comparator, UpdateFunction<Insert, Existing> updateF) {
        ...
        if (isEmpty(insert))
            // do nothing if update is empty
            return toUpdate;
        if (isEmpty(toUpdate)) {
            if (isSimple(updateF))
                // if update is empty and updateF is trivial, return our new input
                return insert;
    ...
    public static <Compare, Existing extends Compare, Insert extends Compare> Object[] updateLeaves(Object[] unode, Object[] inode, Comparator<? super Compare> comparator, UpdateFunction<Insert, Existing> updateF) {
        ...
        int c = -1;
        while (// optimistic: find the first point in the original leaf that is modified (if any)
        c <= 0) {
            if (c < 0) {
                ...
                if (upos < usz && ipos < isz) {
                    ...
                    while (true) {
                        if (c == 0) {
                            ...
                            // must find greater or equal; set >= 0 (equal) to 0; set < 0 (greater) to c=+ve
                            c = until < 0 ? 1 : 0;
                            if (until < 0)
                                until = -(1 + until);
                            ...
                            // must find less or equal; set >= 0 (equal) to 0, otherwise leave intact
                            c = until & 0x80000000;
                            if (until < 0)
                                until = -(1 + until);
    ...
    private static void reverseInSitu(Object[] tree, int height, boolean copySizeMaps) {
        if (isLeaf(tree)) {
            ...
            for (int i = keyCount; i <= keyCount * 2; ++i) reverseInSitu((Object[]) tree[i], height - 1, copySizeMaps);
            int[] sizeMap = (int[]) tree[2 * keyCount + 1];
            if (// no need to reverse a dense map; same in both directions
            sizeMap != DENSE_SIZE_MAPS[height - 2]) {
                if (copySizeMaps)
    ...
    public static <V> Iterator<V> iterator(Object[] btree, Dir dir) {
        return isLeaf(btree) ? new LeafBTreeSearchIterator<>(btree, null, dir) : new FullBTreeSearchIterator<>(btree, null, dir);
    }

    public static <V> Iterator<V> iterator(Object[] btree, int lb, int ub, Dir dir) {
        return isLeaf(btree) ? new LeafBTreeSearchIterator<>(btree, null, dir, lb, ub) : new FullBTreeSearchIterator<>(btree, null, dir, lb, ub);
    }
    ...
    public static <K, V> BTreeSearchIterator<K, V> slice(Object[] btree, Comparator<? super K> comparator, Dir dir) {
        return isLeaf(btree) ? new LeafBTreeSearchIterator<>(btree, comparator, dir) : new FullBTreeSearchIterator<>(btree, comparator, dir);
    }
    ...
    public static <K, V extends K> BTreeSearchIterator<K, V> slice(Object[] btree, Comparator<? super K> comparator, int startIndex, int endIndex, Dir dir) {
        return isLeaf(btree) ? new LeafBTreeSearchIterator<>(btree, comparator, dir, startIndex, endIndex) : new FullBTreeSearchIterator<>(btree, comparator, dir, startIndex, endIndex);
    }
    ...
    public static <K, V extends K> BTreeSearchIterator<K, V> slice(Object[] btree, Comparator<? super K> comparator, K start, boolean startInclusive, K end, boolean endInclusive, Dir dir) {
        ...
        return isLeaf(btree) ? new LeafBTreeSearchIterator<>(btree, comparator, dir, inclusiveLowerBound, inclusiveUpperBound) : new FullBTreeSearchIterator<>(btree, comparator, dir, inclusiveLowerBound, inclusiveUpperBound);
    }
    ...
    public static long sizeOfStructureOnHeap(Object[] tree) {
        ...
        if (isLeaf(tree))
            return size;
        for (int i = getChildStart(tree); i < getChildEnd(tree); i++) size += sizeOfStructureOnHeap((Object[]) tree[i]);
        return size;
    }
    ...
    public static int toArray(Object[] tree, int treeStart, int treeEnd, Object[] target, int targetOffset) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childStart <= treeEnd && childEnd >= treeStart) {
                newTargetOffset += toArray((Object[]) tree[childOffset + i], max(0, treeStart - childStart), min(childEnd, treeEnd) - childStart, target, newTargetOffset);
                if (// this check will always fail for the non-existent key
                treeStart <= childEnd && treeEnd > childEnd)
    ...
    public static <I, O> Object[] transform(Object[] tree, Function<? super I, ? extends O> function) {
        if (// isEmpty determined by identity; must return input
        isEmpty(tree))
            return tree;
        if (// escape hatch for fast leaf transformation
        isLeaf(tree))
            return transformLeaf(tree, function);
        // optimistically assume we'll return our input unmodified
        Object[] result = tree;
        int keyCount = shallowSizeOfBranch(tree);
        for (int i = 0; i < keyCount; ++i) {
            ...
            if (result == tree) {
                if (curChild == updChild && curKey == updKey)
                    // if output still same as input, loop
                    continue;
        ...
        // take the original sizeMap, as we are exactly the same shape
    ...
    public static int hashCode(Object[] btree) {
        ...
        int result = 1;
        for (Object v : iterable(btree)) result = 31 * result + Objects.hashCode(v);
        return result;
    }
    ...
    public static class Builder<V> {
        ...
        transient Comparator<? super V> comparator;

        transient Object[] values;

        transient int count;

        // true if we have managed to cheaply ensure sorted (+ filtered, if resolver == null) as we have added
        transient boolean detected = true;

        // false if the user has promised to enforce the sort order and resolve any duplicates
        transient boolean auto = true;

        transient QuickResolver<V> quickResolver;
        ...
        private Builder<V> mergeAll(int addCount) {
            ...
            if (j == addEnd)
                // already a superset of the new values
                return this;
    ...
    public static <V, A> void applyLeaf(Object[] btree, BiConsumer<A, V> function, A argument) {
        ...
        for (int i = 0; i < limit; i++) applyValue((V) btree[i], function, argument);
    }
    ...
    private static int sizesToSizeMap(int[] sizeMap) {
        int total = sizeMap[0];
        for (int i = 1; i < sizeMap.length; ++i) sizeMap[i] = total += 1 + sizeMap[i];
        return total;
    }

    private static int sizesToSizeMap(int[] sizes, int count) {
        int total = sizes[0];
        for (int i = 1; i < count; ++i) sizes[i] = total += 1 + sizes[i];
        return total;
    }
    ...
    private static void sizeMapToSizes(int[] sizeMap) {
        for (int i = sizeMap.length; i > 1; --i) sizeMap[i] -= 1 + sizeMap[i - 1];
    }
    ...
    private static <Compare> int exponentialSearch(Comparator<? super Compare> comparator, Object[] in, int from, int to, Compare find) {
        ...
        while (from + step < to) {
            ...
            if (c == 0)
                return i;
            from = i + 1;
            // jump in perfect binary search increments
            step = step * 2 + 1;
        }
    ...
    private static long[] sizeOnHeapOfPerfectTrees(int branchShift) {
        ...
        for (int i = 1; i < result.length; ++i) result[i] = sizeOnHeapOfPerfectTree(i + 1, branchFactor);
        return result;
    }
    ...
    public static long sizeOnHeapOf(Object[] tree) {
        ...
        if (isLeaf(tree))
            return size;
        for (int i = childOffset(tree); i < childEndOffset(tree); i++) size += sizeOnHeapOf((Object[]) tree[i]);
        // may overcount, since we share size maps
        size += ObjectSizes.sizeOfArray(sizeMap(tree));
        return size;
    }

    // Arbitrary boundaries
    private static transient Object POSITIVE_INFINITY = new Object();

    private static transient Object NEGATIVE_INFINITY = new Object();
    ...
    private static abstract class LeafOrBranchBuilder {
        ...
        final transient int height;

        final transient LeafOrBranchBuilder child;

        transient BranchBuilder parent;
    ...
    private static abstract class LeafBuilder extends LeafOrBranchBuilder {
        ...
        <Insert, Existing> void copy(Object[] source, int offset, int length, UpdateFunction<Insert, Existing> apply) {
            ...
            if (count + length > MAX_KEYS) {
                int copy = MAX_KEYS - count;
                for (int i = 0; i < copy; ++i) buffer[count + i] = apply.insert((Insert) source[offset + i]);
                offset += copy;
                // implicitly:  leaf().count = MAX_KEYS;
                overflow(apply.insert((Insert) source[offset++]));
                length -= 1 + copy;
            }
            for (int i = 0; i < length; ++i) buffer[count + i] = apply.insert((Insert) source[offset + i]);
            count += length;
        }
    ...
    static class BranchBuilder extends LeafOrBranchBuilder {
        ...
        final transient LeafBuilder leaf;
        ...
        Object[] drainAndPropagate(Object[] unode, BranchBuilder propagateTo) {
            ...
            if (mustRedistribute()) {
                ...
                if (!hasOverflow() && usz == count && areIdentical(buffer, 0, unode, 0, usz) && areIdentical(buffer, MAX_KEYS, unode, usz, usz + 1)) {
                    branch = unode;
                    sizeOfBranch = sizeOfBranch(branch);
                } else {
                    if (hasOverflow())
        ...
        static void copySizeMapToSizes(int[] in, int inOffset, int[] out, int outOffset, int count) {
            ...
            for (int i = 0; i < count; ++i) out[outOffset + i] = in[inOffset + i] - (1 + in[inOffset + i - 1]);
        }
    }
    ...
    public static class FastBuilder<V> extends AbstractFastBuilder implements AutoCloseable {
        ...
        private static final transient TinyThreadLocalPool<FastBuilder<?>> POOL = new TinyThreadLocalPool<>();

        private transient TinyThreadLocalPool.TinyPool<FastBuilder<?>> pool;
        ...
        void reset() {
            ...
            while (branch != null && branch.inUse) {
                ...
                if (branch.savedBuffer != null && branch.savedBuffer[0] != null)
                    // by definition full, if non-empty
    ...
    private static class Updater<Compare, Existing extends Compare, Insert extends Compare> extends AbstractUpdater implements AutoCloseable {
        ...
        static final transient TinyThreadLocalPool<Updater> POOL = new TinyThreadLocalPool<>();

        transient TinyThreadLocalPool.TinyPool<Updater> pool;

        // the new tree we navigate linearly, and are always on a key or at the end
        final transient SimpleTreeKeysIterator<Compare, Insert> insert = new SimpleTreeKeysIterator<>();

        transient Comparator<? super Compare> comparator;

        transient UpdateFunction<Insert, Existing> updateF;
        ...
        Object[] update(Object[] update, Object[] insert, Comparator<? super Compare> comparator, UpdateFunction<Insert, Existing> updateF) {
            ...
            for (int i = 0; i < leafDepth; ++i) builder = builder.ensureParent();
        ...
        private Insert updateRecursive(Insert ik, Object[] unode, Existing uub, LeafOrBranchBuilder builder) {
            return builder == leaf() ? updateRecursive(ik, unode, uub, (LeafBuilder) builder) : updateRecursive(ik, unode, uub, (BranchBuilder) builder);
        }
        ...
        private Insert updateRecursive(Insert ik, Object[] unode, Existing uub, BranchBuilder builder) {
            ...
            while (ik != null) {
                ...
                if (find < 0)
                    find = -1 - find;
                if (find > usz)
                    // nothing else needs to be inserted in this branch
                    break;
                if (find > upos)
                ...
                if (c < 0) {
                    ...
                    if (// this was the right-most child, branch is complete and we can return immediately
                    find == usz)
                        return ik;
        ...
        private Insert updateRecursive(Insert ik, Object[] unode, Existing uub, LeafBuilder builder) {
            ...
            while (true) {
                if (c == 0) {
                    ...
                    c = -searchResultToComparison(ulim);
                    if (ulim < 0)
                        ulim = -(1 + ulim);
                    ...
                    c = insert.copyKeysSmallerThan(uk, comparator, builder, updateF);
                    ik = insert.next();
                    if (ik == null) {
                        builder.copy(unode, upos, usz - upos);
                        return null;
            ...
            if (uub == null || comparator.compare(ik, uub) < 0) {
                builder.addKey(isSimple(updateF) ? ik : updateF.insert(ik));
                // 0 on match, -1 otherwise
                insert.copyKeysSmallerThan(uub, comparator, builder, updateF);
                ik = insert.next();
            }
            return ik;
        }
    ...
    private static abstract class AbstractTransformer<I, O> extends AbstractUpdater implements AutoCloseable {
         ...
         */
        final transient SimpleTreeIterator update = new SimpleTreeIterator();
         ...
         */
        transient Object[][] queuedToFinish = new Object[1][];
        ...
        private Object[] apply() {
            ...
            while (true) {
                ...
                O nextKey;
                do {
                    // always have a node above leaf level, else we'd invoke transformLeaf
                    update.ascendToParent();
                    ...
                    if (// if we don't have a key, try to steal from leaf().buffer
                    nextKey == null && leaf().count > MIN_KEYS)
                    ...
                    while (nextKey == null && upos < usz) nextKey = apply((I) unode[upos++]);
    ...
    private static class Transformer<I, O> extends AbstractTransformer<I, O> {
        ...
        static final transient TinyThreadLocalPool<Transformer> POOL = new TinyThreadLocalPool<>();

        transient TinyThreadLocalPool.TinyPool<Transformer> pool;

        transient Function<? super I, ? extends O> apply;
    ...
    private static class BiTransformer<I, I2, O> extends AbstractTransformer<I, O> {
        ...
        static final transient TinyThreadLocalPool<BiTransformer> POOL = new TinyThreadLocalPool<>();

        transient BiFunction<? super I, ? super I2, ? extends O> apply;

        transient I2 i2;

        transient TinyThreadLocalPool.TinyPool<BiTransformer> pool;
    ...
    private static class SimpleTreeKeysIterator<Compare, Insert extends Compare> {
        ...
        transient int leafSize;

        transient int leafPos;

        transient Object[] leaf;
        ...
        private void advanceBranch(Object[] node, int position) {
            ...
            if (position < count)
                ...
                // no more children in this branch, remove from stack
                --depth;
        ...
        <Update> int copyKeysSmallerThan(Compare bound, Comparator<? super Compare> comparator, LeafBuilder builder, UpdateFunction<Insert, Update> transformer) {
            while (true) {
                ...
                if (end < leafSize)
                    // 0 if next is a match for bound, -1 otherwise
