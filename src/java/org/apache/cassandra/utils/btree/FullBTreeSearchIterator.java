...
public class FullBTreeSearchIterator<K, V> extends TreeCursor<K> implements BTreeSearchIterator<K, V> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(FullBTreeSearchIterator.class);
    ...
    // only "exists" as an absence of other states
    private static final transient int MIDDLE = 0;

    // may only co-exist with LAST (or MIDDLE, which is 0)
    private static final transient int ON_ITEM = 1;

    // may not coexist with any other state
    private static final transient int BEFORE_FIRST = 2;

    // may co-exist with ON_ITEM, in which case we are also at END
    private static final transient int LAST = 4;

    // equal to LAST | ON_ITEM
    private static final transient int END = 5;
