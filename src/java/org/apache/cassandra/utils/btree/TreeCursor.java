...
class TreeCursor<K> extends NodeCursor<K> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TreeCursor.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TreeCursor.class);

    // TODO: spend some time optimising compiler inlining decisions: many of these methods have only one primary call-site
    transient NodeCursor<K> cur;
    ...
    boolean seekTo(K key, boolean forwards, boolean skipOne) {
        ...
        if (tryOne) {
            ...
            K test = cur.value();
            int cmp;
            if (// check object identity first, since we utilise that in some places and it's very cheap
            key == test)
                // check object identity first, since we utilise that in some places and it's very cheap
                cmp = 0;
            else
                // order of provision matters for asymmetric comparators
                cmp = comparator.compare(test, key);
        ...
        while (cur != root()) {
            ...
            if (bound == null)
                ...
                break;
            // order of provision matters for asymmetric comparators
            int cmpbound = comparator.compare(bound.bound(forwards), key);
            if (forwards ? cmpbound > 0 : cmpbound < 0)
                // already in correct sub-tree
                break;
            ...
            if (// it was an exact match, so terminate here
            cmpbound == 0) {
                this.cur = cur;
                return true;
            }
    ...
    void seekTo(int index) {
        ...
        while (true) {
            // index within subtree rooted at cur
