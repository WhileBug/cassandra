...
public class CassandraVersion implements Comparable<CassandraVersion> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CassandraVersion.class);
     ...
     */
    private static final transient String VERSION_REGEXP = "(\\d+)\\.(\\d+)(?:\\.(\\w+))?(?:\\.(\\w+))?(\\-[-.\\w]+)?([.+][.\\w]+)?";

    private static final transient Pattern PATTERN_WORDS = Pattern.compile("\\w+");

    @VisibleForTesting
    static final transient int NO_HOTFIX = -1;

    private static final transient Pattern PATTERN = Pattern.compile(VERSION_REGEXP);

    public static final transient CassandraVersion CASSANDRA_4_0 = new CassandraVersion("4.0").familyLowerBound.get();

    public static final transient CassandraVersion CASSANDRA_4_0_RC2 = new CassandraVersion(4, 0, 0, NO_HOTFIX, new String[] { "rc2" }, null);

    public static final transient CassandraVersion CASSANDRA_3_4 = new CassandraVersion("3.4").familyLowerBound.get();

    public final transient int major;

    public final transient int minor;

    public final transient int patch;

    public final transient int hotfix;

    public final transient Supplier<CassandraVersion> familyLowerBound = Suppliers.memoize(this::getFamilyLowerBound);

    private final transient String[] preRelease;

    private final transient String[] build;
    ...
    private CassandraVersion getFamilyLowerBound() {
        return patch == 0 && hotfix == NO_HOTFIX && preRelease != null && preRelease.length == 0 && build == null ? this : new CassandraVersion(major, minor, 0, NO_HOTFIX, ArrayUtils.EMPTY_STRING_ARRAY, null);
    }
    ...
    public boolean equals(Object o) {
        ...
        return major == that.major && minor == that.minor && patch == that.patch && hotfix == that.hotfix && Arrays.equals(preRelease, that.preRelease) && Arrays.equals(build, that.build);
    }
