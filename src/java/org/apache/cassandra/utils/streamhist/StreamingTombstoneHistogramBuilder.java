...
public class StreamingTombstoneHistogramBuilder {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamingTombstoneHistogramBuilder.class);
    ...
    private transient Spool spool;

    // voluntarily give up resolution for speed
    private final transient int roundSeconds;
    ...
    public void update(int point, int value) {
        ...
        if (spool.capacity > 0) {
            if (!spool.tryAddOrAccumulate(point, value)) {
                ...
                // after spool flushing we should always be able to insert new value
    ...
    static class DataHolder {
        ...
        private static final transient long EMPTY = Long.MAX_VALUE;
        ...
        boolean addValue(int point, int delta) {
            ...
            if (index < 0) {
                ...
                if (// ok, someone else at this point, let's shift array and insert
                unwrapPoint(data[index]) != point) {
    ...
    public static int saturatingCastToMaxDeletionTime(long value) {
        return (value < 0L || value > Cell.MAX_DELETION_TIME) ? Cell.MAX_DELETION_TIME : (int) value;
    }
}
