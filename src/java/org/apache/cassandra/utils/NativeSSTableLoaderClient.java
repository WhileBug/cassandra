...
public class NativeSSTableLoaderClient extends SSTableLoader.Client {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(NativeSSTableLoaderClient.class);

    protected final transient Map<String, TableMetadataRef> tables;

    private final transient Collection<InetSocketAddress> hosts;

    private final transient int storagePort;

    private final transient AuthProvider authProvider;

    private final transient SSLOptions sslOptions;
    ...
    private static TableMetadataRef createTableMetadata(String keyspace, Session session, IPartitioner partitioner, boolean isView, Row row, String name, Types types) {
        ...
        String columnsQuery = String.format("SELECT * FROM %s.%s WHERE keyspace_name = ? AND table_name = ?", SchemaConstants.SCHEMA_KEYSPACE_NAME, SchemaKeyspaceTables.COLUMNS);
        for (Row colRow : session.execute(columnsQuery, keyspace, name)) builder.addColumn(createDefinitionFromRow(colRow, keyspace, name, types));
        String droppedColumnsQuery = String.format("SELECT * FROM %s.%s WHERE keyspace_name = ? AND table_name = ?", SchemaConstants.SCHEMA_KEYSPACE_NAME, SchemaKeyspaceTables.DROPPED_COLUMNS);
