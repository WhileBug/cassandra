...
public class MerkleTree {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MerkleTree.class);

    private static final transient Logger logger = LoggerFactory.getLogger(MerkleTree.class);

    // 2xMM3_128 = 32 bytes.
    private static final transient int HASH_SIZE = 32;

    private static final transient byte[] EMPTY_HASH = new byte[HASH_SIZE];

    /*
     * Thread-local byte array, large enough to host 32B of digest or MM3/Random partitoners' tokens
     */
    private static final transient ThreadLocal<byte[]> byteArray = ThreadLocal.withInitial(() -> new byte[HASH_SIZE]);

    private static byte[] getTempArray(int minimumSize) {
        return minimumSize <= HASH_SIZE ? byteArray.get() : new byte[minimumSize];
    }

    public static final transient byte RECOMMENDED_DEPTH = Byte.MAX_VALUE - 1;

    private final transient int hashdepth;

    /**
     * The top level range that this MerkleTree covers.
     */
    final transient Range<Token> fullRange;

    private final transient IPartitioner partitioner;

    private transient long maxsize;
    ...
    enum Difference {
        ...
        CONSISTENT, FULLY_INCONSISTENT, PARTIALLY_INCONSISTENT
    }
    ...
    private Node findHelper(Node current, Range<Token> activeRange, Range<Token> find) throws StopRecursion {
        while (true) {
            ...
            if (// this node is fully contained in the range
            find.contains(activeRange))
            ...
            if (// left child contains/matches the range
            leftRange.contains(find)) {
                activeRange = leftRange;
                current = inner.left();
            } else if (// right child contains/matches the range
            rightRange.contains(find)) {
    ...
    private OnHeapNode splitHelper(Node node, Token pleft, Token pright, int depth, Token t) throws StopRecursion.TooDeep {
        ...
        if (// left child contains token
        Range.contains(pleft, inner.token(), t))
    ...
    public static class TreeRange extends Range<Token> {
        ...
        private final transient MerkleTree tree;

        public final transient int depth;
    ...
    public static class TreeRangeIterator extends AbstractIterator<TreeRange> implements Iterable<TreeRange>, PeekingIterator<TreeRange> {
        ...
        private final transient ArrayDeque<TreeRange> tovisit;
    ...
    public static class RowHash {
        ...
        public final transient byte[] hash;

        public final transient long size;
    ...
    public long serializedSize(int version) {
        long size = // mt.hashdepth
        1 + sizeof(maxsize) + sizeof(this.size) + sizeof(partitioner.getClass().getCanonicalName());
    ...
    private static transient boolean warnedOnce;
    ...
    private static Node deserializeTree(DataInputPlus in, IPartitioner partitioner, int innerNodeCount, boolean offHeapRequested, int version) throws IOException {
        return shouldUseOffHeapTrees(partitioner, offHeapRequested) ? deserializeOffHeap(in, partitioner, innerNodeCount, version) : OnHeapNode.deserialize(in, partitioner, version);
    }
    ...
    MerkleTree tryMoveOffHeap() throws IOException {
        return root instanceof OnHeapNode && shouldUseOffHeapTrees(partitioner, DatabaseDescriptor.useOffheapMerkleTrees()) ? moveOffHeap() : this;
    }
    ...
    static abstract class OnHeapNode implements Node {
        ...
        transient long sizeOfRange;

        transient long partitionsInRange;
        ...
        public boolean hashesDiffer(Node other) {
            return other instanceof OnHeapNode ? hashesDiffer((OnHeapNode) other) : hashesDiffer((OffHeapNode) other);
        }
    ...
    static abstract class OffHeapNode implements Node {
        ...
        protected final transient ByteBuffer buffer;
        ...
        private boolean hashesDiffer(OffHeapNode other) {
            ...
            for (int i = 0; i < HASH_SIZE; i += 8) if (buffer().getLong(thisOffset + i) != other.buffer().getLong(otherOffset + i))
                return true;
            return false;
        }
    ...
    interface Leaf extends Node {
        ...
        static final transient byte IDENT = 1;
    ...
    static class OffHeapLeaf extends OffHeapNode implements Leaf {
        ...
        static final transient int HASH_BYTES_OFFSET = 0;
    ...
    interface Inner extends Node {
        ...
        static final transient byte IDENT = 2;
        ...
        default int serializedSize(int version) {
            return 1 + (int) Token.serializer.serializedSize(token(), version) + left().serializedSize(version) + right().serializedSize(version);
        }

        default void toString(StringBuilder buff, int maxdepth) {
            buff.append("#<").append(getClass().getSimpleName()).append(' ').append(token()).append(" hash=").append(Node.toString(hash())).append(" children=[");
            if (maxdepth < 1) {
                buff.append('#');
            } else {
                Node left = left();
    ...
    static class OnHeapInner extends OnHeapNode implements Inner {
        ...
        private transient OnHeapNode left;

        private transient OnHeapNode right;
        ...
        public Node fillInnerHashes() {
            if (// hash and size haven't been calculated; compute children then compute this
            !computed) {
        ...
        int serializeOffHeap(ByteBuffer buffer, IPartitioner partitioner) throws IOException {
            ...
            for (int i = 0; i < HASH_SIZE; i += 8) {
                buffer.putLong(offset + OffHeapInner.HASH_BYTES_OFFSET + i, buffer.getLong(leftHashOffset + i) ^ buffer.getLong(rightHashOffset + i));
            }
            return offset;
        }
    ...
    static class OffHeapInner extends OffHeapNode implements Inner {
         ...
         */
        static final transient int LEFT_CHILD_POINTER_OFFSET = 0;

        static final transient int RIGHT_CHILD_POINTER_OFFSET = 4;

        static final transient int HASH_BYTES_OFFSET = 8;

        static final transient int TOKEN_LENGTH_OFFSET = 8 + HASH_SIZE;

        static final transient int TOKEN_BYTES_OFFSET = TOKEN_LENGTH_OFFSET + 2;
        ...
        static int maxOffHeapSize(IPartitioner partitioner) {
            return // left pointer
            4 + // right pointer
            4 + HASH_SIZE + 2 + partitioner.getMaxTokenSize();
        }
    ...
    static byte[] xor(byte[] left, byte[] right) {
        ...
        for (int i = 0; i < left.length; i++) out[i] = (byte) ((left[i] & 0xFF) ^ (right[i] & 0xFF));
        return out;
    }
    ...
    private static void xorOntoLeft(byte[] left, byte[] right) {
        assert left.length == right.length;
        for (int i = 0; i < left.length; i++) left[i] = (byte) ((left[i] & 0xFF) ^ (right[i] & 0xFF));
    }
    ...
    private void unsafeInvalidateHelper(Node node, Token pleft, Token t) {
        ...
        if (Range.contains(pleft, inner.token(), t))
            ...
            unsafeInvalidateHelper(inner.left(), pleft, t);
        else
            // right child contains token
            unsafeInvalidateHelper(inner.right(), inner.token(), t);
    }
