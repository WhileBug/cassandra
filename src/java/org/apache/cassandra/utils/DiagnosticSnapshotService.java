...
public class DiagnosticSnapshotService {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(DiagnosticSnapshotService.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(DiagnosticSnapshotService.class);

    private static final transient Logger logger = LoggerFactory.getLogger(DiagnosticSnapshotService.class);

    public static final transient DiagnosticSnapshotService instance = new DiagnosticSnapshotService(Executors.newSingleThreadExecutor(new NamedThreadFactory("DiagnosticSnapshot")));

    public static final transient String REPAIRED_DATA_MISMATCH_SNAPSHOT_PREFIX = "RepairedDataMismatch-";

    public static final transient String DUPLICATE_ROWS_DETECTED_SNAPSHOT_PREFIX = "DuplicateRows-";
    ...
    private static final transient long SNAPSHOT_INTERVAL_NANOS = TimeUnit.MINUTES.toNanos(1);

    private static final transient DateTimeFormatter DATE_FORMAT = DateTimeFormatter.BASIC_ISO_DATE;

    private final transient ConcurrentHashMap<TableId, AtomicLong> lastSnapshotTimes = new ConcurrentHashMap<>();
    ...
    private void maybeTriggerSnapshot(TableMetadata metadata, String prefix, Iterable<InetAddressAndPort> endpoints) {
        ...
        if (now - last > interval && cached.compareAndSet(last, now)) {
            Message<SnapshotCommand> msg = Message.out(Verb.SNAPSHOT_REQ, new SnapshotCommand(metadata.keyspace, metadata.name, getSnapshotName(prefix), false));
            for (InetAddressAndPort replica : endpoints) MessagingService.instance().send(msg, replica);
        } else {
    ...
    private static class DiagnosticSnapshotTask implements Runnable {
        ...
        final transient SnapshotCommand command;
        ...
        public void run() {
            try {
                ...
                if (ks == null) {
                    logger.info("Snapshot request received from {} for {}.{} but keyspace not found", from, command.keyspace, command.column_family);
                    return;
                }
                ColumnFamilyStore cfs = ks.getColumnFamilyStore(command.column_family);
                if (cfs.snapshotExists(command.snapshot_name)) {
                    logger.info("Received diagnostic snapshot request from {} for {}.{}, " + "but snapshot with tag {} already exists", from, command.keyspace, command.column_family, command.snapshot_name);
                    return;
                }
                logger.info("Creating snapshot requested by {} of {}.{} tag: {}", from, command.keyspace, command.column_family, command.snapshot_name);
                cfs.snapshot(command.snapshot_name);
            } catch (IllegalArgumentException e) {
                logger.warn("Snapshot request received from {} for {}.{} but CFS not found", from, command.keyspace, command.column_family);
            }
        }
    }
}
