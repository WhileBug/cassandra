...
public abstract class MemoryUtil {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MemoryUtil.class);

    // copied from java.nio.Bits
    private static final transient long UNSAFE_COPY_THRESHOLD = 1024 * 1024L;

    private static final transient Unsafe unsafe;

    private static final transient Class<?> DIRECT_BYTE_BUFFER_CLASS, RO_DIRECT_BYTE_BUFFER_CLASS;

    private static final transient long DIRECT_BYTE_BUFFER_ADDRESS_OFFSET;

    private static final transient long DIRECT_BYTE_BUFFER_CAPACITY_OFFSET;

    private static final transient long DIRECT_BYTE_BUFFER_LIMIT_OFFSET;

    private static final transient long DIRECT_BYTE_BUFFER_POSITION_OFFSET;

    private static final transient long DIRECT_BYTE_BUFFER_ATTACHMENT_OFFSET;

    private static final transient Class<?> BYTE_BUFFER_CLASS;

    private static final transient long BYTE_BUFFER_OFFSET_OFFSET;

    private static final transient long BYTE_BUFFER_HB_OFFSET;

    private static final transient long BYTE_ARRAY_BASE_OFFSET;

    private static final transient boolean BIG_ENDIAN = ByteOrder.nativeOrder().equals(ByteOrder.BIG_ENDIAN);

    public static final transient boolean INVERTED_ORDER = Architecture.IS_UNALIGNED && !BIG_ENDIAN;
    ...
    public static int getIntByByte(long address) {
        if (BIG_ENDIAN) {
            return (((int) unsafe.getByte(address)) << 24) | (((int) unsafe.getByte(address + 1) & 0xff) << 16) | (((int) unsafe.getByte(address + 2) & 0xff) << 8) | (((int) unsafe.getByte(address + 3) & 0xff));
        } else {
            return (((int) unsafe.getByte(address + 3)) << 24) | (((int) unsafe.getByte(address + 2) & 0xff) << 16) | (((int) unsafe.getByte(address + 1) & 0xff) << 8) | (((int) unsafe.getByte(address) & 0xff));
        }
    }
