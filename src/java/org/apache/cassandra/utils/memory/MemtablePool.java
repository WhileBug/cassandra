...
public abstract class MemtablePool {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MemtablePool.class);

    final transient MemtableCleanerThread<?> cleaner;

    // the total memory used by this pool
    public final transient SubPool onHeap;

    public final transient SubPool offHeap;

    public final transient Timer blockedOnAllocating;

    public final transient Gauge<Long> numPendingTasks;

    final transient WaitQueue hasRoom = new WaitQueue();
    ...
    MemtablePool(long maxOnHeapMemory, long maxOffHeapMemory, float cleanThreshold, MemtableCleaner cleaner) {
        ...
        numPendingTasks = CassandraMetricsRegistry.Metrics.register(nameFactory.createMetricName("PendingFlushTasks"), () -> (long) this.cleaner.numPendingTasks());
    }
    ...
    private static final transient AtomicLongFieldUpdater<SubPool> reclaimingUpdater = AtomicLongFieldUpdater.newUpdater(SubPool.class, "reclaiming");

    private static final transient AtomicLongFieldUpdater<SubPool> allocatedUpdater = AtomicLongFieldUpdater.newUpdater(SubPool.class, "allocated");

    private static final transient AtomicLongFieldUpdater<SubPool> nextCleanUpdater = AtomicLongFieldUpdater.newUpdater(SubPool.class, "nextClean");
}
