...
public class MemtableCleanerThread<P extends MemtablePool> extends InfiniteLoopExecutor {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MemtableCleanerThread.class);

    private static final transient Logger logger = LoggerFactory.getLogger(MemtableCleanerThread.class);
    ...
    public static class Clean<P extends MemtablePool> implements InterruptibleRunnable {
         ...
         */
        final transient AtomicInteger numPendingTasks = new AtomicInteger(0);

        /**
         * The pool we're cleaning
         */
        final transient P pool;

        /**
         * should ensure that at least some memory has been marked reclaiming after completion
         */
        final transient MemtableCleaner cleaner;

        /**
         * signalled whenever needsCleaning() may return true
         */
        final transient WaitQueue wait = new WaitQueue();
    ...
    private final transient Runnable trigger;

    private final transient Clean<P> clean;
