...
public class BufferPool {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(BufferPool.class);

    /**
     * The size of a page aligned buffer, 128KiB
     */
    public static final transient int NORMAL_CHUNK_SIZE = 128 << 10;

    public static final transient int NORMAL_ALLOCATION_UNIT = NORMAL_CHUNK_SIZE / 64;

    public static final transient int TINY_CHUNK_SIZE = NORMAL_ALLOCATION_UNIT;

    public static final transient int TINY_ALLOCATION_UNIT = TINY_CHUNK_SIZE / 64;

    public static final transient int TINY_ALLOCATION_LIMIT = TINY_CHUNK_SIZE / 2;

    private static final transient Logger logger = LoggerFactory.getLogger(BufferPool.class);

    private static final transient NoSpamLogger noSpamLogger = NoSpamLogger.getLogger(logger, 15L, TimeUnit.MINUTES);

    private static final transient ByteBuffer EMPTY_BUFFER = ByteBuffer.allocateDirect(0);

    private volatile transient Debug debug = Debug.NO_OP;

    protected final transient String name;

    protected final transient BufferPoolMetrics metrics;

    private final transient long memoryUsageThreshold;

    private final transient String readableMemoryUsageThreshold;

    /**
     * Size of unpooled buffer being allocated outside of buffer pool in bytes.
     */
    private final transient LongAdder overflowMemoryUsage = new LongAdder();

    /**
     * Size of buffer being used in bytes, including pooled buffer and unpooled buffer.
     */
    private final transient LongAdder memoryInUse = new LongAdder();

    /**
     * Size of allocated buffer pool slabs in bytes
     */
    private final transient AtomicLong memoryAllocated = new AtomicLong();

    /**
     * A global pool of chunks (page aligned buffers)
     */
    private final transient GlobalPool globalPool;

    /**
     * Allow partially freed chunk to be recycled for allocation
     */
    private final transient boolean recyclePartially;

    /**
     * A thread local pool of chunks, where chunks come from the global pool
     */
    private final transient FastThreadLocal<LocalPool> localPool = new FastThreadLocal<LocalPool>() {
    ...
    private final transient Set<LocalPoolRef> localPoolReferences = Collections.newSetFromMap(new ConcurrentHashMap<>());

    private final transient ReferenceQueue<Object> localPoolRefQueue = new ReferenceQueue<>();

    private final transient InfiniteLoopExecutor localPoolCleaner;
    ...
    interface Debug {
        ...
        public static transient Debug NO_OP = new Debug() {
    ...
    final class GlobalPool implements Supplier<Chunk>, Recycler {
         ...
         */
        static final transient int MACRO_CHUNK_SIZE = 64 * NORMAL_CHUNK_SIZE;

        private final transient String READABLE_MACRO_CHUNK_SIZE = prettyPrintMemory(MACRO_CHUNK_SIZE);

        private final transient Queue<Chunk> macroChunks = new ConcurrentLinkedQueue<>();
        ...
        private final transient Queue<Chunk> chunks = new ConcurrentLinkedQueue<>();
        ...
        private final transient Queue<Chunk> partiallyFreedChunks = new ConcurrentLinkedQueue<>();

        /**
         * Used in logging statements to lazily build a human-readable current memory usage.
         */
        private final transient Object readableMemoryUsage = new Object() {
            ...
            while (true) {
                ...
                if (cur + MACRO_CHUNK_SIZE > memoryUsageThreshold) {
                    if (memoryUsageThreshold > 0) {
                        noSpamLogger.info("Maximum memory usage reached ({}), cannot allocate chunk of {}", readableMemoryUsageThreshold, READABLE_MACRO_CHUNK_SIZE);
                    }
                    return null;
                }
        ...
        @VisibleForTesting
        void unsafeFree() {
            while (!chunks.isEmpty()) chunks.poll().unsafeFree();
            while (!partiallyFreedChunks.isEmpty()) partiallyFreedChunks.poll().unsafeFree();
            while (!macroChunks.isEmpty()) macroChunks.poll().unsafeFree();
        }
    ...
    private static class MicroQueueOfChunks {
        ...
        private transient Chunk chunk0, chunk1, chunk2;
    ...
    public final class LocalPool implements Recycler {
        ...
        private final transient Queue<ByteBuffer> reuseObjects;

        private final transient Supplier<Chunk> parent;

        private final transient LocalPoolRef leakRef;

        private final transient MicroQueueOfChunks chunks = new MicroQueueOfChunks();

        private final transient Thread owningThread = Thread.currentThread();
         ...
         */
        private transient LocalPool tinyPool;

        private final transient int tinyLimit;

        private transient boolean recycleWhenFree = true;
        ...
        private LocalPool(LocalPool parent) {
            ...
            // we only currently permit one layer of nesting (which brings us down to 32 byte allocations, so is plenty)
            this.tinyLimit = 0;
            // we share the same ByteBuffer object reuse pool, as we both have the same exclusive access to it
        ...
        private ByteBuffer get(int size, boolean sizeIsLowerBound) {
            ...
            if (size > NORMAL_CHUNK_SIZE) {
                if (logger.isTraceEnabled())
                    logger.trace("Requested buffer size {} is bigger than {}; allocating directly", prettyPrintMemory(size), prettyPrintMemory(NORMAL_CHUNK_SIZE));
            } else {
                if (logger.isTraceEnabled())
        ...
        public void recycle(Chunk chunk) {
            ...
            // tiny chunk always has a parent chunk
            assert parentChunk != null;
    ...
    private static final class LocalPoolRef extends PhantomReference<LocalPool> {
        ...
        private final transient MicroQueueOfChunks chunks;
    ...
    final static class Chunk {
        ...
        final transient long baseAddress;

        private final transient int shift;

        // it may be 0L when all slots are allocated after "get" or when all slots are freed after "free"
        private volatile transient long freeSlots;

        private static final transient AtomicLongFieldUpdater<Chunk> freeSlotsUpdater = AtomicLongFieldUpdater.newUpdater(Chunk.class, "freeSlots");
        ...
        private volatile transient LocalPool owner;

        private final transient Recycler recycler;

        private static final transient AtomicReferenceFieldUpdater<Chunk, Status> statusUpdater = AtomicReferenceFieldUpdater.newUpdater(Chunk.class, Status.class, "status");

        private volatile transient Status status = Status.IN_USE;

        @VisibleForTesting
        transient Object debugAttachment;
        ...
        private void tryRecycleFullyOrPartially() {
            ...
            if (free() > 0 && setInUse()) {
                assert owner == null;
                if (// prefer to recycle fully, as fully free chunks are returned to a higher priority queue
                !tryRecycleFully())
                    recyclePartially();
            }
        }
