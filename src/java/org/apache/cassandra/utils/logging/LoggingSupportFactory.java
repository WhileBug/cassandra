...
public class LoggingSupportFactory {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(LoggingSupportFactory.class);

    private static final transient Logger logger = LoggerFactory.getLogger(LoggingSupportFactory.class);

    private static volatile transient LoggingSupport loggingSupport;
    ...
    public static LoggingSupport getLoggingSupport() {
        if (loggingSupport == null) {
            ...
            if (loggerFactoryClass.contains("logback")) {
                ...
                logger.warn("You are using Cassandra with an unsupported deployment. The intended logging implementation library logback is not used by slf4j. Detected slf4j logger factory: {}. " + "You will not be able to dynamically manage log levels via JMX and may have performance or other issues.", loggerFactoryClass);
            }
        }
