...
public class IntegerInterval {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(IntegerInterval.class);

    volatile transient long interval;

    private static transient AtomicLongFieldUpdater<IntegerInterval> intervalUpdater = AtomicLongFieldUpdater.newUpdater(IntegerInterval.class, "interval");
    ...
    public static class Set {
        ...
        static transient long[] EMPTY = new long[0];

        private volatile transient long[] ranges = EMPTY;
        ...
        public synchronized void add(int start, int end) {
            ...
            {
                // take local copy to avoid risk of it changing in the midst of operation
                ranges = this.ranges;
                ...
                // floor (i.e. greatest <=) of the end position
                int rpos = Arrays.binarySearch(ranges, ((end & 0xFFFFFFFFL) << 32) | 0xFFFFFFFFL);
                if (rpos < 0)
                    rpos = (-1 - rpos) - 1;
                if (rpos >= 0) {
                ...
                // lower (i.e. greatest <) of the start position
                int lpos = Arrays.binarySearch(ranges, ((start & 0xFFFFFFFFL) << 32) | 0);
                if (lpos < 0)
                    lpos = -1 - lpos;
                lpos -= 1;
                if (lpos >= 0) {
                ...
                int dest = 0;
                for (int i = 0; i <= lpos; ++i) newRanges[dest++] = ranges[i];
                newRanges[dest++] = make(start, end);
                for (int i = rpos + 1; i < ranges.length; ++i) newRanges[dest++] = ranges[i];
            }
