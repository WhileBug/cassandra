...
public final class Ref<T> implements RefCounted<T> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Ref.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Ref.class);

    static final transient Logger logger = LoggerFactory.getLogger(Ref.class);

    public static final transient boolean DEBUG_ENABLED = System.getProperty("cassandra.debugrefcount", "false").equalsIgnoreCase("true");

    final transient State state;

    final transient T referent;
    ...
    static final class State extends PhantomReference<Ref> {
        ...
        final transient Debug debug = DEBUG_ENABLED ? new Debug() : null;

        final transient GlobalState globalState;

        private volatile transient int released;

        private static final transient AtomicIntegerFieldUpdater<State> releasedUpdater = AtomicIntegerFieldUpdater.newUpdater(State.class, "released");
        ...
        void assertNotReleased() {
            if (DEBUG_ENABLED && released == 1)
                debug.log(toString());
            assert released == 0;
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.utils.concurrent.Ref.State.debug]=" + org.json.simple.JSONValue.toJSONString(debug).replace("\n", "").replace("\r", ""));
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.utils.concurrent.Ref.State.debug]=" + org.json.simple.JSONValue.toJSONString(debug).replace("\n", "").replace("\r", ""));
        }
        ...
        Throwable ensureReleased(Throwable accumulate) {
            ...
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.utils.concurrent.Ref.State.debug]=" + org.json.simple.JSONValue.toJSONString(debug).replace("\n", "").replace("\r", ""));
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.utils.concurrent.Ref.State.releasedUpdater]=" + org.json.simple.JSONValue.toJSONString(releasedUpdater).replace("\n", "").replace("\r", ""));
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.utils.concurrent.Ref.State.debug]=" + org.json.simple.JSONValue.toJSONString(debug).replace("\n", "").replace("\r", ""));
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.utils.concurrent.Ref.State.releasedUpdater]=" + org.json.simple.JSONValue.toJSONString(releasedUpdater).replace("\n", "").replace("\r", ""));
            return accumulate;
        }
    ...
    static final class Debug {
        ...
        transient String allocateThread, deallocateThread;

        transient StackTraceElement[] allocateTrace, deallocateTrace;
    ...
    static final class GlobalState {
        ...
        private final transient Collection<State> locallyExtant = new ConcurrentLinkedDeque<>();

        // the number of live refs
        private final transient AtomicInteger counts = new AtomicInteger();

        // the object to call to cleanup when our refs are all finished with
        private final transient Tidy tidy;
        ...
        void register(Ref.State ref) {
            ...
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.utils.concurrent.Ref.GlobalState.locallyExtant]=" + org.json.simple.JSONValue.toJSONString(locallyExtant).replace("\n", "").replace("\r", ""));
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.utils.concurrent.Ref.GlobalState.locallyExtant]=" + org.json.simple.JSONValue.toJSONString(locallyExtant).replace("\n", "").replace("\r", ""));
        }
    ...
    private static final transient Class<?>[] concurrentIterableClasses = new Class<?>[] { ConcurrentLinkedQueue.class, ConcurrentLinkedDeque.class, ConcurrentSkipListSet.class, CopyOnWriteArrayList.class, CopyOnWriteArraySet.class, DelayQueue.class, NonBlockingHashMap.class };

    static final transient Set<Class<?>> concurrentIterables = Collections.newSetFromMap(new IdentityHashMap<>());

    private static final transient Set<GlobalState> globallyExtant = Collections.newSetFromMap(new ConcurrentHashMap<>());

    static final transient ReferenceQueue<Object> referenceQueue = new ReferenceQueue<>();

    private static final transient InfiniteLoopExecutor EXEC = new InfiniteLoopExecutor("Reference-Reaper", Ref::reapOneReference).start();

    static final transient ScheduledExecutorService STRONG_LEAK_DETECTOR = !DEBUG_ENABLED ? null : Executors.newScheduledThreadPool(1, new NamedThreadFactory("Strong-Reference-Leak-Detector"));
    ...
    static final transient Deque<InProgressVisit> inProgressVisitPool = new ArrayDeque<InProgressVisit>();
    ...
    static class InProgressVisit {
        ...
        transient Object mapEntryValue;
    ...
    static class Visitor implements Runnable {
        ...
        final transient Deque<InProgressVisit> path = new ArrayDeque<>();

        final transient Set<Object> visited = Collections.newSetFromMap(new IdentityHashMap<>());

        @VisibleForTesting
        transient int lastVisitedCount;

        @VisibleForTesting
        transient long iterations = 0;

        transient GlobalState visiting;

        transient Set<GlobalState> haveLoops;
    ...
    static final transient Map<Class<?>, List<Field>> fieldMap = new HashMap<>();
    ...
    public static class IdentityCollection {
        ...
        final transient Set<Tidy> candidates;
    ...
    private static class StrongLeakDetector implements Runnable {
        ...
        transient Set<Tidy> candidates = new HashSet<>();
