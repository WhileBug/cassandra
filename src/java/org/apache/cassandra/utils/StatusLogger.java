...
public class StatusLogger {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StatusLogger.class);

    private static final transient Logger logger = LoggerFactory.getLogger(StatusLogger.class);

    private static final transient ReentrantLock busyMonitor = new ReentrantLock();
    ...
    private static void logStatus() {
        ...
        for (ThreadPoolMetrics tpool : CassandraMetricsRegistry.Metrics.allThreadPoolMetrics()) {
            logger.info(String.format("%-28s%10s%10s%15s%10s%18s", tpool.poolName, tpool.activeTasks.getValue(), tpool.pendingTasks.getValue(), tpool.completedTasks.getValue(), tpool.currentBlocked.getCount(), tpool.totalBlocked.getCount()));
        }
        // one offs
