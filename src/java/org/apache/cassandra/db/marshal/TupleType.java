...
public class TupleType extends AbstractType<ByteBuffer> {
    ...
    private static final transient Pattern COLON_PAT = Pattern.compile(COLON);

    private static final transient String ESCAPED_COLON = "\\\\:";

    private static final transient Pattern ESCAPED_COLON_PAT = Pattern.compile(ESCAPED_COLON);

    private static final transient String AT = "@";

    private static final transient Pattern AT_PAT = Pattern.compile(AT);

    private static final transient String ESCAPED_AT = "\\\\@";

    private static final transient Pattern ESCAPED_AT_PAT = Pattern.compile(ESCAPED_AT);

    protected final transient List<AbstractType<?>> types;

    private final transient TupleSerializer serializer;
    ...
    public static TupleType getInstance(TypeParser parser) throws ConfigurationException, SyntaxException {
        ...
        for (int i = 0; i < types.size(); i++) types.set(i, types.get(i).freeze());
    ...
    public TupleType withUpdatedUserType(UserType udt) {
        return referencesUserType(udt.name) ? new TupleType(Lists.newArrayList(transform(types, t -> t.withUpdatedUserType(udt)))) : this;
    }
    ...
    public ByteBuffer fromString(String source) {
        ...
        if (fieldStrings.size() > size())
            throw new MarshalException(String.format("Invalid tuple literal: too many elements. Type %s expects %d but got %d", asCQL3Type(), size(), fieldStrings.size()));
