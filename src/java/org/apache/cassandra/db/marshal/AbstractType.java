...
public abstract class AbstractType<T> implements Comparator<ByteBuffer>, AssignmentTestable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AbstractType.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AbstractType.class);

    public final transient Comparator<ByteBuffer> reverseComparator;
    ...
    public final transient ComparisonType comparisonType;

    public final transient boolean isByteOrderComparable;

    public final transient ValueComparators comparatorSet;
    ...
    protected AbstractType(ComparisonType comparisonType) {
        ...
        try {
            ...
            if ((custom.getDeclaringClass() == AbstractType.class) == (comparisonType == CUSTOM))
                throw new IllegalStateException((comparisonType == CUSTOM ? "compareCustom must be overridden if ComparisonType is CUSTOM" : "compareCustom should not be overridden if ComparisonType is not CUSTOM") + " (" + getClass().getSimpleName() + ")");
    ...
    public <V> void writeValue(V value, ValueAccessor<V> accessor, DataOutputPlus out) throws IOException {
        ...
        if (expectedValueLength >= 0) {
            ...
            if (actualValueLength == expectedValueLength)
                ...
                throw new IOException(String.format("Expected exactly %d bytes, but was %d", expectedValueLength, actualValueLength));
        } else {
    ...
    public <V> long writtenLength(V value, ValueAccessor<V> accessor) {
        assert !accessor.isEmpty(value) : "bytes should not be empty for type " + this;
        return // if the size is wrong, this will be detected in writeValue
        valueLengthIfFixed() >= 0 ? accessor.size(value) : accessor.sizeWithVIntLength(value);
    }
