...
public class DynamicCompositeType extends AbstractCompositeType {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(DynamicCompositeType.class);

    private static final transient Logger logger = LoggerFactory.getLogger(DynamicCompositeType.class);

    private final transient Map<Byte, AbstractType<?>> aliases;

    // interning instances
    private static final transient ConcurrentHashMap<Map<Byte, AbstractType<?>>, DynamicCompositeType> instances = new ConcurrentHashMap<>();
    ...
    private class DynamicParsedComparator implements ParsedComparator {
        ...
        final transient AbstractType<?> type;

        final transient boolean isAlias;

        final transient String comparatorName;

        final transient String remainingPart;
    ...
    private static class FixedValueComparator extends AbstractType<Void> {
        ...
        public static final transient FixedValueComparator alwaysLesserThan = new FixedValueComparator(-1);

        public static final transient FixedValueComparator alwaysGreaterThan = new FixedValueComparator(1);

        private final transient int cmp;
