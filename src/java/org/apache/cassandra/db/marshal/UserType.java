...
public class UserType extends TupleType implements SchemaElement {
    ...
    private static final transient Logger logger = LoggerFactory.getLogger(UserType.class);

    private static final transient ConflictBehavior CONFLICT_BEHAVIOR = ConflictBehavior.get();

    public final transient String keyspace;

    public final transient ByteBuffer name;

    private final transient List<FieldIdentifier> fieldNames;

    private final transient List<String> stringFieldNames;

    private final transient boolean isMultiCell;

    private final transient UserTypeSerializer serializer;
    ...
    public UserType withUpdatedUserType(UserType udt) {
        ...
        if (name.equals(udt.name)) {
            return isMultiCell == udt.isMultiCell ? udt : new UserType(keyspace, name, udt.fieldNames(), udt.fieldTypes(), isMultiCell);
        }
        return new UserType(keyspace, name, fieldNames, Lists.newArrayList(transform(fieldTypes(), t -> t.withUpdatedUserType(udt))), isMultiCell());
    }
    ...
    private enum ConflictBehavior {
        ...
        LOG {
            ...
            void onConflict(String keyspace, String name, String fieldName) {
                logger.error("Duplicate names found in UDT {}.{} for column {}", maybeQuote(keyspace), maybeQuote(name), maybeQuote(fieldName));
            }
        }
        , REJECT {
            ...
            @Override
            void onConflict(String keyspace, String name, String fieldName) {
                throw new AssertionError(String.format("Duplicate names found in UDT %s.%s for column %s; " + "to resolve set -D" + UDT_CONFLICT_BEHAVIOR + "=LOG on startup and remove the type", maybeQuote(keyspace), maybeQuote(name), maybeQuote(fieldName)));
            }
        }
        ;
