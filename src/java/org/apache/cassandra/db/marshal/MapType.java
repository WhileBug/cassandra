...
public class MapType<K, V> extends CollectionType<Map<K, V>> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MapType.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MapType.class);

    // interning instances
    private static final transient ConcurrentHashMap<Pair<AbstractType<?>, AbstractType<?>>, MapType> instances = new ConcurrentHashMap<>();

    private static final transient ConcurrentHashMap<Pair<AbstractType<?>, AbstractType<?>>, MapType> frozenInstances = new ConcurrentHashMap<>();

    private final transient AbstractType<K> keys;

    private final transient AbstractType<V> values;

    private final transient MapSerializer<K, V> serializer;
