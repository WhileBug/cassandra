...
public class SSTableIterator extends AbstractSSTableIterator {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SSTableIterator.class);
    ...
    private class ForwardReader extends Reader {
        ...
        protected transient ClusteringBound<?> start;

        // The end of the current slice. Will never be null.
        protected transient ClusteringBound<?> end = BufferClusteringBound.TOP;

        // the next element to return: this is computed by hasNextInternal().
        protected transient Unfiltered next;

        // set to true once we know we have no more result for the slice. This is in particular
        protected transient boolean sliceDone;
    ...
    private class ForwardIndexedReader extends ForwardReader {
        ...
        // the last index block that has data for the current query
        ...
        private ForwardIndexedReader(RowIndexEntry indexEntry, FileDataInput file, boolean shouldCloseFile) {
            ...
            // if we never call setForSlice, that's where we want to stop
        ...
        public void setForSlice(Slice slice) throws IOException {
            ...
            if (indexState.currentBlockIdx() == lastBlockIdx && metadata().comparator.compare(slice.end(), indexState.currentIndex().firstName) < 0 && openMarker == null) {
                sliceDone = true;
            }
        }
        ...
        protected Unfiltered computeNext() throws IOException {
            while (true) {
                ...
                if (indexState.isDone() || indexState.currentBlockIdx() > lastBlockIdx || !deserializer.hasNext() || (indexState.currentBlockIdx() == lastBlockIdx && deserializer.compareNextTo(end) >= 0))
                    return null;
                Unfiltered next = deserializer.readNext();
                UnfilteredValidation.maybeValidateUnfiltered(next, metadata(), key, sstable);
                // We may get empty row for the same reason expressed on UnfilteredSerializer.deserializeOne.
