...
public enum ConsistencyLevel {
    ...
    public static int localQuorumFor(AbstractReplicationStrategy replicationStrategy, String dc) {
        return (replicationStrategy instanceof NetworkTopologyStrategy) ? (((NetworkTopologyStrategy) replicationStrategy).getReplicationFactor(dc).allReplicas / 2) + 1 : quorumFor(replicationStrategy);
    }
    ...
    public static ObjectIntHashMap<String> eachQuorumForRead(AbstractReplicationStrategy replicationStrategy) {
        if (replicationStrategy instanceof NetworkTopologyStrategy) {
            ...
            for (String dc : npStrategy.getDatacenters()) perDc.put(dc, ConsistencyLevel.localQuorumFor(replicationStrategy, dc));
            return perDc;
        } else {
        ...
        switch(this) {
            ...
            case EACH_QUORUM:
                if (replicationStrategy instanceof NetworkTopologyStrategy) {
                    ...
                    int n = 0;
                    for (String dc : strategy.getDatacenters()) n += localQuorumFor(replicationStrategy, dc);
                    return n;
                } else {
                    return quorumFor(replicationStrategy);
                }
