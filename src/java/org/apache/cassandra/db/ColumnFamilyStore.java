...
public class ColumnFamilyStore implements ColumnFamilyStoreMBean {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ColumnFamilyStore.class);
    ...
    */
    private static final transient ThreadPoolExecutor flushExecutor = new JMXEnabledThreadPoolExecutor(DatabaseDescriptor.getFlushWriters(), Stage.KEEP_ALIVE_SECONDS, TimeUnit.SECONDS, new LinkedBlockingQueue<>(), new NamedThreadFactory("MemtableFlushWriter"), "internal");

    private static final transient PerDiskFlushExecutors perDiskflushExecutors = new PerDiskFlushExecutors(DatabaseDescriptor.getFlushWriters(), DatabaseDescriptor.getNonLocalSystemKeyspacesDataFileLocations(), DatabaseDescriptor.useSpecificLocationForLocalSystemData());

    // post-flush executor is single threaded to provide guarantee that any flush Future on a CF will never return until prior flushes have completed
    private static final transient ThreadPoolExecutor postFlushExecutor = new JMXEnabledThreadPoolExecutor(1, Stage.KEEP_ALIVE_SECONDS, TimeUnit.SECONDS, new LinkedBlockingQueue<>(), new NamedThreadFactory("MemtablePostFlush"), "internal");

    private static final transient ThreadPoolExecutor reclaimExecutor = new JMXEnabledThreadPoolExecutor(1, Stage.KEEP_ALIVE_SECONDS, TimeUnit.SECONDS, new LinkedBlockingQueue<>(), new NamedThreadFactory("MemtableReclaimMemory"), "internal");

    private static final transient String[] COUNTER_NAMES = new String[] { "table", "count", "error", "value" };

    private static final transient String[] COUNTER_DESCS = new String[] { "keyspace.tablename", "number of occurances", "error bounds", "value" };

    private static final transient CompositeType COUNTER_COMPOSITE_TYPE;

    private static final transient String SAMPLING_RESULTS_NAME = "SAMPLING_RESULTS";

    public static final transient String SNAPSHOT_TRUNCATE_PREFIX = "truncated";

    public static final transient String SNAPSHOT_DROP_PREFIX = "dropped";
    ...
    public final transient Keyspace keyspace;

    public final transient String name;

    public final transient TableMetadataRef metadata;

    private final transient String mbeanName;

    @Deprecated
    private final transient String oldMBeanName;

    private volatile transient boolean valid = true;
     ...
     */
    private final transient Tracker data;

    /* The read order, used to track accesses to off-heap memtable storage */
    public final transient OpOrder readOrdering = new OpOrder();

    /* This is used to generate the next index for a SSTable */
    private final transient AtomicInteger fileIndexGenerator = new AtomicInteger(0);

    public final transient SecondaryIndexManager indexManager;

    public final transient TableViews viewManager;

    /* These are locally held copies to be changed from the config during runtime */
    private volatile transient DefaultValue<Integer> minCompactionThreshold;

    private volatile transient DefaultValue<Integer> maxCompactionThreshold;

    private volatile transient DefaultValue<Double> crcCheckChance;

    private final transient CompactionStrategyManager compactionStrategyManager;

    private final transient Directories directories;

    public final transient TableMetrics metric;

    public volatile transient long sampleReadLatencyNanos;

    public volatile transient long additionalWriteLatencyNanos;

    private final transient CassandraTableWriteHandler writeHandler;

    private final transient CassandraStreamManager streamManager;

    private final transient TableRepairManager repairManager;

    private final transient SSTableImporter sstableImporter;

    private volatile transient boolean compactionSpaceCheck = true;

    @VisibleForTesting
    final transient DiskBoundaryManager diskBoundaryManager = new DiskBoundaryManager();

    private volatile transient boolean neverPurgeTombstones = false;
    ...
    public static void scrubDataDirectories(TableMetadata metadata) throws StartupException {
        ...
        if (dir.exists()) {
            ...
            for (File file : Objects.requireNonNull(dir.listFiles())) if (tmpCacheFilePattern.matcher(file.getName()).matches())
                if (!file.delete())
    ...
    Descriptor getUniqueDescriptorFor(Descriptor descriptor, File targetDirectory) {
        ...
        do {
            newDescriptor = new // Increment the generation until we find a filename that doesn't exist. This is needed because the new
            Descriptor(// Increment the generation until we find a filename that doesn't exist. This is needed because the new
            descriptor.version, // Increment the generation until we find a filename that doesn't exist. This is needed because the new
            targetDirectory, // Increment the generation until we find a filename that doesn't exist. This is needed because the new
            descriptor.ksname, // Increment the generation until we find a filename that doesn't exist. This is needed because the new
            descriptor.cfname, // SSTables that are being loaded might already use these generation numbers.
            fileIndexGenerator.incrementAndGet(), descriptor.formatType);
    ...
    private final class PostFlush implements Callable<CommitLogPosition> {
        ...
        final transient CountDownLatch latch = new CountDownLatch(1);

        final transient List<Memtable> memtables;

        volatile transient Throwable flushFailure = null;
    ...
    private final class Flush implements Runnable {
        ...
        final transient OpOrder.Barrier writeBarrier;

        final transient List<Memtable> memtables = new ArrayList<>();

        final transient ListenableFutureTask<CommitLogPosition> postFlushTask;

        final transient PostFlush postFlush;

        final transient boolean truncate;
        ...
        public Collection<SSTableReader> flushMemtable(Memtable memtable, boolean flushNonCf2i) {
            ...
            try (LifecycleTransaction txn = LifecycleTransaction.offline(OperationType.FLUSH)) {
                ...
                try {
                    ...
                    for (int i = 0; i < flushRunnables.size(); i++) futures.add(executors[i].submit(flushRunnables.get(i)));
                    /**
    ...
    public void apply(PartitionUpdate update, UpdateTransaction indexer, OpOrder.Group opGroup, CommitLogPosition commitLogPosition) {
        ...
        try {
            ...
            if (// dont compute datasize if not needed
            metric.topWritePartitionSize.isEnabled())
    ...
    public Collection<SSTableReader> getOverlappingLiveSSTables(Iterable<SSTableReader> sstables) {
        ...
        for (SSTableReader sstable : sortedByFirst) {
            if (first == null) {
                ...
                if (// we do overlap
                sstable.first.compareTo(last) <= 0) {
    ...
    public List<CompositeData> finishLocalSampling(String sampler, int count) throws OpenDataException {
        ...
        for (Sample counter : samplerResults) {
            ...
            result.add(new CompositeDataSupport(COUNTER_COMPOSITE_TYPE, COUNTER_NAMES, new Object[] { // string
            keyspace.getName() + "." + name, // string
            counter.count, // string
            counter.error, samplerImpl.toString(counter.value) }));
        }
        return result;
    }
        ...
        for (ColumnFamilyStore cfs : concatWithIndexes()) {
            try (RefViewFragment currentView = cfs.selectAndReference(View.select(SSTableSet.CANONICAL, (x) -> predicate == null || predicate.apply(x)))) {
                for (SSTableReader ssTable : currentView.sstables) {
                    ...
                    // hard links
                    ssTable.createLinks(snapshotDirectory.getPath(), rateLimiter);
    ...
    public ClockAndCount getCachedCounter(ByteBuffer partitionKey, Clustering<?> clustering, ColumnMetadata column, CellPath path) {
        if (// counter cache disabled.
        CacheService.instance.counterCache.getCapacity() == 0L)
            return null;
    ...
    private static CompactionManager.CompactionPauser pauseCompactionStrategies(Iterable<ColumnFamilyStore> toPause) {
        ...
        try {
            for (ColumnFamilyStore cfs : toPause) {
                // to avoid OOM:ing after pausing the strategies
    ...
    public static class ViewFragment {
        ...
        public final transient List<SSTableReader> sstables;

        public final transient Iterable<Memtable> memtables;
    ...
    public static class RefViewFragment extends ViewFragment implements AutoCloseable {
        ...
        public final transient Refs<SSTableReader> refs;
    ...
    private static final class PerDiskFlushExecutors {
         ...
         */
        private final transient ExecutorService[] nonLocalSystemflushExecutors;

        /**
         * The flush executors for the local system keyspaces.
         */
        private final transient ExecutorService[] localSystemDiskFlushExecutors;

        /**
         * {@code true} if local system keyspaces are stored in their own directory and use an extra flush executor,
         * {@code false} otherwise.
         */
        private final transient boolean useSpecificExecutorForSystemKeyspaces;
        ...
        public PerDiskFlushExecutors(int flushWriters, String[] locationsForNonSystemKeyspaces, boolean useSpecificLocationForSystemKeyspaces) {
            ...
            localSystemDiskFlushExecutors = useSpecificLocationForSystemKeyspaces ? new ExecutorService[] { newThreadPool("LocalSystemKeyspacesDiskMemtableFlushWriter", flushWriters) } : new ExecutorService[] { flushExecutors[0] };
        }
        ...
        private static JMXEnabledThreadPoolExecutor newThreadPool(String poolName, int size) {
            return new JMXEnabledThreadPoolExecutor(size, Stage.KEEP_ALIVE_SECONDS, TimeUnit.SECONDS, new LinkedBlockingQueue<>(), new NamedThreadFactory(poolName), "internal");
        }
        ...
        public ExecutorService[] getExecutorsFor(String keyspaceName, String tableName) {
            return Directories.isStoredInLocalSystemKeyspacesDataLocation(keyspaceName, tableName) ? localSystemDiskFlushExecutors : nonLocalSystemflushExecutors;
        }
