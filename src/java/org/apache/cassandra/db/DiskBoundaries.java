...
public class DiskBoundaries {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(DiskBoundaries.class);

    public final transient List<Directories.DataDirectory> directories;

    public final transient ImmutableList<PartitionPosition> positions;

    final transient long ringVersion;

    final transient int directoriesVersion;

    private final transient ColumnFamilyStore cfs;

    private volatile transient boolean isInvalid = false;
    ...
    public boolean equals(Object o) {
        ...
        if (ringVersion != that.ringVersion)
            return false;
        if (directoriesVersion != that.directoriesVersion)
            return false;
        if (!directories.equals(that.directories))
            return false;
    ...
    public String toString() {
        return "DiskBoundaries{" + "directories=" + directories + ", positions=" + positions + ", ringVersion=" + ringVersion + ", directoriesVersion=" + directoriesVersion + '}';
    }
    ...
    public int getDiskIndex(SSTableReader sstable) {
        ...
        // boundaries are .minkeybound and .maxkeybound so they should never be equal
        assert pos < 0;
        return -pos - 1;
    }
