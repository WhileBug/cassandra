...
public abstract class DataLimits {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(DataLimits.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(DataLimits.class);

    public static final transient Serializer serializer = new Serializer();

    public static final transient int NO_LIMIT = Integer.MAX_VALUE;

    public static final transient DataLimits NONE = new CQLLimits(NO_LIMIT) {
    ...
    public static final transient DataLimits DISTINCT_NONE = new CQLLimits(NO_LIMIT, 1, true);
    ...
    public static abstract class Counter extends StoppingTransformation<BaseRowIterator<?>> {
        ...
        protected final transient int nowInSec;

        protected final transient boolean assumeLiveData;
        ...
        protected transient boolean enforceLimits = true;
    ...
    private static class CQLLimits extends DataLimits {
        ...
        protected final transient int rowLimit;

        protected final transient int perPartitionLimit;

        // Whether the query is a distinct query or not.
        protected final transient boolean isDistinct;
        ...
        protected class CQLCounter extends Counter {
            ...
            protected transient int rowsCounted;

            protected transient int rowsInCurrentPartition;

            protected final transient boolean countPartitionsWithOnlyStaticData;

            protected transient boolean hasLiveStaticRow;

            public CQLCounter(int nowInSec, boolean assumeLiveData, boolean countPartitionsWithOnlyStaticData, boolean enforceStrictLiveness) {
    ...
    private static class CQLPagingLimits extends CQLLimits {
        ...
        private final transient ByteBuffer lastReturnedKey;

        private final transient int lastReturnedKeyRemaining;
        ...
        private class PagingAwareCounter extends CQLCounter {
            ...
            private PagingAwareCounter(int nowInSec, boolean assumeLiveData, boolean countPartitionsWithOnlyStaticData, boolean enforceStrictLiveness) {
    ...
    private static class CQLGroupByLimits extends CQLLimits {
         ...
         */
        protected final transient GroupingState state;

        /**
         * The GROUP BY specification
         */
        protected final transient AggregationSpecification groupBySpec;

        /**
         * The limit on the number of groups
         */
        protected final transient int groupLimit;

        /**
         * The limit on the number of groups per partition
         */
        protected final transient int groupPerPartitionLimit;
        ...
        @Override
        public DataLimits forPaging(int pageSize) {
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.filter.DataLimits.CQLGroupByLimits.state]=" + org.json.simple.JSONValue.toJSONString(state).replace("\n", "").replace("\r", ""));
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.filter.DataLimits.CQLGroupByLimits.state]=" + org.json.simple.JSONValue.toJSONString(state).replace("\n", "").replace("\r", ""));
            return new CQLGroupByLimits(pageSize, groupPerPartitionLimit, rowLimit, groupBySpec, state);
        }
        ...
        public DataLimits forPaging(int pageSize, ByteBuffer lastReturnedKey, int lastReturnedKeyRemaining) {
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.filter.DataLimits.CQLGroupByLimits.state]=" + org.json.simple.JSONValue.toJSONString(state).replace("\n", "").replace("\r", ""));
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.filter.DataLimits.CQLGroupByLimits.state]=" + org.json.simple.JSONValue.toJSONString(state).replace("\n", "").replace("\r", ""));
            return new CQLGroupByPagingLimits(pageSize, groupPerPartitionLimit, rowLimit, groupBySpec, state, lastReturnedKey, lastReturnedKeyRemaining);
        }
        ...
        public DataLimits forGroupByInternalPaging(GroupingState state) {
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.filter.DataLimits.CQLGroupByLimits.state]=" + org.json.simple.JSONValue.toJSONString(state).replace("\n", "").replace("\r", ""));
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.filter.DataLimits.CQLGroupByLimits.state]=" + org.json.simple.JSONValue.toJSONString(state).replace("\n", "").replace("\r", ""));
            return new CQLGroupByLimits(rowLimit, groupPerPartitionLimit, rowLimit, groupBySpec, state);
        }
        ...
        public DataLimits withoutState() {
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.filter.DataLimits.CQLGroupByLimits.state]=" + org.json.simple.JSONValue.toJSONString(state).replace("\n", "").replace("\r", ""));
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.filter.DataLimits.CQLGroupByLimits.state]=" + org.json.simple.JSONValue.toJSONString(state).replace("\n", "").replace("\r", ""));
            return state == GroupingState.EMPTY_STATE ? this : new CQLGroupByLimits(groupLimit, groupPerPartitionLimit, rowLimit, groupBySpec);
        }
        ...
        @Override
        public boolean isExhausted(Counter counter) {
            return ((GroupByAwareCounter) counter).rowsCounted < rowLimit && counter.counted() < groupLimit;
        }

        protected class GroupByAwareCounter extends Counter {

            private final transient GroupMaker groupMaker;
             ...
             */
            protected transient DecoratedKey currentPartitionKey;
             ...
             */
            protected transient int rowsCountedInCurrentPartition;

            /**
             * The number of groups counted so far. A group is counted only once it is complete
             * (e.g the next one has been reached).
             */
            protected transient int groupCounted;

            /**
             * The number of groups in the current partition.
             */
            protected transient int groupInCurrentPartition;

            protected transient boolean hasUnfinishedGroup;

            protected transient boolean hasLiveStaticRow;

            protected transient boolean hasReturnedRowsFromCurrentPartition;

            private GroupByAwareCounter(int nowInSec, boolean assumeLiveData, boolean countPartitionsWithOnlyStaticData, boolean enforceStrictLiveness) {
                super(nowInSec, assumeLiveData, enforceStrictLiveness);
