...
public class ClusteringIndexNamesFilter extends AbstractClusteringIndexFilter {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ClusteringIndexNamesFilter.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ClusteringIndexNamesFilter.class);

    static final transient InternalDeserializer deserializer = new NamesDeserializer();

    // This could be empty if selectedColumns only has static columns (in which case the filter still
    // selects the static row)
    private final transient NavigableSet<Clustering<?>> clusterings;
    ...
    private final transient NavigableSet<Clustering<?>> clusteringsInQueryOrder;
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(clusterings, that.clusterings) && Objects.equals(reversed, that.reversed);
    }
    ...
    private static class NamesDeserializer implements InternalDeserializer {
        ...
        public ClusteringIndexFilter deserialize(DataInputPlus in, int version, TableMetadata metadata, boolean reversed) throws IOException {
            ...
            try (BTree.FastBuilder<Clustering<?>> builder = BTree.fastBuilder()) {
                for (int i = 0; i < size; i++) builder.add(Clustering.serializer.deserialize(in, version, comparator.subtypes()));
