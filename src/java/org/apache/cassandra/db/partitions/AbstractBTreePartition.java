...
public abstract class AbstractBTreePartition implements Partition, Iterable<Row> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AbstractBTreePartition.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AbstractBTreePartition.class);

    protected static final transient Holder EMPTY = new Holder(RegularAndStaticColumns.NONE, BTree.empty(), DeletionInfo.LIVE, Rows.EMPTY_STATIC_ROW, EncodingStats.NO_STATS);

    public static final transient long HOLDER_UNSHARED_HEAP_SIZE = ObjectSizes.measure(EMPTY);
    ...
    public static final class Holder {
        ...
        public final transient RegularAndStaticColumns columns;

        public final transient DeletionInfo deletionInfo;

        // the btree of rows
        public final transient Object[] tree;

        public final transient Row staticRow;

        public final transient EncodingStats stats;
    ...
    public UnfilteredRowIterator unfilteredIterator(Holder current, ColumnFilter selection, Slices slices, boolean reversed) {
        ...
        return slices.size() == 1 ? sliceIterator(selection, slices.get(0), reversed, current, staticRow) : new SlicesIterator(selection, slices, reversed, current, staticRow);
    }
    ...
    private abstract class AbstractIterator extends AbstractUnfilteredRowIterator {
        ...
        final transient Holder current;

        final transient ColumnFilter selection;

        private AbstractIterator(Holder current, Row staticRow, ColumnFilter selection, boolean isReversed) {
            super(// non-selected columns will be filtered in subclasses by RowAndDeletionMergeIterator
            AbstractBTreePartition.this.metadata(), // non-selected columns will be filtered in subclasses by RowAndDeletionMergeIterator
            AbstractBTreePartition.this.partitionKey(), // non-selected columns will be filtered in subclasses by RowAndDeletionMergeIterator
            current.deletionInfo.getPartitionDeletion(), // it would also be more precise to return the intersection of the selection and current.columns,
            selection.fetchedColumns(), // but its probably not worth spending time on computing that.
            staticRow, isReversed, current.stats);
    ...
    private class SlicesIterator extends AbstractIterator {
        ...
        private final transient Slices slices;

        private transient int idx;

        private transient Iterator<Unfiltered> currentSlice;
    ...
    private class ClusteringsIterator extends AbstractIterator {
        ...
        private final transient Iterator<Clustering<?>> clusteringsInQueryOrder;

        private final transient SearchIterator<Clustering<?>, Row> rowSearcher;

        private transient Iterator<Unfiltered> currentIterator;
