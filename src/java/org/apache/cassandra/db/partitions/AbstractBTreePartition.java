...
public abstract class AbstractBTreePartition implements Partition, Iterable<Row> {
    ...
    private abstract class AbstractIterator extends AbstractUnfilteredRowIterator {
        ...
        private AbstractIterator(Holder current, Row staticRow, ColumnFilter selection, boolean isReversed) {
            super(AbstractBTreePartition.this.metadata(), AbstractBTreePartition.this.partitionKey(), current.deletionInfo.getPartitionDeletion(), // non-selected columns will be filtered in subclasses by RowAndDeletionMergeIterator
            selection.fetchedColumns(), // it would also be more precise to return the intersection of the selection and current.columns,
