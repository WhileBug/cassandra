...
public final class SystemKeyspace {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SystemKeyspace.class);
    ...
    private static final transient Logger logger = LoggerFactory.getLogger(SystemKeyspace.class);
    ...
    public static final transient CassandraVersion UNREADABLE_VERSION = new CassandraVersion("0.0.0-unknown");
    ...
    public static final transient CassandraVersion NULL_VERSION = new CassandraVersion("0.0.0-absent");

    public static final transient CassandraVersion CURRENT_VERSION = new CassandraVersion(FBUtilities.getReleaseVersionString());

    public static final transient String BATCHES = "batches";

    public static final transient String PAXOS = "paxos";

    public static final transient String BUILT_INDEXES = "IndexInfo";

    public static final transient String LOCAL = "local";

    public static final transient String PEERS_V2 = "peers_v2";

    public static final transient String PEER_EVENTS_V2 = "peer_events_v2";

    public static final transient String COMPACTION_HISTORY = "compaction_history";

    public static final transient String SSTABLE_ACTIVITY = "sstable_activity";

    public static final transient String TABLE_ESTIMATES = "table_estimates";

    public static final transient String TABLE_ESTIMATES_TYPE_PRIMARY = "primary";

    public static final transient String TABLE_ESTIMATES_TYPE_LOCAL_PRIMARY = "local_primary";

    public static final transient String AVAILABLE_RANGES_V2 = "available_ranges_v2";

    public static final transient String TRANSFERRED_RANGES_V2 = "transferred_ranges_v2";

    public static final transient String VIEW_BUILDS_IN_PROGRESS = "view_builds_in_progress";

    public static final transient String BUILT_VIEWS = "built_views";

    public static final transient String PREPARED_STATEMENTS = "prepared_statements";

    public static final transient String REPAIRS = "repairs";
     ...
     */
    public static final transient Set<String> TABLES_SPLIT_ACROSS_MULTIPLE_DISKS = ImmutableSet.of(BATCHES, PAXOS, COMPACTION_HISTORY, PREPARED_STATEMENTS, REPAIRS);

    @Deprecated
    public static final transient String LEGACY_PEERS = "peers";

    @Deprecated
    public static final transient String LEGACY_PEER_EVENTS = "peer_events";

    @Deprecated
    public static final transient String LEGACY_TRANSFERRED_RANGES = "transferred_ranges";

    @Deprecated
    public static final transient String LEGACY_AVAILABLE_RANGES = "available_ranges";

    @Deprecated
    public static final transient String LEGACY_SIZE_ESTIMATES = "size_estimates";

    public static final transient TableMetadata Batches = parse(BATCHES, "batches awaiting replay", "CREATE TABLE %s (" + "id timeuuid," + "mutations list<blob>," + "version int," + "PRIMARY KEY ((id)))").partitioner(new LocalPartitioner(TimeUUIDType.instance)).compaction(CompactionParams.stcs(singletonMap("min_threshold", "2"))).build();

    private static final transient TableMetadata Paxos = parse(PAXOS, "in-progress paxos proposals", "CREATE TABLE %s (" + "row_key blob," + "cf_id UUID," + "in_progress_ballot timeuuid," + "most_recent_commit blob," + "most_recent_commit_at timeuuid," + "most_recent_commit_version int," + "proposal blob," + "proposal_ballot timeuuid," + "proposal_version int," + "PRIMARY KEY ((row_key), cf_id))").compaction(CompactionParams.lcs(emptyMap())).build();

    private static final transient TableMetadata BuiltIndexes = parse(BUILT_INDEXES, "built column indexes", // table_name here is the name of the keyspace - don't be fooled
    "CREATE TABLE \"%s\" (" + "table_name text," + // Table used to be compact in previous versions
    "index_name text," + "value blob," + "PRIMARY KEY ((table_name), index_name)) ").build();
    ...
    private static final transient TableMetadata PeerEventsV2 = parse(PEER_EVENTS_V2, "events related to peers", "CREATE TABLE %s (" + "peer inet," + "peer_port int," + "hints_dropped map<uuid, int>," + "PRIMARY KEY ((peer), peer_port))").build();

    private static final transient TableMetadata CompactionHistory = parse(COMPACTION_HISTORY, "week-long compaction history", "CREATE TABLE %s (" + "id uuid," + "bytes_in bigint," + "bytes_out bigint," + "columnfamily_name text," + "compacted_at timestamp," + "keyspace_name text," + "rows_merged map<int, bigint>," + "PRIMARY KEY ((id)))").defaultTimeToLive((int) TimeUnit.DAYS.toSeconds(7)).build();

    private static final transient TableMetadata SSTableActivity = parse(SSTABLE_ACTIVITY, "historic sstable read rates", "CREATE TABLE %s (" + "keyspace_name text," + "columnfamily_name text," + "generation int," + "rate_120m double," + "rate_15m double," + "PRIMARY KEY ((keyspace_name, columnfamily_name, generation)))").build();
    ...
    private static final transient TableMetadata AvailableRangesV2 = parse(AVAILABLE_RANGES_V2, "available keyspace/ranges during bootstrap/replace that are ready to be served", "CREATE TABLE %s (" + "keyspace_name text," + "full_ranges set<blob>," + "transient_ranges set<blob>," + "PRIMARY KEY ((keyspace_name)))").build();

    private static final transient TableMetadata TransferredRangesV2 = parse(TRANSFERRED_RANGES_V2, "record of transferred ranges for streaming operation", "CREATE TABLE %s (" + "operation text," + "peer inet," + "peer_port int," + "keyspace_name text," + "ranges set<blob>," + "PRIMARY KEY ((operation, keyspace_name), peer, peer_port))").build();

    private static final transient TableMetadata ViewBuildsInProgress = parse(VIEW_BUILDS_IN_PROGRESS, "views builds current progress", "CREATE TABLE %s (" + "keyspace_name text," + "view_name text," + "start_token varchar," + "end_token varchar," + "last_token varchar," + "keys_built bigint," + "PRIMARY KEY ((keyspace_name), view_name, start_token, end_token))").build();

    private static final transient TableMetadata BuiltViews = parse(BUILT_VIEWS, "built views", "CREATE TABLE %s (" + "keyspace_name text," + "view_name text," + "status_replicated boolean," + "PRIMARY KEY ((keyspace_name), view_name))").build();

    private static final transient TableMetadata PreparedStatements = parse(PREPARED_STATEMENTS, "prepared statements", "CREATE TABLE %s (" + "prepared_id blob," + "logged_keyspace text," + "query_string text," + "PRIMARY KEY ((prepared_id)))").build();
    ...
    @Deprecated
    private static final transient TableMetadata LegacyPeers = parse(LEGACY_PEERS, "information about known peers in the cluster", "CREATE TABLE %s (" + "peer inet," + "data_center text," + "host_id uuid," + "preferred_ip inet," + "rack text," + "release_version text," + "rpc_address inet," + "schema_version uuid," + "tokens set<varchar>," + "PRIMARY KEY ((peer)))").build();

    @Deprecated
    private static final transient TableMetadata LegacyPeerEvents = parse(LEGACY_PEER_EVENTS, "events related to peers", "CREATE TABLE %s (" + "peer inet," + "hints_dropped map<uuid, int>," + "PRIMARY KEY ((peer)))").build();

    @Deprecated
    private static final transient TableMetadata LegacyTransferredRanges = parse(LEGACY_TRANSFERRED_RANGES, "record of transferred ranges for streaming operation", "CREATE TABLE %s (" + "operation text," + "peer inet," + "keyspace_name text," + "ranges set<blob>," + "PRIMARY KEY ((operation, keyspace_name), peer))").build();

    @Deprecated
    private static final transient TableMetadata LegacyAvailableRanges = parse(LEGACY_AVAILABLE_RANGES, "available keyspace/ranges during bootstrap/replace that are ready to be served", "CREATE TABLE %s (" + "keyspace_name text," + "ranges set<blob>," + "PRIMARY KEY ((keyspace_name)))").build();
    ...
    private static volatile transient Map<TableId, Pair<CommitLogPosition, Long>> truncationRecords;

    public enum BootstrapState {

        NEEDS_BOOTSTRAP, COMPLETED, IN_PROGRESS, DECOMMISSIONED
    }
    ...
    public static synchronized AvailableRanges getAvailableRanges(String keyspace, IPartitioner partitioner) {
        ...
        for (UntypedResultSet.Row row : rs) {
            Optional.ofNullable(row.getSet("full_ranges", BytesType.instance)).ifPresent(full_ranges -> full_ranges.stream().map(buf -> byteBufferToRange(buf, partitioner)).forEach(full::add));
            Optional.ofNullable(row.getSet("transient_ranges", BytesType.instance)).ifPresent(transient_ranges -> transient_ranges.stream().map(buf -> byteBufferToRange(buf, partitioner)).forEach(trans::add));
        }
    ...
    public static class AvailableRanges {
        ...
        public transient Set<Range<Token>> full;

        public transient Set<Range<Token>> trans;
