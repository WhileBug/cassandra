...
public class RowAndDeletionMergeIterator extends AbstractUnfilteredRowIterator {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RowAndDeletionMergeIterator.class);
    ...
    private final transient boolean removeShadowedData;

    private final transient Comparator<Clusterable> comparator;

    private final transient ColumnFilter selection;

    private final transient Iterator<Row> rows;

    private transient Row nextRow;

    private final transient Iterator<RangeTombstone> ranges;

    private transient RangeTombstone nextRange;

    // The currently open tombstone. Note that unless this is null, there is no point in checking nextRange.
    private transient RangeTombstone openRange;
    ...
    private void updateNextRange() {
        while (nextRange == null && ranges.hasNext()) {
            ...
            // tombstone time.
            if ((removeShadowedData && !nextRange.deletionTime().supersedes(partitionLevelDeletion())) || nextRange.deletedSlice().isEmpty(metadata.comparator))
                nextRange = null;
        }
    }
