...
public interface RangeTombstoneMarker extends Unfiltered {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RangeTombstoneMarker.class);
    ...
    public static class Merger {
        ...
        private final transient DeletionTime partitionDeletion;

        private final transient boolean reversed;

        private transient ClusteringBoundOrBoundary<?> bound;

        private final transient RangeTombstoneMarker[] markers;

        // For each iterator, what is the currently open marker deletion time (or null if there is no open marker on that iterator)
        private final transient DeletionTime[] openMarkers;
        ...
        private transient int biggestOpenMarker = -1;
        ...
        public RangeTombstoneMarker merge() {
            ...
            if (previousDeletionTimeInMerged.isLive()) {
                merged = isBeforeClustering ? RangeTombstoneBoundMarker.inclusiveOpen(reversed, bound, newDeletionTimeInMerged) : RangeTombstoneBoundMarker.exclusiveOpen(reversed, bound, newDeletionTimeInMerged);
            } else if (newDeletionTimeInMerged.isLive()) {
                merged = isBeforeClustering ? RangeTombstoneBoundMarker.exclusiveClose(reversed, bound, previousDeletionTimeInMerged) : RangeTombstoneBoundMarker.inclusiveClose(reversed, bound, previousDeletionTimeInMerged);
            } else {
                merged = isBeforeClustering ? RangeTombstoneBoundaryMarker.exclusiveCloseInclusiveOpen(reversed, bound, previousDeletionTimeInMerged, newDeletionTimeInMerged) : RangeTombstoneBoundaryMarker.inclusiveCloseExclusiveOpen(reversed, bound, previousDeletionTimeInMerged, newDeletionTimeInMerged);
            }
            return merged;
        }
