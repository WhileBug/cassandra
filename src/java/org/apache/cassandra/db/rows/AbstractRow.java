...
public abstract class AbstractRow implements Row {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AbstractRow.class);
    ...
    public boolean hasInvalidDeletions() {
        ...
        if (!deletion().time().validate())
            return true;
        for (ColumnData cd : this) if (cd.hasInvalidDeletions())
            return true;
        return false;
    }
            ...
            if (fullDetails) {
                ...
                if (cd.column().isSimple()) {
                    ...
                    if (cd.column().type.isCollection()) {
                        ...
                        transform = cell -> String.format("%s -> %s", ct.nameComparator().getString(cell.path().get(0)), Cells.valueString(cell, ct.valueComparator()));
                        ...
                        transform = cell -> {
                            ...
                            return String.format("%s -> %s", ut.fieldNameAsString(fId), Cells.valueString(cell, ut.fieldType(fId)));
                        };
                    } else {
                        transform = cell -> "";
                    }
    ...
    public int hashCode() {
        ...
        for (ColumnData cd : this) hash += 31 * cd.hashCode();
        return hash;
    }
}
