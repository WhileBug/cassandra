...
public class BTreeRow extends AbstractRow {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(BTreeRow.class);

    private static final transient long EMPTY_SIZE = ObjectSizes.measure(emptyRow(Clustering.EMPTY));

    private final transient Clustering<?> clustering;

    private final transient LivenessInfo primaryKeyLivenessInfo;

    private final transient Deletion deletion;

    // The data for each columns present in this row in column sorted order.
    private final transient Object[] btree;

    private static final transient ColumnData FIRST_COMPLEX_STATIC = new ComplexColumnData(Columns.FIRST_COMPLEX_STATIC, new Object[0], new DeletionTime(0, 0));

    private static final transient ColumnData FIRST_COMPLEX_REGULAR = new ComplexColumnData(Columns.FIRST_COMPLEX_REGULAR, new Object[0], new DeletionTime(0, 0));

    private static final transient Comparator<ColumnData> COLUMN_COMPARATOR = (cd1, cd2) -> cd1.column.compareTo(cd2.column);
    ...
    public Row markCounterLocalToBeCleared() {
        return transform((cd) -> cd.column().isCounterColumn() ? cd.markCounterLocalToBeCleared() : cd);
    }
    ...
    public long unsharedHeapSizeExcludingData() {
        long heapSize = EMPTY_SIZE + clustering.unsharedHeapSizeExcludingData() + primaryKeyLivenessInfo.unsharedHeapSize() + deletion.unsharedHeapSize() + BTree.sizeOfStructureOnHeap(btree);
    ...
    private class CellIterator extends AbstractIterator<Cell<?>> {
        ...
        private transient Iterator<ColumnData> columnData = iterator();

        private transient Iterator<Cell<?>> complexCells;
    ...
    private class CellInLegacyOrderIterator extends AbstractIterator<Cell<?>> {
        ...
        private final transient int firstComplexIdx;

        private transient int simpleIdx;

        private transient int complexIdx;
    ...
    public static class Builder implements Row.Builder {
        ...
        private static class CellResolver implements BTree.Builder.Resolver {
            ...
            static final transient CellResolver instance = new CellResolver();

            public ColumnData resolve(Object[] cells, int lb, int ub) {
                Cell<?> cell = (Cell<?>) cells[lb];
                ColumnMetadata column = cell.column;
                if (cell.column.isSimple()) {
                    while (++lb < ub) cell = Cells.reconcile(cell, (Cell<?>) cells[lb]);
                    return cell;
                }
        ...
        protected transient Clustering<?> clustering;

        protected transient LivenessInfo primaryKeyLivenessInfo = LivenessInfo.EMPTY;

        protected transient Deletion deletion = Deletion.LIVE;

        private final transient boolean isSorted;

        private transient BTree.Builder<Cell<?>> cells_;

        private transient boolean hasComplex = false;
        ...
        public void newRow(Clustering<?> clustering) {
            // Ensures we've properly called build() if we've use this builder before
