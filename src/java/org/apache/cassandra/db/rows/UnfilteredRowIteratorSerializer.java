...
public class UnfilteredRowIteratorSerializer {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(UnfilteredRowIteratorSerializer.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(UnfilteredRowIteratorSerializer.class);

    protected static final transient Logger logger = LoggerFactory.getLogger(UnfilteredRowIteratorSerializer.class);

    private static final transient int IS_EMPTY = 0x01;

    private static final transient int IS_REVERSED = 0x02;

    private static final transient int HAS_PARTITION_DELETION = 0x04;

    private static final transient int HAS_STATIC_ROW = 0x08;

    private static final transient int HAS_ROW_ESTIMATE = 0x10;

    public static final transient UnfilteredRowIteratorSerializer serializer = new UnfilteredRowIteratorSerializer();
    ...
    public long serializedSize(UnfilteredRowIterator iterator, ColumnFilter selection, int version, int rowEstimate) {
        ...
        long size = // flags
        ByteBufferUtil.serializedSizeWithVIntLength(iterator.partitionKey().getKey()) + 1;
        if (iterator.isEmpty())
            return size;
        DeletionTime partitionDeletion = iterator.partitionLevelDeletion();
        Row staticRow = iterator.staticRow();
    ...
    public static class Header {
        ...
        public final transient SerializationHeader sHeader;

        public final transient DecoratedKey key;

        public final transient boolean isReversed;

        public final transient boolean isEmpty;

        public final transient DeletionTime partitionDeletion;

        public final transient Row staticRow;

        // -1 if no estimate
        public final transient int rowEstimate;
