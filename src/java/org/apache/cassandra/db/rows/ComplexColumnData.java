...
public class ComplexColumnData extends ColumnData implements Iterable<Cell<?>> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ComplexColumnData.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ComplexColumnData.class);

    static final transient Cell<?>[] NO_CELLS = new Cell<?>[0];

    private static final transient long EMPTY_SIZE = ObjectSizes.measure(new ComplexColumnData(ColumnMetadata.regularColumn("", "", "", SetType.getInstance(ByteType.instance, true)), NO_CELLS, new DeletionTime(0, 0)));

    // The cells for 'column' sorted by cell path.
    private final transient Object[] cells;

    private final transient DeletionTime complexDeletion;
    ...
    public boolean hasInvalidDeletions() {
        if (!complexDeletion.validate())
            return true;
        for (Cell<?> cell : this) if (cell.hasInvalidDeletions())
            return true;
        return false;
    }
    ...
    public String toString() {
        return String.format("[%s=%s %s]", column().name, complexDeletion.toString(), BTree.toString(cells));
    }
    ...
    public static class Builder {
        ...
        private transient DeletionTime complexDeletion;

        private transient ColumnMetadata column;

        private transient BTree.Builder<Cell<?>> builder;
