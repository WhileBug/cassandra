...
public abstract class Cell<V> extends ColumnData {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Cell.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Cell.class);

    public static final transient int NO_TTL = 0;

    public static final transient int NO_DELETION_TIME = Integer.MAX_VALUE;

    public static final transient int MAX_DELETION_TIME = Integer.MAX_VALUE - 1;

    public final static transient Comparator<Cell<?>> comparator = (c1, c2) -> {
    ...
    public static final transient Serializer serializer = new BufferCell.Serializer();
    ...
    @Override
    public abstract // Overrides super type to provide a more precise return type.
    ...
    static class Serializer {
        ...
        // Whether the cell is a tombstone or not.
        private final static transient int IS_DELETED_MASK = 0x01;

        // Whether the cell is expiring.
        private final static transient int IS_EXPIRING_MASK = 0x02;

        // Wether the cell has an empty value. This will be the case for tombstone in particular.
        private final static transient int HAS_EMPTY_VALUE_MASK = 0x04;

        // Wether the cell has the same timestamp than the row this is a cell of.
        private final static transient int USE_ROW_TIMESTAMP_MASK = 0x08;

        // Wether the cell has the same ttl than the row this is a cell of.
        private final static transient int USE_ROW_TTL_MASK = 0x10;
