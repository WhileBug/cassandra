...
public abstract class AbstractCell<V> extends Cell<V> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AbstractCell.class);
    ...
    public int dataSize() {
        CellPath path = path();
        return TypeSizes.sizeof(timestamp()) + TypeSizes.sizeof(ttl()) + TypeSizes.sizeof(localDeletionTime()) + valueSize() + (path == null ? 0 : path.dataSize());
    }
    ...
    public void digest(Digest digest) {
        ...
        digest.updateWithLong(timestamp()).updateWithInt(ttl()).updateWithBoolean(isCounterCell());
        if (path() != null)
            path().digest(digest);
    }
    ...
    public String toString() {
        ...
        if (type instanceof CollectionType && type.isMultiCell()) {
            ...
            return String.format("[%s[%s]=%s %s]", column().name, ct.nameComparator().getString(path().get(0)), ct.valueComparator().getString(value(), accessor()), livenessInfoString());
        }
        if (isTombstone())
