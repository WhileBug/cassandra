...
public class EncodingStats implements IMeasurableMemory {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(EncodingStats.class);
    ...
    public static final transient long TIMESTAMP_EPOCH;

    private static final transient int DELETION_TIME_EPOCH;

    private static final transient int TTL_EPOCH = 0;
    ...
    static {
        ...
        // timestamps should be in microseconds by convention
        TIMESTAMP_EPOCH = c.getTimeInMillis() * 1000;
        // local deletion times are in seconds
        DELETION_TIME_EPOCH = (int) (c.getTimeInMillis() / 1000);
    }

    // We should use this sparingly obviously
    public static final transient EncodingStats NO_STATS = new EncodingStats(TIMESTAMP_EPOCH, DELETION_TIME_EPOCH, TTL_EPOCH);

    public static transient long HEAP_SIZE = ObjectSizes.measure(NO_STATS);
    ...
    public long unsharedHeapSize() {
        if (this == NO_STATS) {
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.rows.EncodingStats.NO_STATS]=" + org.json.simple.JSONValue.toJSONString(NO_STATS).replace("\n", "").replace("\r", ""));
            logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.rows.EncodingStats.NO_STATS]=" + org.json.simple.JSONValue.toJSONString(NO_STATS).replace("\n", "").replace("\r", ""));
            return 0;
        }
        logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.rows.EncodingStats.NO_STATS]=" + org.json.simple.JSONValue.toJSONString(NO_STATS).replace("\n", "").replace("\r", ""));
    ...
    public static class Collector implements PartitionStatisticsCollector {
        ...
        private transient boolean isTimestampSet;

        private transient long minTimestamp = Long.MAX_VALUE;

        private transient boolean isDelTimeSet;

        private transient int minDeletionTime = Integer.MAX_VALUE;

        private transient boolean isTTLSet;

        private transient int minTTL = Integer.MAX_VALUE;
        ...
        public EncodingStats get() {
            return new EncodingStats(isTimestampSet ? minTimestamp : TIMESTAMP_EPOCH, isDelTimeSet ? minDeletionTime : DELETION_TIME_EPOCH, isTTLSet ? minTTL : TTL_EPOCH);
        }
