...
public final class VirtualSchemaKeyspace extends VirtualKeyspace {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(VirtualSchemaKeyspace.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(VirtualSchemaKeyspace.class);

    public static final transient VirtualSchemaKeyspace instance = new VirtualSchemaKeyspace();
    ...
    private static final class VirtualKeyspaces extends AbstractVirtualTable {
        ...
        private static final transient String KEYSPACE_NAME = "keyspace_name";

        private VirtualKeyspaces(String keyspace) {
            super(builder(keyspace, "keyspaces").comment("virtual keyspace definitions").kind(TableMetadata.Kind.VIRTUAL).partitioner(new LocalPartitioner(UTF8Type.instance)).addPartitionKeyColumn(KEYSPACE_NAME, UTF8Type.instance).build());
        }

        public DataSet data() {
            SimpleDataSet result = new SimpleDataSet(metadata());
            for (KeyspaceMetadata keyspace : VirtualKeyspaceRegistry.instance.virtualKeyspacesMetadata()) result.row(keyspace.name);
            return result;
        }
    }
    ...
    private static final class VirtualTables extends AbstractVirtualTable {
        ...
        private static final transient String TABLE_NAME = "table_name";

        private static final transient String COMMENT = "comment";

        private VirtualTables(String keyspace) {
            super(builder(keyspace, "tables").comment("virtual table definitions").kind(TableMetadata.Kind.VIRTUAL).partitioner(new LocalPartitioner(UTF8Type.instance)).addPartitionKeyColumn(KEYSPACE_NAME, UTF8Type.instance).addClusteringColumn(TABLE_NAME, UTF8Type.instance).addRegularColumn(COMMENT, UTF8Type.instance).build());
        }
    ...
    private static final class VirtualColumns extends AbstractVirtualTable {
        ...
        private static final transient String COLUMN_NAME = "column_name";

        private static final transient String CLUSTERING_ORDER = "clustering_order";

        private static final transient String COLUMN_NAME_BYTES = "column_name_bytes";

        private static final transient String KIND = "kind";

        private static final transient String POSITION = "position";
