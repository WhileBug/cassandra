...
public class ViewManager {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ViewManager.class);

    private static final transient Logger logger = LoggerFactory.getLogger(ViewManager.class);

    private static final transient Striped<Lock> LOCKS = Striped.lazyWeakLock(DatabaseDescriptor.getConcurrentViewWriters() * 1024);

    private static final transient boolean enableCoordinatorBatchlog = Boolean.getBoolean("cassandra.mv_enable_coordinator_batchlog");

    private final transient ConcurrentMap<String, View> viewsByName = new ConcurrentHashMap<>();

    private final transient ConcurrentMap<TableId, TableViews> viewsByBaseTable = new ConcurrentHashMap<>();
