...
public final class RTBoundValidator extends Transformation<UnfilteredRowIterator> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RTBoundValidator.class);

    public enum Stage {

        MEMTABLE, SSTABLE, MERGED, PURGED, PROCESSED
    }

    private final transient Stage stage;

    private final transient boolean enforceIsClosed;
    ...
    private final static class RowsTransformation extends Transformation {
        ...
        private final transient boolean isReverseOrder;
        ...
        private IllegalStateException ise(String why) {
            String message = String.format("%s UnfilteredRowIterator for %s has an illegal RT bounds sequence: %s", stage, metadata, why);
