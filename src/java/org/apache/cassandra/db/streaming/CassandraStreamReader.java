...
public class CassandraStreamReader implements IStreamReader {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CassandraStreamReader.class);

    private static final transient Logger logger = LoggerFactory.getLogger(CassandraStreamReader.class);

    protected final transient TableId tableId;

    protected final transient long estimatedKeys;

    protected final transient Collection<SSTableReader.PartitionPositionBounds> sections;

    protected final transient StreamSession session;

    protected final transient Version inputVersion;

    protected final transient long repairedAt;

    protected final transient UUID pendingRepair;

    protected final transient SSTableFormat.Type format;

    protected final transient int sstableLevel;

    protected final transient SerializationHeader.Component header;

    protected final transient int fileSeqNum;
    ...
    protected SerializationHeader getHeader(TableMetadata metadata) throws UnknownColumnException {
        // pre-3.0 sstable have no SerializationHeader
    ...
    protected long totalSize() {
        long size = 0;
        for (SSTableReader.PartitionPositionBounds section : sections) size += section.upperPosition - section.lowerPosition;
        return size;
    }
    ...
    public static class StreamDeserializer extends UnmodifiableIterator<Unfiltered> implements UnfilteredRowIterator {
        ...
        private final transient DataInputPlus in;

        private final transient SerializationHeader header;

        private final transient DeserializationHelper helper;

        private transient DecoratedKey key;

        private transient DeletionTime partitionLevelDeletion;

        private transient SSTableSimpleIterator iterator;

        private transient Row staticRow;

        private transient IOException exception;
