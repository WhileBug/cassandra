...
public class CompressedInputStream extends RebufferingInputStream implements AutoCloseable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CompressedInputStream.class);

    private static final transient double GROWTH_FACTOR = 1.5;

    private final transient DataInputPlus input;

    private final transient Iterator<CompressionMetadata.Chunk> compressedChunks;

    private final transient CompressionParams compressionParams;

    private final transient ChecksumType checksumType;

    private final transient DoubleSupplier validateChecksumChance;

    /**
     * The base offset of the current {@link #buffer} into the original sstable as if it were uncompressed.
     */
    private transient long uncompressedChunkPosition = Long.MIN_VALUE;
    ...
    private void loadNextChunk() throws IOException {
        ...
        // chunk length + checksum or CRC length
        chunkBytesRead += (chunkLength + 4);
        /*
    ...
    private transient ByteBuffer compressedChunk;
    ...
    private transient byte[] copyArray;
    ...
    private void maybeValidateChecksum(ByteBuffer buffer, int expectedChecksum) throws IOException {
        ...
        if (validateChance >= 1.0d || (validateChance > 0.0d && validateChance > ThreadLocalRandom.current().nextDouble())) {
            ...
            // checksum calculation consumes the buffer, so we must reset its position afterwards
    ...
    private transient long chunkBytesRead = 0;
}
