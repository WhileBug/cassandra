...
public class CassandraStreamWriter {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CassandraStreamWriter.class);

    private static final transient int DEFAULT_CHUNK_SIZE = 64 * 1024;

    private static final transient Logger logger = LoggerFactory.getLogger(CassandraStreamWriter.class);

    protected final transient SSTableReader sstable;

    private final transient LZ4Compressor compressor = LZ4Factory.fastestInstance().fastCompressor();

    protected final transient Collection<SSTableReader.PartitionPositionBounds> sections;

    protected final transient StreamRateLimiter limiter;
