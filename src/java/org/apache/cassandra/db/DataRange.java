...
public class DataRange {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(DataRange.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(DataRange.class);

    public static final transient Serializer serializer = new Serializer();

    protected final transient AbstractBounds<PartitionPosition> keyRange;

    protected final transient ClusteringIndexFilter clusteringIndexFilter;
    ...
    public static class Paging extends DataRange {
        ...
        private final transient Clustering<?> lastReturned;

        private final transient boolean inclusive;
        ...
        @Override
        public ClusteringIndexFilter clusteringIndexFilter(DecoratedKey key) {
            return key.equals(startKey()) ? clusteringIndexFilter.forPaging(comparator, lastReturned, inclusive) : clusteringIndexFilter;
        }
        ...
        public DataRange forSubRange(AbstractBounds<PartitionPosition> range) {
            ...
            return range.left.equals(keyRange().left) ? new Paging(range, clusteringIndexFilter, comparator, lastReturned, inclusive) : new DataRange(range, clusteringIndexFilter);
        }
        ...
        @Override
        public String toString(TableMetadata metadata) {
            return String.format("range=%s (paging) pfilter=%s lastReturned=%s (%s)", keyRange.getString(metadata.partitionKeyType), clusteringIndexFilter.toString(metadata), lastReturned.toString(metadata), inclusive ? "included" : "excluded");
        }
    }
    ...
    public static class Serializer {
        ...
        public long serializedSize(DataRange range, int version, TableMetadata metadata) {
            long size = // isPaging boolean
            AbstractBounds.rowPositionSerializer.serializedSize(range.keyRange, version) + ClusteringIndexFilter.serializer.serializedSize(range.clusteringIndexFilter, version) + 1;
