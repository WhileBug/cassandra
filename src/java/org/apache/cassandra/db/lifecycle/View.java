...
public class View {
     ...
     */
    public final transient List<Memtable> liveMemtables;
     ...
     */
    public final transient List<Memtable> flushingMemtables;

    final transient Set<SSTableReader> compacting;

    final transient Set<SSTableReader> sstables;
    ...
    final transient Map<SSTableReader, SSTableReader> sstablesMap;

    final transient Map<SSTableReader, SSTableReader> compactingMap;

    final transient SSTableIntervalTree intervalTree;
    ...
    public Iterable<SSTableReader> select(SSTableSet sstableSet) {
        switch(sstableSet) {
            ...
            case CANONICAL:
                ...
                for (SSTableReader sstable : compacting) if (sstable.openReason != SSTableReader.OpenReason.EARLY)
                ...
                for (SSTableReader sstable : sstables) if (!compacting.contains(sstable) && sstable.openReason != SSTableReader.OpenReason.EARLY)
                    canonicalSSTables.add(sstable);
    ...
    static Predicate<View> permitCompacting(final Iterable<SSTableReader> readers) {
        return new Predicate<View>() {
            ...
            public boolean apply(View view) {
                for (SSTableReader reader : readers) if (view.compacting.contains(reader) || view.sstablesMap.get(reader) != reader || reader.isMarkedCompacted())
                    return false;
                return true;
            }
        };
    ...
    static Function<View, View> markFlushing(final Memtable toFlush) {
        return new Function<View, View>() {
            ...
            public View apply(View view) {
                ...
                List<Memtable> newFlushing = copyOf(concat(filter(flushing, lessThan(toFlush)), of(toFlush), filter(flushing, not(lessThan(toFlush)))));
