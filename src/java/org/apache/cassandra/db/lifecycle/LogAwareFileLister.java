...
final class LogAwareFileLister {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(LogAwareFileLister.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(LogAwareFileLister.class);

    private static final transient Logger logger = LoggerFactory.getLogger(LogAwareFileLister.class);
    ...
    // file, file type
    private final transient BiPredicate<File, FileType> filter;

    // The behavior when we fail to list files
    private final transient OnTxnErr onTxnErr;

    // The unfiltered result
    transient NavigableMap<File, Directories.FileType> files = new TreeMap<>();
