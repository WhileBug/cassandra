...
public class RowIndexEntry<T> implements IMeasurableMemory {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RowIndexEntry.class);

    private static final transient long EMPTY_SIZE = ObjectSizes.measure(new RowIndexEntry(0));

    // constants for type of row-index-entry as serialized for saved-cache
    static final transient int CACHE_NOT_INDEXED = 0;

    static final transient int CACHE_INDEXED = 1;

    static final transient int CACHE_INDEXED_SHALLOW = 2;

    static final transient Histogram indexEntrySizeHistogram;

    static final transient Histogram indexInfoCountHistogram;

    static final transient Histogram indexInfoGetsHistogram;

    static final transient Histogram indexInfoReadsHistogram;
    ...
    public final transient long position;
    ...
    public static final class Serializer implements IndexSerializer<IndexInfo> {
        ...
        private final transient IndexInfo.Serializer idxInfoSerializer;
    ...
    private static final class IndexedEntry extends RowIndexEntry<IndexInfo> {
        ...
        private static final transient long BASE_SIZE;
        ...
        private final transient long headerLength;

        private final transient IndexInfo[] columnsIndex;

        private final transient int[] offsets;

        private final transient int indexedPartSize;

        @Unmetered
        private final transient ISerializer<IndexInfo> idxInfoSerializer;
    ...
    private static final class ShallowIndexedEntry extends RowIndexEntry<IndexInfo> {
        ...
        private final transient long indexFilePosition;

        private final transient DeletionTime deletionTime;

        private final transient long headerLength;

        private final transient int columnsIndexCount;

        private final transient int indexedPartSize;

        private final transient int offsetsOffset;

        @Unmetered
        private final transient ISerializer<IndexInfo> idxInfoSerializer;

        private final transient int fieldsSerializedSize;
    ...
    private abstract static class FileIndexInfoRetriever implements IndexInfoRetriever {
        ...
        final transient long indexInfoFilePosition;

        final transient ISerializer<IndexInfo> idxInfoSerializer;

        final transient FileDataInput indexReader;

        transient int retrievals;
