...
public class RangeTombstoneList implements Iterable<RangeTombstone>, IMeasurableMemory {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RangeTombstoneList.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RangeTombstoneList.class);

    private static transient long EMPTY_SIZE = ObjectSizes.measure(new RangeTombstoneList(null, 0));
    ...
    private transient ClusteringBound<?>[] starts;

    private transient ClusteringBound<?>[] ends;

    private transient long[] markedAts;

    private transient int[] delTimes;

    private transient long boundaryHeapSize;
    ...
    public ClusteringComparator comparator() {
        logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.RangeTombstoneList.comparator]=" + org.json.simple.JSONValue.toJSONString(comparator).replace("\n", "").replace("\r", ""));
        logger_IC.info("[InconsistencyDetector][org.apache.cassandra.db.RangeTombstoneList.comparator]=" + org.json.simple.JSONValue.toJSONString(comparator).replace("\n", "").replace("\r", ""));
        return comparator;
    }
    ...
    private static <T> ClusteringBound<ByteBuffer> clone(ClusteringBound<T> bound, ByteBufferCloner cloner) {
        ...
        for (int i = 0; i < values.length; i++) values[i] = cloner.clone(bound.get(i), bound.accessor());
