...
public class CompactionIterator extends CompactionInfo.Holder implements UnfilteredPartitionIterator {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CompactionIterator.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CompactionIterator.class);

    private static final transient long UNFILTERED_TO_UPDATE_PROGRESS = 100;

    private final transient OperationType type;

    private final transient AbstractCompactionController controller;

    private final transient List<ISSTableScanner> scanners;
    ...
    private final transient long totalBytes;

    private transient long bytesRead;

    private transient long totalSourceCQLRows;
     ...
     */
    private final transient long[] mergeCounters;

    private final transient UnfilteredPartitionIterator compacted;

    private final transient ActiveCompactionsTracker activeCompactions;
    ...
    public CompactionIterator(OperationType type, List<ISSTableScanner> scanners, AbstractCompactionController controller, int nowInSec, UUID compactionId, ActiveCompactionsTracker activeCompactions) {
        ...
        // note that CompactionTask also calls this, but CT only creates CompactionIterator with a NOOP ActiveCompactions
    ...
    public CompactionInfo getCompactionInfo() {
        return new CompactionInfo(controller.cfs.metadata(), type, bytesRead, totalBytes, compactionId, sstables);
    }
    ...
    private class Purger extends PurgeFunction {
        ...
        private transient LongPredicate purgeEvaluator;

        private transient long compactedUnfiltered;

        private Purger(AbstractCompactionController controller, int nowInSec) {
            super(nowInSec, controller.gcBefore, controller.compactingRepaired() ? Integer.MAX_VALUE : Integer.MIN_VALUE, controller.cfs.getCompactionStrategyManager().onlyPurgeRepairedTombstones(), controller.cfs.metadata.get().enforceStrictLiveness());
            this.controller = controller;
        }
    ...
    private static class GarbageSkippingUnfilteredRowIterator extends WrappingUnfilteredRowIterator {
        ...
        final transient UnfilteredRowIterator tombSource;

        final transient DeletionTime partitionLevelDeletion;

        final transient Row staticRow;

        final transient ColumnFilter cf;

        final transient TableMetadata metadata;

        final transient boolean cellLevelGC;

        transient DeletionTime tombOpenDeletionTime = DeletionTime.LIVE;

        transient DeletionTime dataOpenDeletionTime = DeletionTime.LIVE;

        transient DeletionTime openDeletionTime = DeletionTime.LIVE;

        transient DeletionTime partitionDeletionTime;

        transient DeletionTime activeDeletionTime;

        transient Unfiltered tombNext = null;

        transient Unfiltered dataNext = null;

        transient Unfiltered next = null;
    ...
    private static class GarbageSkipper extends Transformation<UnfilteredRowIterator> {
        ...
        final transient AbstractCompactionController controller;
    ...
    private static class AbortableUnfilteredPartitionTransformation extends Transformation<UnfilteredRowIterator> {
        ...
        private final transient AbortableUnfilteredRowTransformation abortableIter;
    ...
    private static class AbortableUnfilteredRowTransformation extends Transformation {
        ...
        private final transient CompactionIterator iter;
