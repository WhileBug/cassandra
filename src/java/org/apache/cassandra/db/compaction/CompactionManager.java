...
public class CompactionManager implements CompactionManagerMBean {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CompactionManager.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CompactionManager.class);

    public static final transient String MBEAN_OBJECT_NAME = "org.apache.cassandra.db:type=CompactionManager";

    private static final transient Logger logger = LoggerFactory.getLogger(CompactionManager.class);

    public static final transient CompactionManager instance;

    @VisibleForTesting
    public final transient AtomicInteger currentlyBackgroundUpgrading = new AtomicInteger(0);

    public static final transient int NO_GC = Integer.MIN_VALUE;

    public static final transient int GC_ALL = Integer.MAX_VALUE;

    // A thread local that tells us if the current thread is owned by the compaction manager. Used
    // by CounterContext to figure out if it should log a warning for invalid counter shards.
    public static final transient FastThreadLocal<Boolean> isCompactionManager = new FastThreadLocal<Boolean>() {
    ...
    private final transient CompactionExecutor executor = new CompactionExecutor();

    private final transient ValidationExecutor validationExecutor = new ValidationExecutor();

    private final transient CompactionExecutor cacheCleanupExecutor = new CacheCleanupExecutor();

    private final transient CompactionExecutor viewBuildExecutor = new ViewBuildExecutor();

    private final transient CompactionMetrics metrics = new CompactionMetrics(executor, validationExecutor, viewBuildExecutor);

    @VisibleForTesting
    final transient Multiset<ColumnFamilyStore> compactingCF = ConcurrentHashMultiset.create();

    public final transient ActiveCompactions active = new ActiveCompactions();

    // used to temporarily pause non-strategy managed compactions (like index summary redistribution)
    private final transient AtomicInteger globalCompactionPauseCount = new AtomicInteger(0);

    private final transient RateLimiter compactionRateLimiter = RateLimiter.create(Double.MAX_VALUE);
    ...
    public boolean isCompacting(Iterable<ColumnFamilyStore> cfses, Predicate<SSTableReader> sstablePredicate) {
        for (ColumnFamilyStore cfs : cfses) if (cfs.getTracker().getCompacting().stream().anyMatch(sstablePredicate))
            return true;
        return false;
    }
    ...
    class BackgroundCompactionCandidate implements Runnable {
        ...
        public void run() {
            ...
            if (// only submit background if we actually ran a compaction - otherwise we end up in an infinite loop submitting noop background tasks
            ranCompaction)
    ...
    public enum AllSSTableOpStatus {
        ...
        SUCCESSFUL(0), ABORTED(1), UNABLE_TO_CANCEL(2);
    ...
    public AllSSTableOpStatus relocateSSTables(final ColumnFamilyStore cfs, int jobs) throws ExecutionException, InterruptedException {
        ...
        return parallelAllSSTableOperation(cfs, new OneSSTableOperation() {
            ...
            public Iterable<SSTableReader> filterSSTables(LifecycleTransaction transaction) {
                ...
                for (int i = 0; i < maxSize; i++) for (List<SSTableReader> diskSSTables : groupedByDisk.values()) if (i < diskSSTables.size())
    ...
    public void forceCompactionForTokenRange(ColumnFamilyStore cfStore, Collection<Range<Token>> ranges) {
        ...
        try (CompactionTasks tasks = cfStore.runWithCompactionsDisabled(taskCreator, (sstable) -> new Bounds<>(sstable.first.getToken(), sstable.last.getToken()).intersects(ranges), false, false, false)) {
            ...
            Runnable runnable = new WrappedRunnable() {
                ...
                protected void runMayThrow() {
                    for (AbstractCompactionTask task : tasks) if (task != null)
                        task.execute(active);
                }
            };
    ...
    private static abstract class CleanupStrategy {
        ...
        protected final transient Collection<Range<Token>> ranges;
        ...
        private static final class Bounded extends CleanupStrategy {
            ...
            private final transient Collection<Range<Token>> transientRanges;

            private final transient boolean isRepaired;
    ...
    public static class ValidationExecutor extends CompactionExecutor {
        ...
        public ValidationExecutor() {
            super(DatabaseDescriptor.getConcurrentValidations(), DatabaseDescriptor.getConcurrentValidations(), "ValidationExecutor", new LinkedBlockingQueue());
