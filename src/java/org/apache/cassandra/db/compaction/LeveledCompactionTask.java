...
public class LeveledCompactionTask extends CompactionTask {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(LeveledCompactionTask.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(LeveledCompactionTask.class);

    private final transient int level;

    private final transient long maxSSTableBytes;

    private final transient boolean majorCompaction;
    ...
    public boolean reduceScopeForLimitedSpace(Set<SSTableReader> nonExpiredSSTables, long expectedSize) {
        if (transaction.originals().size() > 1 && level <= 1) {
            ...
            logger.warn("insufficient space to do L0 -> L{} compaction. {}MiB required, {} for compaction {}", level, (float) expectedSize / 1024 / 1024, transaction.originals().stream().map(sstable -> String.format("%s (level=%s, size=%s)", sstable, sstable.getSSTableLevel(), sstable.onDiskLength())).collect(Collectors.joining(",")), transaction.opId());
            ...
            if (largestL0SSTable != null && l0SSTableCount > 1) {
                logger.info("Removing {} (level={}, size={}) from compaction {}", largestL0SSTable, largestL0SSTable.getSSTableLevel(), largestL0SSTable.onDiskLength(), transaction.opId());
