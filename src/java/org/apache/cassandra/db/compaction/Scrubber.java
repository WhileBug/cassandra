...
public class Scrubber implements Closeable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Scrubber.class);
    ...
    private final transient LifecycleTransaction transaction;

    private final transient File destination;

    private final transient boolean skipCorrupted;

    private final transient boolean reinsertOverflowedTTLRows;

    private final transient boolean isCommutative;

    private final transient boolean isIndex;

    private final transient boolean checkData;

    private final transient long expectedBloomFilterSize;

    private final transient ReadWriteLock fileAccessLock;

    private final transient RandomAccessReader dataFile;

    private final transient RandomAccessReader indexFile;

    private final transient ScrubInfo scrubInfo;

    private final transient RowIndexEntry.IndexSerializer rowIndexEntrySerializer;

    private transient int goodPartitions;

    private transient int badPartitions;

    private transient int emptyPartitions;

    private transient ByteBuffer currentIndexKey;

    private transient ByteBuffer nextIndexKey;

    private transient long currentPartitionPositionFromIndex;

    private transient long nextPartitionPositionFromIndex;

    private transient NegativeLocalDeletionInfoMetrics negativeLocalDeletionInfoMetrics = new NegativeLocalDeletionInfoMetrics();

    private final transient OutputHandler outputHandler;

    private static final transient Comparator<Partition> partitionComparator = Comparator.comparing(Partition::partitionKey);

    private final transient SortedSet<Partition> outOfOrder = new TreeSet<>(partitionComparator);
    ...
    public Scrubber(ColumnFamilyStore cfs, LifecycleTransaction transaction, boolean skipCorrupted, OutputHandler outputHandler, boolean checkData, boolean reinsertOverflowedTTLRows) {
        ...
        // LocalByPartitionerType does not support validation
            ...
            while (!dataFile.isEOF()) {
                ...
                try {
                    ...
                    if (currentIndexKey != null && !key.getKey().equals(currentIndexKey)) {
                        throw new IOError(new IOException(// ByteBufferUtil.bytesToHex(key.getKey()), ByteBufferUtil.bytesToHex(currentIndexKey))));
                        String.format(// ByteBufferUtil.bytesToHex(key.getKey()), ByteBufferUtil.bytesToHex(currentIndexKey))));
                        "Key from data file (%s) does not match key from index file (%s)", "_too big_", ByteBufferUtil.bytesToHex(currentIndexKey))));
                    }
    ...
    private void updateIndexKey() {
        ...
        try {
            ...
            nextPartitionPositionFromIndex = !indexAvailable() ? dataFile.length() : rowIndexEntrySerializer.deserializePositionAndSkip(indexFile);
        } catch (Throwable th) {
    ...
    private void saveOutOfOrderPartition(DecoratedKey prevKey, DecoratedKey key, UnfilteredRowIterator iterator) {
        ...
        outputHandler.warn(String.format("Out of order partition detected (%s found after %s)", keyString(key), keyString(prevKey)));
    ...
    private void throwIfCannotContinue(DecoratedKey key, Throwable th) {
        if (isIndex) {
            outputHandler.warn(String.format("An error occurred while scrubbing the partition with key '%s' for an index table. " + "Scrubbing will abort for this table and the index will be rebuilt.", keyString(key)));
            throw new IOError(th);
        }
        if (isCommutative && !skipCorrupted) {
            outputHandler.warn(String.format("An error occurred while scrubbing the partition with key '%s'.  Skipping corrupt " + "data in counter tables will result in undercounts for the affected " + "counters (see CASSANDRA-2759 for more details), so by default the scrub will " + "stop at this point.  If you would like to skip the row anyway and continue " + "scrubbing, re-run the scrub with the --skip-corrupted option.", keyString(key)));
            throw new IOError(th);
        }
    }
    ...
    private static class ScrubInfo extends CompactionInfo.Holder {
        ...
        private final transient UUID scrubCompactionId;

        private final transient Lock fileReadLock;
        ...
        public CompactionInfo getCompactionInfo() {
            fileReadLock.lock();
            try {
                return new CompactionInfo(sstable.metadata(), OperationType.SCRUB, dataFile.getFilePointer(), dataFile.length(), scrubCompactionId, ImmutableSet.of(sstable));
            } catch (Exception e) {
    ...
    public static final class ScrubResult {
        ...
        public final transient int goodPartitions;

        public final transient int badPartitions;

        public final transient int emptyPartitions;
    ...
    public class NegativeLocalDeletionInfoMetrics {
        ...
        public volatile transient int fixedRows = 0;
    }
    ...
    private static class RowMergingSSTableIterator extends WrappingUnfilteredRowIterator {
        ...
        transient Unfiltered nextToOffer = null;
    ...
    private static final class OrderCheckerIterator extends AbstractIterator<Unfiltered> implements UnfilteredRowIterator {
         ...
         */
        private final transient UnfilteredRowIterator iterator;

        private final transient ClusteringComparator comparator;

        private transient Unfiltered previous;

        /**
         * The partition containing the rows which are out of order.
         */
        private transient Partition rowsOutOfOrder;
    ...
    private static final class FixNegativeLocalDeletionTimeIterator extends AbstractIterator<Unfiltered> implements UnfilteredRowIterator {
        ...
        private final transient NegativeLocalDeletionInfoMetrics negativeLocalExpirationTimeMetrics;
        ...
        private Unfiltered fixNegativeLocalExpirationTime(Row row) {
            ...
            return row.transformAndFilter(livenessInfo, row.deletion(), cd -> {
                if (cd.column().isSimple()) {
                    Cell cell = (Cell) cd;
                    return cell.isExpiring() && cell.localDeletionTime() < 0 ? cell.withUpdatedTimestampAndLocalDeletionTime(cell.timestamp() + 1, AbstractCell.MAX_DELETION_TIME) : cell;
                } else {
                    ComplexColumnData complexData = (ComplexColumnData) cd;
                    return complexData.transformAndFilter(cell -> cell.isExpiring() && cell.localDeletionTime() < 0 ? cell.withUpdatedTimestampAndLocalDeletionTime(cell.timestamp() + 1, AbstractCell.MAX_DELETION_TIME) : cell);
                }
