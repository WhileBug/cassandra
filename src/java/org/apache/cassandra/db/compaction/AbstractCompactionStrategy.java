...
public abstract class AbstractCompactionStrategy {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AbstractCompactionStrategy.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AbstractCompactionStrategy.class);

    private static final transient Logger logger = LoggerFactory.getLogger(AbstractCompactionStrategy.class);

    protected static final transient float DEFAULT_TOMBSTONE_THRESHOLD = 0.2f;

    // minimum interval needed to perform tombstone removal compaction in seconds, default 86400 or 1 day.
    protected static final transient long DEFAULT_TOMBSTONE_COMPACTION_INTERVAL = 86400;

    protected static final transient boolean DEFAULT_UNCHECKED_TOMBSTONE_COMPACTION_OPTION = false;

    protected static final transient boolean DEFAULT_LOG_ALL_OPTION = false;

    protected static final transient String TOMBSTONE_THRESHOLD_OPTION = "tombstone_threshold";

    protected static final transient String TOMBSTONE_COMPACTION_INTERVAL_OPTION = "tombstone_compaction_interval";

    // disable range overlap check when deciding if an SSTable is candidate for tombstone compaction (CASSANDRA-6563)
    protected static final transient String UNCHECKED_TOMBSTONE_COMPACTION_OPTION = "unchecked_tombstone_compaction";

    protected static final transient String LOG_ALL_OPTION = "log_all";

    protected static final transient String COMPACTION_ENABLED = "enabled";

    public static final transient String ONLY_PURGE_REPAIRED_TOMBSTONES = "only_purge_repaired_tombstones";

    protected transient Map<String, String> options;

    protected final transient ColumnFamilyStore cfs;

    protected transient float tombstoneThreshold;

    protected transient long tombstoneCompactionInterval;

    protected transient boolean uncheckedTombstoneCompaction;

    protected transient boolean disableTombstoneCompactions = false;

    protected transient boolean logAll = true;
    ...
    public ScannerList getScanners(Collection<SSTableReader> sstables, Collection<Range<Token>> ranges) {
        ...
        try {
            for (SSTableReader sstable : sstables) scanners.add(sstable.getScanner(ranges));
        } catch (Throwable t) {
    ...
    public synchronized void addSSTables(Iterable<SSTableReader> added) {
        for (SSTableReader sstable : added) addSSTable(sstable);
    }
    ...
    public static class ScannerList implements AutoCloseable {
        ...
        public final transient List<ISSTableScanner> scanners;
        ...
        public long getTotalBytesScanned() {
            ...
            for (int i = 0, isize = scanners.size(); i < isize; i++) bytesScanned += scanners.get(i).getBytesScanned();
            return bytesScanned;
        }

        public long getTotalCompressedSize() {
            long compressedSize = 0;
            for (int i = 0, isize = scanners.size(); i < isize; i++) compressedSize += scanners.get(i).getCompressedLengthInBytes();
    ...
    public SSTableMultiWriter createSSTableMultiWriter(Descriptor descriptor, long keyCount, long repairedAt, UUID pendingRepair, boolean isTransient, MetadataCollector meta, SerializationHeader header, Collection<Index> indexes, LifecycleNewTracker lifecycleNewTracker) {
