...
public final class TimeWindowCompactionStrategyOptions {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TimeWindowCompactionStrategyOptions.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TimeWindowCompactionStrategyOptions.class);

    private static final transient Logger logger = LoggerFactory.getLogger(TimeWindowCompactionStrategyOptions.class);

    protected static final transient TimeUnit DEFAULT_TIMESTAMP_RESOLUTION = TimeUnit.MICROSECONDS;

    protected static final transient TimeUnit DEFAULT_COMPACTION_WINDOW_UNIT = TimeUnit.DAYS;

    protected static final transient int DEFAULT_COMPACTION_WINDOW_SIZE = 1;

    protected static final transient int DEFAULT_EXPIRED_SSTABLE_CHECK_FREQUENCY_SECONDS = 60 * 10;

    protected static final transient Boolean DEFAULT_UNSAFE_AGGRESSIVE_SSTABLE_EXPIRATION = false;

    protected static final transient String TIMESTAMP_RESOLUTION_KEY = "timestamp_resolution";

    protected static final transient String COMPACTION_WINDOW_UNIT_KEY = "compaction_window_unit";

    protected static final transient String COMPACTION_WINDOW_SIZE_KEY = "compaction_window_size";
    ...
    protected static final transient String UNSAFE_AGGRESSIVE_SSTABLE_EXPIRATION_KEY = "unsafe_aggressive_sstable_expiration";

    static final transient String UNSAFE_AGGRESSIVE_SSTABLE_EXPIRATION_PROPERTY = Config.PROPERTY_PREFIX + "allow_unsafe_aggressive_sstable_expiration";

    protected final transient int sstableWindowSize;

    protected final transient TimeUnit sstableWindowUnit;

    protected final transient TimeUnit timestampResolution;

    protected final transient long expiredSSTableCheckFrequency;

    protected final transient boolean ignoreOverlaps;

    transient SizeTieredCompactionStrategyOptions stcsOptions;

    protected final static transient ImmutableList<TimeUnit> validTimestampTimeUnits = ImmutableList.of(TimeUnit.SECONDS, TimeUnit.MILLISECONDS, TimeUnit.MICROSECONDS, TimeUnit.NANOSECONDS);

    protected final static transient ImmutableList<TimeUnit> validWindowTimeUnits = ImmutableList.of(TimeUnit.MINUTES, TimeUnit.HOURS, TimeUnit.DAYS);
