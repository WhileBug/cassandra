...
public class PendingRepairHolder extends AbstractStrategyHolder {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(PendingRepairHolder.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(PendingRepairHolder.class);

    private final transient List<PendingRepairManager> managers = new ArrayList<>();
    ...
    public void setStrategyInternal(CompactionParams params, int numTokenPartitions) {
        managers.clear();
        for (int i = 0; i < numTokenPartitions; i++) managers.add(new PendingRepairManager(cfs, params, isTransient));
    }

    @Override
    public boolean managesRepairedGroup(boolean isRepaired, boolean isPendingRepair, boolean isTransient) {
        Preconditions.checkArgument(!isPendingRepair || !isRepaired, "SSTables cannot be both repaired and pending repair");
    ...
    public Collection<TaskSupplier> getBackgroundTaskSuppliers(int gcBefore) {
        ...
        for (PendingRepairManager manager : managers) suppliers.add(new TaskSupplier(manager.getMaxEstimatedRemainingTasks(), () -> manager.getNextBackgroundTask(gcBefore)));
        return suppliers;
    }
    ...
    public SSTableMultiWriter createSSTableMultiWriter(Descriptor descriptor, long keyCount, long repairedAt, UUID pendingRepair, boolean isTransient, MetadataCollector collector, SerializationHeader header, Collection<Index> indexes, LifecycleNewTracker lifecycleNewTracker) {
        Preconditions.checkArgument(repairedAt == ActiveRepairService.UNREPAIRED_SSTABLE, "PendingRepairHolder can't create sstablewriter with repaired at set");
        Preconditions.checkArgument(pendingRepair != null, "PendingRepairHolder can't create sstable writer without pendingRepair id");
