...
class PendingRepairManager {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(PendingRepairManager.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(PendingRepairManager.class);

    private static final transient Logger logger = LoggerFactory.getLogger(PendingRepairManager.class);

    private final transient ColumnFamilyStore cfs;

    private final transient CompactionParams params;

    private final transient boolean isTransient;

    private volatile transient ImmutableMap<UUID, AbstractCompactionStrategy> strategies = ImmutableMap.of();
    ...
    public static class CleanupTask {
        ...
        private final transient List<Pair<UUID, RepairFinishedCompactionTask>> tasks;
        ...
        public Throwable abort(Throwable accumulate) {
            for (Pair<UUID, RepairFinishedCompactionTask> pair : tasks) accumulate = pair.right.transaction.abort(accumulate);
            return accumulate;
        }
    }
