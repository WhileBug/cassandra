...
public class SplittingSizeTieredCompactionWriter extends CompactionAwareWriter {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SplittingSizeTieredCompactionWriter.class);

    private static final transient Logger logger = LoggerFactory.getLogger(SplittingSizeTieredCompactionWriter.class);

    public static final transient long DEFAULT_SMALLEST_SSTABLE_BYTES = 50_000_000;

    private final transient double[] ratios;

    private final transient long totalSize;

    private final transient Set<SSTableReader> allSSTables;

    private transient long currentBytesToWrite;

    private transient int currentRatioIndex = 0;

    private transient Directories.DataDirectory location;
