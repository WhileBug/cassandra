...
public class Verifier implements Closeable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Verifier.class);

    private final transient ColumnFamilyStore cfs;

    private final transient SSTableReader sstable;

    private final transient CompactionController controller;
    ...
    private final transient VerifyInfo verifyInfo;
     ...
     * the token ranges.
     */
    private final transient Function<String, ? extends Collection<Range<Token>>> tokenLookup;

    private transient int goodRows;

    private final transient OutputHandler outputHandler;

    private transient FileDigestValidator validator;
    ...
    public static class RangeOwnHelper {
        ...
        private final transient List<Range<Token>> normalizedRanges;

        private transient int rangeIndex = 0;
        ...
        public boolean check(DecoratedKey key) {
            ...
            lastKey = key;
            if (// handle tests etc where we don't have any ranges
            normalizedRanges.isEmpty())
                return true;
    ...
    private void markAndThrow(Throwable cause, boolean mutateRepaired) {
        if (// if we are able to mutate repaired flag, an incremental repair should be enough
        mutateRepaired && options.mutateRepairStatus) {
            try {
    ...
    private static class VerifyInfo extends CompactionInfo.Holder {
        ...
        private final transient UUID verificationCompactionId;
        ...
        public CompactionInfo getCompactionInfo() {
            fileReadLock.lock();
            try {
                return new CompactionInfo(sstable.metadata(), OperationType.VERIFY, dataFile.getFilePointer(), dataFile.length(), verificationCompactionId, ImmutableSet.of(sstable));
            } catch (Exception e) {
    ...
    public static class Options {
        ...
        public final transient boolean invokeDiskFailurePolicy;

        public final transient boolean extendedVerification;

        public final transient boolean checkVersion;

        public final transient boolean mutateRepairStatus;

        public final transient boolean checkOwnsTokens;

        public final transient boolean quick;

        public final transient Function<String, ? extends Collection<Range<Token>>> tokenLookup;
        ...
        public String toString() {
            return "Options{" + "invokeDiskFailurePolicy=" + invokeDiskFailurePolicy + ", extendedVerification=" + extendedVerification + ", checkVersion=" + checkVersion + ", mutateRepairStatus=" + mutateRepairStatus + ", checkOwnsTokens=" + checkOwnsTokens + ", quick=" + quick + '}';
        }

        public static class Builder {

            // invoking disk failure policy can stop the node if we find a corrupt stable
            private transient boolean invokeDiskFailurePolicy = false;

            private transient boolean extendedVerification = false;

            private transient boolean checkVersion = false;

            // mutating repair status can be dangerous
            private transient boolean mutateRepairStatus = false;

            private transient boolean checkOwnsTokens = false;

            private transient boolean quick = false;

            private transient Function<String, ? extends Collection<Range<Token>>> tokenLookup = StorageService.instance::getLocalAndPendingRanges;
