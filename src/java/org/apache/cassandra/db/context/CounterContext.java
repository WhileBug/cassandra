...
public class CounterContext {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CounterContext.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CounterContext.class);

    private static final transient int HEADER_SIZE_LENGTH = TypeSizes.sizeof(Short.MAX_VALUE);

    private static final transient int HEADER_ELT_LENGTH = TypeSizes.sizeof(Short.MAX_VALUE);

    private static final transient int CLOCK_LENGTH = TypeSizes.sizeof(Long.MAX_VALUE);

    private static final transient int COUNT_LENGTH = TypeSizes.sizeof(Long.MAX_VALUE);

    private static final transient int STEP_LENGTH = CounterId.LENGTH + CLOCK_LENGTH + COUNT_LENGTH;
     ...
     */
    static final transient CounterId UPDATE_CLOCK_ID = CounterId.fromInt(0);

    private static final transient Logger logger = LoggerFactory.getLogger(CounterContext.class);
    ...
    private static class LazyHolder {
        ...
        private static final transient CounterContext counterContext = new CounterContext();
    }
    ...
    private Relationship compare(ContextState leftState, ContextState rightState) {
        ...
        if (leftState.isLocal() || rightState.isLocal()) {
            ...
            if (leftState.isLocal() && rightState.isLocal())
                ...
                // only one is local - keep that one
        ...
        if (leftClock == rightClock) {
            ...
            if ((leftClock >= 0 && rightClock > 0 && leftClock >= rightClock) || (leftClock < 0 && (rightClock > 0 || leftClock < rightClock)))
                return Relationship.GREATER_THAN;
            else
                return Relationship.LESS_THAN;
        }
    ...
    public <V> boolean hasLegacyShards(V context, ValueAccessor<V> accessor) {
        ...
        if (localAndGlobalCount < totalCount)
            // remote shard(s) present
            return true;
        for (int i = 0; i < localAndGlobalCount; i++) if (accessor.getShort(context, HEADER_SIZE_LENGTH + i * HEADER_ELT_LENGTH) >= 0)
            // found a local shard
            return true;
        return false;
    }
    ...
    public ByteBuffer markLocalToBeCleared(ByteBuffer context) {
        ...
        if (count <= 0)
            // already marked or all are remote.
            return context;
        ...
        if (!hasLocalShards)
            // all shards are global or remote.
            return context;
    ...
    public <V> V clearAllLocal(V context, ValueAccessor<V> accessor) {
        ...
        if (count == 0)
            // no local or global shards present.
            return context;
    ...
    public static class ContextState {
        ...
        public final transient ByteBuffer context;

        public final transient int headerLength;

        // offset from context.position()
        private transient int headerOffset;

        // offset from context.position()
        private transient int bodyOffset;

        private transient boolean currentIsGlobal;

        private transient boolean currentIsLocal;
