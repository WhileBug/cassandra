...
public class CommitLogArchiver {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CommitLogArchiver.class);

    private static final transient Logger logger = LoggerFactory.getLogger(CommitLogArchiver.class);

    public static final transient SimpleDateFormat format = new SimpleDateFormat("yyyy:MM:dd HH:mm:ss");

    private static final transient String DELIMITER = ",";

    private static final transient Pattern NAME = Pattern.compile("%name");

    private static final transient Pattern PATH = Pattern.compile("%path");

    private static final transient Pattern FROM = Pattern.compile("%from");

    private static final transient Pattern TO = Pattern.compile("%to");

    static {
        format.setTimeZone(TimeZone.getTimeZone("GMT"));
    }

    public final transient Map<String, Future<?>> archivePending = new ConcurrentHashMap<String, Future<?>>();

    private final transient ExecutorService executor;

    final transient String archiveCommand;

    final transient String restoreCommand;

    final transient String restoreDirectories;

    public transient long restorePointInTime;

    public final transient TimeUnit precision;
