...
public abstract class CommitLogSegment {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CommitLogSegment.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CommitLogSegment.class);

    private final static transient long idBase;

    private transient CDCState cdcState = CDCState.PERMITTED;

    public enum CDCState {

        PERMITTED, FORBIDDEN, CONTAINS
    }

    transient Object cdcStateLock = new Object();

    private final static transient AtomicInteger nextId = new AtomicInteger(1);

    private static transient long replayLimitId;
    ...
    public static final transient int ENTRY_OVERHEAD_SIZE = 4 + 4 + 4;

    // The commit log (chained) sync marker/header size in bytes (int: length + int: checksum [segmentId, position])
    static final transient int SYNC_MARKER_SIZE = 4 + 4;

    // The OpOrder used to order appends wrt sync
    private final transient OpOrder appendOrder = new OpOrder();

    private final transient AtomicInteger allocatePosition = new AtomicInteger();
    ...
    @VisibleForTesting
    volatile transient int lastSyncedOffset;
     ...
     */
    private volatile transient int lastMarkerOffset;
    ...
    private transient int endOfBuffer;

    // a signal for writers to wait on to confirm the log message they provided has been written to disk
    private final transient WaitQueue syncComplete = new WaitQueue();

    // a map of Cf->dirty interval in this segment; if interval is not covered by the clean set, the log contains unflushed data
    private final transient NonBlockingHashMap<TableId, IntegerInterval> tableDirty = new NonBlockingHashMap<>(1024);

    // a map of Cf->clean intervals; separate map from above to permit marking Cfs clean whilst the log is still in use
    private final transient ConcurrentHashMap<TableId, IntegerInterval.Set> tableClean = new ConcurrentHashMap<>();

    public final transient long id;

    final transient File logFile;

    final transient FileChannel channel;

    final transient int fd;

    protected final transient AbstractCommitLogSegmentManager manager;

    transient ByteBuffer buffer;

    private volatile transient boolean headerWritten;

    public final transient CommitLogDescriptor descriptor;
    ...
    Allocation allocate(Mutation mutation, int size) {
        ...
        try {
            ...
            for (PartitionUpdate update : mutation.getPartitionUpdates()) coverInMap(tableDirty, update.metadata().id, position);
    ...
    private int allocate(int size) {
        while (true) {
            ...
            // ConstantBackoffCAS Algorithm from https://arxiv.org/pdf/1305.5800.pdf
    ...
    protected static class Allocation {
        ...
        private final transient CommitLogSegment segment;

        private final transient OpOrder.Group appendOp;
