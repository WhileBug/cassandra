...
public class CommitLog implements CommitLogMBean {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CommitLog.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CommitLog.class);

    private static final transient Logger logger = LoggerFactory.getLogger(CommitLog.class);

    public static final transient CommitLog instance = CommitLog.construct();

    final public transient AbstractCommitLogSegmentManager segmentManager;

    public final transient CommitLogArchiver archiver;

    public final transient CommitLogMetrics metrics;

    final transient AbstractCommitLogService executor;

    volatile transient Configuration configuration;
        ...
        for (Iterator<CommitLogSegment> iter = segmentManager.getActiveSegments().iterator(); iter.hasNext(); ) {
            ...
            if (segment.isUnused()) {
                ...
                if (logger.isTraceEnabled())
                    logger.trace("Not safe to delete{} commit log segment {}; dirty is {}", (iter.hasNext() ? "" : " active"), segment, segment.dirtyString());
            }
    ...
    public static final class Configuration {
         ...
         */
        private final transient ParameterizedClass compressorClass;
         ...
         */
        private transient EncryptionContext encryptionContext;
