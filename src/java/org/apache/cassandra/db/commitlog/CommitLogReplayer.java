...
public class CommitLogReplayer implements CommitLogReadHandler {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CommitLogReplayer.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CommitLogReplayer.class);

    @VisibleForTesting
    public static transient long MAX_OUTSTANDING_REPLAY_BYTES = Long.getLong("cassandra.commitlog_max_outstanding_replay_bytes", 1024 * 1024 * 64);

    @VisibleForTesting
    public static transient MutationInitiator mutationInitiator = new MutationInitiator();

    static final transient String IGNORE_REPLAY_ERRORS_PROPERTY = Config.PROPERTY_PREFIX + "commitlog.ignorereplayerrors";

    private static final transient Logger logger = LoggerFactory.getLogger(CommitLogReplayer.class);

    private static final transient int MAX_OUTSTANDING_REPLAY_COUNT = Integer.getInteger(Config.PROPERTY_PREFIX + "commitlog_max_outstanding_replay_count", 1024);

    private final transient Set<Keyspace> keyspacesReplayed;

    private final transient Queue<Future<Integer>> futures;

    private final transient AtomicInteger replayedCount;

    private final transient Map<TableId, IntervalSet<CommitLogPosition>> cfPersisted;

    private final transient CommitLogPosition globalPosition;

    // Used to throttle speed of replay of mutations if we pass the max outstanding count
    private transient long pendingMutationBytes = 0;

    private final transient ReplayFilter replayFilter;

    private final transient CommitLogArchiver archiver;

    @VisibleForTesting
    protected transient boolean sawCDCMutation;

    @VisibleForTesting
    protected transient CommitLogReader commitLogReader;
    ...
    public static class MutationInitiator {
        ...
        protected Future<Integer> initiateMutation(final Mutation mutation, final long segmentId, final int serializedSize, final int entryLocation, final CommitLogReplayer commitLogReplayer) {
    ...
    private static class CustomReplayFilter extends ReplayFilter {
        ...
        private transient Multimap<String, String> toReplay;
