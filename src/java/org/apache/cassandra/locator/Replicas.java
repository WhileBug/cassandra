...
public class Replicas {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Replicas.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Replicas.class);

    public static int countFull(ReplicaCollection<?> replicas) {
        int count = 0;
        for (Replica replica : replicas) if (replica.isFull())
            ++count;
        return count;
    }
    ...
    public static class ReplicaCount {
        ...
        transient int fullReplicas;

        transient int transientReplicas;
    ...
    public static ReplicaCount countInOurDc(ReplicaCollection<?> replicas) {
        ...
        for (Replica replica : replicas) if (inOurDc.test(replica))
