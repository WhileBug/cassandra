...
public class RangesAtEndpoint extends AbstractReplicaCollection<RangesAtEndpoint> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RangesAtEndpoint.class);

    private static ReplicaMap<Range<Token>> rangeMap(ReplicaList list) {
        return new ReplicaMap<>(list, Replica::range);
    }

    private static final transient ReplicaMap<Range<Token>> EMPTY_MAP = rangeMap(EMPTY_LIST);
    ...
    private transient ReplicaMap<Range<Token>> byRange;

    private transient RangesAtEndpoint onlyFull;

    private transient RangesAtEndpoint onlyTransient;
                ...
                switch(ignoreConflict) {
                    case DUPLICATE:
                        if (byRange().get(replica.range()).equals(replica))
                            break;
                    case NONE:
                        throw new IllegalArgumentException("Conflicting replica added (expected unique ranges): " + replica + "; existing: " + byRange().get(replica.range()));
                    case ALL:
                }
                return this;
            }
