...
public class TokenMetadata {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TokenMetadata.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TokenMetadata.class);

    private static final transient Logger logger = LoggerFactory.getLogger(TokenMetadata.class);
     ...
     */
    private final transient BiMultiValMap<Token, InetAddressAndPort> tokenToEndpointMap;

    /**
     * Maintains endpoint to host ID map of every node in the cluster
     */
    private final transient BiMap<InetAddressAndPort, UUID> endpointToHostIdMap;
    ...
    private final transient BiMultiValMap<Token, InetAddressAndPort> bootstrapTokens = new BiMultiValMap<>();

    private final transient BiMap<InetAddressAndPort, InetAddressAndPort> replacementToOriginal = HashBiMap.create();

    // (don't need to record Token here since it's still part of tokenToEndpointMap until it's done leaving)
    private final transient Set<InetAddressAndPort> leavingEndpoints = new HashSet<>();
    ...
    private final transient ConcurrentMap<String, PendingRangeMaps> pendingRanges = new ConcurrentHashMap<String, PendingRangeMaps>();

    // nodes which are migrating to the new tokens in the ring
    private final transient Set<Pair<Token, InetAddressAndPort>> movingEndpoints = new HashSet<>();

    /* Use this lock for manipulating the token map */
    private final transient ReadWriteLock lock = new ReentrantReadWriteLock(true);

    // safe to be read without a lock, as it's never mutated
    private volatile transient ArrayList<Token> sortedTokens;

    private volatile transient Topology topology;
    ...
    @GuardedBy("lock")
    private transient long ringVersion = 0;
    ...
    public void updateNormalTokens(Multimap<InetAddressAndPort, Token> endpointTokens) {
        ...
        try {
            ...
            for (InetAddressAndPort endpoint : endpointTokens.keySet()) {
                ...
                // also removing this endpoint from moving
    ...
    private final transient AtomicReference<TokenMetadata> cachedTokenMap = new AtomicReference<>();
    ...
    public TokenMetadata cloneOnlyTokenMap() {
        lock.readLock().lock();
        try {
            return new TokenMetadata(SortedBiMultiValMap.create(tokenToEndpointMap), HashBiMap.create(endpointToHostIdMap), topology, partitioner, ringVersion);
        } finally {
            lock.readLock().unlock();
        }
    }
    ...
    public Set<InetAddressAndPort> getAllMembers() {
        return getAllEndpoints().stream().filter(this::isMember).collect(Collectors.toSet());
    }
    ...
    public static class Topology {
         ...
         */
        private final transient ImmutableMultimap<String, InetAddressAndPort> dcEndpoints;

        /**
         * map of DC to multi-map of rack to endpoints in that rack
         */
        private final transient ImmutableMap<String, ImmutableMultimap<String, InetAddressAndPort>> dcRacks;

        /**
         * reverse-lookup map for endpoint to current known dc/rack assignment
         */
        private final transient ImmutableMap<InetAddressAndPort, Pair<String, String>> currentLocations;

        private final transient Supplier<IEndpointSnitch> snitchSupplier;
        ...
        private Topology(Builder builder) {
            ...
            for (Map.Entry<String, Multimap<String, InetAddressAndPort>> entry : builder.dcRacks.entrySet()) dcRackBuilder.put(entry.getKey(), ImmutableMultimap.copyOf(entry.getValue()));
        ...
        private static class Builder {
             ...
             * multi-map of DC to endpoints in that DC
             */
            private final transient Multimap<String, InetAddressAndPort> dcEndpoints;

            /**
             * map of DC to multi-map of rack to endpoints in that rack
             */
            private final transient Map<String, Multimap<String, InetAddressAndPort>> dcRacks;

            /**
             * reverse-lookup map for endpoint to current known dc/rack assignment
             */
            private final transient Map<InetAddressAndPort, Pair<String, String>> currentLocations;
            ...
            Builder(Topology from) {
                ...
                for (Map.Entry<String, ImmutableMultimap<String, InetAddressAndPort>> entry : from.dcRacks.entrySet()) dcRacks.put(entry.getKey(), HashMultimap.create(entry.getValue()));
            ...
            Builder updateEndpoints() {
                ...
                if (snitch == null)
                    return this;
                for (InetAddressAndPort ep : currentLocations.keySet()) updateEndpoint(ep, snitch);
                return this;
            }
