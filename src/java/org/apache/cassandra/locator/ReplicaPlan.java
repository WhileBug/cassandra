...
public abstract class ReplicaPlan<E extends Endpoints<E>> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ReplicaPlan.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ReplicaPlan.class);

    protected final transient Keyspace keyspace;

    protected final transient ConsistencyLevel consistencyLevel;
    ...
    protected final transient AbstractReplicationStrategy replicationStrategy;
    ...
    private final transient E contacts;
    ...
    public static abstract class ForRead<E extends Endpoints<E>> extends ReplicaPlan<E> {
        ...
        private final transient E candidates;
        ...
        public int blockFor() {
            return consistencyLevel.blockFor(replicationStrategy);
        }
    ...
    public static class ForRangeRead extends ForRead<EndpointsForRange> {
        ...
        final transient int vnodeCount;
    ...
    public static abstract class ForWrite<E extends Endpoints<E>> extends ReplicaPlan<E> {
        ...
        final transient E liveAndDown;

        final transient E live;
        ...
        public int blockFor() {
            return consistencyLevel.blockForWrite(replicationStrategy, pending());
        }
        ...
        public E liveUncontacted() {
            return live().filter(r -> !contacts(r));
        }
        ...
        public boolean isAlive(Replica replica) {
            return live.endpoints().contains(replica.endpoint());
        }
    ...
    public static class ForPaxosWrite extends ForWrite<EndpointsForToken> {
        ...
        final transient int requiredParticipants;
    ...
    public static class SharedForTokenRead implements Shared<EndpointsForToken, ForTokenRead> {
        ...
        private transient ForTokenRead replicaPlan;
        ...
        public void addToContacts(Replica replica) {
            replicaPlan = replicaPlan.withContact(Endpoints.append(replicaPlan.contacts(), replica));
        }
    ...
    public static class SharedForRangeRead implements Shared<EndpointsForRange, ForRangeRead> {
        ...
        private transient ForRangeRead replicaPlan;
