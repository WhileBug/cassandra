...
public class TraceStateImpl extends TraceState {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TraceStateImpl.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TraceStateImpl.class);

    private static final transient Logger logger = LoggerFactory.getLogger(TraceStateImpl.class);

    @VisibleForTesting
    public static transient int WAIT_FOR_PENDING_EVENTS_TIMEOUT_SECS = Integer.parseInt(System.getProperty("cassandra.wait_for_tracing_events_timeout_secs", "0"));

    private final transient Set<Future<?>> pendingFutures = ConcurrentHashMap.newKeySet();
    ...
    protected void waitForPendingEvents() {
        ...
        try {
            if (logger.isTraceEnabled())
                logger.trace("Waiting for up to {} seconds for {} trace events to complete", +WAIT_FOR_PENDING_EVENTS_TIMEOUT_SECS, pendingFutures.size());
            CompletableFuture.allOf(pendingFutures.toArray(new CompletableFuture<?>[pendingFutures.size()])).get(WAIT_FOR_PENDING_EVENTS_TIMEOUT_SECS, TimeUnit.SECONDS);
