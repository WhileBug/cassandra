...
public class RepairSession extends AbstractFuture<RepairSessionResult> implements IEndpointStateChangeSubscriber, IFailureDetectionEventListener, LocalSessions.Listener {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RepairSession.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RepairSession.class);

    private static final transient Logger logger = LoggerFactory.getLogger(RepairSession.class);
    ...
    private final transient String[] cfnames;

    public final transient RepairParallelism parallelismDegree;

    public final transient boolean pullRepair;

    /**
     * Range to repair
     */
    public final transient CommonRange commonRange;

    public final transient boolean isIncremental;

    public final transient PreviewKind previewKind;

    private final transient AtomicBoolean isFailed = new AtomicBoolean(false);

    // Each validation task waits response from replica in validating ConcurrentMap (keyed by CF name and endpoint address)
    private final transient ConcurrentMap<Pair<RepairJobDesc, InetAddressAndPort>, ValidationTask> validating = new ConcurrentHashMap<>();

    // Remote syncing jobs wait response in syncingTasks map
    private final transient ConcurrentMap<Pair<RepairJobDesc, SyncNodePair>, CompletableRemoteSyncTask> syncingTasks = new ConcurrentHashMap<>();

    // Tasks(snapshot, validate request, differencing, ...) are run on taskExecutor
    public final transient ListeningExecutorService taskExecutor;

    public final transient boolean optimiseStreams;
     ...
     */
    public RepairSession(UUID parentRepairSession, UUID id, CommonRange commonRange, String keyspace, RepairParallelism parallelismDegree, boolean isIncremental, boolean pullRepair, PreviewKind previewKind, boolean optimiseStreams, String... cfnames) {
    ...
    public void onIRStateChange(LocalSession session) {
        ...
        if (previewKind == PreviewKind.REPAIRED && session.getState() == ConsistentSession.State.FINALIZED && includesTables(session.tableIds)) {
