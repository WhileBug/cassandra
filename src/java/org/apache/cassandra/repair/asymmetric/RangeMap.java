...
public class RangeMap<T> implements Map<Range<Token>, T> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RangeMap.class);

    private static final transient Comparator<Range<Token>> comparator = Comparator.comparing((Range<Token> o) -> o.left);

    private final transient NavigableMap<Range<Token>, T> byStart;
    ...
    public Set<Map.Entry<Range<Token>, T>> removeIntersecting(Range<Token> range) {
        ...
        for (Map.Entry<Range<Token>, T> entry : intersecting) byStart.remove(entry.getKey());
        return intersecting;
    }

    private class WrappingIntersectingIterator extends AbstractIterator<Map.Entry<Range<Token>, T>> {

        private final transient Iterator<Iterator<Map.Entry<Range<Token>, T>>> iterators;

        private transient Iterator<Map.Entry<Range<Token>, T>> currentIter;

        public WrappingIntersectingIterator(Range<Token> range) {
            List<Iterator<Map.Entry<Range<Token>, T>>> iters = new ArrayList<>(2);
            for (Range<Token> unwrapped : range.unwrap()) iters.add((new IntersectingIterator(unwrapped)));
    ...
    private class IntersectingIterator extends AbstractIterator<Map.Entry<Range<Token>, T>> {
        ...
        private final transient Iterator<Map.Entry<Range<Token>, T>> tailIterator;
        ...
        private transient boolean shouldReturnLast = false;

        public IntersectingIterator(Range<Token> range) {
            Range<Token> startKey = byStart.floorKey(range);
            tailIterator = startKey == null ? byStart.entrySet().iterator() : byStart.tailMap(startKey, true).entrySet().iterator();
        ...
        protected Map.Entry<Range<Token>, T> computeNext() {
            ...
            while (tailIterator.hasNext()) {
                ...
                if (// we know we already returned any wrapping range
                candidateRange.isWrapAround())
                    continue;
                if (// range is unwrapped, but that means one range has right == min token and is still wrapping
                candidateRange.left.compareTo(range.right) >= 0 && (!range.isWrapAround()))
                    return endOfData();
    ...
    static class Entry<K, V> implements Map.Entry<K, V> {
        ...
        public boolean equals(Object o) {
            ...
            return v.equals(entry.getValue()) && k.equals(entry.getKey());
        }
