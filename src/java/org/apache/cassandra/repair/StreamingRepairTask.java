...
public class StreamingRepairTask implements Runnable, StreamEventHandler {
    ...
    StreamPlan createStreamPlan(InetAddressAndPort dest) {
        StreamPlan sp = // sstables are isolated at the beginning of an incremental repair session, so flushing isn't neccessary
        new StreamPlan(StreamOperation.REPAIR, 1, false, pendingRepair, previewKind).listeners(this).flushBeforeTransfer(// request ranges from the remote node
        pendingRepair == null).requestRanges(// request ranges from the remote node
        dest, // request ranges from the remote node
        desc.keyspace, // request ranges from the remote node
        RangesAtEndpoint.toDummyList(ranges), // request ranges from the remote node
