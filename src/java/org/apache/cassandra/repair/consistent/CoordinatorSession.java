...
public class CoordinatorSession extends ConsistentSession {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CoordinatorSession.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CoordinatorSession.class);

    private static final transient Logger logger = LoggerFactory.getLogger(CoordinatorSession.class);

    private final transient Map<InetAddressAndPort, State> participantStates = new HashMap<>();

    private final transient SettableFuture<Boolean> prepareFuture = SettableFuture.create();

    private final transient SettableFuture<Boolean> finalizeProposeFuture = SettableFuture.create();

    private volatile transient long sessionStart = Long.MIN_VALUE;

    private volatile transient long repairStart = Long.MIN_VALUE;

    private volatile transient long finalizeStart = Long.MIN_VALUE;
    ...
    public synchronized void setParticipantState(InetAddressAndPort participant, State state) {
        ...
        Preconditions.checkArgument(participantStates.containsKey(participant), "Session %s doesn't include %s", sessionID, participant);
        Preconditions.checkArgument(participantStates.get(participant).canTransitionTo(state), "Invalid state transition %s -> %s", participantStates.get(participant), state);
    ...
    public synchronized void fail() {
        Set<Map.Entry<InetAddressAndPort, State>> cantFail = participantStates.entrySet().stream().filter(entry -> !entry.getValue().canTransitionTo(State.FAILED)).collect(Collectors.toSet());
        if (!cantFail.isEmpty()) {
