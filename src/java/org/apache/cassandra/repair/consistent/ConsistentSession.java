...
public abstract class ConsistentSession {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ConsistentSession.class);
    ...
    public final transient ImmutableSet<TableId> tableIds;

    public final transient long repairedAt;

    public final transient ImmutableSet<Range<Token>> ranges;

    public final transient ImmutableSet<InetAddressAndPort> participants;
    ...
    public boolean equals(Object o) {
        ...
        if (repairedAt != that.repairedAt)
            return false;
        if (state != that.state)
            return false;
        if (!sessionID.equals(that.sessionID))
            return false;
        if (!coordinator.equals(that.coordinator))
            return false;
        if (!tableIds.equals(that.tableIds))
            return false;
    ...
    public String toString() {
        return "ConsistentSession{" + "state=" + state + ", sessionID=" + sessionID + ", coordinator=" + coordinator + ", tableIds=" + tableIds + ", repairedAt=" + repairedAt + ", ranges=" + ranges + ", participants=" + participants + '}';
    }
    ...
    abstract static class AbstractBuilder {
        ...
        private transient UUID sessionID;

        private transient InetAddressAndPort coordinator;

        private transient Set<TableId> ids;

        private transient long repairedAt;

        private transient Collection<Range<Token>> ranges;

        private transient Set<InetAddressAndPort> participants;
        ...
        void validate() {
            ...
            Preconditions.checkArgument(repairedAt > 0 || repairedAt == ActiveRepairService.UNREPAIRED_SSTABLE);
