...
public class RepairJob extends AbstractFuture<RepairResult> implements Runnable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RepairJob.class);

    private static final transient Logger logger = LoggerFactory.getLogger(RepairJob.class);

    private final transient RepairSession session;

    private final transient RepairJobDesc desc;

    private final transient RepairParallelism parallelismDegree;

    private final transient ListeningExecutorService taskExecutor;

    private final transient List<SyncTask> syncTasks = new CopyOnWriteArrayList<>();
    ...
    public void run() {
        ...
        ListenableFuture<List<SyncStat>> syncResults = Futures.transformAsync(validations, session.optimiseStreams && !session.pullRepair ? this::optimisedSyncing : this::standardSyncing, taskExecutor);
    ...
    private ListenableFuture<List<SyncStat>> standardSyncing(List<TreeResponse> trees) {
        List<SyncTask> syncTasks = createStandardSyncTasks(desc, trees, FBUtilities.getLocalAddressAndPort(), this::isTransient, session.isIncremental, session.pullRepair, session.previewKind);
    ...
    private ListenableFuture<List<SyncStat>> optimisedSyncing(List<TreeResponse> trees) {
        List<SyncTask> syncTasks = createOptimisedSyncingSyncTasks(desc, trees, FBUtilities.getLocalAddressAndPort(), this::isTransient, this::getDC, session.isIncremental, session.previewKind);
        return executeTasks(syncTasks);
    }
    ...
    static List<SyncTask> createOptimisedSyncingSyncTasks(RepairJobDesc desc, List<TreeResponse> trees, InetAddressAndPort local, Predicate<InetAddressAndPort> isTransient, Function<InetAddressAndPort, String> getDC, boolean isIncremental, PreviewKind previewKind) {
        ...
        logger.info("Created {} optimised sync tasks based on {} merkle tree responses for {} (took: {}ms)", syncTasks.size(), trees.size(), desc.parentSessionId, System.currentTimeMillis() - startedAt);
