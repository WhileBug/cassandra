...
public final class SystemDistributedKeyspace {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SystemDistributedKeyspace.class);

    private SystemDistributedKeyspace() {
    }

    private static final transient Logger logger = LoggerFactory.getLogger(SystemDistributedKeyspace.class);
     ...
     */
    public static final transient long GENERATION = 5;

    public static final transient String REPAIR_HISTORY = "repair_history";

    public static final transient String PARENT_REPAIR_HISTORY = "parent_repair_history";

    public static final transient String VIEW_BUILD_STATUS = "view_build_status";

    private static final transient TableMetadata RepairHistory = parse(REPAIR_HISTORY, "Repair history", "CREATE TABLE %s (" + "keyspace_name text," + "columnfamily_name text," + "id timeuuid," + "parent_id timeuuid," + "range_begin text," + "range_end text," + "coordinator inet," + "coordinator_port int," + "participants set<inet>," + "participants_v2 set<text>," + "exception_message text," + "exception_stacktrace text," + "status text," + "started_at timestamp," + "finished_at timestamp," + "PRIMARY KEY ((keyspace_name, columnfamily_name), id))").defaultTimeToLive((int) TimeUnit.DAYS.toSeconds(30)).compaction(CompactionParams.twcs(ImmutableMap.of("compaction_window_unit", "DAYS", "compaction_window_size", "1"))).build();
    ...
    private static final transient TableMetadata ViewBuildStatus = parse(VIEW_BUILD_STATUS, "Materialized View build status", "CREATE TABLE %s (" + "keyspace_name text," + "view_name text," + "host_id uuid," + "status text," + "PRIMARY KEY ((keyspace_name, view_name), host_id))").build();

    private static TableMetadata.Builder parse(String table, String description, String cql) {
        return CreateTableStatement.parse(format(cql, table), SchemaConstants.DISTRIBUTED_KEYSPACE_NAME).id(TableId.forSystemTable(SchemaConstants.DISTRIBUTED_KEYSPACE_NAME, table)).comment(description);
    }
    ...
    public static void startParentRepair(UUID parent_id, String keyspaceName, String[] cfnames, RepairOption options) {
        ...
        String fmtQry = format(query, SchemaConstants.DISTRIBUTED_KEYSPACE_NAME, PARENT_REPAIR_HISTORY, parent_id.toString(), keyspaceName, Joiner.on("','").join(cfnames), Joiner.on("','").join(ranges), toCQLMap(options.asMap(), RepairOption.RANGES_KEY, RepairOption.COLUMNFAMILIES_KEY));
        ...
        for (String cfname : cfnames) {
            for (Range<Token> range : commonRange.ranges) {
                ...
                if (includeNewColumns) {
                    fmtQry = format(query, SchemaConstants.DISTRIBUTED_KEYSPACE_NAME, REPAIR_HISTORY, keyspaceName, cfname, id.toString(), parent_id.toString(), range.left.toString(), range.right.toString(), coordinator.getHostAddress(false), coordinator.port, Joiner.on("', '").join(participants), Joiner.on("', '").join(participants_v2), RepairState.STARTED.toString());
                } else {
                    fmtQry = format(queryWithoutNewColumns, SchemaConstants.DISTRIBUTED_KEYSPACE_NAME, REPAIR_HISTORY, keyspaceName, cfname, id.toString(), parent_id.toString(), range.left.toString(), range.right.toString(), coordinator.getHostAddress(false), Joiner.on("', '").join(participants), RepairState.STARTED.toString());
                }
                processSilent(fmtQry);
            }
        }
    ...
    public static void failRepairs(UUID id, String keyspaceName, String[] cfnames, Throwable t) {
        for (String cfname : cfnames) failedRepairJob(id, keyspaceName, cfname, t);
    }
    ...
    public static void successfulRepairJob(UUID id, String keyspaceName, String cfname) {
        ...
        String fmtQuery = format(query, SchemaConstants.DISTRIBUTED_KEYSPACE_NAME, REPAIR_HISTORY, RepairState.SUCCESS.toString(), keyspaceName, cfname, id.toString());
        processSilent(fmtQuery);
    }
    ...
    public static void failedRepairJob(UUID id, String keyspaceName, String cfname, Throwable t) {
        ...
        PrintWriter pw = new PrintWriter(sw);
        t.printStackTrace(pw);
        String fmtQry = format(query, SchemaConstants.DISTRIBUTED_KEYSPACE_NAME, REPAIR_HISTORY, RepairState.FAILED.toString(), keyspaceName, cfname, id.toString());
    ...
    public static void startViewBuild(String keyspace, String view, UUID hostId) {
        ...
        QueryProcessor.process(format(query, SchemaConstants.DISTRIBUTED_KEYSPACE_NAME, VIEW_BUILD_STATUS), ConsistencyLevel.ONE, Lists.newArrayList(bytes(keyspace), bytes(view), bytes(hostId), bytes(BuildStatus.STARTED.toString())));
    }
    ...
    public static void successfulViewBuild(String keyspace, String view, UUID hostId) {
        ...
        QueryProcessor.process(format(query, SchemaConstants.DISTRIBUTED_KEYSPACE_NAME, VIEW_BUILD_STATUS), ConsistencyLevel.ONE, Lists.newArrayList(bytes(BuildStatus.SUCCESS.toString()), bytes(keyspace), bytes(view), bytes(hostId)));
    }
