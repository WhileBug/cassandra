...
public class EnableFullQueryLog extends NodeToolCmd {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(EnableFullQueryLog.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(EnableFullQueryLog.class);

    @Option(title = "roll_cycle", name = { "--roll-cycle" }, description = "How often to roll the log file (MINUTELY, HOURLY, DAILY).")
    private transient String rollCycle = null;
    ...
    private transient String blocking = null;
    ...
    private transient int maxQueueWeight = Integer.MIN_VALUE;
    ...
    private transient long maxLogSize = Long.MIN_VALUE;
    ...
    @Option(title = "archive_command", name = { "--archive-command" }, description = "Command that will handle archiving rolled full query log files." + " Format is \"/path/to/script.sh %path\" where %path will be replaced with the file to archive")
    private transient String archiveCommand = null;

    @Option(title = "archive_retries", name = { "--max-archive-retries" }, description = "Max number of archive retries.")
    private transient int archiveRetries = Integer.MIN_VALUE;
