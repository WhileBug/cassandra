...
public class Rebuild extends NodeToolCmd {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Rebuild.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Rebuild.class);

    @Arguments(usage = "<src-dc-name>", description = "Name of DC from which to select sources for streaming. By default, pick any DC")
    private transient String sourceDataCenterName = null;
    ...
    @Option(title = "specific_sources", name = { "-s", "--sources" }, description = "Use -s to specify hosts that this node should stream from when -ts is used. Multiple hosts should be separated using commas (e.g. 127.0.0.1,127.0.0.2,...)")
    private transient String specificSources = null;
