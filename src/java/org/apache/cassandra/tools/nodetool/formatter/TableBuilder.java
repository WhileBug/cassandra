...
public class TableBuilder {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TableBuilder.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TableBuilder.class);

    // column delimiter
    private final transient String columnDelimiter;

    private transient int[] maximumColumnWidth;
    ...
    public static class SharedTable {
        ...
        private transient List<TableBuilder> tables = new ArrayList<>();
        ...
        public List<TableBuilder> complete() {
            ...
            final int columns = tables.stream().max(Comparator.comparing(tb -> tb.maximumColumnWidth.length)).get().maximumColumnWidth.length;
            ...
            return tables.stream().map(tb -> new TableBuilder(tb, maximumColumnWidth)).collect(Collectors.toList());
        }
    }
}
