...
public class Scrub extends NodeToolCmd {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Scrub.class);
    ...
    private transient boolean disableSnapshot = false;
    ...
    private transient boolean skipCorrupted = false;

    @Option(title = "no_validate", name = { "-n", "--no-validate" }, description = "Do not validate columns using column validator")
    private transient boolean noValidation = false;

    @Option(title = "reinsert_overflowed_ttl", name = { "-r", "--reinsert-overflowed-ttl" }, description = StandaloneScrubber.REINSERT_OVERFLOWED_TTL_OPTION_DESCRIPTION)
    private transient boolean reinsertOverflowedTTL = false;

    @Option(title = "jobs", name = { "-j", "--jobs" }, description = "Number of sstables to scrub simultanously, set to 0 to use all available compaction threads")
