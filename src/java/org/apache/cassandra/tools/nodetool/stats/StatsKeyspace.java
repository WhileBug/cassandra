...
public class StatsKeyspace {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StatsKeyspace.class);

    public transient List<StatsTable> tables = new ArrayList<>();

    private final transient NodeProbe probe;

    public transient String name;

    public transient long readCount;

    public transient long writeCount;

    public transient int pendingFlushes;

    private transient double totalReadTime;

    private transient double totalWriteTime;
    ...
    public double readLatency() {
        return readCount > 0 ? totalReadTime / readCount / 1000 : Double.NaN;
    }

    public double writeLatency() {
        return writeCount > 0 ? totalWriteTime / writeCount / 1000 : Double.NaN;
    }
}
