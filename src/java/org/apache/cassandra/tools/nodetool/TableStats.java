...
public class TableStats extends NodeToolCmd {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TableStats.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TableStats.class);

    @Arguments(usage = "[<keyspace.table>...]", description = "List of tables (or keyspace) names")
    private transient List<String> tableNames = new ArrayList<>();
    ...
    private transient boolean humanReadable = false;
    ...
    @Option(title = "sstable_location_check", name = { "-l", "--sstable-location-check" }, description = "Check whether or not the SSTables are in the correct location.")
    private transient boolean locationCheck = false;
