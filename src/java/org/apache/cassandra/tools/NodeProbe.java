...
public class NodeProbe implements AutoCloseable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(NodeProbe.class);

    private static final transient String fmtUrl = "service:jmx:rmi:///jndi/rmi://%s:%d/jmxrmi";

    private static final transient String ssObjName = "org.apache.cassandra.db:type=StorageService";

    private static final transient int defaultPort = 7199;

    static transient long JMX_NOTIFICATION_POLL_INTERVAL_SECONDS = Long.getLong("cassandra.nodetool.jmx_notification_poll_interval_seconds", TimeUnit.SECONDS.convert(5, TimeUnit.MINUTES));

    final transient String host;

    final transient int port;
    ...
    protected transient MBeanServerConnection mbeanServerConn;

    protected transient CompactionManagerMBean compactionProxy;

    protected transient StorageServiceMBean ssProxy;

    protected transient GossiperMBean gossProxy;

    protected transient MemoryMXBean memProxy;

    protected transient GCInspectorMXBean gcProxy;

    protected transient RuntimeMXBean runtimeProxy;

    protected transient StreamManagerMBean streamProxy;

    protected transient MessagingServiceMBean msProxy;

    protected transient FailureDetectorMBean fdProxy;

    protected transient CacheServiceMBean cacheService;

    protected transient StorageProxyMBean spProxy;

    protected transient HintsServiceMBean hsProxy;

    protected transient BatchlogManagerMBean bmProxy;

    protected transient ActiveRepairServiceMBean arsProxy;
    ...
    public CassandraMetricsRegistry.JmxTimerMBean getMessagingQueueWaitMetrics(String verb) {
        try {
            return JMX.newMBeanProxy(mbeanServerConn, new ObjectName("org.apache.cassandra.metrics:name=" + verb + "-WaitLatency,type=Messaging"), CassandraMetricsRegistry.JmxTimerMBean.class);
        } catch (MalformedObjectNameException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public Object getCompactionMetric(String metricName) {
        try {
            switch(metricName) {
                case "BytesCompacted":
                    return JMX.newMBeanProxy(mbeanServerConn, new ObjectName("org.apache.cassandra.metrics:type=Compaction,name=" + metricName), CassandraMetricsRegistry.JmxCounterMBean.class);
                case "CompletedTasks":
                case "PendingTasks":
                case "PendingTasksByTableName":
                    return JMX.newMBeanProxy(mbeanServerConn, new ObjectName("org.apache.cassandra.metrics:type=Compaction,name=" + metricName), CassandraMetricsRegistry.JmxGaugeMBean.class).getValue();
                case "TotalCompactionsCompleted":
                    return JMX.newMBeanProxy(mbeanServerConn, new ObjectName("org.apache.cassandra.metrics:type=Compaction,name=" + metricName), CassandraMetricsRegistry.JmxMeterMBean.class);
                default:
    ...
    public Object getClientMetric(String metricName) {
        try {
            switch(metricName) {
                // List<Map<String,String>> - list of all native connections and their properties
                case "connections":
                // number of connected native clients
                case "connectedNativeClients":
                // number of native clients by username
                case "connectedNativeClientsByUser":
                case // number of native clients by username
                "clientsByProtocolVersion":
                    return JMX.newMBeanProxy(mbeanServerConn, new ObjectName("org.apache.cassandra.metrics:type=Client,name=" + metricName), CassandraMetricsRegistry.JmxGaugeMBean.class).getValue();
                default:
    ...
    public Double[] metricPercentilesAsArray(CassandraMetricsRegistry.JmxHistogramMBean metric) {
        return new Double[] { metric.get50thPercentile(), Double.valueOf(metric.get75thPercentile()), Double.valueOf(metric.get95thPercentile()), Double.valueOf(metric.get98thPercentile()), Double.valueOf(metric.get99thPercentile()), Double.valueOf(metric.getMin()), Double.valueOf(metric.getMax()) };
    }

    public Double[] metricPercentilesAsArray(CassandraMetricsRegistry.JmxTimerMBean metric) {
        return new Double[] { Double.valueOf(metric.get50thPercentile()), Double.valueOf(metric.get75thPercentile()), Double.valueOf(metric.get95thPercentile()), Double.valueOf(metric.get98thPercentile()), Double.valueOf(metric.get99thPercentile()), Double.valueOf(metric.getMin()), Double.valueOf(metric.getMax()) };
    }
...
class ColumnFamilyStoreMBeanIterator implements Iterator<Map.Entry<String, ColumnFamilyStoreMBean>> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ColumnFamilyStoreMBeanIterator.class);

    private transient MBeanServerConnection mbeanServerConn;

    transient Iterator<Entry<String, ColumnFamilyStoreMBean>> mbeans;
