...
public class BulkLoader {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(BulkLoader.class);
    ...
    public static void load(LoaderOptions options) throws BulkLoadException {
        ...
        SSTableLoader loader = new SSTableLoader(options.directory.getAbsoluteFile(), new ExternalClient(options.hosts, options.storagePort, options.authProvider, options.serverEncOptions, buildSSLOptions(options.clientEncOptions)), handler, options.connectionsPerHost, options.targetKeyspace);
    ...
    static class ProgressIndicator implements StreamEventHandler {
        ...
        private transient long lastProgress;

        private transient long lastTime;

        private transient long peak = 0;

        private transient int totalFiles = 0;

        private final transient Multimap<InetAddressAndPort, SessionInfo> sessionsByHost = HashMultimap.create();
    ...
    static class ExternalClient extends NativeSSTableLoaderClient {
        ...
        private final transient EncryptionOptions.ServerEncryptionOptions serverEncOptions;

        public ExternalClient(Set<InetSocketAddress> hosts, int storagePort, AuthProvider authProvider, EncryptionOptions.ServerEncryptionOptions serverEncryptionOptions, SSLOptions sslOptions) {
