...
public class SSTableMetadataViewer {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SSTableMetadataViewer.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SSTableMetadataViewer.class);

    private static final transient Options options = new Options();

    private static transient CommandLine cmd;

    private static final transient String COLORS = "c";

    private static final transient String UNICODE = "u";

    private static final transient String GCGS_KEY = "g";

    private static final transient String TIMESTAMP_UNIT = "t";

    private static final transient String SCAN = "s";

    private static transient Comparator<ValuedByteBuffer> VCOMP = Comparator.comparingLong(ValuedByteBuffer::getValue).reversed();
    ...
    transient boolean unicode;

    transient int gc;

    transient PrintStream out;

    transient String[] files;

    transient TimeUnit tsUnit;
    ...
    private void printScannedOverview(Descriptor descriptor, StatsMetadata stats) throws IOException {
        ...
        try (ISSTableScanner scanner = reader.getScanner()) {
            ...
            MinMaxPriorityQueue<ValuedByteBuffer> widestPartitions = MinMaxPriorityQueue.orderedBy(VCOMP).maximumSize(5).create();
            MinMaxPriorityQueue<ValuedByteBuffer> largestPartitions = MinMaxPriorityQueue.orderedBy(VCOMP).maximumSize(5).create();
            MinMaxPriorityQueue<ValuedByteBuffer> mostTombstones = MinMaxPriorityQueue.orderedBy(VCOMP).maximumSize(5).create();
                        ...
                        switch(unfiltered.kind()) {
                            case ROW:
                                ...
                                for (org.apache.cassandra.db.rows.Cell<?> cell : row.cells()) {
                                    ...
                                    if (lastPercent != (int) (percentComplete * 100) && (System.currentTimeMillis() - lastPercentTime) > 1000) {
                                        lastPercentTime = System.currentTimeMillis();
                                        lastPercent = (int) (percentComplete * 100);
                                        if (color)
                                            out.printf("\r%sAnalyzing SSTable...  %s%s %s(%%%s)", BLUE, CYAN, Util.progress(percentComplete, 30, unicode), RESET, (int) (percentComplete * 100));
                                        else
                                            out.printf("\rAnalyzing SSTable...  %s (%%%s)", Util.progress(percentComplete, 30, unicode), (int) (percentComplete * 100));
                                        out.flush();
                                    }
                                    if (cell.isTombstone()) {
                                        tombstoneCount++;
    ...
    private void printSStableMetadata(String fname, boolean scan) throws IOException {
        ...
        if (stats != null) {
            ...
            field("Estimated droppable tombstones", stats.getEstimatedDroppableTombstoneRatio((int) (System.currentTimeMillis() / 1000) - this.gc));
    ...
    private static void printUsage() {
        try (PrintWriter errWriter = new PrintWriter(System.err, true)) {
            ...
            formatter.printHelp(errWriter, 120, "sstablemetadata <options> <sstable...>", String.format("%nDump information about SSTable[s] for Apache Cassandra 3.x%nOptions:"), options, 2, 1, "", true);
            errWriter.println();
        }
    }
