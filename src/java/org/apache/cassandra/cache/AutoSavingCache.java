...
public class AutoSavingCache<K extends CacheKey, V> extends InstrumentingCache<K, V> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AutoSavingCache.class);
    ...
    private static final transient Logger logger = LoggerFactory.getLogger(AutoSavingCache.class);

    /**
     * True if a cache flush is currently executing: only one may execute at a time.
     */
    public static final transient Set<CacheService.CacheType> flushInProgress = new NonBlockingHashSet<CacheService.CacheType>();

    protected volatile transient ScheduledFuture<?> saveTask;

    protected final transient CacheService.CacheType cacheType;

    private final transient CacheSerializer<K, V> cacheLoader;
     ...
     */
    private static final transient String CURRENT_VERSION = "f";

    private static volatile transient IStreamFactory streamFactory = new IStreamFactory() {
    ...
    public class Writer extends CompactionInfo.Holder {
        ...
        private final transient Iterator<K> keyIterator;

        private final transient CompactionInfo info;

        private transient long keysWritten;

        private final transient long keysEstimate;
        ...
        protected Writer(int keysToSave) {
            ...
            info = CompactionInfo.withoutSSTables(TableMetadata.minimal(SchemaConstants.SYSTEM_KEYSPACE_NAME, cacheType.toString()), type, 0, keysEstimate, Unit.KEYS, UUIDGen.getTimeUUID());
        }
        ...
        public void saveCache() {
            ...
            try (WrappedDataOutputStreamPlus writer = new WrappedDataOutputStreamPlus(streamFactory.getOutputStream(cacheFilePaths.left, cacheFilePaths.right))) {
                ...
                while (keyIterator.hasNext()) {
                    ...
                    if (cfs == null)
                        // the table or 2i has been dropped.
                        continue;
        ...
        private void deleteOldCacheFiles() {
            ...
            if (files != null) {
                ...
                for (File file : files) {
                    if (!file.isFile())
                        // someone's been messing with our directory.  naughty!
                        continue;
