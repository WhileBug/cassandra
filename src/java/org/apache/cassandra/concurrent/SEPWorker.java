...
final class SEPWorker extends AtomicReference<SEPWorker.Work> implements Runnable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SEPWorker.class);

    private static final transient Logger logger = LoggerFactory.getLogger(SEPWorker.class);

    private static final transient boolean SET_THREAD_NAME = Boolean.parseBoolean(System.getProperty("cassandra.set_sep_thread_name", "true"));

    final transient Long workerId;

    final transient Thread thread;

    final transient SharedExecutorPool pool;
    ...
    transient long prevStopCheck = 0;

    transient long soleSpinnerSpinTime = 0;
    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                // we go to sleep)
                if (stop())
                    while (isStopped()) LockSupport.park();
                ...
                set(Work.WORKING);
                boolean shutdown;
                // make sure set if shutdown check short circuits
                SEPExecutor.TakeTaskPermitResult status = null;
                while (true) {
    ...
    private void stopSpinning() {
        if (pool.spinningCount.decrementAndGet() == 0)
            for (SEPExecutor executor : pool.executors) executor.maybeSchedule();
    ...
    private static final transient long stopCheckInterval = TimeUnit.MILLISECONDS.toNanos(10L);
    ...
    static final class Work {
        ...
        static final transient Work STOP_SIGNALLED = new Work();

        static final transient Work STOPPED = new Work();

        static final transient Work SPINNING = new Work();

        static final transient Work WORKING = new Work();

        final transient SEPExecutor assigned;
