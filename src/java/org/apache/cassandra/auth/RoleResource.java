...
public class RoleResource implements IResource, Comparable<RoleResource> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RoleResource.class);
    ...
    private static final transient Set<Permission> ROOT_LEVEL_PERMISSIONS = Sets.immutableEnumSet(Permission.CREATE, Permission.ALTER, Permission.DROP, Permission.AUTHORIZE, Permission.DESCRIBE);

    // permissions which may be granted on role level resources
    private static final transient Set<Permission> ROLE_LEVEL_PERMISSIONS = Sets.immutableEnumSet(Permission.ALTER, Permission.DROP, Permission.AUTHORIZE, Permission.DESCRIBE);

    private static final transient String ROOT_NAME = "roles";

    private static final transient RoleResource ROOT_RESOURCE = new RoleResource();
    ...
    public static RoleResource root() {
        logger_IC.info("[InconsistencyDetector][org.apache.cassandra.auth.RoleResource.ROOT_RESOURCE]=" + org.json.simple.JSONValue.toJSONString(ROOT_RESOURCE).replace("\n", "").replace("\r", ""));
        logger_IC.info("[InconsistencyDetector][org.apache.cassandra.auth.RoleResource.ROOT_RESOURCE]=" + org.json.simple.JSONValue.toJSONString(ROOT_RESOURCE).replace("\n", "").replace("\r", ""));
    ...
    public boolean exists() {
        logger_IC.info("[InconsistencyDetector][org.apache.cassandra.auth.RoleResource.level]=" + org.json.simple.JSONValue.toJSONString(level).replace("\n", "").replace("\r", ""));
        logger_IC.info("[InconsistencyDetector][org.apache.cassandra.auth.RoleResource.level]=" + org.json.simple.JSONValue.toJSONString(level).replace("\n", "").replace("\r", ""));
