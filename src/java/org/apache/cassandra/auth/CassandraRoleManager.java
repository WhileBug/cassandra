...
public class CassandraRoleManager implements IRoleManager {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CassandraRoleManager.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CassandraRoleManager.class);

    private static final transient Logger logger = LoggerFactory.getLogger(CassandraRoleManager.class);

    static final transient String DEFAULT_SUPERUSER_NAME = "cassandra";

    static final transient String DEFAULT_SUPERUSER_PASSWORD = "cassandra";

    // Transform a row in the AuthKeyspace.ROLES to a Role instance
    private static final transient Function<UntypedResultSet.Row, Role> ROW_TO_ROLE = row -> {
        try {
            return new Role(row.getString("role"), row.getBoolean("is_superuser"), row.getBoolean("can_login"), Collections.emptyMap(), row.has("member_of") ? row.getSet("member_of", UTF8Type.instance) : Collections.<String>emptySet());
    ...
    @VisibleForTesting
    public static final transient String GENSALT_LOG2_ROUNDS_PROPERTY = Config.PROPERTY_PREFIX + "auth_bcrypt_gensalt_log2_rounds";

    private static final transient int GENSALT_LOG2_ROUNDS = getGensaltLogRounds();
    ...
    private transient SelectStatement loadRoleStatement;

    private final transient Set<Option> supportedOptions;

    private final transient Set<Option> alterableOptions;

    // Will be set to true when all nodes in the cluster are on a version which supports roles (i.e. 2.2+)
    private volatile transient boolean isClusterReady = false;
    ...
    public void setup() {
        loadRoleStatement = (SelectStatement) prepare("SELECT * from %s.%s WHERE role = ?", SchemaConstants.AUTH_KEYSPACE_NAME, AuthKeyspace.ROLES);
        scheduleSetupTask(() -> {
            setupDefaultRole();
            return null;
        });
