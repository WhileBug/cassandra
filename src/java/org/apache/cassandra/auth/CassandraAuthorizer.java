...
public class CassandraAuthorizer implements IAuthorizer {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CassandraAuthorizer.class);

    private static final transient Logger logger = LoggerFactory.getLogger(CassandraAuthorizer.class);

    private static final transient String ROLE = "role";

    private static final transient String RESOURCE = "resource";

    private static final transient String PERMISSIONS = "permissions";

    private transient SelectStatement authorizeRoleStatement;
    ...
    public void revokeAllOn(IResource droppedResource) {
        try {
            ...
            List<CQLStatement> statements = new ArrayList<>();
            for (UntypedResultSet.Row row : rows) {
                statements.add(QueryProcessor.getStatement(String.format("DELETE FROM %s.%s WHERE role = '%s' AND resource = '%s'", SchemaConstants.AUTH_KEYSPACE_NAME, AuthKeyspace.ROLE_PERMISSIONS, escape(row.getString("role")), escape(droppedResource.getName())), ClientState.forInternalCalls()));
            }
            statements.add(QueryProcessor.getStatement(String.format("DELETE FROM %s.%s WHERE resource = '%s'", SchemaConstants.AUTH_KEYSPACE_NAME, AuthKeyspace.RESOURCE_ROLE_INDEX, escape(droppedResource.getName())), ClientState.forInternalCalls()));
            executeLoggedBatch(statements);
        } catch (RequestExecutionException | RequestValidationException e) {
