...
public class CassandraNetworkAuthorizer implements INetworkAuthorizer {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CassandraNetworkAuthorizer.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CassandraNetworkAuthorizer.class);

    private transient SelectStatement authorizeUserStatement = null;

    public void setup() {
        String query = String.format("SELECT dcs FROM %s.%s WHERE role = ?", SchemaConstants.AUTH_KEYSPACE_NAME, AuthKeyspace.NETWORK_PERMISSIONS);
    ...
    public void setRoleDatacenters(RoleResource role, DCPermissions permissions) {
        String query = String.format("UPDATE %s.%s SET dcs = %s WHERE role = '%s'", SchemaConstants.AUTH_KEYSPACE_NAME, AuthKeyspace.NETWORK_PERMISSIONS, getSetString(permissions), role.getName());
        process(query);
    }

    public void drop(RoleResource role) {
        String query = String.format("DELETE FROM %s.%s WHERE role = '%s'", SchemaConstants.AUTH_KEYSPACE_NAME, AuthKeyspace.NETWORK_PERMISSIONS, role.getName());
        process(query);
    }
