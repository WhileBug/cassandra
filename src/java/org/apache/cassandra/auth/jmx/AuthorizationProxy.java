...
public class AuthorizationProxy implements InvocationHandler {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AuthorizationProxy.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AuthorizationProxy.class);

    private static final transient Logger logger = LoggerFactory.getLogger(AuthorizationProxy.class);
     ...
     */
    private static final transient Set<String> MBEAN_SERVER_ALLOWED_METHODS = ImmutableSet.of("getDefaultDomain", "getDomains", "getMBeanCount", "hashCode", "queryMBeans", "queryNames", "toString");
     ...
     */
    private static final transient Set<String> DENIED_METHODS = ImmutableSet.of("createMBean", "deserialize", "getClassLoader", "getClassLoaderFor", "instantiate", "registerMBean", "unregisterMBean");

    private static final transient JMXPermissionsCache permissionsCache = new JMXPermissionsCache();
     ...
     */
    protected transient Predicate<RoleResource> isSuperuser = Roles::hasSuperuserStatus;
     ...
     */
    protected transient Function<RoleResource, Set<PermissionDetails>> getPermissions = permissionsCache::get;
     ...
     */
    protected transient BooleanSupplier isAuthzRequired = () -> DatabaseDescriptor.getAuthorizer().requireAuthorization();
     ...
     */
    protected transient Function<ObjectName, Set<ObjectName>> queryNames = (name) -> mbs.queryNames(name, null);
     ...
     */
    protected transient BooleanSupplier isAuthSetupComplete = () -> StorageService.instance.isAuthSetupComplete();
    ...
    private Object invoke(Method method, Object[] args) throws Throwable {
        ...
        } catch (// Catch any exception that might have been thrown by the mbeans
        InvocationTargetException e) {
            // Throw the exception that nodetool etc expects
