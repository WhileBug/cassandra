...
public class CqlRecordReader extends RecordReader<Long, Row> implements org.apache.hadoop.mapred.RecordReader<Long, Row>, AutoCloseable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CqlRecordReader.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CqlRecordReader.class);

    private static final transient Logger logger = LoggerFactory.getLogger(CqlRecordReader.class);

    private transient ColumnFamilySplit split;

    private transient RowIterator rowIterator;

    private transient Pair<Long, Row> currentRow;
    ...
    private transient String cfName;

    private transient String cqlQuery;
    ...
    private transient String inputColumns;

    private transient String userDefinedWhereClauses;

    private transient List<String> partitionKeys = new ArrayList<>();

    // partition keys -- key aliases
    private transient LinkedHashMap<String, Boolean> partitionBoundColumns = Maps.newLinkedHashMap();

    protected transient int nativeProtocolVersion = 1;
    ...
    private class RowIterator extends AbstractIterator<Pair<Long, Row>> {
        ...
        private transient long keyId = 0L;

        // total number of cf rows read
        protected transient int totalRead = 0;

        protected transient Iterator<Row> rows;

        // previous CF row key
        private transient Map<String, ByteBuffer> previousRowKey = new HashMap<String, ByteBuffer>();
        ...
        public RowIterator() {
            ...
            for (ColumnMetadata meta : cluster.getMetadata().getKeyspace(quote(keyspace)).getTable(quote(cfName)).getPartitionKey()) partitionBoundColumns.put(meta.getName(), Boolean.TRUE);
            rows = rs.iterator();
        }
