...
public class CipherFactory {
    ...
    public CipherFactory(TransparentDataEncryptionOptions options) {
        ...
        cache = // by default cache is unbounded
        Caffeine.newBuilder().maximumSize(// a value large enough that we should never even get close (so nothing gets evicted)
        64).executor(MoreExecutors.directExecutor()).removalListener((key, value, cause) -> {
