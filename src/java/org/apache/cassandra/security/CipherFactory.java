...
public class CipherFactory {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CipherFactory.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CipherFactory.class);

    private final transient Logger logger = LoggerFactory.getLogger(CipherFactory.class);
     ...
     */
    private static final transient FastThreadLocal<CachedCipher> cipherThreadLocal = new FastThreadLocal<>();

    private final transient SecureRandom secureRandom;

    private final transient LoadingCache<String, Key> cache;

    private final transient int ivLength;

    private final transient KeyProvider keyProvider;
    ...
    public CipherFactory(TransparentDataEncryptionOptions options) {
        ...
        // by default cache is unbounded
        cache = // a value large enough that we should never even get close (so nothing gets evicted)
        Caffeine.newBuilder().maximumSize(64).executor(MoreExecutors.directExecutor()).removalListener((key, value, cause) -> {
    ...
    private static class CachedCipher {
        ...
        public final transient int mode;

        public final transient String keyAlias;

        public final transient Cipher cipher;
