...
public final class SSLFactory {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SSLFactory.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SSLFactory.class);

    private static final transient Logger logger = LoggerFactory.getLogger(SSLFactory.class);
    ...
    @VisibleForTesting
    static volatile transient boolean checkedExpiry = false;
    ...
    // garbage collected.
    static private final transient boolean openSslIsAvailable;
     ...
     */
    private static final transient ConcurrentHashMap<CacheKey, SslContext> cachedSslContexts = new ConcurrentHashMap<>();

    /**
     * List of files that trigger hot reloading of SSL certificates
     */
    private static volatile transient List<HotReloadableFile> hotReloadableFiles = ImmutableList.of();

    /**
     * Default initial delay for hot reloading
     */
    public static final transient int DEFAULT_HOT_RELOAD_INITIAL_DELAY_SEC = 600;

    /**
     * Default periodic check delay for hot reloading
     */
    public static final transient int DEFAULT_HOT_RELOAD_PERIOD_SEC = 600;

    /**
     * State variable to maintain initialization invariant
     */
    private static transient boolean isHotReloadingInitialized = false;
    ...
    private static class HotReloadableFile {
        ...
        private volatile transient long lastModTime;
        ...
        public String toString() {
            return "HotReloadableFile{" + "file=" + file + ", lastModTime=" + lastModTime + '}';
        }
    }
    ...
    static SslContext createNettySslContext(EncryptionOptions options, boolean buildTruststore, SocketType socketType, boolean useOpenSsl) throws IOException {
        return createNettySslContext(options, buildTruststore, socketType, useOpenSsl, LoggingCipherSuiteFilter.QUIET_FILTER);
    }

    /**
     * Create a Netty {@link SslContext} with a supplied cipherFilter
     */
    static SslContext createNettySslContext(EncryptionOptions options, boolean buildTruststore, SocketType socketType, boolean useOpenSsl, CipherSuiteFilter cipherFilter) throws IOException {
        /*
    ...
    public static synchronized void initHotReloading(EncryptionOptions.ServerEncryptionOptions serverOpts, EncryptionOptions clientOpts, boolean force) throws IOException {
        ...
        if (!isHotReloadingInitialized) {
            ScheduledExecutors.scheduledTasks.scheduleWithFixedDelay(() -> checkCertFilesForHotReloading(DatabaseDescriptor.getInternodeMessagingEncyptionOptions(), DatabaseDescriptor.getNativeProtocolEncryptionOptions()), DEFAULT_HOT_RELOAD_INITIAL_DELAY_SEC, DEFAULT_HOT_RELOAD_PERIOD_SEC, TimeUnit.SECONDS);
        }
    ...
    public static final class LoggingCipherSuiteFilter implements CipherSuiteFilter {
        ...
        public static final transient CipherSuiteFilter QUIET_FILTER = new LoggingCipherSuiteFilter();

        final transient String settingDescription;
            ...
            for (String c : ciphers) {
                ...
                if (supportedCiphers.contains(c)) {
                    ...
                    if (settingDescription != null) {
                        logger.warn("Dropping unsupported cipher_suite {} from {} configuration", c, settingDescription.toLowerCase());
                    }
                }
            }
                ...
                try {
                    ...
                    try {
                        if (logProtocolAndCiphers) {
                            ...
                            logger.debug("{} supported TLS protocols: {}", contextDescription, supportedProtocols == null ? "system default" : String.join(", ", supportedProtocols));
                            logger.debug("{} unfiltered enabled TLS protocols: {}", contextDescription, enabledProtocols == null ? "system default" : String.join(", ", enabledProtocols));
                            logger.info("{} enabled TLS protocols: {}", contextDescription, filteredEnabledProtocols);
                            logger.debug("{} supported cipher suites: {}", contextDescription, supportedCiphers == null ? "system default" : String.join(", ", supportedCiphers));
                            logger.info("{} enabled cipher suites: {}", contextDescription, enabledCiphers == null ? "system default" : String.join(", ", enabledCiphers));
                        }
                    } finally {
                        engine.closeInbound();
    ...
    static class CacheKey {
        ...
        private final transient SocketType socketType;

        private final transient boolean useOpenSSL;
        ...
        public boolean equals(Object o) {
            ...
            return (socketType == cacheKey.socketType && useOpenSSL == cacheKey.useOpenSSL && Objects.equals(encryptionOptions, cacheKey.encryptionOptions));
        }
