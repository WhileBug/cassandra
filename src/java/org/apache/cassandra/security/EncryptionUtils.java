...
public class EncryptionUtils {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(EncryptionUtils.class);

    public static final transient int COMPRESSED_BLOCK_HEADER_SIZE = 4;

    public static final transient int ENCRYPTED_BLOCK_HEADER_SIZE = 8;

    private static final transient FastThreadLocal<ByteBuffer> reusableBuffers = new FastThreadLocal<ByteBuffer>() {
    ...
    private static int readInt(byte[] input, int inputOffset) {
        return (input[inputOffset + 3] & 0xFF) | ((input[inputOffset + 2] & 0xFF) << 8) | ((input[inputOffset + 1] & 0xFF) << 16) | ((input[inputOffset] & 0xFF) << 24);
    }
    ...
    private static class DataInputReadChannel implements ReadableByteChannel {
        ...
        private final transient FileDataInput fileDataInput;
    ...
    public static class ChannelProxyReadChannel implements ReadableByteChannel {
        ...
        private final transient ChannelProxy channelProxy;

        private volatile transient long currentPosition;
