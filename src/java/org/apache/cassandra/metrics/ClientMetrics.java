...
public final class ClientMetrics {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ClientMetrics.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ClientMetrics.class);

    public static final transient ClientMetrics instance = new ClientMetrics();

    private static final transient MetricNameFactory factory = new DefaultNameFactory("Client");

    private volatile transient boolean initialized = false;

    private transient Collection<Server> servers = Collections.emptyList();

    private transient Meter authSuccess;

    private transient Meter authFailure;

    private transient AtomicInteger pausedConnections;

    @SuppressWarnings({ "unused", "FieldCanBeLocal" })
    private transient Gauge<Integer> pausedConnectionsGauge;

    private transient Meter requestDiscarded;

    private transient Meter protocolException;

    private transient Meter unknownException;
    ...
    private List<Map<String, String>> recentClientStats() {
        ...
        for (Server server : servers) for (// asMap returns guava, so need to convert to java for jmx
        ClientStat stat : // asMap returns guava, so need to convert to java for jmx
        server.recentClientStats()) stats.add(new HashMap<>(stat.asMap()));
