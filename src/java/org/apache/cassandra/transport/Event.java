...
public abstract class Event {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Event.class);

    public enum Type {

        TOPOLOGY_CHANGE(ProtocolVersion.V3), STATUS_CHANGE(ProtocolVersion.V3), SCHEMA_CHANGE(ProtocolVersion.V3), TRACE_COMPLETE(ProtocolVersion.V4);
    ...
    public static abstract class NodeEvent extends Event {
        ...
        public final transient InetSocketAddress node;
    ...
    public static class TopologyChange extends NodeEvent {
        ...
        public final transient Change change;
    ...
    public static class SchemaChange extends Event {
        ...
        public final transient Target target;
        ...
        public int eventSerializedSize(ProtocolVersion version) {
            if (target == Target.FUNCTION || target == Target.AGGREGATE) {
                if (version.isGreaterOrEqualTo(ProtocolVersion.V4))
                    return CBUtil.sizeOfEnumValue(change) + CBUtil.sizeOfEnumValue(target) + CBUtil.sizeOfAsciiString(keyspace) + CBUtil.sizeOfAsciiString(name) + CBUtil.sizeOfStringList(argTypes);
                if (version.isGreaterOrEqualTo(ProtocolVersion.V3))
                    return CBUtil.sizeOfEnumValue(Change.UPDATED) + CBUtil.sizeOfEnumValue(Target.KEYSPACE) + CBUtil.sizeOfAsciiString(keyspace);
                return CBUtil.sizeOfEnumValue(Change.UPDATED) + CBUtil.sizeOfAsciiString(keyspace) + CBUtil.sizeOfAsciiString("");
            }
            if (version.isGreaterOrEqualTo(ProtocolVersion.V3)) {
                int size = CBUtil.sizeOfEnumValue(change) + CBUtil.sizeOfEnumValue(target) + CBUtil.sizeOfAsciiString(keyspace);
                if (target != Target.KEYSPACE)
                ...
                return CBUtil.sizeOfEnumValue(change) + CBUtil.sizeOfAsciiString(keyspace) + CBUtil.sizeOfAsciiString(target == Target.KEYSPACE ? "" : name);
            }
        }

        @Override
        public String toString() {
            StringBuilder sb = new StringBuilder().append(change).append(' ').append(target).append(' ').append(keyspace);
            if (name != null)
        ...
        public boolean equals(Object other) {
            ...
            return Objects.equal(change, scc.change) && Objects.equal(target, scc.target) && Objects.equal(keyspace, scc.keyspace) && Objects.equal(name, scc.name) && Objects.equal(argTypes, scc.argTypes);
        }
    }
}
