...
public class ErrorMessage extends Message.Response {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ErrorMessage.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ErrorMessage.class);

    private static final transient Logger logger = LoggerFactory.getLogger(ErrorMessage.class);

    public static final transient Message.Codec<ErrorMessage> codec = new Message.Codec<ErrorMessage>() {
            ...
            switch(err.code()) {
                ...
                case READ_TIMEOUT:
                    ...
                    if (isWrite && version.isGreaterOrEqualTo(ProtocolVersion.V5) && rte instanceof CasWriteTimeoutException)
                        // CasWriteTimeoutException appends a short for contentions occured.
                        size += 2;
                    break;
                case FUNCTION_FAILURE:
                    FunctionExecutionException fee = (FunctionExecutionException) msg.error;
                ...
                case CAS_WRITE_UNKNOWN:
                    assert version.isGreaterOrEqualTo(ProtocolVersion.V5);
                    CasWriteUnknownResultException cwue = (CasWriteUnknownResultException) err;
                    // receivedFor: 4, blockFor: 4
                    size += CBUtil.sizeOfConsistencyLevel(cwue.consistency) + 4 + 4;
                    break;
            }
            return size;
        }
    ...
    public final transient TransportException error;
