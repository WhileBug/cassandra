...
public enum ProtocolVersion implements Comparable<ProtocolVersion> {
    ...
    public static List<String> supportedVersions() {
        ...
        for (ProtocolVersion version : SUPPORTED) ret.add(version.toString());
        return ret;
    }
    ...
    public static List<ProtocolVersion> supportedVersionsStartingWith(ProtocolVersion smallestVersion) {
        ...
        for (ProtocolVersion version : SUPPORTED_VERSIONS) if (version.isGreaterOrEqualTo(smallestVersion))
            versions.add(version);
        return versions;
    }

    public static ProtocolVersion decode(int versionNum, boolean allowOlderProtocols) {
        ProtocolVersion ret = versionNum >= MIN_SUPPORTED_VERSION.num && versionNum <= MAX_SUPPORTED_VERSION.num ? SUPPORTED_VERSIONS[versionNum - MIN_SUPPORTED_VERSION.num] : null;
        if (ret == null) {
