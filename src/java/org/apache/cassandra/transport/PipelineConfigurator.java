...
public class PipelineConfigurator {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(PipelineConfigurator.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(PipelineConfigurator.class);

    private static final transient Logger logger = LoggerFactory.getLogger(PipelineConfigurator.class);
    ...
    private static final transient boolean DEBUG = Boolean.getBoolean("cassandra.unsafe_verbose_debug_client_protocol");

    // Stateless handlers
    private static final transient ConnectionLimitHandler connectionLimitHandler = new ConnectionLimitHandler();

    // Names of handlers used regardless of protocol version
    private static final transient String CONNECTION_LIMIT_HANDLER = "connectionLimitHandler";

    private static final transient String IDLE_STATE_HANDLER = "idleStateHandler";

    private static final transient String INITIAL_HANDLER = "initialHandler";

    private static final transient String EXCEPTION_HANDLER = "exceptionHandler";

    private static final transient String DEBUG_HANDLER = "debugHandler";

    private static final transient String SSL_HANDLER = "ssl";
    ...
    private static final transient String MESSAGE_DECOMPRESSOR = "decompressor";

    private static final transient String MESSAGE_COMPRESSOR = "compressor";
    ...
    private static final transient String LEGACY_MESSAGE_PROCESSOR = "legacyCqlProcessor";
    ...
    private static final transient String MESSAGE_PROCESSOR = "cqlProcessor";

    private final transient boolean epoll;

    private final transient boolean keepAlive;

    private final transient EncryptionOptions.TlsEncryptionPolicy tlsEncryptionPolicy;
    ...
    public ChannelFuture initializeChannel(final EventLoopGroup workerGroup, final InetSocketAddress socket, final Connection.Factory connectionFactory) {
        ServerBootstrap bootstrap = new ServerBootstrap().channel(epoll ? EpollServerSocketChannel.class : NioServerSocketChannel.class).childOption(ChannelOption.TCP_NODELAY, true).childOption(ChannelOption.SO_LINGER, 0).childOption(ChannelOption.SO_KEEPALIVE, keepAlive).childOption(ChannelOption.ALLOCATOR, CBUtil.allocator).childOption(ChannelOption.WRITE_BUFFER_WATER_MARK, new WriteBufferWaterMark(8 * 1024, 32 * 1024));
        if (workerGroup != null)
    ...
    public void configureModernPipeline(ChannelHandlerContext ctx, ClientResourceLimits.Allocator resourceAllocator, ProtocolVersion version, Map<String, String> options) {
        ...
        CQLMessageHandler<Message.Request> processor = new CQLMessageHandler<>(ctx.channel(), version, frameDecoder, envelopeDecoder, messageDecoder, messageConsumer, payloadAllocator, queueCapacity, resourceProvider, onClosed, errorHandler, throwOnOverload);
        // remove old outbound cql envelope encoder
        ...
        ctx.channel().attr(Dispatcher.EVENT_DISPATCHER).set(dispatcher.eventDispatcher(ctx.channel(), version, payloadAllocator));
