...
public class SimpleClient implements Closeable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SimpleClient.class);

    static {
        InternalLoggerFactory.setDefaultFactory(new Slf4JLoggerFactory());
    }

    private static final transient Logger logger = LoggerFactory.getLogger(SimpleClient.class);

    public final transient String host;

    public final transient int port;

    private final transient EncryptionOptions encryptionOptions;

    private final transient int largeMessageThreshold;

    protected final transient ResponseHandler responseHandler = new ResponseHandler();

    protected final transient Connection.Tracker tracker = new ConnectionTracker();

    protected final transient ProtocolVersion version;

    // We don't track connection really, so we don't need one Connection per channel
    protected transient Connection connection;

    protected transient Bootstrap bootstrap;

    protected transient Channel channel;

    protected transient ChannelFuture lastWriteFuture;

    protected transient String compression;
    ...
    public static class Builder {
        ...
        private transient EncryptionOptions encryptionOptions = new EncryptionOptions();

        private transient ProtocolVersion version = ProtocolVersion.CURRENT;

        private transient boolean useBeta = false;

        private transient int largeMessageThreshold = FrameEncoder.Payload.MAX_SIZE;
    ...
    public SimpleClient(String host, int port, ProtocolVersion version, boolean useBeta, EncryptionOptions encryptionOptions) {
        ...
        this.largeMessageThreshold = FrameEncoder.Payload.MAX_SIZE - Math.max(FrameEncoderCrc.HEADER_AND_TRAILER_LENGTH, FrameEncoderLZ4.HEADER_AND_TRAILER_LENGTH);
    }
    ...
    public static class SimpleEventHandler implements EventHandler {
        ...
        public final transient LinkedBlockingQueue<Event> queue = new LinkedBlockingQueue<>();
    ...
    private static class HandlerNames {
        ...
        private static final transient String ENVELOPE_DECODER = "envelopeDecoder";

        private static final transient String ENVELOPE_ENCODER = "envelopeEncoder";

        private static final transient String COMPRESSOR = "compressor";

        private static final transient String DECOMPRESSOR = "decompressor";

        private static final transient String MESSAGE_DECODER = "messageDecoder";

        private static final transient String MESSAGE_ENCODER = "messageEncoder";

        private static final transient String INITIAL_HANDLER = "intitialHandler";

        private static final transient String RESPONSE_HANDLER = "responseHandler";

        private static final transient String FRAME_DECODER = "frameDecoder";

        private static final transient String FRAME_ENCODER = "frameEncoder";

        private static final transient String PROCESSOR = "processor";
    }

    private static class InitialHandler extends MessageToMessageDecoder<Envelope> {

        final transient ProtocolVersion version;

        final transient ResponseHandler responseHandler;

        final transient int largeMessageThreshold;
        ...
        protected void decode(ChannelHandlerContext ctx, Envelope response, List<Object> results) {
            switch(response.header.type) {
                ...
                case SUPPORTED:
                    ...
                    break;
                default:
                    throw new ProtocolException(String.format("Unexpected %s response expecting " + "READY, AUTHENTICATE or SUPPORTED", response.header.type));
            }
        }
        ...
        private void configureModernPipeline(ChannelHandlerContext ctx, Envelope response) {
            ...
            CQLMessageHandler<Message.Response> processor = new CQLMessageHandler<Message.Response>(ctx.channel(), version, frameDecoder, envelopeDecoder, messageDecoder, responseConsumer, payloadAllocator, queueCapacity, resources, handler -> {
            }, errorHandler, ctx.channel().attr(Connection.attributeKey).get().isThrowOnOverload()) {
            ...
            pipeline.addLast(HandlerNames.MESSAGE_ENCODER, new ChannelOutboundHandlerAdapter() {
                ...
                public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
                    ...
                    for (Message message : (List<Message>) msg) flusher.enqueue(message.encode(version));
    ...
    static class MessageBatchEncoder extends MessageToMessageEncoder<List<Message>> {
        ...
        public static final transient MessageBatchEncoder instance = new MessageBatchEncoder();
    ...
    private class Initializer extends ChannelInitializer<Channel> {
        ...
        private transient int largeMessageThreshold;
    ...
    private class SecureInitializer extends Initializer {
        ...
        protected void initChannel(Channel channel) throws Exception {
            ...
            SslContext sslContext = SSLFactory.getOrCreateSslContext(encryptionOptions, encryptionOptions.require_client_auth, SSLFactory.SocketType.CLIENT);
    ...
    static class ResponseHandler extends SimpleChannelInboundHandler<Message.Response> {
        ...
        public final transient BlockingQueue<Message.Response> responses = new SynchronousQueue<>(true);

        public transient EventHandler eventHandler;
    ...
    public static class SimpleFlusher {
        ...
        private static final transient ChannelFuture[] EMPTY_FUTURES_ARRAY = new ChannelFuture[0];

        final transient Queue<Envelope> outbound = new ConcurrentLinkedQueue<>();

        final transient FrameEncoder frameEncoder;

        private final transient AtomicBoolean scheduled = new AtomicBoolean(false);
        ...
        public void releaseAll() {
            Envelope e;
            while ((e = outbound.poll()) != null) e.release();
        }

        public void schedule(ChannelHandlerContext ctx) {
            if (scheduled.compareAndSet(false, true))
                ctx.executor().scheduleAtFixedRate(() -> maybeWrite(ctx, ctx.voidPromise()), 10, 10, TimeUnit.MILLISECONDS);
        }
        ...
        private ChannelFuture flushBuffer(ChannelHandlerContext ctx, List<Envelope> messages, long bufferSize) {
            ...
            for (Envelope e : messages) e.encodeInto(payload.buffer);
            payload.finish();
            ChannelPromise release = AsyncChannelPromise.withListener(ctx, future -> {
                for (Envelope e : messages) e.release();
            });
            return ctx.writeAndFlush(payload, release);
        }

        private FrameEncoder.Payload allocate(int size, boolean selfContained) {
            FrameEncoder.Payload payload = frameEncoder.allocator().allocate(selfContained, Math.min(size, MAX_FRAMED_PAYLOAD_SIZE));
