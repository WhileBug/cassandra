...
public abstract class CBUtil {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CBUtil.class);

    public static final transient boolean USE_HEAP_ALLOCATOR = Boolean.getBoolean(Config.PROPERTY_PREFIX + "netty_use_heap_allocator");

    public static final transient ByteBufAllocator allocator = USE_HEAP_ALLOCATOR ? new UnpooledByteBufAllocator(false) : new PooledByteBufAllocator(true);

    private static final transient int UUID_SIZE = 16;

    private final static transient FastThreadLocal<CharsetDecoder> TL_UTF8_DECODER = new FastThreadLocal<CharsetDecoder>() {
    ...
    private final static transient FastThreadLocal<CharBuffer> TL_CHAR_BUFFER = new FastThreadLocal<>();
    ...
    public static ByteBuffer readBoundValue(ByteBuf cb, ProtocolVersion protocolVersion) {
        int length = cb.readInt();
        if (length < 0) {
            if (// backward compatibility for pre-version 4
            protocolVersion.isSmallerThan(ProtocolVersion.V4))
                return null;
            if (length == -1)
                return null;
            else if (length == -2)
