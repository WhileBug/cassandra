...
public enum DataType {
    ...
    public Object readValue(ByteBuf cb, ProtocolVersion version) {
        switch(this) {
            ...
            case TUPLE:
                ...
                for (int i = 0; i < n; i++) types.add(DataType.toType(codec.decodeOne(cb, version)));
    ...
    public void writeValue(Object value, ByteBuf cb, ProtocolVersion version) {
        ...
        switch(this) {
            ...
            case TUPLE:
                ...
                for (int i = 0; i < tt.size(); i++) codec.writeOne(DataType.fromType(tt.type(i), version), cb, version);
                break;
        }
    }
    ...
    public int serializedValueSize(Object value, ProtocolVersion version) {
        ...
        switch(this) {
            ...
            case TUPLE:
                TupleType tt = (TupleType) value;
                size = 2;
                for (int i = 0; i < tt.size(); i++) size += codec.oneSerializedSize(DataType.fromType(tt.type(i), version), version);
                return size;
            default:
                return 0;
        }
    ...
    public static final class Codec {
        ...
        public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Codec.class);

        public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Codec.class);

        private final transient DataType[] ids;
        ...
        private int getMaxId(DataType[] values) {
            int maxId = -1;
            for (DataType opt : values) maxId = Math.max(maxId, opt.getId(ProtocolVersion.CURRENT));
            return maxId;
        }
