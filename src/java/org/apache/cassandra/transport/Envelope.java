...
public class Envelope {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Envelope.class);

    public static final transient byte PROTOCOL_VERSION_MASK = 0x7f;

    public final transient Header header;

    public final transient ByteBuf body;
    ...
    public static class Header {
        ...
        public static final transient int LENGTH = 9;

        public static final transient int BODY_LENGTH_SIZE = 4;

        public final transient ProtocolVersion version;

        public final transient EnumSet<Flag> flags;

        public final transient int streamId;

        public final transient Message.Type type;

        public final transient long bodySizeInBytes;
    ...
    public static class Decoder extends ByteToMessageDecoder {
        ...
        private static final transient int MAX_TOTAL_LENGTH = DatabaseDescriptor.getNativeTransportMaxFrameSize();

        private transient boolean discardingTooLongMessage;

        private transient long tooLongTotalLength;

        private transient long bytesToDiscard;

        private transient int tooLongStreamId;
        ...
        HeaderExtractionResult extractHeader(ByteBuffer buffer) {
            Preconditions.checkArgument(buffer.remaining() >= Header.LENGTH, "Undersized buffer supplied. Expected %s, actual %s", Header.LENGTH, buffer.remaining());
            ...
            if (bodyLength < 0)
                return new HeaderExtractionResult.Error(new ProtocolException("Invalid value for envelope header body length field: " + bodyLength), streamId, bodyLength);
        ...
        public static abstract class HeaderExtractionResult {
            ...
            private final transient int streamId;

            private final transient long bodyLength;
            ...
            private static class Error extends HeaderExtractionResult {
                ...
                private final transient ProtocolException error;
    ...
    public static class Encoder extends MessageToMessageEncoder<Envelope> {
        ...
        public static final transient Encoder instance = new Envelope.Encoder();
    ...
    public static class Decompressor extends MessageToMessageDecoder<Envelope> {
        ...
        public static transient Decompressor instance = new Envelope.Decompressor();
    ...
    public static class Compressor extends MessageToMessageEncoder<Envelope> {
        ...
        public static transient Compressor instance = new Compressor();
