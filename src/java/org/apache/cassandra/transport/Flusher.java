...
abstract class Flusher implements Runnable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Flusher.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(Flusher.class);

    private static final transient Logger logger = LoggerFactory.getLogger(Flusher.class);

    @VisibleForTesting
    public static final transient int MAX_FRAMED_PAYLOAD_SIZE = Math.min(BufferPool.NORMAL_CHUNK_SIZE, FrameEncoder.Payload.MAX_SIZE - Math.max(FrameEncoderCrc.HEADER_AND_TRAILER_LENGTH, FrameEncoderLZ4.HEADER_AND_TRAILER_LENGTH));
    ...
    static class FlushItem<T> {
        ...
        final transient T response;

        final transient Envelope request;

        final transient Consumer<FlushItem<T>> tidy;
        ...
        static class Framed extends FlushItem<Envelope> {
            ...
            final transient FrameEncoder.PayloadAllocator allocator;
    ...
    protected final transient EventLoop eventLoop;

    private final transient ConcurrentLinkedQueue<FlushItem<?>> queued = new ConcurrentLinkedQueue<>();

    protected final transient AtomicBoolean scheduled = new AtomicBoolean(false);

    protected final transient List<FlushItem<?>> processed = new ArrayList<>();

    private final transient HashSet<Channel> channels = new HashSet<>();

    private final transient Map<Channel, FlushBuffer> payloads = new HashMap<>();
    ...
    private void processFramedResponse(FlushItem.Framed flush) {
        ...
        if (envelopeSize(outbound.header) >= MAX_FRAMED_PAYLOAD_SIZE) {
            ...
            payloads.computeIfAbsent(flush.channel, channel -> new FlushBuffer(channel, flush.allocator, 5)).add(flush.response);
        }
    }
    ...
    private void flushLargeMessage(Channel channel, Envelope outbound, FrameEncoder.PayloadAllocator allocator) {
        ...
        while (body.readableBytes() > 0 || firstFrame) {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace("Allocated initial buffer of {} for 1 large item", FBUtilities.prettyPrintMemory(payload.buffer.capacity()));
            }
            buf = payload.buffer;
    ...
    protected void flushWrittenChannels() {
        ...
        for (FlushBuffer buffer : payloads.values()) buffer.finish();
        ...
        for (FlushItem<?> item : processed) item.release();
        payloads.clear();
        channels.clear();
        processed.clear();
    }
    ...
    private class FlushBuffer extends ArrayList<Envelope> {
        ...
        private FrameEncoder.Payload allocate(int requiredBytes, int maxItems) {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace("Allocated initial buffer of {} for up to {} items", FBUtilities.prettyPrintMemory(payload.buffer.capacity()), maxItems);
            }
            return payload;
        }
    ...
    private static final class LegacyFlusher extends Flusher {
        ...
        transient int runsSinceFlush = 0;

        transient int runsWithNoWork = 0;
