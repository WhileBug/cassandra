...
public class SequentialWriter extends BufferedDataOutputStreamPlus implements Transactional {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SequentialWriter.class);

    // absolute path to the given file
    private final transient String filePath;

    // Offset for start of buffer relative to underlying file
    protected transient long bufferOffset;

    protected final transient FileChannel fchannel;
    ...
    private final transient boolean strictFlushing;

    // whether to do trickling fsync() to avoid sudden bursts of dirty buffer flushing by kernel causing read
    // latency spikes
    private final transient SequentialWriterOption option;

    private transient int bytesSinceTrickleFsync = 0;

    protected transient long lastFlushOffset;

    protected transient Runnable runPostFlush;

    private final transient TransactionalProxy txnProxy = txnProxy();
