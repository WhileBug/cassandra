...
public class MmappedRegions extends SharedCloseableImpl {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MmappedRegions.class);

    /**
     * In a perfect world, MAX_SEGMENT_SIZE would be final, but we need to test with a smaller size
     */
    public static transient int MAX_SEGMENT_SIZE = Integer.MAX_VALUE;

    /**
     * When we need to grow the arrays, we add this number of region slots
     */
    static final transient int REGION_ALLOC_SIZE = 15;
     ...
     */
    private volatile transient State copy;
    ...
    private static final class State {
         ...
         * The file channel
         */
        private final transient ChannelProxy channel;
        ...
        private Throwable close(Throwable accumulate) {
            ...
            return perform(accumulate, channel.filePath(), Throwables.FileOpType.READ, of(buffers).map((buffer) -> () -> {
                if (buffer != null)
