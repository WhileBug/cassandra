...
public class IndexSummaryBuilder implements AutoCloseable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(IndexSummaryBuilder.class);

    private static final transient Logger logger = LoggerFactory.getLogger(IndexSummaryBuilder.class);

    static final transient String defaultExpectedKeySizeName = Config.PROPERTY_PREFIX + "index_summary_expected_key_size";

    static transient long defaultExpectedKeySize = Long.valueOf(System.getProperty(defaultExpectedKeySizeName, "64"));

    // the offset in the keys memory region to look for a given summary boundary
    private final transient SafeMemoryWriter offsets;

    private final transient SafeMemoryWriter entries;

    private final transient int minIndexInterval;

    private final transient int samplingLevel;

    private final transient int[] startPoints;

    private transient long keysWritten = 0;

    private transient long indexIntervalMatches = 0;

    private transient long nextSamplePosition;
    ...
    private transient TreeMap<Long, ReadableBoundary> lastReadableByData = new TreeMap<>();
    ...
    private transient TreeMap<Long, ReadableBoundary> lastReadableByIndex = new TreeMap<>();

    // the last synced data file position
    private transient long dataSyncPosition;

    // the last synced index file position
    private transient long indexSyncPosition;

    // the last summary interval boundary that is fully readable in both data and index files
    private transient ReadableBoundary lastReadableBoundary;
    ...
    public static class ReadableBoundary {
        ...
        public final transient DecoratedKey lastKey;

        public final transient long indexLength;

        public final transient long dataLength;

        public final transient int summaryCount;

        public final transient long entriesLength;
    ...
    private void setNextSamplePosition(long position) {
        tryAgain: while (true) {
            ...
            for (int start : startPoints) if ((test - start) % BASE_SAMPLING_LEVEL == 0)
                continue tryAgain;
