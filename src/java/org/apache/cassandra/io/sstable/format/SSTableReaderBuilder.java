...
public abstract class SSTableReaderBuilder {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SSTableReaderBuilder.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SSTableReaderBuilder.class);

    private static final transient Logger logger = LoggerFactory.getLogger(SSTableReaderBuilder.class);

    protected final transient SSTableReader.Factory readerFactory;

    protected final transient Descriptor descriptor;

    protected final transient TableMetadataRef metadataRef;

    protected final transient TableMetadata metadata;

    protected final transient long maxDataAge;

    protected final transient Set<Component> components;

    protected final transient StatsMetadata statsMetadata;

    protected final transient SSTableReader.OpenReason openReason;

    protected final transient SerializationHeader header;

    protected transient IndexSummary summary;

    protected transient DecoratedKey first;

    protected transient DecoratedKey last;

    protected transient IFilter bf;

    protected transient FileHandle ifile;

    protected transient FileHandle dfile;
    ...
    void buildSummaryAndBloomFilter(boolean recreateBloomFilter, boolean summaryLoaded, Set<Component> components, StatsMetadata statsMetadata) throws IOException {
        ...
        try (RandomAccessReader primaryIndex = RandomAccessReader.open(new File(descriptor.filenameFor(Component.PRIMARY_INDEX)))) {
            ...
            long estimatedKeys = // statistics is supposed to be optional
            histogramCount > 0 && !statsMetadata.estimatedPartitionSize.isOverflowed() ? // statistics is supposed to be optional
            histogramCount : SSTable.estimateRowsFromIndex(primaryIndex, descriptor);
    ...
    public static class ForBatch extends SSTableReaderBuilder {
        ...
        public SSTableReader build() {
            ...
            try (FileHandle.Builder ibuilder = new FileHandle.Builder(descriptor.filenameFor(Component.PRIMARY_INDEX)).mmapped(DatabaseDescriptor.getIndexAccessMode() == Config.DiskAccessMode.mmap).withChunkCache(ChunkCache.instance);
                FileHandle.Builder dbuilder = new FileHandle.Builder(descriptor.filenameFor(Component.DATA)).compressed(compression).mmapped(DatabaseDescriptor.getDiskAccessMode() == Config.DiskAccessMode.mmap).withChunkCache(ChunkCache.instance)) {
    ...
    public static class ForRead extends SSTableReaderBuilder {
        ...
        private final transient ValidationMetadata validationMetadata;
        ...
        public SSTableReader build() {
            ...
            sstable.first = first;
            sstable.last = last;
            // Don't track hotness if we're offline.
        ...
        private void load(ValidationMetadata validation, boolean isOffline, Set<Component> components, DiskOptimizationStrategy optimizationStrategy, StatsMetadata statsMetadata) throws IOException {
            if (metadata.params.bloomFilterFpChance == 1.0) {
                ...
                bf = FilterFactory.AlwaysPresent;
            } else if (// What happens if filter component and primary index is missing?
        ...
        void load(boolean recreateBloomFilter, boolean saveSummaryIfCreated, DiskOptimizationStrategy optimizationStrategy, StatsMetadata statsMetadata, Set<Component> components) throws IOException {
            try (FileHandle.Builder ibuilder = new FileHandle.Builder(descriptor.filenameFor(Component.PRIMARY_INDEX)).mmapped(DatabaseDescriptor.getIndexAccessMode() == Config.DiskAccessMode.mmap).withChunkCache(ChunkCache.instance);
                FileHandle.Builder dbuilder = new FileHandle.Builder(descriptor.filenameFor(Component.DATA)).compressed(components.contains(Component.COMPRESSION_INFO)).mmapped(DatabaseDescriptor.getDiskAccessMode() == Config.DiskAccessMode.mmap).withChunkCache(ChunkCache.instance)) {
                loadSummary();
