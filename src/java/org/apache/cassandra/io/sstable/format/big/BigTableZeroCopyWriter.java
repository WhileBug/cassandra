...
public class BigTableZeroCopyWriter extends SSTable implements SSTableMultiWriter {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(BigTableZeroCopyWriter.class);

    private static final transient Logger logger = LoggerFactory.getLogger(BigTableZeroCopyWriter.class);

    private final transient TableMetadataRef metadata;

    private volatile transient SSTableReader finalReader;

    private final transient Map<Component.Type, SequentialWriter> componentWriters;

    private static final transient SequentialWriterOption WRITER_OPTION = SequentialWriterOption.newBuilder().trickleFsync(false).bufferSize(2 << 20).bufferType(BufferType.OFF_HEAP).build();

    private static final transient ImmutableSet<Component> SUPPORTED_COMPONENTS = ImmutableSet.of(Component.DATA, Component.PRIMARY_INDEX, Component.SUMMARY, Component.STATS, Component.COMPRESSION_INFO, Component.FILTER, Component.DIGEST, Component.CRC);
    ...
    public BigTableZeroCopyWriter(Descriptor descriptor, TableMetadataRef metadata, LifecycleNewTracker lifecycleNewTracker, final Collection<Component> components) {
        ...
        if (!SUPPORTED_COMPONENTS.containsAll(components))
            throw new AssertionError(format("Unsupported streaming component detected %s", Sets.difference(ImmutableSet.copyOf(components), SUPPORTED_COMPONENTS)));
    ...
    private void write(DataInputPlus in, long size, SequentialWriter out) throws FSWriteError {
        ...
        try {
            ...
            // finish will also call sync(). Leaving here to get stuff flushed as early as possible
            out.sync();
        } catch (IOException e) {
    ...
    public Collection<SSTableReader> finish(boolean openResult) {
        ...
        for (SequentialWriter writer : componentWriters.values()) writer.finish();
        return finished();
    }
    ...
    public Throwable commit(Throwable accumulate) {
        for (SequentialWriter writer : componentWriters.values()) accumulate = writer.commit(accumulate);
        return accumulate;
    }

    @Override
    public Throwable abort(Throwable accumulate) {
        for (SequentialWriter writer : componentWriters.values()) accumulate = writer.abort(accumulate);
        return accumulate;
    }

    @Override
    public void prepareToCommit() {
        for (SequentialWriter writer : componentWriters.values()) writer.prepareToCommit();
    }

    @Override
    public void close() {
        for (SequentialWriter writer : componentWriters.values()) writer.close();
    }
