...
public class SSTableLoader implements StreamEventHandler {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SSTableLoader.class);
    ...
    private final transient Set<InetAddressAndPort> failedHosts = new HashSet<>();

    private final transient List<SSTableReader> sstables = new ArrayList<>();

    private final transient Multimap<InetAddressAndPort, OutgoingStream> streamingDetails = HashMultimap.create();
    ...
    protected Collection<SSTableReader> openSSTables(final Map<InetAddressAndPort, Collection<Range<Token>>> ranges) {
        ...
        LifecycleTransaction.getFiles(directory.toPath(), (file, type) -> {
            ...
            // and it's likely we hit CASSANDRA-16235
            metadata == null && directory.getName().equals(Directories.BACKUPS_SUBDIR)) {
    ...
    public static abstract class Client {
        ...
        private final transient Map<InetAddressAndPort, Collection<Range<Token>>> endpointToRanges = new HashMap<>();
