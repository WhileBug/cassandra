...
public class Descriptor {
    ...
    private final static transient String LEGACY_TMP_REGEX_STR = "^((.*)\\-(.*)\\-)?tmp(link)?\\-((?:l|k).)\\-(\\d)*\\-(.*)$";

    private final static transient Pattern LEGACY_TMP_REGEX = Pattern.compile(LEGACY_TMP_REGEX_STR);

    public static transient String TMP_EXT = ".tmp";

    private static final transient Splitter filenameSplitter = Splitter.on('-');
    ...
    public final transient int generation;

    public final transient SSTableFormat.Type formatType;
