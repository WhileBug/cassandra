...
public class SSTableRewriter extends Transactional.AbstractTransactional implements Transactional {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SSTableRewriter.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SSTableRewriter.class);

    @VisibleForTesting
    public static transient boolean disableEarlyOpeningForTests = false;

    private final transient long preemptiveOpenInterval;

    private final transient long maxAge;

    private transient long repairedAt = -1;

    // the set of final readers we will expose on commit
    // the readers we are rewriting (updated as they are replaced)
    private final transient ILifecycleTransaction transaction;

    private final transient List<SSTableReader> preparedForCommit = new ArrayList<>();

    // the position (in MB) in the target file we last (re)opened at
    private transient long currentlyOpenedEarlyAt;

    private final transient List<SSTableWriter> writers = new ArrayList<>();

    // true if we do not want to obsolete the originals
    private final transient boolean keepOriginals;

    // true if the writer metadata should be released when switch is called
    private final transient boolean eagerWriterMetaRelease;

    private transient SSTableWriter writer;

    private transient Map<DecoratedKey, RowIndexEntry> cachedKeys = new HashMap<>();

    // for testing (TODO: remove when have byteman setup)
    private transient boolean throwEarly, throwLate;
    ...
    private static final class InvalidateKeys implements Runnable {
        ...
        final transient List<KeyCacheKey> cacheKeys = new ArrayList<>();

        final transient WeakReference<InstrumentingCache<KeyCacheKey, ?>> cacheRef;
