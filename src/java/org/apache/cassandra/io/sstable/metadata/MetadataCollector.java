...
public class MetadataCollector implements PartitionStatisticsCollector {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MetadataCollector.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MetadataCollector.class);

    public static final transient double NO_COMPRESSION_RATIO = -1.0;

    private static final transient ByteBuffer[] EMPTY_CLUSTERING = new ByteBuffer[0];
    ...
    public static StatsMetadata defaultStatsMetadata() {
        return new StatsMetadata(defaultPartitionSizeHistogram(), defaultCellPerPartitionCountHistogram(), IntervalSet.empty(), Long.MIN_VALUE, Long.MAX_VALUE, Integer.MAX_VALUE, Integer.MAX_VALUE, 0, Integer.MAX_VALUE, NO_COMPRESSION_RATIO, defaultTombstoneDropTimeHistogram(), 0, Collections.<ByteBuffer>emptyList(), Collections.<ByteBuffer>emptyList(), true, ActiveRepairService.UNREPAIRED_SSTABLE, -1, -1, null, null, false);
    }

    protected transient EstimatedHistogram estimatedPartitionSize = defaultPartitionSizeHistogram();

    // TODO: cound the number of row per partition (either with the number of cells, or instead)
    protected transient EstimatedHistogram estimatedCellPerPartitionCount = defaultCellPerPartitionCountHistogram();

    protected transient IntervalSet<CommitLogPosition> commitLogIntervals = IntervalSet.empty();

    protected final transient MinMaxLongTracker timestampTracker = new MinMaxLongTracker();

    protected final transient MinMaxIntTracker localDeletionTimeTracker = new MinMaxIntTracker(Cell.NO_DELETION_TIME, Cell.NO_DELETION_TIME);

    protected final transient MinMaxIntTracker ttlTracker = new MinMaxIntTracker(Cell.NO_TTL, Cell.NO_TTL);

    protected transient double compressionRatio = NO_COMPRESSION_RATIO;

    protected transient StreamingTombstoneHistogramBuilder estimatedTombstoneDropTime = new StreamingTombstoneHistogramBuilder(SSTable.TOMBSTONE_HISTOGRAM_BIN_SIZE, SSTable.TOMBSTONE_HISTOGRAM_SPOOL_SIZE, SSTable.TOMBSTONE_HISTOGRAM_TTL_ROUND_SECONDS);

    protected transient int sstableLevel;

    private transient ClusteringPrefix<?> minClustering = null;

    private transient ClusteringPrefix<?> maxClustering = null;

    protected transient boolean hasLegacyCounterShards = false;

    protected transient long totalColumnsSet;

    protected transient long totalRows;
     ...
     */
    protected transient ICardinality cardinality = new HyperLogLogPlus(13, 25);

    private final transient ClusteringComparator comparator;

    private final transient UUID originatingHostId;
    ...
    public static class MinMaxIntTracker {
        ...
        private final transient int defaultMin;

        private final transient int defaultMax;
