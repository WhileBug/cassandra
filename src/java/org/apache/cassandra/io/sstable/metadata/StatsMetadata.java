...
public class StatsMetadata extends MetadataComponent {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StatsMetadata.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StatsMetadata.class);

    public static final transient IMetadataComponentSerializer serializer = new StatsMetadataSerializer();

    public static final transient ISerializer<IntervalSet<CommitLogPosition>> commitLogPositionSetSerializer = IntervalSet.serializer(CommitLogPosition.serializer);

    public final transient EstimatedHistogram estimatedPartitionSize;

    public final transient EstimatedHistogram estimatedCellPerPartitionCount;

    public final transient IntervalSet<CommitLogPosition> commitLogIntervals;

    public final transient long minTimestamp;

    public final transient long maxTimestamp;

    public final transient int minLocalDeletionTime;

    public final transient int maxLocalDeletionTime;

    public final transient int minTTL;

    public final transient int maxTTL;

    public final transient double compressionRatio;

    public final transient TombstoneHistogram estimatedTombstoneDropTime;

    public final transient int sstableLevel;

    public final transient List<ByteBuffer> minClusteringValues;

    public final transient List<ByteBuffer> maxClusteringValues;

    public final transient boolean hasLegacyCounterShards;

    public final transient long repairedAt;

    public final transient long totalColumnsSet;

    public final transient long totalRows;

    public final transient UUID originatingHostId;

    public final transient UUID pendingRepair;

    public final transient boolean isTransient;

    // just holds the current encoding stats to avoid allocating - it is not serialized
    public final transient EncodingStats encodingStats;

    public StatsMetadata(EstimatedHistogram estimatedPartitionSize, EstimatedHistogram estimatedCellPerPartitionCount, IntervalSet<CommitLogPosition> commitLogIntervals, long minTimestamp, long maxTimestamp, int minLocalDeletionTime, int maxLocalDeletionTime, int minTTL, int maxTTL, double compressionRatio, TombstoneHistogram estimatedTombstoneDropTime, int sstableLevel, List<ByteBuffer> minClusteringValues, List<ByteBuffer> maxClusteringValues, boolean hasLegacyCounterShards, long repairedAt, long totalColumnsSet, long totalRows, UUID originatingHostId, UUID pendingRepair, boolean isTransient) {
    ...
    public static class StatsMetadataSerializer implements IMetadataComponentSerializer<StatsMetadata> {
        ...
        public int serializedSize(Version version, StatsMetadata component) throws IOException {
            ...
            // mix/max timestamp(long), min/maxLocalDeletionTime(int), min/max TTL, compressionRatio(double), repairedAt (long)
            ...
            // min column names
            size += 4;
            for (// with short length
            ByteBuffer value : // with short length
            component.minClusteringValues) size += 2 + value.remaining();
            // max column names
            size += 4;
            for (// with short length
            ByteBuffer value : // with short length
            component.maxClusteringValues) size += 2 + value.remaining();
            ...
            if (version.hasOriginatingHostId()) {
                // boolean: is originatingHostId present
                size += 1;
        ...
        public StatsMetadata deserialize(Version version, DataInputPlus in) throws IOException {
            ...
            if (partitionSizes.isOverflowed()) {
                logger.warn("Deserialized partition size histogram with {} values greater than the maximum of {}. " + "Clearing the overflow bucket to allow for degraded mean and percentile calculations...", partitionSizes.overflowCount(), partitionSizes.getLargestBucketOffset());
                partitionSizes.clearOverflow();
            }
            EstimatedHistogram columnCounts = EstimatedHistogram.serializer.deserialize(in);
            if (columnCounts.isOverflowed()) {
                logger.warn("Deserialized partition cell count histogram with {} values greater than the maximum of {}. " + "Clearing the overflow bucket to allow for degraded mean and percentile calculations...", columnCounts.overflowCount(), columnCounts.getLargestBucketOffset());
