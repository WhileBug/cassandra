...
public class CompressedSequentialWriter extends SequentialWriter {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CompressedSequentialWriter.class);
    ...
    private transient long chunkOffset = 0;

    // index file writer (random I/O)
    private final transient CompressionMetadata.Writer metadataWriter;

    private final transient ICompressor compressor;

    // used to store compressed data
    private transient ByteBuffer compressed;

    // holds a number of already written chunks
    private transient int chunkCount = 0;

    private transient long uncompressedSize = 0, compressedSize = 0;

    private final transient MetadataCollector sstableMetadataCollector;

    private final transient ByteBuffer crcCheckBuffer = ByteBuffer.allocate(4);
    ...
    protected void flushData() {
        ...
        if (compressedLength >= maxCompressedLength) {
            ...
            if (uncompressedLength >= maxCompressedLength) {
                ...
                // verified by CompressionParams.validate
                assert maxCompressedLength <= buffer.capacity();
    ...
    protected static class CompressedFileWriterMark implements DataPosition {
        ...
        final transient long chunkOffset;

        // uncompressed data offset (real data offset)
        final transient long uncDataOffset;

        final transient int validBufferBytes;

        final transient int nextChunkIndex;
