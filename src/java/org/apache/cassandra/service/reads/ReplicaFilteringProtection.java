...
public class ReplicaFilteringProtection<E extends Endpoints<E>> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ReplicaFilteringProtection.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ReplicaFilteringProtection.class);

    private static final transient Logger logger = LoggerFactory.getLogger(ReplicaFilteringProtection.class);

    private static final transient NoSpamLogger oneMinuteLogger = NoSpamLogger.getLogger(logger, 1, TimeUnit.MINUTES);

    private static final transient Function<UnfilteredRowIterator, EncodingStats> NULL_TO_NO_STATS = rowIterator -> rowIterator == null ? EncodingStats.NO_STATS : rowIterator.stats();
    ...
    private final transient E sources;

    private final transient TableMetrics tableMetrics;

    private final transient int cachedRowsWarnThreshold;

    private final transient int cachedRowsFailThreshold;

    /**
     * Tracks whether or not we've already hit the warning threshold while evaluating a partition.
     */
    private transient boolean hitWarningThreshold = false;

    // tracks the current number of cached rows
    private transient int currentRowsCached = 0;

    // tracks the high watermark for the number of cached rows
    private transient int maxRowsCached = 0;

    /**
     * Per-source list of the pending partitions seen by the merge listener, to be merged with the extra fetched rows.
     */
    private final transient List<Queue<PartitionBuilder>> originalPartitions;

    ReplicaFilteringProtection(Keyspace keyspace, ReadCommand command, ConsistencyLevel consistency, long queryStartNanoTime, E sources, int cachedRowsWarnThreshold, int cachedRowsFailThreshold) {
    ...
    private UnfilteredPartitionIterator executeReadCommand(ReadCommand cmd, Replica source, ReplicaPlan.Shared<EndpointsForToken, ReplicaPlan.ForTokenRead> replicaPlan) {
        ...
        DataResolver<EndpointsForToken, ReplicaPlan.ForTokenRead> resolver = new DataResolver<>(cmd, replicaPlan, (NoopReadRepair<EndpointsForToken, ReplicaPlan.ForTokenRead>) NoopReadRepair.instance, queryStartNanoTime);
    ...
    private void incrementCachedRows() {
        ...
        if (currentRowsCached == cachedRowsFailThreshold + 1) {
            String message = String.format("Replica filtering protection has cached over %d rows during query %s. " + "(See 'cached_replica_rows_fail_threshold' in cassandra.yaml.)", cachedRowsFailThreshold, command.toCQLString());
            ...
            String message = String.format("Replica filtering protection has cached over %d rows during query %s. " + "(See 'cached_replica_rows_warn_threshold' in cassandra.yaml.)", cachedRowsWarnThreshold, command.toCQLString());
    ...
    private class PartitionBuilder {
        ...
        private final transient RegularAndStaticColumns columns;

        private final transient EncodingStats stats;

        private transient DeletionTime deletionTime;

        private transient Row staticRow = Rows.EMPTY_STATIC_ROW;

        private final transient Queue<Unfiltered> contents = new ArrayDeque<>();

        private transient BTreeSet.Builder<Clustering<?>> toFetch;

        private transient int partitionRowsCached;
        ...
        private UnfilteredPartitionIterator fetchFromSource() {
            ...
            if (logger.isTraceEnabled())
                logger.trace("Requesting rows {} in partition {} from {} for replica filtering protection", clusterings, key, source);
            Tracing.trace("Requesting {} rows in partition {} from {} for replica filtering protection", clusterings.size(), key, source);
            ...
            SinglePartitionReadCommand cmd = SinglePartitionReadCommand.create(command.metadata(), command.nowInSec(), command.columnFilter(), RowFilter.NONE, limits, key, filter);
