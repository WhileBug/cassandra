...
class ReplicaPlanMerger extends AbstractIterator<ReplicaPlan.ForRangeRead> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ReplicaPlanMerger.class);

    private final transient Keyspace keyspace;

    private final transient ConsistencyLevel consistency;

    private final transient PeekingIterator<ReplicaPlan.ForRangeRead> ranges;
    ...
    protected ReplicaPlan.ForRangeRead computeNext() {
        ...
        while (ranges.hasNext()) {
            ...
            if (merged == null)
                break;
            current = merged;
            // consume the range we just merged since we've only peeked so far
            ranges.next();
        }
        return current;
    }
