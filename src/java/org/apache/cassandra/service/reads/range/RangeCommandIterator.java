...
class RangeCommandIterator extends AbstractIterator<RowIterator> implements PartitionIterator {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RangeCommandIterator.class);

    private static final transient Logger logger = LoggerFactory.getLogger(RangeCommandIterator.class);

    private static final transient ClientRequestMetrics rangeMetrics = new ClientRequestMetrics("RangeSlice");

    private final transient CloseableIterator<ReplicaPlan.ForRangeRead> replicaPlans;

    private final transient int totalRangeCount;
    ...
    private transient DataLimits.Counter counter;

    private transient PartitionIterator sentQueryIterator;

    private final transient int maxConcurrencyFactor;

    private transient int concurrencyFactor;

    // The two following "metric" are maintained to improve the concurrencyFactor
    // when it was not good enough initially.
    private transient int liveReturned;

    private transient int rangesQueried;

    private transient int batchesRequested = 0;
    ...
    private PartitionIterator sendNextRequests() {
        ...
        try {
            for (int i = 0; i < concurrencyFactor && replicaPlans.hasNext(); ) {
                ...
                // response will be closed by concatAndBlockOnRepair, or in the catch block below
