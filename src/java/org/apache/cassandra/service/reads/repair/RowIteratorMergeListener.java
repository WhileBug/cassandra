...
public class RowIteratorMergeListener<E extends Endpoints<E>> implements UnfilteredRowIterators.MergeListener {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RowIteratorMergeListener.class);
    ...
    private final transient BitSet writeBackTo;

    private final transient boolean buildFullDiff;

    /**
     * the repairs we will send to each source, suffixed by a complete repair of all differences, if {@link #buildFullDiff}
     */
    private final transient PartitionUpdate.Builder[] repairs;

    private final transient Row.Builder[] currentRows;

    private final transient RowDiffListener diffListener;

    private final transient ReplicaPlan.ForRead<E> readPlan;
    ...
    private transient DeletionTime mergedDeletionTime;

    // For each source, the time of the current deletion as known by the source.
    private final transient DeletionTime[] sourceDeletionTime;

    // For each source, record if there is an open range to send as repair, and from where.
    private final transient ClusteringBound<?>[] markerToRepair;
