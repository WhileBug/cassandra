...
public abstract class AbstractReadExecutor {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AbstractReadExecutor.class);

    private static final transient Logger logger = LoggerFactory.getLogger(AbstractReadExecutor.class);

    protected final transient ReadCommand command;

    private final transient ReplicaPlan.SharedForTokenRead replicaPlan;

    protected final transient ReadRepair<EndpointsForToken, ReplicaPlan.ForTokenRead> readRepair;

    protected final transient DigestResolver<EndpointsForToken, ReplicaPlan.ForTokenRead> digestResolver;

    protected final transient ReadCallback<EndpointsForToken, ReplicaPlan.ForTokenRead> handler;

    protected final transient TraceState traceState;

    protected final transient ColumnFamilyStore cfs;

    protected final transient long queryStartNanoTime;

    private final transient int initialDataRequestCount;

    protected volatile transient PartitionIterator result = null;
    ...
    AbstractReadExecutor(ColumnFamilyStore cfs, ReadCommand command, ReplicaPlan.ForTokenRead replicaPlan, int initialDataRequestCount, long queryStartNanoTime) {
        ...
        for (Replica replica : replicaPlan.contacts()) digestVersion = Math.min(digestVersion, MessagingService.instance().versions.get(replica.endpoint()));
        command.setDigestVersion(digestVersion);
    }

    public DecoratedKey getKey() {
        Preconditions.checkState(command instanceof SinglePartitionReadCommand, "Can only get keys for SinglePartitionReadCommand");
    ...
    public static class NeverSpeculatingReadExecutor extends AbstractReadExecutor {
         ...
         */
        private final transient boolean logFailedSpeculation;
    ...
    static class SpeculatingReadExecutor extends AbstractReadExecutor {
        ...
        private volatile transient boolean speculated = false;

        public SpeculatingReadExecutor(ColumnFamilyStore cfs, ReadCommand command, ReplicaPlan.ForTokenRead replicaPlan, long queryStartNanoTime) {
        ...
        public void maybeTryAdditionalReplicas() {
            if (shouldSpeculateAndMaybeWait()) {
                ...
                if (handler.resolver.isDataPresent()) {
                    ...
                    retryCommand = extraReplica.isTransient() ? command.copyAsTransientQuery(extraReplica) : command.copyAsDigestQuery(extraReplica);
                } else {
    ...
    private static class AlwaysSpeculatingReadExecutor extends AbstractReadExecutor {
        ...
        public AlwaysSpeculatingReadExecutor(ColumnFamilyStore cfs, ReadCommand command, ReplicaPlan.ForTokenRead replicaPlan, long queryStartNanoTime) {
