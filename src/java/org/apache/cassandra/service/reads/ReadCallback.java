...
public class ReadCallback<E extends Endpoints<E>, P extends ReplicaPlan.ForRead<E>> implements RequestCallback<ReadResponse> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ReadCallback.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ReadCallback.class);

    protected static final transient Logger logger = LoggerFactory.getLogger(ReadCallback.class);

    public final transient ResponseResolver<E, P> resolver;

    final transient SimpleCondition condition = new SimpleCondition();

    private final transient long queryStartNanoTime;

    // TODO: move to replica plan as well?
    final transient int blockFor;
    ...
    final transient ReplicaPlan.Shared<E, P> replicaPlan;

    private final transient ReadCommand command;

    private static final transient AtomicIntegerFieldUpdater<ReadCallback> failuresUpdater = AtomicIntegerFieldUpdater.newUpdater(ReadCallback.class, "failures");
    ...
    private void assertWaitingFor(InetAddressAndPort from) {
        assert !replicaPlan().consistencyLevel().isDatacenterLocal() || DatabaseDescriptor.getLocalDataCenter().equals(DatabaseDescriptor.getEndpointSnitch().getDatacenter(from)) : "Received read response from unexpected replica: " + from;
    }
}
