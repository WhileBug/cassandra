...
public class StartupChecks {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StartupChecks.class);

    private static final transient Logger logger = LoggerFactory.getLogger(StartupChecks.class);

    // List of checks to run before starting up. If any test reports failure, startup will be halted.
    private final transient List<StartupCheck> preFlightChecks = new ArrayList<>();
    ...
    private final transient List<StartupCheck> DEFAULT_TESTS = ImmutableList.of(checkJemalloc, checkLz4Native, checkValidLaunchDate, checkJMXPorts, checkJMXProperties, inspectJvmOptions, checkNativeLibraryInitialization, initSigarLibrary, checkMaxMapCount, checkDataDirs, checkSSTablesFormat, checkSystemKeyspaceState, checkDatacenter, checkRack, checkLegacyAuthTables);
    ...
    public void verify() throws StartupException {
        for (StartupCheck test : preFlightChecks) test.execute();
    }

    public static final transient StartupCheck checkJemalloc = new StartupCheck() {
    ...
    public static final transient StartupCheck checkLz4Native = () -> {
        try {
            // make sure native loads
    ...
    public static final transient StartupCheck checkValidLaunchDate = new StartupCheck() {
        ...
        public void execute() throws StartupException {
            ...
            if (now < EARLIEST_LAUNCH_DATE)
                throw new StartupException(StartupException.ERR_WRONG_MACHINE_STATE, String.format("current machine time is %s, but that is seemingly incorrect. exiting now.", new Date(now).toString()));
        }
    };

    public static final transient StartupCheck checkJMXPorts = new StartupCheck() {
    ...
    public static final transient StartupCheck checkJMXProperties = new StartupCheck() {
    ...
    public static final transient StartupCheck inspectJvmOptions = new StartupCheck() {
        ...
        private boolean jvmOptionsContainsOneOf(String... optionNames) {
            ...
            for (String argument : inputArguments) {
                for (String optionName : optionNames) if (argument.startsWith(optionName))
                    return true;
            }
            return false;
        }
    ...
    public static final transient StartupCheck checkNativeLibraryInitialization = new StartupCheck() {
    ...
    public static final transient StartupCheck initSigarLibrary = new StartupCheck() {
    ...
    public static final transient StartupCheck checkMaxMapCount = new StartupCheck() {
        ...
        public void execute() {
            ...
            if (DatabaseDescriptor.getDiskAccessMode() == Config.DiskAccessMode.standard && DatabaseDescriptor.getIndexAccessMode() == Config.DiskAccessMode.standard)
                // no need to check if disk access mode is only standard and not mmap
                return;
            long maxMapCount = getMaxMapCount();
            if (maxMapCount < EXPECTED_MAX_MAP_COUNT)
                logger.warn("Maximum number of memory map areas per process (vm.max_map_count) {} " + "is too low, recommended value: {}, you can change it with sysctl.", maxMapCount, EXPECTED_MAX_MAP_COUNT);
        }
    };

    public static final transient StartupCheck checkDataDirs = () -> {
        // check all directories(data, commitlog, saved cache) for existence and permission
        Iterable<String> dirs = Iterables.concat(Arrays.asList(DatabaseDescriptor.getAllDataFileLocations()), Arrays.asList(DatabaseDescriptor.getCommitLogLocation(), DatabaseDescriptor.getSavedCachesLocation(), DatabaseDescriptor.getHintsDirectory().getAbsolutePath()));
        for (String dataDir : dirs) {
            ...
            if (!dir.exists()) {
                ...
                if (!dir.mkdirs())
                    throw new StartupException(StartupException.ERR_WRONG_DISK_STATE, "Has no permission to create directory " + dataDir);
            }
            // if directories exist verify their permissions
            if (!Directories.verifyFullPermissions(dir, dataDir))
                throw new StartupException(StartupException.ERR_WRONG_DISK_STATE, "Insufficient permissions on directory " + dataDir);
        }
    };

    public static final transient StartupCheck checkSSTablesFormat = new StartupCheck() {
        ...
        public void execute() throws StartupException {
            ...
            FileVisitor<Path> sstableVisitor = new SimpleFileVisitor<Path>() {
                ...
                public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) throws IOException {
                    ...
                    return (name.equals(Directories.SNAPSHOT_SUBDIR) || name.equals(Directories.BACKUPS_SUBDIR) || nonSSTablePaths.contains(dir.toFile().getCanonicalPath())) ? FileVisitResult.SKIP_SUBTREE : FileVisitResult.CONTINUE;
                }
            };
            ...
            if (!invalid.isEmpty())
                throw new StartupException(StartupException.ERR_WRONG_DISK_STATE, String.format("Detected unreadable sstables %s, please check " + "NEWS.txt and ensure that you have upgraded through " + "all required intermediate versions, running " + "upgradesstables", Joiner.on(",").join(invalid)));
        }
    };

    public static final transient StartupCheck checkSystemKeyspaceState = new StartupCheck() {

        public void execute() throws StartupException {
            // check the system keyspace to keep user from shooting self in foot by changing partitioner, cluster name, etc.
            // we do a one-off scrub of the system keyspace first; we can't load the list of the rest of the keyspaces,
            // until system keyspace is opened.
            for (TableMetadata cfm : Schema.instance.getTablesAndViews(SchemaConstants.SYSTEM_KEYSPACE_NAME)) ColumnFamilyStore.scrubDataDirectories(cfm);
            try {
    ...
    public static final transient StartupCheck checkDatacenter = new StartupCheck() {
    ...
    public static final transient StartupCheck checkRack = new StartupCheck() {
    ...
    public static final transient StartupCheck checkLegacyAuthTables = () -> {
    ...
    static Optional<String> checkLegacyAuthTablesMessage() {
        List<String> existing = new ArrayList<>(SchemaConstants.LEGACY_AUTH_TABLES).stream().filter((legacyAuthTable) -> {
            UntypedResultSet result = QueryProcessor.executeOnceInternal(String.format("SELECT table_name FROM %s.%s WHERE keyspace_name='%s' AND table_name='%s'", SchemaConstants.SCHEMA_KEYSPACE_NAME, "tables", SchemaConstants.AUTH_KEYSPACE_NAME, legacyAuthTable));
            return result != null && !result.isEmpty();
        }).collect(Collectors.toList());
        if (!existing.isEmpty())
            return Optional.of(String.format("Legacy auth tables %s in keyspace %s still exist and have not been properly migrated.", Joiner.on(", ").join(existing), SchemaConstants.AUTH_KEYSPACE_NAME));
        else
