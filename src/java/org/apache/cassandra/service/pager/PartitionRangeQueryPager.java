...
public class PartitionRangeQueryPager extends AbstractQueryPager<PartitionRangeReadQuery> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(PartitionRangeQueryPager.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(PartitionRangeQueryPager.class);

    private volatile transient DecoratedKey lastReturnedKey;

    private volatile transient PagingState.RowMark lastReturnedRow;
    ...
    public PartitionRangeQueryPager withUpdatedLimit(DataLimits newLimits) {
        return new PartitionRangeQueryPager(query.withUpdatedLimit(newLimits), protocolVersion, lastReturnedKey, lastReturnedRow, maxRemaining(), remainingInPartition());
    }

    public PagingState state() {
        return lastReturnedKey == null ? null : new PagingState(lastReturnedKey.getKey(), lastReturnedRow, maxRemaining(), remainingInPartition());
    }
    ...
    protected PartitionRangeReadQuery nextPageReadQuery(int pageSize) {
        ...
        } else // if the last key was the one of the end of the range we know that we are done
        if (lastReturnedKey.equals(fullRange.keyRange().right) && remainingInPartition() == 0 && lastReturnedRow == null) {
            return null;
        } else {
    ...
    private AbstractBounds<PartitionPosition> makeKeyBounds(PartitionPosition lastReturnedKey, boolean includeLastKey) {
        ...
        if (bounds instanceof Range || bounds instanceof Bounds) {
            return includeLastKey ? new Bounds<>(lastReturnedKey, bounds.right) : new Range<>(lastReturnedKey, bounds.right);
        }
        return includeLastKey ? new IncludingExcludingBounds<>(lastReturnedKey, bounds.right) : new ExcludingBounds<>(lastReturnedKey, bounds.right);
    }
}
