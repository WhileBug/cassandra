...
public class PagingState {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(PagingState.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(PagingState.class);

    // Can be null for single partition queries.
    public final transient ByteBuffer partitionKey;

    // Can be null if not needed.
    public final transient RowMark rowMark;

    public final transient int remaining;

    public final transient int remainingInPartition;
    ...
    public static PagingState deserialize(ByteBuffer bytes, ProtocolVersion protocolVersion) {
        ...
        try {
            ...
            if (protocolVersion.isGreaterThan(ProtocolVersion.V3)) {
                if (isModernSerialized(bytes))
                    return modernDeserialize(bytes, protocolVersion);
                if (isLegacySerialized(bytes))
                    return legacyDeserialize(bytes, ProtocolVersion.V3);
            }
            if (protocolVersion.isSmallerThan(ProtocolVersion.V4)) {
                if (isLegacySerialized(bytes))
                    return legacyDeserialize(bytes, protocolVersion);
                if (isModernSerialized(bytes))
                    return modernDeserialize(bytes, ProtocolVersion.V4);
            }
        } catch (IOException e) {
    ...
    private int modernSerializedSize() {
        return serializedSizeWithVIntLength(null == partitionKey ? EMPTY_BYTE_BUFFER : partitionKey) + serializedSizeWithVIntLength(null == rowMark ? EMPTY_BYTE_BUFFER : rowMark.mark) + sizeofUnsignedVInt(remaining) + sizeofUnsignedVInt(remainingInPartition);
    }
    ...
    int legacySerializedSize(boolean withRemainingInPartition) {
        return serializedSizeWithShortLength(null == partitionKey ? EMPTY_BYTE_BUFFER : partitionKey) + serializedSizeWithShortLength(null == rowMark ? EMPTY_BYTE_BUFFER : rowMark.mark) + sizeof(remaining) + (withRemainingInPartition ? sizeof(remainingInPartition) : 0);
    }
    ...
    public String toString() {
        return String.format("PagingState(key=%s, cellname=%s, remaining=%d, remainingInPartition=%d", partitionKey != null ? bytesToHex(partitionKey) : null, rowMark, remaining, remainingInPartition);
    }
    ...
    public static class RowMark {
        ...
        private final transient ByteBuffer mark;
        ...
        private static List<AbstractType<?>> makeClusteringTypes(TableMetadata metadata) {
            ...
            for (int i = 0; i < size; i++) l.add(BytesType.instance);
            return l;
        }
        ...
        public Clustering<?> clustering(TableMetadata metadata) {
            if (mark == null)
                return null;
            return protocolVersion.isSmallerOrEqualTo(ProtocolVersion.V3) ? decodeClustering(metadata, mark) : Clustering.serializer.deserialize(mark, MessagingService.VERSION_30, makeClusteringTypes(metadata));
        }
