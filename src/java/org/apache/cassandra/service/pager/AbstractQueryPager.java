...
abstract class AbstractQueryPager<T extends ReadQuery> implements QueryPager {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AbstractQueryPager.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AbstractQueryPager.class);

    protected final transient T query;

    protected final transient DataLimits limits;

    protected final transient ProtocolVersion protocolVersion;
    ...
    private transient int remainingInPartition;
    ...
    private abstract class Pager<T extends Unfiltered> extends Transformation<BaseRowIterator<T>> {
        ...
        private final transient DataLimits pageLimits;

        protected final transient DataLimits.Counter counter;

        private transient DecoratedKey currentKey;

        private transient Row lastRow;

        private transient boolean isFirstPartition = true;
        ...
        public void onClose() {
            ...
            if (lastRow != null && (lastRow.clustering() == Clustering.STATIC_CLUSTERING || lastRow.clustering().isEmpty())) {
