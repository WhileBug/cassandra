...
public class GossipDigestSynVerbHandler extends GossipVerbHandler<GossipDigestSyn> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(GossipDigestSynVerbHandler.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(GossipDigestSynVerbHandler.class);

    public static final transient GossipDigestSynVerbHandler instance = new GossipDigestSynVerbHandler();

    private static final transient Logger logger = LoggerFactory.getLogger(GossipDigestSynVerbHandler.class);
    ...
    public void doVerb(Message<GossipDigestSyn> message) {
        ...
        if (!Gossiper.instance.isEnabled() && Gossiper.instance.isInShadowRound()) {
            ...
            MessagingService.instance().send(Message.out(GOSSIP_DIGEST_ACK, new GossipDigestAck(Collections.emptyList(), Collections.emptyMap())), from);
            return;
        }
        if (logger.isTraceEnabled()) {
        ...
        Message<GossipDigestAck> gDigestAckMessage = gDigestList.isEmpty() ? createShadowReply() : createNormalReply(gDigestList);
        if (logger.isTraceEnabled())
