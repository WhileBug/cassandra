...
public class EndpointState {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(EndpointState.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(EndpointState.class);

    protected static final transient Logger logger = LoggerFactory.getLogger(EndpointState.class);

    public final static transient IVersionedSerializer<EndpointState> serializer = new EndpointStateSerializer();

    private volatile transient HeartBeatState hbState;

    private final transient AtomicReference<Map<ApplicationState, VersionedValue>> applicationState;

    /* fields below do not get serialized */
    private volatile transient long updateTimestamp;

    private volatile transient boolean isAlive;
    ...
    void setHeartBeatState(HeartBeatState newHbState) {
        updateTimestamp();
        hbState = newHbState;
        logger_IC.info("[InconsistencyDetector][org.apache.cassandra.gms.EndpointState.hbState]=" + org.json.simple.JSONValue.toJSONString(hbState).replace("\n", "").replace("\r", ""));
        logger_IC.info("[InconsistencyDetector][org.apache.cassandra.gms.EndpointState.hbState]=" + org.json.simple.JSONValue.toJSONString(hbState).replace("\n", "").replace("\r", ""));
    }
    ...
    void removeMajorVersion3LegacyApplicationStates() {
        while (hasLegacyFields()) {
            ...
            if (orig.size() == updatedStates.size() || applicationState.compareAndSet(orig, updatedStates))
                return;
        }
    }
...
class EndpointStateSerializer implements IVersionedSerializer<EndpointState> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(EndpointStateSerializer.class);
