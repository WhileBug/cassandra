...
public class SecondaryIndexManager implements IndexRegistry, INotificationConsumer {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SecondaryIndexManager.class);

    private static final transient Logger logger = LoggerFactory.getLogger(SecondaryIndexManager.class);
     ...
     */
    private final transient Map<String, Index> indexes = Maps.newConcurrentMap();

    /**
     * The indexes that had a build failure.
     */
    private final transient Set<String> needsFullRebuild = Sets.newConcurrentHashSet();

    /**
     * The indexes that are available for querying.
     */
    private final transient Set<String> queryableIndexes = Sets.newConcurrentHashSet();

    /**
     * The indexes that are available for writing.
     */
    private final transient Map<String, Index> writableIndexes = Maps.newConcurrentMap();

    /**
     * The count of pending index builds for each index.
     */
    private final transient Map<String, AtomicInteger> inProgressBuilds = Maps.newConcurrentMap();

    // executes tasks returned by Indexer#addIndexColumn which may require index(es) to be (re)built
    private static final transient ListeningExecutorService asyncExecutor = MoreExecutors.listeningDecorator(new JMXEnabledThreadPoolExecutor(1, Stage.KEEP_ALIVE_SECONDS, TimeUnit.SECONDS, new LinkedBlockingQueue<>(), new NamedThreadFactory("SecondaryIndexManagement"), "internal"));

    // executes all blocking tasks produced by Indexers e.g. getFlushTask, getMetadataReloadTask etc
    private static final transient ListeningExecutorService blockingExecutor = MoreExecutors.newDirectExecutorService();

    /**
     * The underlying column family containing the source data for these indexes
     */
    public final transient ColumnFamilyStore baseCfs;
    ...
    public Set<IndexMetadata> getDependentIndexes(ColumnMetadata column) {
        ...
        for (Index index : indexes.values()) if (index.dependsOn(column))
    ...
    public void validate(PartitionUpdate update) throws InvalidRequestException {
        for (Index index : indexes.values()) index.validate(update);
    }
    ...
    private static final class WriteTimeTransaction implements UpdateTransaction {
        ...
        private final transient Index.Indexer[] indexers;
        ...
        public void start() {
            for (Index.Indexer indexer : indexers) indexer.begin();
        }

        public void onPartitionDeletion(DeletionTime deletionTime) {
            for (Index.Indexer indexer : indexers) indexer.partitionDelete(deletionTime);
        }

        public void onRangeTombstone(RangeTombstone tombstone) {
            for (Index.Indexer indexer : indexers) indexer.rangeTombstone(tombstone);
        }

        public void onInserted(Row row) {
            for (Index.Indexer indexer : indexers) indexer.insertRow(row);
        }
        ...
        public void commit() {
            for (Index.Indexer indexer : indexers) indexer.finish();
        }
    ...
    private static final class IndexGCTransaction implements CompactionTransaction {
        ...
        private final transient Collection<Index> indexes;
        ...
        public void onRowMerge(Row merged, Row... versions) {
            ...
            for (int i = 0; i < builders.length; i++) if (builders[i] != null)
    ...
    private static final class CleanupGCTransaction implements CleanupTransaction {
        ...
        private transient DeletionTime partitionDelete;
    ...
    public void handleNotification(INotification notification, Object sender) {
        if (!indexes.isEmpty() && notification instanceof SSTableAddedNotification) {
            ...
            if (!notice.memtable().isPresent())
                buildIndexesBlocking(Lists.newArrayList(notice.added), indexes.values().stream().filter(Index::shouldBuildBlocking).collect(Collectors.toSet()), false);
        }
    }
