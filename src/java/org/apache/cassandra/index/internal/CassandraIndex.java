...
public abstract class CassandraIndex implements Index {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CassandraIndex.class);

    private static final transient Logger logger = LoggerFactory.getLogger(CassandraIndex.class);

    public final transient ColumnFamilyStore baseCfs;

    protected transient IndexMetadata metadata;

    protected transient ColumnFamilyStore indexCfs;

    protected transient ColumnMetadata indexedColumn;

    protected transient CassandraIndexFunctions functions;
     ...
     */
    protected abstract <T> CBuilder buildIndexClusteringPrefix(ByteBuffer partitionKey, ClusteringPrefix<T> prefix, CellPath path);
    ...
    public Index.Searcher searcherFor(ReadCommand command) {
        ...
        if (target.isPresent()) {
            switch(getIndexMetadata().kind) {
                ...
                default:
                    throw new IllegalStateException(String.format("Unsupported index type %s for index %s on %s", metadata.kind, metadata.name, indexedColumn.name.toString()));
            }
        }
        return null;
    }
    ...
    public Indexer indexerFor(final DecoratedKey key, final RegularAndStaticColumns columns, final int nowInSec, final WriteContext ctx, final IndexTransaction.Type transactionType) {
        /**
        ...
        return new Indexer() {
            ...
            public void updateRow(Row oldRow, Row newRow) {
                ...
                if (newRow.isStatic() != indexedColumn.isStatic())
                    return;
                if (isPrimaryKeyIndex())
                    indexPrimaryKey(newRow.clustering(), getPrimaryKeyIndexLiveness(newRow), newRow.deletion());
                if (indexedColumn.isComplex()) {
            ...
            private void indexCell(Clustering<?> clustering, Cell<?> cell) {
                if (cell == null || !cell.isLive(nowInSec))
                    return;
                insert(key.getKey(), clustering, cell, LivenessInfo.withExpirationTime(cell.timestamp(), cell.ttl(), cell.localDeletionTime()), ctx);
            }
    ...
    private void validateIndexedValue(ByteBuffer value) {
        if (value != null && value.remaining() >= FBUtilities.MAX_UNSIGNED_SHORT)
            throw new InvalidRequestException(String.format("Cannot index value of size %d for index %s on %s(%s) (maximum allowed size=%d)", value.remaining(), metadata.name, baseCfs.metadata, indexedColumn.name.toString(), FBUtilities.MAX_UNSIGNED_SHORT));
    }
