...
public class TermIterator extends RangeIterator<Long, Token> {
    ...
    private static final transient Logger logger = LoggerFactory.getLogger(TermIterator.class);

    private static final transient FastThreadLocal<ExecutorService> SEARCH_EXECUTOR = new FastThreadLocal<ExecutorService>() {
        ...
        public ExecutorService initialValue() {
            ...
            return (concurrencyFactor <= 1) ? MoreExecutors.newDirectExecutorService() : Executors.newFixedThreadPool(concurrencyFactor, new ThreadFactory() {
    ...
    private final transient RangeIterator<Long, Token> union;

    private final transient Set<SSTableIndex> referencedIndexes;
