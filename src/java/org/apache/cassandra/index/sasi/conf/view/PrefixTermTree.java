...
public class PrefixTermTree extends RangeTermTree {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(PrefixTermTree.class);

    private final transient OnDiskIndexBuilder.Mode mode;

    private final transient Trie<ByteBuffer, Set<SSTableIndex>> trie;
    ...
    public Set<SSTableIndex> search(Expression e) {
        Map<ByteBuffer, Set<SSTableIndex>> indexes = (e == null || e.lower == null || mode == OnDiskIndexBuilder.Mode.CONTAINS) ? trie : trie.prefixMap(e.lower.value);
    ...
    public static class Builder extends RangeTermTree.Builder {
        ...
        private final transient PatriciaTrie<ByteBuffer, Set<SSTableIndex>> trie;
    ...
    private static class ByteBufferKeyAnalyzer implements KeyAnalyzer<ByteBuffer> {
         ...
         */
        private static final transient int MSB = 1 << Byte.SIZE - 1;
