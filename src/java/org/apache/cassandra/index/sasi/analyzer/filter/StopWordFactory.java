...
public class StopWordFactory {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StopWordFactory.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StopWordFactory.class);

    private static final transient Logger logger = LoggerFactory.getLogger(StopWordFactory.class);

    private static final transient String DEFAULT_RESOURCE_EXT = "_ST.txt";

    private static final transient String DEFAULT_RESOURCE_PREFIX = StopWordFactory.class.getPackage().getName().replace(".", File.separator);

    private static final transient Set<String> SUPPORTED_LANGUAGES = new HashSet<>(Arrays.asList("ar", "bg", "cs", "de", "en", "es", "fi", "fr", "hi", "hu", "it", "pl", "pt", "ro", "ru", "sv"));

    private static final transient LoadingCache<String, Set<String>> STOP_WORDS_CACHE = Caffeine.newBuilder().executor(MoreExecutors.directExecutor()).build(StopWordFactory::getStopWordsFromResource);
