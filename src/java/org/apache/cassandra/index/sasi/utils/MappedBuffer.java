...
public class MappedBuffer implements Closeable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MappedBuffer.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(MappedBuffer.class);

    private final transient MappedByteBuffer[] pages;

    private transient long position, limit;

    private final transient long capacity;

    private final transient int pageSize, sizeBits;
    ...
    public void close() {
         ...
         */
        try {
            for (MappedByteBuffer segment : pages) FileUtils.clean(segment);
        } catch (Exception e) {
