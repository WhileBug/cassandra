...
public abstract class AbstractTokenTreeBuilder implements TokenTreeBuilder {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(AbstractTokenTreeBuilder.class);

    protected transient int numBlocks;

    protected transient Node root;

    protected transient InteriorNode rightmostParent;

    protected transient Leaf leftmostLeaf;

    protected transient Leaf rightmostLeaf;

    protected transient long tokenCount = 0;

    protected transient long treeMinToken;

    protected transient long treeMaxToken;
    ...
    public int serializedSize() {
        if (numBlocks == 1)
            return BLOCK_HEADER_BYTES + ((int) tokenCount * BLOCK_ENTRY_BYTES) + (((Leaf) root).overflowCollisionCount() * OVERFLOW_ENTRY_BYTES);
        else
    ...
    protected abstract class Node {
        ...
        protected transient InteriorNode parent;

        protected transient Node next;

        protected transient Long nodeMinToken, nodeMaxToken;
        ...
        private abstract class Header {
            ...
            public void serialize(ByteBuffer buf) {
                buf.put(infoByte()).putShort((short) (tokenCount())).putLong(nodeMinToken).putLong(nodeMaxToken);
            }
        ...
        private class RootHeader extends Header {
            ...
            public void serialize(ByteBuffer buf) {
                super.serialize(buf);
                writeMagic(buf);
                buf.putLong(tokenCount).putLong(treeMinToken).putLong(treeMaxToken);
            }
    ...
    protected abstract class Leaf extends Node {
        ...
        protected transient LongArrayList overflowCollisions;
        ...
        protected void serializeOverflowCollisions(ByteBuffer buf) {
            if (overflowCollisions != null)
                for (LongCursor offset : overflowCollisions) buf.putLong(offset.value);
        }
        ...
        protected LeafEntry createEntry(final long tok, final LongSet offsets) {
            ...
            switch(offsetCount) {
                ...
                case 2:
                    ...
                    if (rawOffsets[0] <= Integer.MAX_VALUE && rawOffsets[1] <= Integer.MAX_VALUE && (rawOffsets[0] <= Short.MAX_VALUE || rawOffsets[1] <= Short.MAX_VALUE))
        ...
        protected abstract class LeafEntry {
            ...
            protected final transient long token;
            ...
            public void serialize(ByteBuffer buf) {
                buf.putShort((short) type().ordinal()).putShort(offsetExtra()).putLong(token).putInt(offsetData());
            }
        }
        ...
        private class PackedCollisionLeafEntry extends LeafEntry {
            ...
            private transient short smallerOffset;

            private transient int largerOffset;
        ...
        private class OverflowCollisionLeafEntry extends LeafEntry {
            ...
            private final transient short startIndex;

            private final transient short count;
    ...
    protected class InteriorNode extends Node {
        ...
        protected transient List<Long> tokens = new ArrayList<>(TOKENS_PER_BLOCK);

        protected transient List<Node> children = new ArrayList<>(TOKENS_PER_BLOCK + 1);
        ...
        private void serializeChildOffsets(long childBlockIndex, ByteBuffer buf) {
            for (int i = 0; i < children.size(); i++) buf.putLong((childBlockIndex + i) * BLOCK_BYTES);
        }
    }
