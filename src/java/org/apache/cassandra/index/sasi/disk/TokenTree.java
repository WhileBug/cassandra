...
public class TokenTree {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(TokenTree.class);

    private static final transient int LONG_BYTES = Long.SIZE / 8;

    private static final transient int SHORT_BYTES = Short.SIZE / 8;

    private final transient Descriptor descriptor;

    private final transient MappedBuffer file;

    private final transient long startPos;

    private final transient long treeMinToken;

    private final transient long treeMaxToken;

    private final transient long tokenCount;
    ...
    public class TokenTreeIterator extends RangeIterator<Long, Token> {
        ...
        private final transient Function<Long, DecoratedKey> keyFetcher;

        private final transient MappedBuffer file;

        private transient long currentLeafStart;

        private transient int currentTokenIndex;

        private transient long leafMinToken;

        private transient long leafMaxToken;

        private transient short leafSize;

        protected transient boolean firstIteration = true;

        private transient boolean lastLeaf;
        ...
        protected Token computeNext() {
            ...
            if (currentTokenIndex >= leafSize && lastLeaf)
                return endOfData();
            if (// tokens remaining in this leaf
            currentTokenIndex < leafSize) {
        ...
        protected void performSkipTo(Long nextToken) {
            maybeFirstIteration();
            if (// next is in this leaf block
            nextToken <= leafMaxToken) {
    ...
    public static class OnDiskToken extends Token {
        ...
        private final transient Set<TokenInfo> info = new HashSet<>(2);

        private final transient Set<DecoratedKey> loadedKeys = new TreeSet<>(DecoratedKey.comparator);
        ...
        public LongSet getOffsets() {
            ...
            for (TokenInfo i : info) {
                for (long offset : i.fetchOffsets()) offsets.add(offset);
            }
            return offsets;
        }
    ...
    private static class TokenInfo {
        ...
        private final transient MappedBuffer buffer;

        private final transient Function<Long, DecoratedKey> keyFetcher;

        private final transient long position;

        private final transient short leafSize;
        ...
        private long[] fetchOffsets() {
            ...
            switch(type) {
                ...
                case OVERFLOW:
                    // offsetShort contains count of tokens
    ...
    private static class KeyIterator extends AbstractIterator<DecoratedKey> {
        ...
        private final transient long[] offsets;
