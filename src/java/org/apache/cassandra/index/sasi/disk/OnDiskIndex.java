...
public class OnDiskIndex implements Iterable<OnDiskIndex.DataTerm>, Closeable {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(OnDiskIndex.class);
    ...
    public enum IteratorOrder {
        ...
        public int startAt(OnDiskBlock<DataTerm> block, Expression e) {
            switch(this) {
                case DESC:
                    return e.lower == null ? 0 : startAt(block.search(e.validator, e.lower.value), e.lower.inclusive);
                case ASC:
                    return e.upper == null ? block.termCount() - 1 : startAt(block.search(e.validator, e.upper.value), e.upper.inclusive);
                default:
        ...
        public int startAt(SearchResult<DataTerm> found, boolean inclusive) {
            switch(this) {
                ...
                case ASC:
                    if (// search term was bigger then whole data set
                    found.cmp < 0)
                        return found.index;
    ...
    public final transient Descriptor descriptor;

    protected final transient OnDiskIndexBuilder.Mode mode;

    protected final transient OnDiskIndexBuilder.TermSize termSize;

    protected final transient AbstractType<?> comparator;

    protected final transient MappedBuffer indexFile;

    protected final transient long indexSize;

    protected final transient boolean hasMarkedPartials;

    protected final transient Function<Long, DecoratedKey> keyFetcher;

    protected final transient String indexPath;

    protected final transient PointerLevel[] levels;

    protected final transient DataLevel dataLevel;

    protected final transient ByteBuffer minTerm, maxTerm, minKey, maxKey;
    ...
    public RangeIterator<Long, Token> search(Expression exp) {
        ...
        final Expression expression = (exp.getOp() != Op.NOT_EQ) ? exp : new Expression(exp).setOp(Op.RANGE).setLower(new Expression.Bound(minTerm, true)).setUpper(new Expression.Bound(maxTerm, true)).addExclusion(exp.lower.value);
    ...
    private RangeIterator<Long, Token> searchRange(Expression range) {
        ...
        int upperBlock = // optimization so we don't have to fetch upperBlock when query has lower == upper
        upper == null ? // optimization so we don't have to fetch upperBlock when query has lower == upper
        dataLevel.blockCount - 1 : (lower != null && comparator.compare(lower.value, upper.value) == 0) ? lowerBlock : getDataBlock(upper.value);
    ...
    protected class DataLevel extends Level<DataBlock> {
        ...
        protected final transient int superBlockCnt;

        protected final transient long superBlocksOffset;
    ...
    protected class OnDiskSuperBlock {
        ...
        private final transient TokenTree tokenTree;
    ...
    protected abstract class Level<T extends OnDiskBlock> {
        ...
        protected final transient long blockOffsets;

        protected final transient int blockCount;
    ...
    public class DataTerm extends Term implements Comparable<DataTerm> {
        ...
        private final transient TokenTree perBlockIndex;
    ...
    private static class PrefetchedTokensIterator extends RangeIterator<Long, Token> {
        ...
        private final transient NavigableMap<Long, Token> tokens;

        private transient PeekingIterator<Token> currentIterator;
    ...
    private class TermIterator extends AbstractIterator<DataTerm> {
        ...
        private final transient IteratorOrder order;

        protected transient OnDiskBlock<DataTerm> currentBlock;

        protected transient int blockIndex, offset;

        private transient boolean checkLower = true, checkUpper = true;
