...
public class OnDiskIndexBuilder {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(OnDiskIndexBuilder.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(OnDiskIndexBuilder.class);

    private static final transient Logger logger = LoggerFactory.getLogger(OnDiskIndexBuilder.class);

    public enum Mode {

        PREFIX(EnumSet.of(Op.EQ, Op.MATCH, Op.PREFIX, Op.NOT_EQ, Op.RANGE)), CONTAINS(EnumSet.of(Op.EQ, Op.MATCH, Op.CONTAINS, Op.PREFIX, Op.SUFFIX, Op.NOT_EQ)), SPARSE(EnumSet.of(Op.EQ, Op.NOT_EQ, Op.RANGE));
    ...
    public static final transient int BLOCK_SIZE = 4096;

    public static final transient int MAX_TERM_SIZE = 1024;

    public static final transient int SUPER_BLOCK_SIZE = 64;

    public static final transient int IS_PARTIAL_BIT = 15;

    private static final transient SequentialWriterOption WRITER_OPTION = SequentialWriterOption.newBuilder().bufferSize(BLOCK_SIZE).build();

    private final transient List<MutableLevel<InMemoryPointerTerm>> levels = new ArrayList<>();

    private transient MutableLevel<InMemoryDataTerm> dataLevel;

    private final transient TermSize termSize;

    private final transient AbstractType<?> keyComparator, termComparator;

    private final transient Map<ByteBuffer, TokenTreeBuilder> terms;

    private final transient Mode mode;

    private final transient boolean marksPartials;

    private transient ByteBuffer minKey, maxKey;

    private transient long estimatedBytes;
    ...
    protected void finish(Descriptor descriptor, Pair<ByteBuffer, ByteBuffer> range, File file, TermIterator terms) {
        ...
        try {
            ...
            for (// flush all of the buffers
            MutableLevel l : // flush all of the buffers
            levels) l.flush();
    ...
    protected static void alignToBlock(SequentialWriter out) throws IOException {
        ...
        if (// align on the block boundary if needed
        (endOfBlock & (BLOCK_SIZE - 1)) != 0)
            out.skipBytes((int) (FBUtilities.align(endOfBlock, BLOCK_SIZE) - endOfBlock));
    }

    private class InMemoryTerm {

        protected final transient IndexedTerm term;
    ...
    private class InMemoryPointerTerm extends InMemoryTerm {
        ...
        protected final transient int blockCnt;
    ...
    private class InMemoryDataTerm extends InMemoryTerm {
        ...
        private final transient TokenTreeBuilder keys;
    ...
    private class MutableLevel<T extends InMemoryTerm> {
        ...
        private final transient LongArrayList blockOffsets = new LongArrayList();

        protected final transient SequentialWriter out;

        private final transient MutableBlock<T> inProcessBlock;

        private transient InMemoryPointerTerm lastTerm;
        ...
        protected void flushMetadata(LongArrayList longArrayList) throws IOException {
            ...
            for (int i = 0; i < longArrayList.size(); i++) out.writeLong(longArrayList.get(i));
        }
    }
    ...
    private class DataBuilderLevel extends MutableLevel<InMemoryDataTerm> {
        ...
        private final transient LongArrayList superBlockOffsets = new LongArrayList();

        /**
         * count of regular data blocks written since current super block was init'd
         */
        private transient int dataBlocksCnt;

        private transient TokenTreeBuilder superBlockTree;
    ...
    private static class MutableBlock<T extends InMemoryTerm> {
        ...
        protected final transient DataOutputBufferFixed buffer;

        protected final transient ShortArrayList offsets;
    ...
    private static class MutableDataBlock extends MutableBlock<InMemoryDataTerm> {
        ...
        private static final transient int MAX_KEYS_SPARSE = 5;
        ...
        private final transient List<TokenTreeBuilder> containers = new ArrayList<>();

        private transient TokenTreeBuilder combinedIndex;
        ...
        protected void addInternal(InMemoryDataTerm term) throws IOException {
            ...
            if (mode == Mode.SPARSE) {
                if (keys.getTokenCount() > MAX_KEYS_SPARSE)
                    throw new IOException(String.format("Term - '%s' belongs to more than %d keys in %s mode, which is not allowed.", comparator.getString(term.term.getBytes()), MAX_KEYS_SPARSE, mode.name()));
                writeTerm(term, keys);
            } else {
        ...
        private int ptrLength(InMemoryDataTerm term) {
            return // 1 byte type + 4 byte offset to the tree
            (term.keys.getTokenCount() > 5) ? // 1 byte size + n 8 byte tokens
            5 : 1 + (8 * (int) term.keys.getTokenCount());
        }
        ...
        private void writeTerm(InMemoryTerm term, TokenTreeBuilder keys) throws IOException {
            ...
            for (Pair<Long, LongSet> key : keys) buffer.writeLong(key.left);
        }
