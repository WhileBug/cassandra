...
public class DynamicTokenTreeBuilder extends AbstractTokenTreeBuilder {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(DynamicTokenTreeBuilder.class);

    private final transient SortedMap<Long, LongSet> tokens = new TreeMap<>();
    ...
    public void add(Iterator<Pair<Long, LongSet>> data) {
        while (data.hasNext()) {
            ...
            for (LongCursor l : entry.right) add(entry.left, l.value);
        }
    }
    ...
    public void add(SortedMap<Long, LongSet> data) {
        for (Map.Entry<Long, LongSet> newEntry : data.entrySet()) {
            ...
            for (LongCursor offset : newEntry.getValue()) found.add(offset.value);
        }
    }
    ...
    private class DynamicLeaf extends Leaf {
        ...
        private final transient SortedMap<Long, LongSet> tokens;
        ...
        protected void serializeData(ByteBuffer buf) {
            for (Map.Entry<Long, LongSet> entry : tokens.entrySet()) createEntry(entry.getKey(), entry.getValue()).serialize(buf);
        }
    }
}
