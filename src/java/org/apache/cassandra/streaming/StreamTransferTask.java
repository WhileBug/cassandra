...
public class StreamTransferTask extends StreamTask {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamTransferTask.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamTransferTask.class);

    private static final transient Logger logger = LoggerFactory.getLogger(StreamTransferTask.class);

    private static final transient ScheduledThreadPoolExecutor timeoutExecutor = createTimeoutExecutor();

    private final transient AtomicInteger sequenceNumber = new AtomicInteger(0);

    private transient boolean aborted = false;

    @VisibleForTesting
    protected final transient Map<Integer, OutgoingStreamMessage> streams = new HashMap<>();

    private final transient Map<Integer, ScheduledFuture<?>> timeoutTasks = new HashMap<>();
