...
public class StreamSession implements IEndpointStateChangeSubscriber {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamSession.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamSession.class);

    private static final transient Logger logger = LoggerFactory.getLogger(StreamSession.class);

    // for test purpose to record received message and state transition
    public volatile static transient MessageStateSink sink = MessageStateSink.NONE;

    private final transient StreamOperation streamOperation;
    ...
    private final transient OutboundConnectionSettings template;

    private final transient int index;

    // should not be null when session is started
    private transient StreamResultFuture streamResult;

    // stream requests to send to the peer
    protected final transient Set<StreamRequest> requests = Sets.newConcurrentHashSet();

    // streaming tasks are created and managed per ColumnFamily ID
    @VisibleForTesting
    protected final transient ConcurrentHashMap<TableId, StreamTransferTask> transfers = new ConcurrentHashMap<>();

    // data receivers, filled after receiving prepare message
    private final transient Map<TableId, StreamReceiveTask> receivers = new ConcurrentHashMap<>();

    private final transient StreamingMetrics metrics;

    final transient Map<String, Set<Range<Token>>> transferredRangesPerKeyspace = new HashMap<>();

    private final transient boolean isFollower;

    private final transient NettyStreamingMessageSender messageSender;

    // contains both inbound and outbound channels
    private final transient ConcurrentMap<ChannelId, Channel> channels = new ConcurrentHashMap<>();
    ...
    private transient boolean maybeCompleted = false;

    private transient Future<?> closeFuture;
    ...
    private volatile transient State state = State.INITIALIZED;
    ...
    public void start() {
        ...
        try {
            logger.info("[Stream #{}] Starting streaming to {}{}", planId(), peer, template.connectTo == null ? "" : " through " + template.connectTo);
    ...
    public synchronized Future<?> onError(Throwable e) {
        ...
        if (isEofException) {
            if (state.finalState) {
                ...
                logger.error("[Stream #{}] Socket closed before session completion, peer {} is probably down.", planId(), peer.getHostAddressAndPort(), e);
                return closeSession(State.FAILED);
            }
        }
        logError(e);
        if (messageSender.connected()) {
            // make sure subsequent error handling sees the session in a final state
            state(State.FAILED);
    ...
    private void logError(Throwable e) {
        if (e instanceof SocketTimeoutException) {
            logger.error("[Stream #{}] Did not receive response from peer {}{} for {} secs. Is peer down? " + "If not, maybe try increasing streaming_keep_alive_period_in_secs.", planId(), peer.getHostAddressAndPort(), template.connectTo == null ? "" : " through " + template.connectTo.getHostAddressAndPort(), 2 * DatabaseDescriptor.getStreamingKeepAlivePeriod(), e);
        } else {
            logger.error("[Stream #{}] Streaming error occurred on session with peer {}{}", planId(), peer.getHostAddressAndPort(), template.connectTo == null ? "" : " through " + template.connectTo.getHostAddressAndPort(), e);
        }
    }
    ...
    private void prepareAsync(Collection<StreamRequest> requests, Collection<StreamSummary> summaries) {
        for (// always flush on stream request
        StreamRequest request : // always flush on stream request
        requests) addTransferRanges(request.keyspace, RangesAtEndpoint.concat(request.full, request.transientReplicas), request.columnFamilies, true);
        ...
        if (!peer.equals(FBUtilities.getBroadcastAddressAndPort()))
            for (StreamTransferTask task : transfers.values()) prepareSynAck.summaries.add(task.getSummary());
    ...
    private void prepareSynAck(PrepareSynAckMessage msg) {
        if (!msg.summaries.isEmpty()) {
            for (StreamSummary summary : msg.summaries) prepareReceiving(summary);
    ...
    public static interface MessageStateSink {
        ...
        static final transient MessageStateSink NONE = new MessageStateSink() {
