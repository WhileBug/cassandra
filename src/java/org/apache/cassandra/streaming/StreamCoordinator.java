...
public class StreamCoordinator {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamCoordinator.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamCoordinator.class);

    private static final transient Logger logger = LoggerFactory.getLogger(StreamCoordinator.class);

    private final transient boolean connectSequentially;

    private final transient Map<InetAddressAndPort, HostStreamingData> peerSessions = new ConcurrentHashMap<>();

    private final transient StreamOperation streamOperation;

    private final transient int connectionsPerHost;

    private final transient boolean follower;

    private transient StreamConnectionFactory factory;

    private transient Iterator<StreamSession> sessionsToConnect = null;
    ...
    private class HostStreamingData {
        ...
        private final transient Map<Integer, StreamSession> streamSessions = new HashMap<>();

        private final transient Map<Integer, SessionInfo> sessionInfos = new HashMap<>();

        private transient int lastReturned = -1;
        ...
        public StreamSession getOrCreateNextSession(InetAddressAndPort peer) {
            ...
            if (streamSessions.size() < connectionsPerHost) {
                StreamSession session = new StreamSession(streamOperation, peer, factory, isFollower(), streamSessions.size(), pendingRepair, previewKind);
