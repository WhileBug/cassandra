...
public class StreamManager implements StreamManagerMBean {
    ...
    public static class StreamRateLimiter {
        ...
        private static double calculateRateInBytes() {
            return DatabaseDescriptor.getStreamThroughputOutboundMegabitsPerSec() > 0 ? DatabaseDescriptor.getStreamThroughputOutboundMegabitsPerSec() * BYTES_PER_MEGABIT : // if throughput is set to 0 or negative value, throttling is disabled
            Double.MAX_VALUE;
        }

        private static double calculateInterDCRateInBytes() {
            return DatabaseDescriptor.getInterDCStreamThroughputOutboundMegabitsPerSec() > 0 ? DatabaseDescriptor.getInterDCStreamThroughputOutboundMegabitsPerSec() * BYTES_PER_MEGABIT : // if throughput is set to 0 or negative value, throttling is disabled
            Double.MAX_VALUE;
        }
