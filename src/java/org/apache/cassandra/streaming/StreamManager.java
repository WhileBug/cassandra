...
public class StreamManager implements StreamManagerMBean {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamManager.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamManager.class);

    public static final transient StreamManager instance = new StreamManager();
    ...
    public static class StreamRateLimiter {
        ...
        public static final transient double BYTES_PER_MEGABIT = (1000 * 1000) / 8.0;

        private static final transient RateLimiter limiter = RateLimiter.create(calculateRateInBytes());

        private static final transient RateLimiter interDCLimiter = RateLimiter.create(calculateInterDCRateInBytes());

        private final transient boolean isLocalDC;
        ...
        private static double calculateRateInBytes() {
            return // if throughput is set to 0 or negative value, throttling is disabled
            DatabaseDescriptor.getStreamThroughputOutboundMegabitsPerSec() > 0 ? // if throughput is set to 0 or negative value, throttling is disabled
            DatabaseDescriptor.getStreamThroughputOutboundMegabitsPerSec() * BYTES_PER_MEGABIT : Double.MAX_VALUE;
        }

        private static double calculateInterDCRateInBytes() {
            return // if throughput is set to 0 or negative value, throttling is disabled
            DatabaseDescriptor.getInterDCStreamThroughputOutboundMegabitsPerSec() > 0 ? // if throughput is set to 0 or negative value, throttling is disabled
            DatabaseDescriptor.getInterDCStreamThroughputOutboundMegabitsPerSec() * BYTES_PER_MEGABIT : Double.MAX_VALUE;
        }
    ...
    private final transient StreamEventJMXNotifier notifier = new StreamEventJMXNotifier();
     ...
     */
    private final transient Map<UUID, StreamResultFuture> initiatorStreams = new NonBlockingHashMap<>();

    private final transient Map<UUID, StreamResultFuture> followerStreams = new NonBlockingHashMap<>();
