...
public class StreamPlan {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamPlan.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamPlan.class);

    private static final transient String[] EMPTY_COLUMN_FAMILIES = new String[0];

    private final transient UUID planId = UUIDGen.getTimeUUID();

    private final transient StreamOperation streamOperation;

    private final transient List<StreamEventHandler> handlers = new ArrayList<>();

    private final transient StreamCoordinator coordinator;

    private transient boolean flushBeforeTransfer = true;
    ...
    public StreamPlan(StreamOperation streamOperation, int connectionsPerHost, boolean connectSequentially, UUID pendingRepair, PreviewKind previewKind) {
        this.streamOperation = streamOperation;
        this.coordinator = new StreamCoordinator(streamOperation, connectionsPerHost, new DefaultConnectionFactory(), false, connectSequentially, pendingRepair, previewKind);
    }
