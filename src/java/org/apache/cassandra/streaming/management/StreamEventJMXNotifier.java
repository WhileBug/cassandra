...
public class StreamEventJMXNotifier extends NotificationBroadcasterSupport implements StreamEventHandler {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamEventJMXNotifier.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamEventJMXNotifier.class);

    // interval in millisec to use for progress notification
    private static final transient long PROGRESS_NOTIFICATION_INTERVAL = 1000;

    private final transient AtomicLong seq = new AtomicLong();

    private transient long progressLastSent;
    ...
    public void onFailure(Throwable t) {
        Notification notif = new Notification(StreamEvent.class.getCanonicalName() + ".failure", StreamManagerMBean.OBJECT_NAME, seq.getAndIncrement());
