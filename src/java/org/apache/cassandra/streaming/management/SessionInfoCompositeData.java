...
public class SessionInfoCompositeData {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SessionInfoCompositeData.class);

    private static final transient String[] ITEM_NAMES = new String[] { "planId", "peer", "peer_port", "connecting", "connecting_port", "receivingSummaries", "sendingSummaries", "state", "receivingFiles", "sendingFiles", "sessionIndex" };

    private static final transient String[] ITEM_DESCS = new String[] { "Plan ID", "Session peer", "Session peer storage port", "Connecting address", "Connecting storage port", "Summaries of receiving data", "Summaries of sending data", "Current session state", "Receiving files", "Sending files", "Session index" };

    private static final transient OpenType<?>[] ITEM_TYPES;
    ...
    public static SessionInfo fromCompositeData(CompositeData cd) {
        ...
        SessionInfo info = new SessionInfo(peer, (int) values[10], connecting, fromArrayOfCompositeData((CompositeData[]) values[5], toStreamSummary), fromArrayOfCompositeData((CompositeData[]) values[6], toStreamSummary), StreamSession.State.valueOf((String) values[7]));
