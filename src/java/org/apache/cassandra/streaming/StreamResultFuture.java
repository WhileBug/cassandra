...
public final class StreamResultFuture extends AbstractFuture<StreamState> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(StreamResultFuture.class);

    private static final transient Logger logger = LoggerFactory.getLogger(StreamResultFuture.class);

    public final transient UUID planId;

    public final transient StreamOperation streamOperation;

    private final transient StreamCoordinator coordinator;

    private final transient Collection<StreamEventHandler> eventListeners = new ConcurrentLinkedQueue<>();
    ...
    public static synchronized StreamResultFuture createFollower(int sessionIndex, UUID planId, StreamOperation streamOperation, InetAddressAndPort from, Channel channel, UUID pendingRepair, PreviewKind previewKind) {
        ...
        if (future == null) {
            logger.info("[Stream #{} ID#{}] Creating new streaming plan for {} from {} channel.remote {} channel.local {}" + " channel.id {}", planId, sessionIndex, streamOperation.getDescription(), from, channel.remoteAddress(), channel.localAddress(), channel.id());
        ...
        logger.info("[Stream #{}, ID#{}] Received streaming plan for {} from {} channel.remote {} channel.local {} channel.id {}", planId, sessionIndex, streamOperation.getDescription(), from, channel.remoteAddress(), channel.localAddress(), channel.id());
        return future;
    }
    ...
    void handleSessionPrepared(StreamSession session) {
        ...
        logger.info("[Stream #{} ID#{}] Prepare completed. Receiving {} files({}), sending {} files({})", session.planId(), session.sessionIndex(), sessionInfo.getTotalFilesToReceive(), FBUtilities.prettyPrintMemory(sessionInfo.getTotalSizeToReceive()), sessionInfo.getTotalFilesToSend(), FBUtilities.prettyPrintMemory(sessionInfo.getTotalSizeToSend()));
    ...
    synchronized void fireStreamEvent(StreamEvent event) {
        ...
        for (StreamEventHandler listener : eventListeners) listener.handleStreamEvent(event);
    }
