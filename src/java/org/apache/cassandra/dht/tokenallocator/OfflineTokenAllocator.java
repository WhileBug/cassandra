...
public class OfflineTokenAllocator {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(OfflineTokenAllocator.class);
    ...
    public static class FakeNode {
        ...
        private final transient InetAddressAndPort fakeAddressAndPort;

        private final transient int rackId;
    ...
    private static class MultinodeAllocator {
        ...
        private final transient FakeSnitch fakeSnitch;

        private final transient TokenMetadata fakeMetadata;

        private final transient TokenAllocation allocation;

        private final transient Map<Integer, SummaryStatistics> lastCheckPoint = Maps.newHashMap();

        private final transient OutputHandler logger;
        ...
        private void validateAllocation(int nodeId, int rackId) {
            ...
            if (oldOwnership != null)
                logger.debug(String.format("Replicated node load in rack=%d before allocating node %d: %s.", rackId, nodeId, TokenAllocation.statToString(oldOwnership)));
            logger.debug(String.format("Replicated node load in rack=%d after allocating node %d: %s.", rackId, nodeId, TokenAllocation.statToString(newOwnership)));
            if (oldOwnership != null && oldOwnership.getStandardDeviation() != 0.0) {
                ...
                if (stdDevGrowth > TokenAllocation.WARN_STDEV_GROWTH) {
                    logger.warn(String.format("Growth of %.2f%% in token ownership standard deviation after allocating node %d on rack %d above warning threshold of %d%%", stdDevGrowth * 100, nodeId, rackId, (int) (TokenAllocation.WARN_STDEV_GROWTH * 100)));
                }
            }
        }
    }
    ...
    private static class FakeSnitch extends SimpleSnitch {
        ...
        final transient Map<InetAddressAndPort, Integer> nodeByRack = new HashMap<>();
