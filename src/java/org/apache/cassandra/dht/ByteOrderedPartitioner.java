...
public class ByteOrderedPartitioner implements IPartitioner {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ByteOrderedPartitioner.class);

    public static final transient BytesToken MINIMUM = new BytesToken(ArrayUtils.EMPTY_BYTE_ARRAY);

    public static final transient BigInteger BYTE_MASK = new BigInteger("255");

    private static final transient long EMPTY_SIZE = ObjectSizes.measure(MINIMUM);

    public static final transient ByteOrderedPartitioner instance = new ByteOrderedPartitioner();

    public static class BytesToken extends Token {

        static final transient long serialVersionUID = -2630749093733680626L;

        final transient byte[] token;
