...
public class RangeStreamer {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RangeStreamer.class);

    private static final transient Logger logger = LoggerFactory.getLogger(RangeStreamer.class);

    public static transient Predicate<Replica> ALIVE_PREDICATE = replica -> (!Gossiper.instance.isEnabled() || (Gossiper.instance.getEndpointStateForEndpoint(replica.endpoint()) == null || Gossiper.instance.getEndpointStateForEndpoint(replica.endpoint()).isAlive())) && FailureDetector.instance.isAlive(replica.endpoint());

    /* bootstrap tokens. can be null if replacing the node. */
    private final transient Collection<Token> tokens;

    /* current token ring */
    private final transient TokenMetadata metadata;
    ...
    private final transient Map<String, Multimap<InetAddressAndPort, FetchReplica>> toFetch = new HashMap<>();

    private final transient List<SourceFilter> sourceFilters = new ArrayList<>();

    private final transient StreamPlan streamPlan;

    private final transient boolean useStrictConsistency;

    private final transient IEndpointSnitch snitch;

    private final transient StreamStateStore stateStore;

    public static class FetchReplica {

        public final transient Replica local;

        // Source replica
        public final transient Replica remote;
        ...
        public String toString() {
            return "FetchReplica{" + "local=" + local + ", remote=" + remote + '}';
        }
    ...
    public static class FailureDetectorSourceFilter implements SourceFilter {
        ...
        private final transient IFailureDetector fd;
    ...
    public static class SingleDatacenterFilter implements SourceFilter {
        ...
        private final transient String sourceDc;
    ...
    public static class AllowedSourcesFilter implements SourceFilter {
        ...
        private final transient Set<InetAddressAndPort> allowedSources;
