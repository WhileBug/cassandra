...
public class RandomPartitioner implements IPartitioner {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(RandomPartitioner.class);

    public static final transient BigInteger ZERO = new BigInteger("0");

    public static final transient BigIntegerToken MINIMUM = new BigIntegerToken("-1");

    public static final transient BigInteger MAXIMUM = new BigInteger("2").pow(127);

    public static final transient int MAXIMUM_TOKEN_SIZE = MAXIMUM.bitLength() / 8 + 1;
     ...
     */
    private static final transient ThreadLocal<MessageDigest> localMD5Digest = new ThreadLocal<MessageDigest>() {
    ...
    private static final transient int HEAP_SIZE = (int) ObjectSizes.measureDeep(new BigIntegerToken(hashToBigInteger(ByteBuffer.allocate(1))));

    public static final transient RandomPartitioner instance = new RandomPartitioner();

    public static final transient AbstractType<?> partitionOrdering = new PartitionerDefinedOrder(instance);

    private final transient Splitter splitter = new Splitter(this) {
    ...
    public Token split(Token ltoken, Token rtoken, double ratioToLeft) {
        BigDecimal left = ltoken.equals(MINIMUM) ? BigDecimal.ZERO : new BigDecimal(((BigIntegerToken) ltoken).token), right = rtoken.equals(MINIMUM) ? BigDecimal.ZERO : new BigDecimal(((BigIntegerToken) rtoken).token), ratio = BigDecimal.valueOf(ratioToLeft);
        BigInteger newToken;
    ...
    public static class BigIntegerToken extends ComparableObjectToken<BigInteger> {
        ...
        static final transient long serialVersionUID = -5833589141319293006L;
