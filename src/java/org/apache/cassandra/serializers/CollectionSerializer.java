...
public abstract class CollectionSerializer<T> extends TypeSerializer<T> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(CollectionSerializer.class);
    ...
    public static <V> V pack(Collection<V> values, ValueAccessor<V> accessor, int elements, ProtocolVersion version) {
        int size = 0;
        for (V value : values) size += sizeOfValue(value, accessor, version);
     ...
     */
    public abstract ByteBuffer getSliceFromSerialized(ByteBuffer collection, ByteBuffer from, ByteBuffer to, AbstractType<?> comparator, boolean frozen);
