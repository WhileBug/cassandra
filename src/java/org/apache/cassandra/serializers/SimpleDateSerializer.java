...
public class SimpleDateSerializer extends TypeSerializer<Integer> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SimpleDateSerializer.class);

    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(SimpleDateSerializer.class);

    private static final transient DateTimeFormatter formatter = DateTimeFormatter.ISO_LOCAL_DATE.withZone(UTC).withResolverStyle(STRICT);

    private static final transient long minSupportedDateMillis = TimeUnit.DAYS.toMillis(Integer.MIN_VALUE);

    private static final transient long maxSupportedDateMillis = TimeUnit.DAYS.toMillis(Integer.MAX_VALUE);

    private static final transient long maxSupportedDays = (long) Math.pow(2, 32) - 1;

    private static final transient long byteOrderShift = (long) Math.pow(2, 31) * 2;

    private static final transient Pattern rawPattern = Pattern.compile("^-?\\d+$");

    public static final transient SimpleDateSerializer instance = new SimpleDateSerializer();
    ...
    public static int dateStringToDays(String source) throws MarshalException {
        ...
        try {
            ...
            if (millis < minSupportedDateMillis)
                throw new MarshalException(String.format("Input date %s is less than min supported date %s", source, ZonedDateTime.ofInstant(Instant.ofEpochMilli(minSupportedDateMillis), UTC).toString()));
            if (millis > maxSupportedDateMillis)
                throw new MarshalException(String.format("Input date %s is greater than max supported date %s", source, ZonedDateTime.ofInstant(Instant.ofEpochMilli(maxSupportedDateMillis), UTC).toString()));
