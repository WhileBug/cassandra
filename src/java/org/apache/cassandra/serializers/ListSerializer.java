...
public class ListSerializer<T> extends CollectionSerializer<List<T>> {
    ...
    public static transient org.slf4j.Logger logger_IC = org.slf4j.LoggerFactory.getLogger(ListSerializer.class);

    // interning instances
    private static final transient ConcurrentMap<TypeSerializer<?>, ListSerializer> instances = new ConcurrentHashMap<TypeSerializer<?>, ListSerializer>();

    public final transient TypeSerializer<T> elements;
    ...
    protected List<ByteBuffer> serializeValues(List<T> values) {
        ...
        for (T value : values) output.add(elements.serialize(value));
        return output;
    }
